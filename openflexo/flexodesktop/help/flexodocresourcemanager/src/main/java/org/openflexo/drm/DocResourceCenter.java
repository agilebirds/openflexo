...
public class DocResourceCenter extends DRMObject implements XMLStorageResourceData {
   ...
   public DocResourceCenter()
    ...
    protected static DocResourceCenter createDefaultDocResourceCenter()
    {
        ...
        DocItemFolder rootFolder = DocItemFolder.createDocItemFolder(DocResourceManager.DOC_RESOURCE_CENTER,"Root folder for Flexo documentation",null,returned);
        returned.setRootFolder(rootFolder);
        DocItemFolder flexoModelFolder = DocItemFolder.createDocItemFolder(DocResourceManager.FLEXO_MODEL,"Description of Flexo model",rootFolder,returned);
        DocItemFolder flexoToolSetFolder = DocItemFolder.createDocItemFolder(DocResourceManager.FLEXO_TOOL_SET,"Description for the FlexoToolSet application",rootFolder,returned);
        DocItemFolder abstractModuleFolder = DocItemFolder.createDocItemFolder(DocResourceManager.ABSTRACT_MODULE,"Description of what is a module",flexoToolSetFolder,returned);
        DocItem abstractMainPaneItem = DocItem.createDocItem(DocResourceManager.ABSTRACT_MAIN_PANE,"Description of what is the main pane",abstractModuleFolder,returned,false);
        DocItem abstractControlPanelItem = DocItem.createDocItem(DocResourceManager.ABSTRACT_CONTROL_PANEL,"Description of what is the control panel",abstractModuleFolder,returned,false);
        DocItem abstractLeftViewItem = DocItem.createDocItem(DocResourceManager.ABSTRACT_LEFT_VIEW,"Description of what is the left view",abstractModuleFolder,returned,false);
        DocItem abstractRightViewItem = DocItem.createDocItem(DocResourceManager.ABSTRACT_RIGHT_VIEW,"Description of what is the right view",abstractModuleFolder,returned,false);
    ...
    public Author getAuthorNamed (String authorIdentifier)
    {
        for (Enumeration en=getAuthors().elements(); en.hasMoreElements();) {
            ...
            if (next.getIdentifier().equals(authorIdentifier)) return next;
        }
        return null;
    }
    ...
    public Language getLanguageNamed (String languageIdentifier)
    {
        for (Enumeration en=getLanguages().elements(); en.hasMoreElements();) {
            ...
            if (next.getIdentifier().equalsIgnoreCase(languageIdentifier)) return next;
        }
        return null;
    }
    ...
    {
        if (getRootFolder() == null) return null;
