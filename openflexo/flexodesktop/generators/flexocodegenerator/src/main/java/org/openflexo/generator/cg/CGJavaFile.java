...
public class CGJavaFile extends AbstractCGFile implements ModelReinjectableFile {
	...
	{
		if ((_parsedJavaSource == null) 
				&& (_parseException == null) 
				&& (getGeneratedResourceData() != null) 
	...
	{
		if ((getResource() != null) 
				&& (getResource().getResourceFile() != null)) {
	...
	/**
	 * This method is called whenever we have to store known Java model.
	 * Since known properties and methods are stored in the data model, we
	 * juste have to store here properties and methods that we explicitely
	 * choose to ignore.
	 * At this point we just do a comparison between parsed java source and
	 * related entity to store the delta.
	 *
	 */
	...
	/**
	 * Maybe some explanation required here.
	 * Reinjection in model will result here in two concurrent changes: one in the model itself, 
	 * and one resulting from the necessary previous acceptation. This situation will result as a
	 * conflict occurring in the next generation iteration (normally changes are both side the same
	 * and this conflict must be automatically resolved).
	 * The goal here is to avoid this situation, by asserting that reinjection is enough and 
	 * round-trip for the related reinjected elements is no more necessary, so we just re-run silently
	 * the code generation, and put the result as the LAST_GENERATED (this will shortcut round-trip
	 * for the next generation, and thus prevent from a conflict to appear).
	 * 
	 * @param entity
	...
	private void updateIsTemplateStatus(Vector<DMObject> newObjects)
	{
		...
		if ((getGeneratedResourceData() != null) 
				&& (getGeneratedResourceData().getCurrentGeneration() != null)) {
			try {
				...
				Vector<DMProperty> parsedProperties = FJPDMMapper.searchForProperties(
	 					parsedClass, 
	 					getProject().getDataModel(), 
	 					null, 
	 					pureGenerationSource, 
	 					true, 
	 					false, 
	 					excludedSignatures);
				Vector<DMMethod> parsedMethods = FJPDMMapper.searchForMethods(
						parsedClass,
						getProject().getDataModel(),
						null,
						pureGenerationSource,
