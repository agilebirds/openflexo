 ...
 *
 */
public abstract class ComponentAPIFileResource<G extends FlexoComponentResourceGenerator> extends APIFileResource<G, CGAPIFile> implements GenerationAvailableFileResource, ComponentFileResource, FlexoObserver
{
		...
		if (resource instanceof TemplateLocator) {
			...
			if (dmRes.isLoaded() && getComponentDefinition() != null && getComponentDefinition().getComponentDMEntity() != null) {
				if (!requestDate.before(getComponentDefinition().getComponentDMEntity().getLastUpdate())) {
					if (logger.isLoggable(Level.FINER)) logger.finer("OPTIMIST DEPENDANCY CHECKING for API COMPONENT "+getComponentDefinition().getName());
					return false;
				}
			}
		}
