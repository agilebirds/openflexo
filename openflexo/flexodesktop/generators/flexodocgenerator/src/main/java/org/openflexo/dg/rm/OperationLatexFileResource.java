...
public class OperationLatexFileResource extends LatexFileResource<DGLatexGenerator<OperationNode>> implements FlexoObserver
{
    ...
    {
        if (getOperation()!=null && getOperation().getAbstractActivityNode()!=null && getOperation().getProcess()!=null && !getFile().getName().equals(DGLatexGenerator.nameForOperation(getOperation(), getGenerator().getRepository()))) {
            if (logger.isLoggable(Level.INFO))
                logger.info("Renaming file from "+getFileName()+ " to "+DGLatexGenerator.nameForOperation(getOperation(), getGenerator().getRepository()));
            try {
