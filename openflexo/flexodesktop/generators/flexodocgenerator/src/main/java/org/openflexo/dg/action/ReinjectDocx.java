...
public class ReinjectDocx extends AbstractGCAction<ReinjectDocx, CGObject>
{
	...
	public static final FlexoActionType<ReinjectDocx, CGObject, CGObject> actionType =  new FlexoActionType<ReinjectDocx, CGObject, CGObject>("reinject_docx", FlexoActionType.importMenu, FlexoActionType.defaultGroup, FlexoActionType.NORMAL_ACTION_TYPE)
	{
	...
	protected void doAction(Object context) throws FlexoException
	{
		...
		try
		{
			...
			resetSecondaryProgress(parsedDocx.getAllParsedFlexoDescriptions().size() + parsedDocx.getAllParsedFlexoNames().size() + parsedDocx.getAllParsedFlexoTitles().size() + parsedDocx.getAllParsedFlexoContents().size() + 1);
			...
			registeredObjects.addAll(getFocusedObject().getProject().getAllRegisteredObjects()); //Create new to avoid concurrent modification exception
	    	...
	    	for(IParsedFlexoObject parsedFlexoObject : parsedDocx.getAllParsedFlexoObjects())
	    	{
	    		...
	    		FlexoModelObject flexoObject = allObjects.get(FlexoModelObject.getSerializationIdentifier(parsedFlexoObject.getUserId(), parsedFlexoObject.getFlexoId()));
	    		...
	    		if(flexoObject != null)
	    		{
	    			if(parsedFlexoObject.getParsedFlexoDescription() != null)
	    			{
		    			...
		    			setSecondaryProgress(FlexoLocalization.localizedForKey("reinjecting_description_in_model_for_object") + " " + flexoObject.getName());
	
			    		for(String target : parsedFlexoObject.getParsedFlexoDescription().getAllTargets())
			    		{
			    			logger.info("Updating object '" +flexoObject.getSerializationIdentifier() + "' description" + (target.length()>0?" for target '" +target+ "'":""));
	    			...
	    			{
	    				setSecondaryProgress(FlexoLocalization.localizedForKey("reinjecting_name_in_model_for_object") + " " + flexoObject.getName());
	    				
	    				String oldName = flexoObject.getName();
	    				String newName = parsedFlexoObject.getParsedFlexoName().getFlexoName();
	    				
	    				if(newName!=null && (oldName == null || !StringUtils.replaceBreakLinesBy(newName, " ").equals(StringUtils.replaceBreakLinesBy(oldName, " "))))
	    				{
		    				try
							{
							...
							{ 
								...
								//This suffix will be removed at the end of the process. Either the object name will be set to the newly set one if possible or it will be reset to its old name.
								//Ideally, this should be performed only on duplicateException but there is too much different possible duplicate Exception type (with no common hierarchy).
								...
								{
									logger.log(Level.WARNING, "Reinject docx: cannot set name for object '" + flexoObject.getName() + " (" + flexoObject.getUserIdentifier() + "_" + flexoObject.getFlexoID() + ")' to '" + newName + "'", e1);
									addToErrorReport("Cannot change name of object '" + oldName + "' to '" + newName + "': " + (e.getMessage()!=null?e.getMessage():e.getClass()));
								}
							}
	    				}
	    			}
	    			...
	    			if(parsedFlexoObject.getParsedFlexoTitle() != null && flexoObject instanceof TOCEntry)
	    			{
	    				...
	    				setSecondaryProgress(FlexoLocalization.localizedForKey("reinjecting_title_in_model_for_tocentry") + " " + tocEntry.getTitle());
	    				
	    				String oldTitle = tocEntry.getTitle();
	    				String newTitle = parsedFlexoObject.getParsedFlexoTitle().getFlexoTitle();
	    				
	    				if(newTitle!=null && (oldTitle == null || !StringUtils.replaceBreakLinesBy(newTitle, " ").equals(StringUtils.replaceBreakLinesBy(oldTitle, " "))))
	    				{
	    			...
	    			{
	    				TOCEntry tocEntry = (TOCEntry)flexoObject;
	    				setSecondaryProgress(FlexoLocalization.localizedForKey("reinjecting_content_in_model_for_tocentry") + " " + tocEntry.getTitle());
						...
						{
							logger.log(Level.WARNING, "Reinject docx: cannot set content for toc entry '" + tocEntry.getTitle() + " (" + flexoObject.getUserIdentifier() + "_" + flexoObject.getFlexoID() + ")'", e);
							addToErrorReport("Cannot change content of toc entry '" + tocEntry.getTitle() + "': " + (e.getMessage()!=null?e.getMessage():e.getClass()));
						}
	    			}
	    		}
	    		else
	    		{
	    			...
	    			logger.warning("ReinjectDocx: cannot find object with flexoid '"+ parsedFlexoObject.getFlexoId()+ "' and user id '" +parsedFlexoObject.getUserId()+ "'");
	    	...
	    	for(FlexoModelObject flexoObject : duplicatedFlexoNames.keySet())
	    	{
	    		...
	    		catch(Exception e)
	    		{
	    			try
					{
	    				...
	    				addToErrorReport("Cannot change name of object '" + duplicatedFlexoNames.get(flexoObject)[0] + "' to '" + duplicatedFlexoNames.get(flexoObject)[1] + "': the new name already exists");
						flexoObject.setName(duplicatedFlexoNames.get(flexoObject)[0]);
					}
					catch (Exception e1)
					{ //Should not occur
						logger.log(Level.WARNING, "Reinject docx: cannot roll back name for object '" + flexoObject.getName() + " (" + flexoObject.getUserIdentifier() + "_" + flexoObject.getFlexoID() + ")' to '" + duplicatedFlexoNames.get(flexoObject)[0] + "'", e1);
						addToErrorReport("Cannot rollback name of object '" + flexoObject.getName() + "' to '" + duplicatedFlexoNames.get(flexoObject)[0] + "': " + e1.getMessage());
					} 
	    		}
	    	}
		}
	...
	{
		for(IParsedHtmlResource htmlResource : parsedHtml.getNeededResources())
		{ //Copy needed resources (images) in imported images directory
			try
			{
			...
			{
				logger.log(Level.WARNING, "Reinject docx: cannot write resource file with identifier '" +htmlResource.getIdentifier()+ "'", e);
