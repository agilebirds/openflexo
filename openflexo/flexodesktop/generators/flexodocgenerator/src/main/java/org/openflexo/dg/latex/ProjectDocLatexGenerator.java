...
public class ProjectDocLatexGenerator extends ProjectDocGenerator
{
     ...
     *
     * @see org.openflexo.dg.DGGenerator#buildResourcesAndSetGenerators(DGRepository,
     *      java.util.Vector)
     */
    @Override
    public void buildResourcesAndSetGenerators(DGRepository repository, Vector<CGRepositoryFileResource> resources)
    {
		...
		if (workflowGenerator == null)
			workflowGenerator = new DGLatexGenerator<FlexoWorkflow>(this, getProject().getFlexoWorkflow(), WKF_TEMPLATE_NAME, getProject()
					.getFlexoWorkflow().getFullyQualifiedName(), DGGenerator
					.nameForObjectNoExt(getProject().getFlexoWorkflow(), repository), repository.getTocRepository().getTOCEntryWithID(
					DocSection.PROCESSES));
		if (clGenerator == null)
		...
		if (readersGuideGenerator == null)
			readersGuideGenerator = new DGLatexGenerator<FlexoProject>(this, getProject(), READERS_TEMPLATE_NAME,"ReadersGuide",DGGenerator.nameForReadersGuideNoExt(getRepository()),getRepository().getTOCEntryWithID(DocSection.READERS_GUIDE));
		if (definitionsGenerator == null)
			definitionsGenerator = new DGLatexGenerator<FlexoProject>(this, getProject(), DEFINITIONS_TEMPLATE_NAME,"Definitions",DGGenerator.nameForDefinitionsNoExt(getRepository()),getRepository().getTOCEntryWithID(DocSection.DEFINITIONS));
		if (rolesGenerator == null)
			rolesGenerator = new DGLatexGenerator<FlexoProject>(this, getProject(), ROLES_TEMPLATE_NAME,"Roles",DGGenerator.nameForDefinitionsNoExt(getRepository()),getRepository().getTOCEntryWithID(DocSection.ROLES));
        ...
        ReadersGuideLatexFileResource readersRes = GeneratedFileResourceFactory.createNewReadersGuideLatexFileResource(repository, readersGuideGenerator);
        entry = readersGuideGenerator.getTOCEntry();
        if (entry!=null)
        	associateEntryWithResource(entry,readersRes);
        resources.add(readersRes);

        DefinitionsLatexFileResource definitionRes = GeneratedFileResourceFactory.createNewDefinitionsLatexFileResource(repository, definitionsGenerator);
        ...
        while (en.hasMoreElements()) {
            ...
            while(en1.hasMoreElements()) {
                ...
                if (generator != null) {
                	...
                	DMEOEntityLatexFileResource res = GeneratedFileResourceFactory.createNewEOEntityLatexFileResource(repository, generator);
                    resources.add(res);
	...
	private void buildResourcesAndSetGeneratorsForComponentLibrary(DGRepository repository, Vector<CGRepositoryFileResource> resources) {
		...
		ComponentLibraryLatexFileResource clRes = GeneratedFileResourceFactory.createNewComponentLibraryLatexFileResource(repository, clGenerator);
		resources.add(clRes);
        ...
        for (ComponentDefinition cd : components) {
            ...
            if (generator != null) {
            	...
            	ComponentLatexFileResource<? extends ComponentDefinition> res = GeneratedFileResourceFactory.createNewComponentLatexFileResource(repository, generator);
                resources.add(res);
                entry = generator.getTOCEntry();
                if (entry!=null)
                	associateEntryWithResource(entry,res);
	...
	private void buildResourcesAndSetGeneratorsForWorkflow(DGRepository repository, Vector<CGRepositoryFileResource> resources) {
		// The Workflow itself
		WorkflowLatexFileResource wkfResource = GeneratedFileResourceFactory.createNewWorkflowLatexFileResource(repository, workflowGenerator);
	 ...
	 * @return
	 */
	private void buildResourcesAndSetGeneratorsForProcess(DGRepository repository,
			Vector<CGRepositoryFileResource> resources, FlexoProcess process) {
    ...
    private DGLatexGenerator<? extends ComponentDefinition> getComponentGenerator(ComponentDefinition cd)
    {
        ...
        if (returned == null) {
            if (cd instanceof PopupComponentDefinition)
                componentGenerators.put(cd,returned=new DGLatexGenerator<PopupComponentDefinition>(this, (PopupComponentDefinition) cd, POPUP_TEMPLATE_NAME));
            else if (cd instanceof OperationComponentDefinition)
                componentGenerators.put(cd,returned=new DGLatexGenerator<OperationComponentDefinition>(this, (OperationComponentDefinition) cd, PAGE_TEMPLATE_NAME));
            else if (cd instanceof TabComponentDefinition)
                componentGenerators.put(cd,returned=new DGLatexGenerator<TabComponentDefinition>(this, (TabComponentDefinition) cd, TAB_TEMPLATE_NAME));
            else
	...
	public List<CGTemplate> getVelocityMacroTemplates() {
		...
		try {
			...
			logger.warning("Should include velocity macro template for project generator but template is not found '" + LATEX_MACRO_LIBRARY_NAME + "'");
			e.printStackTrace();
		}
		return result;
	}
