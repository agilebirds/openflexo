...
/**
 * Represents an accessor as a get/set key-value pair mapping a data stored in a
 * database, implemented in DB as a relationship to an other table.
 * 
 * @author sguerin
 * 
 */
public class DMEORelationship extends DMEOProperty implements Bindable
{
     ...
     */
    public static DMEORelationship createsNewFlattenDMEORelationship(DMModel dmModel, DMEOEntity dmEOEntity, String name, String definition, boolean isReadOnly,
            boolean isSettable, DMPropertyImplementationType implementationType) throws EOAccessException
    {
        EORelationship eoRelationship = new EORelationship();
        eoRelationship.setName(name);
    ...
    public boolean getIsToMany()
    {
    	if (getIsFlattenRelationship()) {
    		if (getFlattenRelationshipDefinition() != null
    				&& getFlattenRelationshipDefinition().getBindingPathLastElement() != null) {
    			...
    			for (int i=0; i<getFlattenRelationshipDefinition().getBindingPathElementCount(); i++) {
    				...
    				if (!r.getIsFlattenRelationship() && r.getIsToMany()) return true;
    			}
    		}
    		return false;
    	}
    ...
    public void setIsToMany(boolean aBoolean)
    {
        ...
        if (getEORelationship() != null) {
        	...
        	if((old && aBoolean) || (!old && !aBoolean))return;
    ...
    {
        if (getEORelationship() != null) {
        	if (_flattenRelationshipDefinition != null
        			&& _lastKnownFlattenRelationshipDefinition != null
	...
	public DMType getType()
    {
       	if (getIsFlattenRelationship()) {
    		if (getFlattenRelationshipDefinition() != null 
    				&& getFlattenRelationshipDefinition().getBindingPathLastElement() != this)
    ...
    /**
     * Overrides parent's method by adressing an NSArray instead of a Vector<Type> in case of
     * a EOF toMany relationship
     */
	@Override
	...
	protected String[] getSetterSignatureCandidates()
	{
		...
		else return super.getSetterSignatureCandidates();
	}
    ...
    public void setDestinationEntity(DMEOEntity destinationEntity, boolean performAutoJoin)
    {
        ...
        if (destinationEntity != oldDestinationEntity) {
            ...
            if (_destinationEntity == null) {
           	...
           	// Warning: notifications must be done after udating completely the model, otherwise, observers will be notified too soon and will get de-synchronized with the model.
            setChanged();
            ...
            if (destinationEntity != null && performAutoJoin) {
                ...
                if (getName() != null && getName().startsWith(FlexoLocalization.localizedForKey("default_new_relationship_name"))
                    ...
                    try {
                        setName(Character.toLowerCase(destinationEntity.getName().charAt(0)) + destinationEntity.getName().substring(1)+(getIsToMany()?"s":""));
                    } catch (IllegalArgumentException e) {
                    	e.printStackTrace();
       	...
       	if (getIsFlattenRelationship()) {
    		if (getFlattenRelationshipDefinition() != null) {
       		...
       		for (DMEOJoin join : getDMEOJoins()) {
       			sb.append((isFirst?"":",")+(join.getSourceEntity()!=null && join.getSourceAttribute()!=null?join.getSourceEntity().getName()+"."+join.getSourceAttribute().getName():join.getSourceEntity()!=null?FlexoLocalization.localizedForKey("unresolved_attribute"):FlexoLocalization.localizedForKey("unresolved_entity"))
       					+"-"+(join.getDestinationEntity()!=null && join.getDestinationAttribute()!=null ?join.getDestinationEntity().getName()+"."+join.getDestinationAttribute().getName():join.getDestinationEntity()!=null?FlexoLocalization.localizedForKey("unresolved_attribute"):FlexoLocalization.localizedForKey("unresolved_entity")));
       			isFirst = false;		
       		}
      		sb.append(")");
      		return sb.toString();
    ...
    /**
     * Creates joins automatically if attributes with the same name (if exact
     * case is not found, an attempt ignoring the case will be made but it won't
     * work all the time) and same type can be found
     * 
    ...
    public static class DMEORelationshipMustReferToAValidEORelationship extends ValidationRule
    {
		...
		public ValidationIssue applyValidation(final Validable object)
        {
            ...
            if (property.getEORelationship() == null) {
            	ValidationError error = new ValidationError(this, object, "relationship_($object.name)_must_refer_to_a_valid_eo_relationship");
                 return error;
            }
            return null;
        }
    ...
    public static class DMEORelationshipToManyIsRarellyMandatory extends ValidationRule
    {
        ...
        {
            final DMEORelationship property = (DMEORelationship) object;
            if ((property.getIsToMany()) 
            		&& (property.getEORelationship() != null) 
            		&& (property.getEORelationship().getIsMandatory())) {
				...
				if (j.getSourceAttribute()!=null && j.getDestinationAttribute()!=null) {
					if (j.getSourceAttribute().getPrototype() != j.getDestinationAttribute().getPrototype()) {
						...
						if (rel.getIsToMany()) {
							...
							return new ValidationError(this,j,"foreign_key_type_($object.sourceAttribute.prototype.name)_must_match_primary_key_type_($object.destinationAttribute.prototype.name)",v);
						} else {
							v.add(new SetPrototype(j.getSourceAttribute(),j.getDestinationAttribute().getPrototype()));
							v.add(new SetPrototype(j.getDestinationAttribute(),j.getDestinationAttribute().getPrototype()));
							return new ValidationError(this,j,"foreign_key_type_($object.destinationAttribute.prototype.name)_must_match_primary_key_type_($object.sourceAttribute.prototype.name)");
						}
					}
				}
