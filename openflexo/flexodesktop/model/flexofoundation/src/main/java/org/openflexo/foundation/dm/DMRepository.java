...
public abstract class DMRepository extends DMObject
{
    ...
    public final void unregisterEntity(DMEntity entity)
    {// GPO: This method is final. If you want to override it, override the 2-args method
        unregisterEntity(entity, true);
    }

    /**
     * Unregisters the given <code>entity</code> from this repository and from
     * its package. A notification will be thrown if <code>notify</code> is
     * true. The purpose of this flag is to avoid notifications for temporary
     * states: if you plan on registering this entity back in this same repository,
     * then the unregister call can be invoked with false (the registration will notify
     * the observers).
     *
     * @param entity
     * @param notify
     ...
     * Take care because all observers are delete here !!!
     *
    ...
    static class PackageComparator implements Comparator<DMPackage>
    {
    	...
    	{
    		if (o1.isDefaultPackage()) return -1;
    		if (o2.isDefaultPackage()) return 1;
