...
public class HTMLListDescriptor {
	...
	public boolean isInSearchArea(IEWidget w){
		if(w==null || searchRow==null)return false;
		IEWidget p = w;
		while(p!=null && p.getParent() instanceof IEWidget){
			if(p.equals(searchRow))return true;
	...
	public RepetitionOperator getRepetitionOperator(){
		...
		if(reply instanceof RepetitionOperator)return (RepetitionOperator)reply;
		return null;
	}
	...
	public Vector<IEHeaderWidget> getHeadersWithKeyPath(){
		Vector<IEHeaderWidget> v=new Vector<IEHeaderWidget>();
			...
			while (en.hasMoreElements()) {
				...
				if(kp!=null && kp.trim().length()>0)v.add(h);
			}
		return v;
	}
		...
		if(firstTR!=null){
			...
			while(en.hasMoreElements()){
				...
				if(td.constraints.gridx==gridx){
					...
					if(reply==null)reply = td.getSequenceWidget().findFirstWidgetOfClass(IEHyperlinkWidget.class);
					if(reply==null && td.getSequenceWidget().size()>0)reply = td.getSequenceWidget().get(0);
					return reply;
				}
			}
			if (logger.isLoggable(Level.FINE)) logger.fine("cannot find a widget matching header :"+h.getValue()+" at gridx = "+gridx);
		}
...
//	}
	...
	public String getBindingValueCodeStringRepresentationForWidget(IEWidget widget)
	{
		...
		if(bv.getCodeStringRepresentation()==null || bv.getCodeStringRepresentation().trim().length()==0)return null;
		String reply = bv.getCodeStringRepresentation();
		BindingValue item = getRepetitionOperator().getItemVariable();
		if(item==null || item.getCodeStringRepresentation()==null || item.getCodeStringRepresentation().trim().length()==0)return null;
...
//							}
//							if (value != null) {
//								keyValues.append("		item.setObjectForKey(\"" + ToolBox.convertStringToJavaString(value.toString()) + "\",\"" + keyPath + "\");").append(StringUtils.LINE_SEPARATOR);
//							}
//						}
//					}
//				}
...
//	}
	...
	public static HTMLListDescriptor createInstanceForBloc(IEBlocWidget w) {
		if(w!=null){
			if(w.getContent() instanceof IEHTMLTableWidget){
        		...
        		if(repeatedRow==null){
        			if(topTable.getTR(0)!=null && topTable.getTR(0).getTD(0)!=null && topTable.getTR(0).getTD(0).getColSpan()==topTable.getColCount()){
        				if(topTable.getTR(0).getTD(0).getSequenceWidget().get(0)!=null && topTable.getTR(0).getTD(0).getSequenceWidget().get(0) instanceof IEHTMLTableWidget){
        					repeatedRow = ((IEHTMLTableWidget)topTable.getTR(0).getTD(0).getSequenceWidget().get(0)).getFirstRepeatedSequence();
        				}
        			}

        			if(repeatedRow==null){
            			if(topTable.getTR(1)!=null && topTable.getTR(1).getTD(0)!=null && topTable.getTR(1).getTD(0).getColSpan()==topTable.getColCount()){
            				if(topTable.getTR(1).getTD(0).getSequenceWidget().get(0)!=null && topTable.getTR(1).getTD(0).getSequenceWidget().get(0) instanceof IEHTMLTableWidget){
            					repeatedRow = ((IEHTMLTableWidget)topTable.getTR(1).getTD(0).getSequenceWidget().get(0)).getFirstRepeatedSequence();
            				}
            			}
            		}
	...
	public Vector<IEControlWidget> getFilterWidgets(){
		if(hasSearch()){
			...
			while (en.hasMoreElements()) {
				...
				if(element instanceof IETextFieldWidget
				|| element instanceof IETextAreaWidget
				|| element instanceof IEDropDownWidget
	...
	public boolean isSearchRow(IEWidget widget) {
		if(widget!=null) return widget.equals(searchRow);
		return false;
	}
    ...
    public static HTMLListDescriptor createInstanceForHTMLTable(IEHTMLTableWidget widget)
    {
        ...
        if(repeatedRow==null){
            ...
            if(repeatedRow==null){
                if(topTable.getTR(1)!=null && topTable.getTR(1).getTD(0)!=null && topTable.getTR(1).getTD(0).getColSpan()==topTable.getColCount()){
                    if(topTable.getTR(1).getTD(0).getSequenceWidget().get(0)!=null && topTable.getTR(0).getTD(0).getSequenceWidget().get(0) instanceof IEHTMLTableWidget){
                        repeatedRow = ((IEHTMLTableWidget)topTable.getTR(1).getTD(0).getSequenceWidget().get(0)).getFirstRepeatedSequence();
                    }
                }
                if(repeatedRow==null){
                    if(topTable.getTR(2)!=null && topTable.getTR(2).getTD(0)!=null && topTable.getTR(2).getTD(0).getColSpan()==topTable.getColCount()){
                        if(topTable.getTR(2).getTD(0).getSequenceWidget().get(0)!=null && topTable.getTR(0).getTD(0).getSequenceWidget().get(0) instanceof IEHTMLTableWidget){
                            repeatedRow = ((IEHTMLTableWidget)topTable.getTR(2).getTD(0).getSequenceWidget().get(0)).getFirstRepeatedSequence();
                        }
                    }
                }
            }
    ...
    public static HTMLListDescriptor createInstanceForWidget(IEWidget widget)
    {
        ...
        if(repetition.getOperatedSequence()==null)return null;
        if(repetition.getOperatedSequence().size()<1)return null;
        IWidget w = repetition.getOperatedSequence().get(0);
        if(!(w instanceof ITableRow)) return null;
    ...
    public String getAnchor(){
    	if(getBloc()!=null)return getBloc().getAnchor();
    ...
    public boolean isFetchingDMEOEntityWithIntegerPK(){
    	if(isFetchingDMEOEntity())return ((DMEOEntity)getRepetitionOperator().getEntity()).isIntegerPrimaryKey();
    	return false;
    }

    public String getEntityName(){
    	if(isFetchingDMEOEntity())return ((DMEOEntity)getRepetitionOperator().getEntity()).getName();
    	return null;
    }
}
