...
/**
 * Represents a connex sub-graph composed of AbstractNode elements linked with
 * FlexoPreCondition and FlexoPostCondition Note that a NodeCompound is created
 * by cloning and thus, contains clones of initial nodes !
 *
 * @author sguerin
 *
 */
public final class NodeCompound extends WKFObject implements LevelledObject
{
    ...
    private void makeConnex(NodeCompound clone, Vector<PetriGraphNode> originalSelectedNodes, Vector<WKFArtefact> originalSelectedArtefacts) {
		...
		while (i.hasNext()) {
			WKFEdge post = i.next();
			if (post.getStartNode()==null || post.getEndNode()==null || !originalSet.contains(post.getStartNode()) || !originalSet.contains(post.getEndNode())) {
				post.delete();
			}
		}
	}
    ...
    public void setLocation(Point2D location,String context)
    {
        ...
        for(Enumeration<WKFObject> e = getAllEmbeddedWKFObjects().elements(); e.hasMoreElements();) {
        	...
        	if (upperLeftCorner==null) {
        		...
        		upperLeftCorner.setLocation(
        				Math.min(upperLeftCorner.getX(), node.getX(context)),
        ...
        offset.setLocation(location.getX() - upperLeftCorner.getX(),
        					location.getY() - upperLeftCorner.getY());
        for (Enumeration<WKFObject> e = getAllEmbeddedWKFObjects().elements(); e.hasMoreElements();) {
        	WKFObject node = e.nextElement();
    ...
    /**
     * Return all operation components contained in this compound containing a
     * WOComponent
     *
