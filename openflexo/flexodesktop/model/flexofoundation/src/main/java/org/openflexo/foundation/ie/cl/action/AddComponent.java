...
public class AddComponent extends FlexoAction<AddComponent,IECLObject,IECLObject> 
{
    ...
    public static FlexoActionType<AddComponent,IECLObject,IECLObject> actionType 
    = new FlexoActionType<AddComponent,IECLObject,IECLObject> (
    		"add_new_component",
    ...
    @Override
	protected void doAction(Object context) throws DuplicateResourceException,NotImplementedException,InvalidParameterException, DuplicateComponentName, DuplicateEntityName
    {
    	if (getFolder() == null) {
       ...
       logger.info ("Add component");
        if (getComponentType() == ComponentType.OPERATION_COMPONENT) {
        	_newComponent = new OperationComponentDefinition(getNewComponentName(), getFolder().getComponentLibrary(), getFolderUsedToCreateComponent(), getProject());
        } else if (getComponentType() == ComponentType.POPUP_COMPONENT) {
        	_newComponent = new PopupComponentDefinition(getNewComponentName(), getFolder().getComponentLibrary(),getFolderUsedToCreateComponent(), getProject());
        } else if (getComponentType() == ComponentType.PARTIAL_COMPONENT) {
        	_newComponent = new SingleWidgetComponentDefinition(getNewComponentName(), getFolder().getComponentLibrary(), getFolderUsedToCreateComponent(), getProject());
        } else if (getComponentType() == ComponentType.TAB_COMPONENT) {
        	_newComponent = new TabComponentDefinition(getNewComponentName(), getFolder().getComponentLibrary(), getFolderUsedToCreateComponent(), getProject());
        	...
        	_newComponent = new MonitoringScreenDefinition(getNewComponentName(), getFolder().getComponentLibrary(), getFolderUsedToCreateComponent(), getProject(),_relatedProcess);
