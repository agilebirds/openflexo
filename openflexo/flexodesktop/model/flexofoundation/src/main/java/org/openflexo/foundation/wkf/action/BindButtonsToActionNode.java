...
public class BindButtonsToActionNode extends FlexoAction<BindButtonsToActionNode,OperationNode,WKFObject>
{
    ...
    public static FlexoActionType<BindButtonsToActionNode,OperationNode,WKFObject> actionType 
    = new FlexoActionType<BindButtonsToActionNode,OperationNode,WKFObject>(
    		"bind_buttons_to_action_nodes", 
    ...
    /**
     * This hashtable keeps a record of the associations made by the end-user.
     * The key list contains all the buttons of the component that needs to be
     * associated with an action. The values are ActionNode that needs to be
     * associated with the button. If the ActionNode returns a <code>null</code>
     * value, then it means that the action needs to be created (it was the
     * dummy ActionNode that was selected).
     */
    private Hashtable associations;

    /**
     * This is the vector of actions (including the dummy) that were selectable
     * to be bound to a button
     */
    ...
    /**
     * This array of FlexoBoolean indicates wheter the associated ActionNode
     * (the order is the same as the one given by the keys of the hashtable)
     * should be linked to the Begin and end node or not.
     */
    private FlexoBoolean[] insertActionNode;

    /**
     * If an exception was thrown then it needs to be passed also (mainly to
     * handle the CANCEL option, information for going back to the previous
     * state is contained in that exception)
     */
