...
public class FlexoWSLibrary extends WSObject implements XMLStorageResourceData
{
    ...
    private static final Logger logger = FlexoLogger.getLogger(FlexoWSLibrary.class.getPackage()
            .getName());

    public static FlexoWSLibrary createNewWSLibrary(FlexoProject project)
    {
        FlexoWSLibrary wsl = new FlexoWSLibrary(project);
        File wsFile = ProjectRestructuration.getExpectedWSLibraryFile(project, project
                .getProjectName());
        ...
        try {
            ...
            try {
                ...
                if (logger.isLoggable(Level.SEVERE))
                    logger.severe("Could not create WS Library with name: "+wsLibraryFile.getRelativePath()+". This should never happen");
                return null;
            }
        }
    ...
    */
    ...
    /**
     * creates an externalWSService, without linking it to the library.
     * call wslibrary.addExternalWSService(wsgroup, wsdlfile) to link a wsgroup.
     * @param name
     * @return
     */
   ...
   /**
    *
    * @param group a WSService to link to the library.
    ...
    */
    public ExternalWSService addExternalWSService(ExternalWSService group, File wsdlFileToCopy) throws DuplicateWSObjectException {
        ...
        // wsdl file
        File copiedFile = new File(ProjectRestructuration.getExpectedWSLibraryDirectory(getWSLibrary().getProject().getProjectDirectory()), wsdlFileToCopy.getName());
       /* if (progress != null) {
...
public void setInternalWSServices(Vector wsServices)
{
    ...
    public ExternalWSService getExternalWSServiceNamed(String name) {
        ...
        while (en.hasMoreElements()) {
        	...
        	if (logger.isLoggable(Level.INFO)) logger.info("looking in :"+group.getName());
    ...
    public WSService getWSServiceNamed(String s){
    		...
    		if(group==null) group = getInternalWSServiceNamed(s);
    		return group;
    }
	...
	if (logger.isLoggable(Level.FINE)) logger.fine("Could not find a WSPortType named: "+s);
	return null;
	...
	if (logger.isLoggable(Level.FINE)) logger.fine("Could not find a WSRepository named: "+s);
	return null;
...
public void delete(){
	  ...
	  if (logger.isLoggable(Level.FINE)) logger.fine("delete : FlexoWSLibrary");
