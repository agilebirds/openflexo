...
public class PropertyListActionListener implements ActionListener
{
    ...
    {
    	if (listAccessor == null) return null;
        ...
        if (getActionMethod(getSelectedObject()) != null) {
			...
			if(_plAction.getMethodExpression() instanceof Function) {
				...
				for (int i=0; i<_plAction.getMethodExpressionArgs().size(); i++) {
					...
					if (arg instanceof StringConstant) params[i] = ((StringConstant)arg).getValue();
					else if (arg == ObjectSymbolicConstant.NULL) params[i] = null;
					else if (arg == ObjectSymbolicConstant.THIS) params[i] = getSelectedObject();
				}
			}
			else {
				params = new Object[0];
    		...
    		if (getActionMethod(paramObject) != null) {
    			...
    			if(_plAction.getMethodExpression() instanceof Function) {
    				...
    				for (int i=0; i<_plAction.getMethodExpressionArgs().size(); i++) {
    					...
    					else if (arg == ObjectSymbolicConstant.THIS) params[i] = paramObject;
    				}
    			}
    			else {
    				params = new Object[1];
        ...
        if (_actionMethod == null) {
            ...
            if (getModel() != null) {
                ...
                if(_plAction.getMethodExpression() instanceof Function) {
                	...
                	for (int i=0; i<_plAction.getMethodExpressionArgs().size(); i++) {
                		Expression arg = _plAction.getMethodExpressionArgs().get(i);
                		if (arg instanceof StringConstant) methodClassParams[i] = String.class;
                		else if (arg == ObjectSymbolicConstant.NULL) methodClassParams[i] = Object.class;
                		else if (arg == ObjectSymbolicConstant.THIS) methodClassParams[i] = paramObject.getClass();
                	}
                }
                else {
                	if (_plAction.type.equals(PropertyListAction.ADD_TYPE) 
                			|| _plAction.type.equals(PropertyListAction.STATIC_ACTION_TYPE)) {
   ...
   private boolean isAvailable(InspectableObject paramObject)
	...
	public static Method lookupMethod (Class aClass, String methodName, Class[] parameterTypes) throws SecurityException, NoSuchMethodException
	{
		/*String s = "";
