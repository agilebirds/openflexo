...
public class Merge extends Observable implements IMerge {
	...
	public Merge (DiffSource original, DiffSource left, DiffSource right, MergedDocumentType docType)
	{
		...
		if (left.getDelimitingMethod() != delimitingMethod
				|| right.getDelimitingMethod() != delimitingMethod) {
	...
	{
		if (changes.size() == 0) return "Merge: no changes";
		else {
	...
	public MergeChange changeBefore(MergeChange change) 
	{
		...
		if (index >= 1) return changes.get(index-1);
		return null;
	}
	...
	private DiffChange getNextLeftChangesFromLine (int lineNb, DiffReport leftReport)
	{
		for (DiffChange change : leftReport.getChanges()) {
			if ((change.getFirst1() >= lineNb) && (!processedChanges.contains(change)))return change;
		}
		return null;
	}
	...
	private DiffChange getNextRightChangesFromLine (int lineNb, DiffReport rightReport)
	{
		for (DiffChange change : rightReport.getChanges()) {
			if ((change.getFirst0() >= lineNb) && (!processedChanges.contains(change))) return change;
		}
		return null;
	}
	...
	protected void computeChanges()
	{
		...
		if (debug) System.out.println("left-diff:\n"+leftReport);
		if (debug) System.out.println("right-diff:\n"+rightReport);
		...
		while (currentLineNb < _original.getTextTokens().length || originalIsEmpty) {
			...
			if ((rightChange == null) && (leftChange == null)) {
				...
				if (debug) System.out.println("BEGIN currentLineNb="+currentLineNb);
				if (debug) System.out.println("Next right="+rightChange);
				if (debug) System.out.println("Next left="+leftChange);
				if (debug) System.out.println("BEFORE NEW INSERTION leftToOriginal="+leftToOriginal);
				if (debug) System.out.println("BEFORE NEW INSERTION originalToRight="+originalToRight);
				// At last one not null
				if (rightChange == null) {
					// Add left one
					MergeChange newChange = addLeftChange(leftChange);
					currentLineNb = newChange.getLast1()+1;
					if (debug) System.out.println("ADD LEFT CHANGE: "+leftChange+" > "+newChange);
				}
				else if (leftChange == null) {
					// Add right one
					MergeChange newChange = addRightChange(rightChange);
					currentLineNb = newChange.getLast1()+1;
					if (debug) System.out.println("ADD RIGHT CHANGE: "+rightChange+" > "+newChange);
				}
				else {
					// Still both side
					if (intersect(leftChange,rightChange)) {
						if (debug) System.out.println("Begin MERGING : "+leftChange+" and "+rightChange);
						...
						if (debug) System.out.println("Add LEFT : "+leftChange);
						rightChangesToMerge.add(rightChange);
						if (debug) System.out.println("Add RIGHT : "+rightChange);
						...
						do {
							...
							while (foundAnOtherOne) {
								...
								if (debug) System.out.println("Examining nextRightChange: "+nextRightChange);
								if (nextRightChange != null && intersect(result,nextRightChange) && (nextRightChange != rightChange)) {
									if (debug) System.out.println("Add nextRightChange: "+nextRightChange);
							...
							while (foundAnOtherOne) {
								...
								if (debug) System.out.println("Examining nextLeftChange: "+nextLeftChange);
								if (nextLeftChange != null && intersect(nextLeftChange, result) && (nextLeftChange != leftChange)) {
									if (debug) System.out.println("Add nextLeftChange: "+nextLeftChange);
						...
						currentLineNb = newChange.getLast1()+1;
						if (debug) System.out.println("ADD MERGED CHANGES:  > "+newChange);
					}
					else {
			...
			if (debug) System.out.println("AFTER NEW INSERTION leftToOriginal="+leftToOriginal);
			if (debug) System.out.println("AFTER NEW INSERTION originalToRight="+originalToRight);
			if (debug) System.out.println("END currentLineNb="+currentLineNb);
		}
	...
	private MergeChange addLeftChange(DiffChange leftChange)
	{
		...
		newChange.setDebug("["+leftChange.toNiceString(false)+"], leftToOriginal="+leftToOriginal+" originalToRight="+originalToRight);
	...
	private MergeChange addRightChange(DiffChange rightChange)
	{
		...
		newChange.setDebug("["+rightChange.toNiceString(false)+"], leftToOriginal="+leftToOriginal+" originalToRight="+originalToRight);
	...
	private MergeChange addMergedChanges(Vector<DiffChange> leftVector, Vector<DiffChange> rightVector)
	{
		...
		if (getDocumentType() != null
				&& getDocumentType().getAutomaticMergeResolvingModel() != null) {
		...
		if (debug) System.out.println("Left merges: "+leftMerges);
		if (debug) System.out.println("Right merges: "+rightMerges);
	...
	{
		if (debug) System.out.println("computeMerge() in Merge");
	...
	public Vector<MergeChange> filteredChangeList(List<ChangeCategory> selectedCategories){
		...
		for(MergeChange item: changes){
			if(selectedCategories.contains(item.category()))reply.add(item);
		}
		return reply;
	}
}
