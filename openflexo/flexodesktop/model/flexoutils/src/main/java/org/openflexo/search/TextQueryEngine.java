...
public class TextQueryEngine {
	 ...
	 * @param text
	 * @param query
	 * @param original the original document, if any. This argument can be null.
	 * @return
	 */
		...
		if (query.isRegularExpression()) {
			...
			for (int i=0;i<text.length();i++) {
				if (text.regionMatches(!query.isCaseSensitive(),i , query.getSearchedText(), 0, query.getSearchedText().length())) {
					if ((query.isWholeWord() && (i==0 || !(Character.isLetterOrDigit(text.charAt(i-1))||text.charAt(i-1)=='_')) && (i+query.getSearchedText().length()==text.length() || !(Character.isLetterOrDigit(text.charAt(i+query.getSearchedText().length()))||text.charAt(i+query.getSearchedText().length())=='_')))||!query.isWholeWord()) {
