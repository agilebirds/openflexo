    ...
    public static String convertHTML2Latex(final String htmlString) {
		...
		HTMLEditorKit.ParserCallback callback = new HTMLEditorKit.ParserCallback() {
			...
			public void handleSimpleTag(Tag t, MutableAttributeSet a, int pos) {
				if('/'==(htmlString.charAt(pos+1))) handleEndTag(t, pos);
			...
			public void handleStartTag(Tag t, MutableAttributeSet a, int pos) {
				...
				if (t==HTML.Tag.B)
					...
					if(a.getAttribute(HTML.Attribute.HREF)!=null){
						...
						if(href.startsWith("#"))sb.append("\\hyperlink{"+href.substring(1)+"}{");
						else sb.append("\\href{"+href+"}{");
						withinA = true;
					...
					if (size != null) {
						try {
							...
							switch (sizeInt) {
							case 1:
								latexSize = "{\\footnotesize "; break;
							case 2:
								latexSize = "{\\small "; break;
							case 3:
								latexSize = "{\\normalsize "; break;
							case 4:
								latexSize = "{\\large "; break;
							case 5:
								latexSize = "{\\Large "; break;
							case 6:
								latexSize = "{\\LARGE "; break;
							case 7:
								latexSize = "{\\huge "; break;
							}
							if (sizeInt > 7) latexSize = "{\\Huge ";
						} catch (NumberFormatException e) {
							// ignore, test on latexSize will cover it
						}
					}
					if (latexSize == null) latexSize = "{"; // always put a { because the end tag will close the }
					sb.append(latexSize);
    ...
    protected static String convertManuallyHTML2Latex(String htmlString) {
		...
		for (int i = 0; i < htmlString.length(); i++) {
			...
			case '\\':
				...
				break;
			case '<':
				if (htmlString.regionMatches(true, i, START_HTML_TAG, 0,
						START_HTML_TAG.length())) {
					i += START_HTML_TAG.length();
				} else if (htmlString.regionMatches(true, i, END_HTML_TAG, 0,
						END_HTML_TAG.length())) {
					i += END_HTML_TAG.length();
				} else if (htmlString.regionMatches(true, i, START_HEAD_TAG, 0,
						START_HEAD_TAG.length())) {
					i += START_HEAD_TAG.length();
					withinHead = true;
				} else if (htmlString.regionMatches(true, i, END_HEAD_TAG, 0,
						END_HEAD_TAG.length())) {
					i += END_HEAD_TAG.length();
					withinHead = false;
				} else if (htmlString.regionMatches(true, i, START_BODY_TAG, 0,
						START_BODY_TAG.length())) {
					i += START_BODY_TAG.length();
				} else if (htmlString.regionMatches(true, i, END_BODY_TAG, 0,
						END_BODY_TAG.length())) {
					i += END_BODY_TAG.length();
				} else if (htmlString.regionMatches(true, i, START_BOLD_TAG, 0,
						START_BOLD_TAG.length())) {
					i += START_BOLD_TAG.length();
					if (!withinHead)
						sb.append("\\textbf{");
				} else if (htmlString.regionMatches(true, i, END_BOLD_TAG, 0,
						END_BOLD_TAG.length())) {
					i += END_BOLD_TAG.length();
					if (!withinHead)
						sb.append("}");
				} else if (htmlString.regionMatches(true, i, START_ITALIC_TAG,
						0, START_ITALIC_TAG.length())) {
					i += START_ITALIC_TAG.length();
					if (!withinHead)
						sb.append("\\textit{");
				} else if (htmlString.regionMatches(true, i, END_ITALIC_TAG, 0,
						END_ITALIC_TAG.length())) {
					i += END_ITALIC_TAG.length();
					if (!withinHead)
						sb.append("}");
				} else if (htmlString.regionMatches(true, i,
						START_UNDERLINE_TAG, 0, START_UNDERLINE_TAG.length())) {
					i += START_UNDERLINE_TAG.length();
					if (!withinHead)
						sb.append("\\underline{");
				} else if (htmlString.regionMatches(true, i, END_UNDERLINE_TAG,
						0, END_UNDERLINE_TAG.length())) {
					i += END_UNDERLINE_TAG.length();
					if (!withinHead)
						sb.append("}");
				} else if (htmlString.regionMatches(true, i,
						START_UNORDERED_TAG, 0, START_UNORDERED_TAG.length())) {
					i += START_UNORDERED_TAG.length();
					if (!withinHead)
						sb.append("\\begin{itemize}");
				} else if (htmlString.regionMatches(true, i, END_UNORDERED_TAG,
						0, END_UNORDERED_TAG.length())) {
					i += END_UNORDERED_TAG.length();
					if (!withinHead)
						sb.append("\\end{itemize}");
				} else if (htmlString.regionMatches(true, i, START_ORDERED_TAG,
						0, START_ORDERED_TAG.length())) {
					i += START_ORDERED_TAG.length();
					if (!withinHead)
						sb.append("\\begin{enumerate}");
				} else if (htmlString.regionMatches(true, i, END_ORDERED_TAG,
						0, END_ORDERED_TAG.length())) {
					i += END_ORDERED_TAG.length();
					if (!withinHead)
						sb.append("\\end{enumerate}");
				} else if (htmlString.regionMatches(true, i,
						START_LIST_ITEM_TAG, 0, START_LIST_ITEM_TAG.length())) {
					i += START_LIST_ITEM_TAG.length();
					if (!withinHead)
						sb.append("\\item ");
				} else if (htmlString.regionMatches(true, i, END_LIST_ITEM_TAG,
						0, END_LIST_ITEM_TAG.length())) {
					i += END_LIST_ITEM_TAG.length();
				} else if (htmlString.regionMatches(true, i, BREAK, 0, BREAK
						.length())) {
					i += BREAK.length();
					if (!withinHead)
						sb.append("\\\\");
				} else if (htmlString.regionMatches(true, i, XHTML_BREAK, 0,
						XHTML_BREAK.length())) {
					i += XHTML_BREAK.length();
					if (!withinHead)
						sb.append("\\\\");
				} else if (htmlString.regionMatches(true, i,
						START_PARAGRAPH_TAG, 0, START_PARAGRAPH_TAG.length())) {
					i += START_PARAGRAPH_TAG.length();
					if (!withinHead)
						sb.append("\\par ");
				} else if (htmlString.regionMatches(true, i, END_PARAGRAPH_TAG,
						0, END_PARAGRAPH_TAG.length())) {
					i += END_PARAGRAPH_TAG.length();
				} else if (htmlString.regionMatches(true, i, "IMG", 0, "IMG"
						.length())) {
					int j = i + 1;
					...
					if (src != null && !withinHead) {
						...
						System.err.println("WARNING:\tI could not find 'src' attribute within: "
											+ img);
					}
					i += (j - i);
				}
				break;
			case '&':
				if (htmlString.regionMatches(true, i, SMALLER, 0, SMALLER
						.length())) {
					i += SMALLER.length();
					if (!withinHead)
						sb.append("\\textsmaller");
				} else if (htmlString.regionMatches(true, i, GREATER, 0,
						GREATER.length())) {
					i += GREATER.length();
					if (!withinHead)
						sb.append("\\textgreater");
				} else if (htmlString.regionMatches(true, i, AMPERSAND, 0,
						AMPERSAND.length())) {
					i += AMPERSAND.length();
					if (!withinHead)
						sb.append("\\&");
				} else if (htmlString.regionMatches(true, i, QUOTE, 0, QUOTE
						.length())) {
					i += QUOTE.length();
					if (!withinHead)
						sb.append("\"");
				} else if (htmlString.regionMatches(true, i, a_GRAVE, 0,
						a_GRAVE.length())) {
					i += a_GRAVE.length();
					if (!withinHead)
						sb.append("à");
					...
					if (!withinHead)
						sb.append("À");
				} else if (htmlString.regionMatches(true, i, a_CIRC, 0, a_CIRC
						.length())) {
					i += a_CIRC.length();
					if (!withinHead)
						sb.append("â");
					...
					if (!withinHead)
						sb.append("Â");
				} else if (htmlString.regionMatches(true, i, a_UML, 0, a_UML
						.length())) {
					i += a_UML.length();
					if (!withinHead)
						sb.append("ä");
					...
					if (!withinHead)
						sb.append("Ä");
				} else if (htmlString.regionMatches(true, i, a_RING, 0, a_RING
						.length())) {
					i += a_RING.length();
					if (!withinHead)
						sb.append("å");
					...
					if (!withinHead)
						sb.append("Å");
				} else if (htmlString.regionMatches(true, i, ae_LIGATURE, 0,
						ae_LIGATURE.length())) {
					...
					if (!withinHead)
						sb.append("Æ");
				} else if (htmlString.regionMatches(true, i, c_CEDILLA, 0,
						c_CEDILLA.length())) {
					i += c_CEDILLA.length();
					if (!withinHead)
						sb.append("ç");
					...
					if (!withinHead)
						sb.append("Ç");
				} else if (htmlString.regionMatches(true, i, e_ACUTE, 0,
						e_ACUTE.length())) {
					i += e_ACUTE.length();
					if (!withinHead)
						sb.append("é");
					...
					if (!withinHead)
						sb.append("É");
				} else if (htmlString.regionMatches(true, i, e_GRAVE, 0,
						e_GRAVE.length())) {
					i += e_GRAVE.length();
					if (!withinHead)
						sb.append("è");
					...
					if (!withinHead)
						sb.append("É");
				} else if (htmlString.regionMatches(true, i, e_CIRC, 0, e_CIRC
						.length())) {
					i += e_CIRC.length();
					if (!withinHead)
						sb.append("ê");
					...
					if (!withinHead)
						sb.append("È");
				} else if (htmlString.regionMatches(true, i, e_UML, 0, e_UML
						.length())) {
					i += e_UML.length();
					if (!withinHead)
						sb.append("ë");
					...
					if (!withinHead)
						sb.append("Ë");
				} else if (htmlString.regionMatches(true, i, i_UML, 0, i_UML
						.length())) {
					i += i_UML.length();
					if (!withinHead)
						sb.append("ë");
					...
					if (!withinHead)
						sb.append("Ë");
				} else if (htmlString.regionMatches(true, i, o_CIRC, 0, o_CIRC
						.length())) {
					i += o_CIRC.length();
					if (!withinHead)
						sb.append("ô");
					...
					if (!withinHead)
						sb.append("Ô");
				} else if (htmlString.regionMatches(true, i, o_UML, 0, o_UML
						.length())) {
					i += o_UML.length();
					if (!withinHead)
						sb.append("ö");
					...
					if (!withinHead)
						sb.append("Ö");
				} else if (htmlString.regionMatches(true, i, u_GRAVE, 0,
						u_GRAVE.length())) {
					i += u_GRAVE.length();
					if (!withinHead)
						sb.append("ù");
					...
					if (!withinHead)
						sb.append("Ù");
				} else if (htmlString.regionMatches(true, i, u_CIRC, 0, u_CIRC
						.length())) {
					i += u_CIRC.length();
					if (!withinHead)
						sb.append("û");
					...
					if (!withinHead)
						sb.append("Ù");
				} else if (htmlString.regionMatches(true, i, u_UML, 0, u_UML
						.length())) {
					i += u_UML.length();
					if (!withinHead)
						sb.append("ü");
					...
					if (!withinHead)
						sb.append("Ü");
				} else if (htmlString.regionMatches(true, i, REGISTERED, 0,
						REGISTERED.length())) {
					i += REGISTERED.length();
					if (!withinHead)
						sb.append("\\textregistered");
				} else if (htmlString.regionMatches(true, i, COPYRIGHT, 0,
						COPYRIGHT.length())) {
					i += COPYRIGHT.length();
					if (!withinHead)
						sb.append("\\copyright");
				} else if (htmlString.regionMatches(true, i, EURO, 0, EURO
						.length())) {
					i += EURO.length();
					if (!withinHead)
						sb.append("€");
				} else if (htmlString.regionMatches(true, i,
						NON_BREAKING_SPACE, 0, NON_BREAKING_SPACE.length())) {
	...
	public static Color extractColorFromString(String color) {
		...
		try {
			if (color.startsWith("#")) {
				...
				if (rgb.length==3) {
					if (color.indexOf('%')>-1) {
						return new Color((float)Float.valueOf(rgb[0])/100,(float)Float.valueOf(rgb[1])/100,(float)Float.valueOf(rgb[2])/100);
					} else {
