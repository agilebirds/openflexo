...
public class BPELExportedPartnerLink implements BPELPartnerLinkInterface{
	...
	public BPELExportedPartnerLink(FlexoProcess pro, AbstractInPort pIN,OutputPort pOUT) {
		...
		for (MessageEntryBinding part:v) {
			if (part.getBindingValue()==null || part.getBindingDefinitionName()==null) {
				...
				System.out.println("EXP PLINKS Adding IN : "+part.getBindingValue().getStringRepresentation()+" - "+part.getBindingDefinitionName());
		...
		for (MessageEntryBinding part:v2) {
			if (part.getBindingValue()==null || part.getBindingDefinitionName()==null) {
				...
				System.out.println("EXP PLINKS ADDING OUT :"+part.getBindingValue().getStringRepresentation()+" - "+part.getBindingDefinitionName());
	...
	public TPartnerLink getTPartnerLink() {
		...
		toReturn.setPartnerLinkType(new QName(BPELConstants.THIS_NAMESPACE+"/wsdl",BPELConstants.normalise(process.getName())+BPELConstants.APPEND_PARTNERLINKTYPE));
