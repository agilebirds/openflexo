	...
	private boolean isLocationAlreadyUsed(SimplifiedCardinalDirection orientation, FGEPoint returned) {
		for(FlexoPreCondition pc : getFlexoPreCondition().getAttachedNode().getPreConditions()) {
			if (pc!=getFlexoPreCondition()) {
				...
				if (gr instanceof ShapeGraphicalRepresentation && pc.hasLocationForContext(SWIMMING_LANE_EDITOR) &&  ((ShapeGraphicalRepresentation)gr).getBounds(1.0).intersects(returned.x, returned.y, PRECONDITION_SIZE, PRECONDITION_SIZE)) {
					return true;
				}
			}
		}
