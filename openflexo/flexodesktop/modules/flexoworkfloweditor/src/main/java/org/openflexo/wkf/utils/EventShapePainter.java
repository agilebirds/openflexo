...
public class EventShapePainter implements ShapePainter {
	...
	FGECircle _internalCircle = new FGECircle(new FGEPoint(0.516, 0.516), 0.42,
			Filling.NOT_FILLED);

	@Override
	public void paintShape(FGEShapeGraphics g) {
		if (model.isIntermediate() || model.isIntermediateDrop()) {
			g.drawCircle(_internalCircle.getX(), _internalCircle.getY(),
					_internalCircle.getWidth(), _internalCircle.getHeight());
		}
		if (model.getEventType()==EVENT_TYPE.NonInteruptiveBoundary) {
			g.drawCircle(_internalCircle.getX(), _internalCircle.getY(),
					_internalCircle.getWidth(), _internalCircle.getHeight(),FGEConstants.DASHED);
		}
		...
		if (model.isTriggerTimer()){
			...
			FGECircle r =new FGECircle(new FGEPoint(xTr, yTr), outerFactor,
					Filling.NOT_FILLED);
