	...
	protected FlexoActionInitializer<CreateNewWebService> getDefaultInitializer() 
	{
		return new FlexoActionInitializer<CreateNewWebService>() {
			...
			public boolean run(ActionEvent e, CreateNewWebService action)
            {
                ...
                RadioButtonMode EXTERNAL_WS = new RadioButtonMode(new FileResource("Resources/WS/SmallWSIcon.gif"), FlexoLocalization
                        .localizedForKey("import_external_ws"));
                RadioButtonMode INTERNAL_WS = new RadioButtonMode(new FileResource("Resources/WS/SmallWSIcon.gif"), FlexoLocalization
                        .localizedForKey("export_internal_ws"));
                ...
                params[7] = new InfoLabelParameter("infoLabelExt", "description", FlexoLocalization
                        .localizedForKey("ws_create_external_ws_description"));
                params[7].setDepends("mode");
                params[7].setConditional("mode=" + '"' + EXTERNAL_WS + '"');
                params[8] = new InfoLabelParameter("infoLabelInt", "description", FlexoLocalization
                        .localizedForKey("ws_create_internal_ws_description"));
                ...
                while (true) {
                    AskParametersDialog dialog = AskParametersDialog.createAskParametersDialog(getProject(), null,
                            FlexoLocalization
							                .localizedForKey("create_new_web_service"), FlexoLocalization.localizedForKey("what_would_you_like_to_do"), params);
	...
	protected FlexoActionFinalizer<CreateNewWebService> getDefaultFinalizer() 
	{
		return new FlexoActionFinalizer<CreateNewWebService>() {
			...
			public boolean run(ActionEvent e, CreateNewWebService action)
            {
                ...
                if (getControllerActionInitializer().getWSEController().getCurrentDisplayedObjectAsModuleView() == action.getNewWebService().getWSLibrary()) {
                    WSELibraryView libraryView = (WSELibraryView)getControllerActionInitializer().getWSEController().getCurrentModuleView();
 	...
 	protected FlexoExceptionHandler<CreateNewWebService> getDefaultExceptionHandler() 
 	{
 		return new FlexoExceptionHandler<CreateNewWebService>() {
			...
			public boolean handleException(FlexoException exception, CreateNewWebService action) {
                if (action instanceof CreateNewWebService
                        && (action).getWebServiceType().equals(CreateNewWebService.EXTERNAL_WS)) {
                    if (exception.getLocalizationKey() != null) {
                        ...
                        && (action).getWebServiceType().equals(CreateNewWebService.INTERNAL_WS)) {
                    if (exception.getLocalizationKey() != null) {
