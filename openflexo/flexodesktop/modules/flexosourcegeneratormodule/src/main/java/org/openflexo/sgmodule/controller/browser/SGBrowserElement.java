...
public abstract class SGBrowserElement extends BrowserElement {

    protected static final Logger logger = Logger.getLogger(SGBrowserElement.class.getPackage().getName());

	public SGBrowserElement(CGObject object, BrowserElementType elementType, ProjectBrowser browser, BrowserElement parent)
	{
	...
	protected Vector<IconMarker> getMarkers(){
		...
		if ((getObject() instanceof CGFile)
			...
			} else if (((CGFile) getObject()).getResource().getGenerator().hasAppendingException()) {
				markers.add(IconLibrary.WARNING);
			}
		}
