...
public class RedefineCustomTemplateFileInitializer extends ActionInitializer {
	...
	RedefineCustomTemplateFileInitializer(SGControllerActionInitializer actionInitializer)
	{
		...
		return new FlexoActionInitializer<RedefineCustomTemplateFile>() {
			...
			public boolean run(ActionEvent e, RedefineCustomTemplateFile action)
			{
				...
				RadioButtonListParameter<String> repositoryChoiceParam = new RadioButtonListParameter<String>("location","location",
						(templates.getCustomCodeRepositoriesVector().size()>0?CHOOSE_EXISTING_REPOSITORY:CREATE_NEW_REPOSITORY),locationChoices);
				...
				= new DynamicDropDownParameter<CustomCGTemplateRepository>("customRepository","custom_templates_repository",
						...
						(getControllerActionInitializer().getSGController().getLastEditedCGRepository()!=null?getControllerActionInitializer().getSGController().getLastEditedCGRepository().getPreferredTemplateRepository():null));
		...
		return new FlexoActionFinalizer<RedefineCustomTemplateFile>() {
			...
			public boolean run(ActionEvent e, RedefineCustomTemplateFile action) {
				if (action.getNewTemplateFile() != null) {
					if ((action.getInvoker() != null) && (action.getInvoker() instanceof CGTemplateFileModuleView) && (((CGTemplateFileModuleView) action.getInvoker()).isOpenedInSeparateWindow())) {
						...
						CGTemplateFileModuleView newView = (CGTemplateFileModuleView) getControllerActionInitializer().getSGController().createModuleViewForObjectAndPerspective(
						...
						getControllerActionInitializer().getSGController().setCurrentEditedObjectAsModuleView(action.getNewTemplateFile());
					}
