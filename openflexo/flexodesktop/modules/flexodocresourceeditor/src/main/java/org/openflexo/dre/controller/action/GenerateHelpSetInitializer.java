...
public class GenerateHelpSetInitializer extends ActionInitializer {
	...
	protected FlexoActionInitializer<GenerateHelpSet> getDefaultInitializer() 
	{
		return new FlexoActionInitializer<GenerateHelpSet>() {
            ...
            {
                ParameterDefinition[] parameters = new ParameterDefinition[4+DocResourceManager.instance().getDocResourceCenter().getLanguages().size()];
                ...
                for (Language language : DocResourceManager.instance().getDocResourceCenter().getLanguages()) {
                    ...
                    parameters[1+langNb] = new TextFieldParameter("title_"+language.getIdentifier(), "title_"+language.getIdentifier(), FlexoLocalization.localizedForKeyAndLanguage("help_for_flexo_tool_set",lang));
                    langNb++;
                }
                CheckboxListParameter<Language> languagesParam = new CheckboxListParameter<Language>("languages", "languages", DocResourceManager.instance().getDocResourceCenter().getLanguages(), DocResourceManager.instance().getDocResourceCenter().getLanguages());
                languagesParam.addParameter("format", "toString");
                parameters[1+langNb] = languagesParam;
                
                CheckboxListParameter<UserType> distributionsParam = new CheckboxListParameter<UserType>("distributions", "distributions", UserType.allKnownUserType(), UserType.allKnownUserType());
               ...
               AskParametersDialog dialog = AskParametersDialog.createAskParametersDialog(getProject(),
                        null, FlexoLocalization.localizedForKey("generate_helpset"), FlexoLocalization.localizedForKey("enter_parameters_for_helpset_generation"), parameters);
			...
			public boolean run(ActionEvent e, GenerateHelpSet action)
            {
                if (FlexoController.confirm(FlexoLocalization.localizedForKey("helpset_has_been_sucessfully_generated")+"\n"
                    ...
                    for (DRMHelpSet set: action.getVectorOfGeneratedHelpset()) {
                    	if (set.getDistributionName().equals(ModuleLoader.getUserType().getIdentifier()) && FlexoLocalization.getCurrentLanguage().getIdentifier().equalsIgnoreCase(set.getLanguage().getIdentifier()))
                    		selected =set;
                    }
                    DynamicDropDownParameter<DRMHelpSet> helpSetParameter = new DynamicDropDownParameter<DRMHelpSet>("helpset","helpset_to_reload",action.getVectorOfGeneratedHelpset(),selected);
                    helpSetParameter.addParameter("format", "localizedName");
                    AskParametersDialog dialog = AskParametersDialog.createAskParametersDialog(getProject(),
                            null, FlexoLocalization.localizedForKey("reload_helpset"), FlexoLocalization.localizedForKey("choose_helpset_to_reload"), helpSetParameter);
                    if (dialog.getStatus() == AskParametersDialog.VALIDATE) {
