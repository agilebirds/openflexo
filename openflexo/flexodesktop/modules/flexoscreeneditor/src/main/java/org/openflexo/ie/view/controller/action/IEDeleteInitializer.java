		...
		return new FlexoActionInitializer<IEDelete>() {
			...
			public boolean run(ActionEvent e, IEDelete action)
			{
				...
				if (components.size()>0) {
				    if (v.size()>1) {
				        FlexoController.notify(FlexoLocalization.localizedForKey("to_delete_a_component_select_it_separately_from_other_object."));
				        return false;
				    } else {
				        ComponentDefinition componentToDelete = components.firstElement();
				        if (componentToDelete == null) return false;
				...
				if (doIt) {
					...
					if (actionNodes.size()>0) {
						...
						def[0].setFormatter("displayString");
						AskParametersDialog dialog = AskParametersDialog.createAskParametersDialog(getProject(), getController().getFlexoFrame(), FlexoLocalization.localizedForKey("select_actions_to_delete"), FlexoLocalization.localizedForKey("<html>the_following_actions_are_linked_to_the_objects_you_are_about_to_delete.<br>select_the_ones_you_would_like_to_delete</html>"), def);
						if (dialog.getStatus() == AskParametersDialog.VALIDATE) {
