    ...
    protected class ViewHeader extends JPanel
    {
        ...
        protected ViewHeader()
        {
            ...
            if (isEdited) {
                ...
                FlexoActionButton editFileAction = new FlexoActionButton(EditGeneratedFile.actionType, "edit", DGFileModuleView.this,_controller.getEditor());
                if(!_cgFile.getMarkedAsDoNotGenerate()) {
					actionButtons.add(editFileAction);
				}

                FlexoActionButton generateAction = new FlexoActionButton(GenerateSourceCode.actionType, "regenerate", DGFileModuleView.this,_controller.getEditor());
                if(!_cgFile.getMarkedAsDoNotGenerate()) {
            ...
            if (!isEdited) {
                ...
                if (_cgFile.getResource() instanceof GenerationAvailableFileResource) {
                    ...
                    if (generator != null) {
						if (generator.hasFormattingException()) {
                            addInfoPanel(CGIconLibrary.UNFIXABLE_WARNING_ICON, FlexoLocalization.localizedForKey("java_formatting_warning_title"),
                ...
                if (_cgFile.hasGenerationErrors()) {
                    addInfoPanel(CGIconLibrary.UNFIXABLE_ERROR_ICON, FlexoLocalization.localizedForKey("generation_error"), FlexoLocalization
                            .localizedForKey("generation_error_description"));
                }
            }
    ...
    private void rebuildView()
    {
        ...
        if(_cgFile.getMarkedAsDoNotGenerate()){
            ...
            if (_cgFile.getGenerationStatus() == GenerationStatus.CodeGenerationNotAvailable) {
                add(new JLabel(FlexoLocalization.localizedForKey("sorry_code_generator_not_available_int_this_version"), SwingConstants.CENTER),
                        BorderLayout.CENTER);
                validate();
                return;
            ...
            if (_cgFile.getResource() instanceof GenerationAvailableFileResource) {
                ...
                if (_cgFile instanceof DGScreenshotFile) {
                    ...
                    validate();
                } else if ((_cgFile instanceof CGPackagedResourceFile) && ((CGPackagedResourceFile)_cgFile).getResource().getResourceFormat().isImage()) {
                	_screenshotDisplayer = new ScreenshotDisplayer(_cgFile);
                    add(_screenshotDisplayer);
                    validate();
