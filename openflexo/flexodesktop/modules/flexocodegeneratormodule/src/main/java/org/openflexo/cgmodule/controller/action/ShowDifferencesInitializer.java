...
public class ShowDifferencesInitializer extends ActionInitializer {
	...
	ShowDifferencesInitializer(GeneratorControllerActionInitializer actionInitializer)
	{
	...
	protected FlexoActionInitializer<ShowDifferences> getDefaultInitializer() 
	{
		return new FlexoActionInitializer<ShowDifferences>() {
			...
			public boolean run(ActionEvent e, ShowDifferences action)
			{
				...
				= new CGFileVersionParameter("versionLeft","left_version",action.getCGFile(),null);
				CGFileVersionParameter versionRightParameter 
				= new CGFileVersionParameter("versionRight","right_version",action.getCGFile(),null);
				AskParametersDialog dialog = AskParametersDialog.createAskParametersDialog( getProject(), null, 
						FlexoLocalization
						        		.localizedForKey("open_in_diff_editor"),
						FlexoLocalization
						.localizedForKey("please_choose_versions_that_you_want_to_compare"), versionLeftParameter, versionRightParameter);
				if (dialog.getStatus() == AskParametersDialog.VALIDATE) {
					...
					ContentSource rightSource = ContentSource.getContentSource(
							ContentSourceType.HistoryVersion,versionRightParameter.getValue().getVersionId());
					ContentSource leftSource = ContentSource.getContentSource(
							ContentSourceType.HistoryVersion,versionLeftParameter.getValue().getVersionId());
		...
		return new FlexoActionFinalizer<ShowDifferences>() {
			...
			public boolean run(ActionEvent e, ShowDifferences action)
			{
				CGFileDiffEditorPopup popup = new CGFileDiffEditorPopup(
						action.getCGFile(),
						action.getLeftSource(), action.getRightSource(),getControllerActionInitializer().getGeneratorController());
