...
public class EditCustomTemplateFileInitializer extends ActionInitializer {
	...
	EditCustomTemplateFileInitializer(GeneratorControllerActionInitializer actionInitializer)
	{
		...
		return new FlexoActionInitializer<EditCustomTemplateFile>() {
			...
			public boolean run(ActionEvent e, EditCustomTemplateFile action)
			{
				...
				if ((action.getInvoker() != null) 
					...
					editor = (CGTemplateFileModuleView) getControllerActionInitializer().getGeneratorController().moduleViewForObject((CGTemplate) action.getFocusedObject());
				}
				if (editor != null) {
