...
public class RegisterNewCGReleaseInitializer extends ActionInitializer {
	...
	RegisterNewCGReleaseInitializer(GeneratorControllerActionInitializer actionInitializer)
	{
	...
	protected FlexoActionInitializer<RegisterNewCGRelease> getDefaultInitializer() 
	{
		return new FlexoActionInitializer<RegisterNewCGRelease>() {
			...
			public boolean run(ActionEvent e, final RegisterNewCGRelease action)
			{
				...
				ReadOnlyTextFieldParameter dateParam = new ReadOnlyTextFieldParameter("date","date",(new SimpleDateFormat("dd/MM HH:mm:ss")).format(registeringDate));
				ReadOnlyTextFieldParameter userParam = new ReadOnlyTextFieldParameter("userId","user_id",FlexoModelObject.getCurrentUserIdentifier());
				final RadioButtonListParameter<RegisterNewCGRelease.IncrementType> incTypeParam 
				= new RadioButtonListParameter<RegisterNewCGRelease.IncrementType>("incType","inc_type_choice",RegisterNewCGRelease.IncrementType.IncrementMinor,RegisterNewCGRelease.IncrementType.values());
				...
				AskParametersDialog dialog = AskParametersDialog.createAskParametersDialog(
						getProject(), 
						null,
						action.getLocalizedName(),FlexoLocalization.localizedForKey("enter_parameters_for_the_new_release"),nameParam,dateParam,userParam,versionIdParam, incTypeParam, descriptionParam);
				if (dialog.getStatus() == AskParametersDialog.VALIDATE) {
					if (nameParam.getValue() == null
							|| nameParam.getValue().trim().equals("")) {
