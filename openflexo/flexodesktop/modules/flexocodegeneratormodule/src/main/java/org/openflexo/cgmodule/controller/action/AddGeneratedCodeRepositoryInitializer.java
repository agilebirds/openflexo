		...
		return new FlexoActionInitializer<AddGeneratedCodeRepository>() {
			...
			public boolean run(ActionEvent e, AddGeneratedCodeRepository action)
			{
				if ((action.getNewGeneratedCodeRepositoryName() == null) || (action.getNewGeneratedCodeRepositoryDirectory() == null)) {
					...
					ChoiceListParameter<CodeType> targetType = new ChoiceListParameter<CodeType>("target", "target", action
							.getNewTargetType());
					targetType.setShowReset(false);
					final TextFieldParameter paramName = new TextFieldParameter("name", "repository_name", (action
							.getNewGeneratedCodeRepositoryName() == null ? gc.getProject().getNextExternalRepositoryIdentifier(FlexoLocalization.localizedForKey("generated_code")) : action
									.getNewGeneratedCodeRepositoryName()));
					DirectoryParameter paramDir = new DirectoryParameter("directory", "source_directory", (action
							.getNewGeneratedCodeRepositoryDirectory() == null ? new File(System.getProperty("user.home"),gc.getProject().getProjectName()) : action
					...
					final DynamicDropDownParameter<DGRepository> readerRepository = new DynamicDropDownParameter<DGRepository>("repository","reader_repository",availableReaderRepositories,availableReaderRepositories.size()>0?availableReaderRepositories.firstElement():null);
					...
					final DirectoryParameter readerDir = new DirectoryParameter("readerDirectory", "reader_directory",getReaderParamDir(paramName, readerRepository.getValue()));
					...
					AskParametersDialog dialog = AskParametersDialog.createAskParametersDialog(
							...
							FlexoLocalization.localizedForKey("enter_parameters_for_the_new_generated_code_repository"),new AskParametersDialog.ValidationCondition() {
								...
								public boolean isValid(ParametersModel model) {
									...
									if (includeReader.getValue() && (!readerRepository.getValue().isConnected() && (readerDir.getValue()==null))) {
			...
			protected File getReaderParamDir(final TextFieldParameter paramName, DGRepository newValue) {
				...
				return newValue.getDirectory()!=null?newValue.getDirectory():
					new File(System.getProperty("user.home")+"/"+getProject().getProjectName()+"/"+newValue.getFormat().name()+"/"+
