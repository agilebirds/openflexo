...
public class ResourceGR extends ShapeGraphicalRepresentation<FlexoResource> {
	...
	public ResourceGR(FlexoResource aResource, Drawing<?> aDrawing) 
	{
		...
		resourceTypeStyle = TextStyle.makeTextStyle(aResource.getResourceType().getMainColor().darker(), new Font("SansSerif", Font.BOLD, 10));
		...
		setShapePainter(new ShapePainter() {
			...
			public void paintShape(FGEShapeGraphics g) 
			{
				...
				g.drawString((new SimpleDateFormat("dd/MM HH:mm:ss SSS")).format(getResource().getLastUpdate()), new FGEPoint(0.5,0.75),TextAlignment.CENTER);
			};
		});
	...
	private void updateStyles()
	{
		...
		background = BackgroundStyle.makeColorGradientBackground(getResource().getResourceType().getMainColor(), Color.WHITE, ColorGradientDirection.SOUTH_WEST_NORTH_EAST);
