 ...
 *
 */
public class DMTypeSelector extends TextFieldCustomPopup<DMType>
implements FIBCustomComponent<DMType,DMTypeSelector>
{
	...
	public DMTypeSelector(FlexoProject project, DMType editedObject, boolean displayTypeAsSimplified)
	{
        ...
        if (editedObject != null && !editedObject.isBasicType()) editionMode = EditionMode.COMPLEX_TYPE;
        completionListKeyAdapter = new KeyAdapter() {
            @Override
			public void keyPressed(KeyEvent e)
            {
              	 if (_selectorPanel != null && _selectorPanel.isUpdating()) return;
	...
	public String renderedString(DMType editedObject)
    {
    	...
    	else return STRING_REPRESENTATION_WHEN_NULL;
    }
    ...
    public class ComplexDMTypeSelectorPanel extends AbstractDMTypeSelectorPanel
    {
        ...
        {
        	if (getEditedObject() != null) return getEditedObject();
        	else {
        		DMType returned = DMType.makeUnresolvedDMType("");
        		returned.setOwner(getOwner());
        		setEditedObject(returned);
        		typeVariableParam.setAvailableValues(getEditedObject()!=null && getEditedObject().getTypeVariableContext() != null?getEditedObject().getTypeVariableContext().getTypeVariables():null);
        		return returned;
        	}
        }
		...
		protected void init()
        {
            ...
            kindOfTypeParameter = new EnumDropDownParameter<DMType.KindOfType>(
            		"KindOfType","",null,DMType.KindOfType.values());
            ...
            kindOfTypeParameter.addValueListener(new ParameterDefinition.ValueListener<DMType.KindOfType>() {
				public void newValueWasSet(ParameterDefinition<DMType.KindOfType> param, DMType.KindOfType oldValue, DMType.KindOfType newValue) {
					if (isUpdating) return;
			       	isUpdating = true;
			       	if (newValue == DMType.KindOfType.RESOLVED) {
						...
						getOrCreateEditedType().setDimensions(0);
						//typeVariableParam.setAvailableValues(getEditedObject()!=null && getEditedObject().getTypeVariableContext() != null?getEditedObject().getTypeVariableContext().getTypeVariables():null);
						if (getOrCreateEditedType()!=null
								&& getOrCreateEditedType().getTypeVariableContext() != null) {
							...
							if (getOrCreateEditedType().getTypeVariableContext().getTypeVariables() != null
								...
								getOrCreateEditedType().setTypeVariable(getOrCreateEditedType().getTypeVariableContext().getTypeVariables().firstElement());
							}
							else {
								getOrCreateEditedType().setTypeVariable(null);
							}
            ...
            unresolvedTypeParameter = new TextFieldParameter("type","type","");
  			...
  			unresolvedTypeParameter.addValueListener(new ParameterDefinition.ValueListener<String>() {
				public void newValueWasSet(ParameterDefinition<String> param, String oldValue, String newValue) {
			       	...
			       	if (newValue != null && !newValue.equals(oldValue)) {
			       		setEditedObject(getProject().getDataModel().getDmTypeConverter().convertFromString(newValue,getOwner(),getProject()));
  			...
  			baseEntityParameter.addValueListener(new ParameterDefinition.ValueListener<DMEntity>() {
				public void newValueWasSet(ParameterDefinition<DMEntity> param, DMEntity oldValue, DMEntity newValue) {
					...
					if (oldValue != newValue) {
						...
						hasParametersParameter.setValue(getEditedObject()!=null && getEditedObject().getBaseEntity() != null ? getEditedObject().getBaseEntity().getTypeVariables().size()>0:false);
  			...
  			baseEntityParameter.setConditional(
  					"KindOfType="+'"'+DMType.KindOfType.RESOLVED.getStringRepresentation()+'"'
  					+" OR "+"KindOfType="+'"'+DMType.KindOfType.RESOLVED_ARRAY.getStringRepresentation()+'"');
			...
			parametersParam
			= new PropertyListParameter<DMType.ParameterizedTypeVariable>("parameters",
					"parameters",
					null,
					20,3);
			...
			parametersParam.setConditional("hasParams=true AND "
					+"(KindOfType="+'"'+DMType.KindOfType.RESOLVED.getStringRepresentation()+'"'
  					+" OR "+"KindOfType="+'"'+DMType.KindOfType.RESOLVED_ARRAY.getStringRepresentation()+'"'+")");
			...
			PropertyListColumn valueAttributeColumn = parametersParam.addCustomColumn("value", "value","org.openflexo.components.widget.DMTypeInspectorWidget",200,true);
			...
			valueAttributeColumn.setValueForParameter("owner", "owner");
 			...
 			domainParam.setConditional(
  					"KindOfType="+'"'+DMType.KindOfType.DKV.getStringRepresentation()+'"');
			...
			upperBoundsParam
			= new PropertyListParameter<DMType.WildcardBound>("upperBounds",
					"upper_bounds",
					null,
					20,2);
			...
			PropertyListColumn upperBoundsValueAttributeColumn = upperBoundsParam.addCustomColumn("bound", "upper_bounds","org.openflexo.components.widget.DMTypeInspectorWidget",200,true);
			...
			lowerBoundsParam
			= new PropertyListParameter<DMType.WildcardBound>("lowerBounds",
					"lower_bounds",
					null,
					20,2);
			...
			PropertyListColumn lowerBoundsValueAttributeColumn = lowerBoundsParam.addCustomColumn("bound", "lower_bounds","org.openflexo.components.widget.DMTypeInspectorWidget",200,true);
			...
			lowerBoundsParam.addDeleteAction("remove", "wildcardActions.removeLowerBound", "wildcardActions.removeLowerBoundEnabled", null);
 			...
 			_dataPanel = new AskParametersPanel(
					getProject(),
					kindOfTypeParameter,
			...
			if (getOwner() != null) _dataPanel.getParametersModel().addObjectForKey(getOwner(), "owner");
		...
		DMEntitySelector<DMEntity> getBaseEntitySelector()
        {
        	...
        	if (widget != null) return widget.getSelector();
        	return null;
        }
		...
		public void update()
         {
        	 ...
        	 if (getEditedObject() == null
        			 || getEditedObject().getStringRepresentation() == null
        			 || getEditedObject().getStringRepresentation().equals(""))
        		 kindOfTypeParameter.setValue(DMType.KindOfType.RESOLVED);
        	 else kindOfTypeParameter.setValue(getEditedObject()!=null?getEditedObject().getKindOfType():null);
    ...
    public DMType parseAndSelect(String aTypeAsString)
    {
    	...
    	while (st.hasMoreTokens()) {
    		...
    		if (" <>,?".indexOf(token) > -1) typeToParse += token; // this is a delimiter
    		else {
    			DMEntity e = getAllUnambigousEntities().get(token);
       			if (e == null) e = getAllUnambigousEntities().get(token.toUpperCase());
       			if (e != null) typeToParse += e.getFullQualifiedName();
    			else typeToParse += token;
    		}
    	}
	...
	{
		if (logger.isLoggable(Level.FINE))
			logger.fine("activateBasicTypeMode() getEditedObject()="+getEditedObject()+" editionMode="+editionMode+" popupIsShown()="+popupIsShown()+" _selectorPanel="+_selectorPanel);
	...
	{
		if (logger.isLoggable(Level.FINE))
			logger.fine("activateComplexTypeMode() getEditedObject()="+getEditedObject()+" editionMode="+editionMode+" popupIsShown()="+popupIsShown()+" _selectorPanel="+_selectorPanel);
		if ((_selectorPanel != null) && (editionMode != EditionMode.COMPLEX_TYPE)) {
			...
			if (_editedObject == null && _selectorPanel instanceof BasicDMTypeSelectorPanel) {
				FlexoModelObject currentSelectedObject = ((BasicDMTypeSelectorPanel)_selectorPanel)._entitySelectorPanel.getSelectedObject();
    ...
    private Hashtable<String,DMEntity> getAllUnambigousEntities()
    {
    	if (unambigousEntities == null) {
    		...
    		while (en.hasMoreElements()) {
    			...
    			if (unambigousEntities.get(fullQualifiedName) == null) unambigousEntities.put(fullQualifiedName, e);
       			String fullQualifiedNameUC = e.getFullQualifiedName().toUpperCase();
    			if (unambigousEntities.get(fullQualifiedNameUC) == null) unambigousEntities.put(fullQualifiedNameUC, e);
    			String name = e.getClassName();
    			if (unambigousEntities.get(name) == null) unambigousEntities.put(name, e);
       			String nameUC = e.getClassName().toUpperCase();
    			if (unambigousEntities.get(nameUC) == null) unambigousEntities.put(nameUC, e);
    		}
    	}
