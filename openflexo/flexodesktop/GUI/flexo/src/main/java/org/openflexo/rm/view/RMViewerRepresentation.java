...
public class RMViewerRepresentation extends DefaultDrawing<FlexoProject> implements GraphicalFlexoObserver {
	...
	{
		if (r1.getFullyQualifiedName().compareTo(r2.getFullyQualifiedName()) < 0) return resourceSynchronizationBetween(r2,r1);
	...
	public void performAutomaticLayout()
	{
		...
		for (FlexoResource res : getProject().getResources().values()) {
			...
			if (res.getResourceOrder() > maxOrder) maxOrder = res.getResourceOrder();
		...
		for (int i : resources.keySet()) {
			...
			for (FlexoResource r : resources.get(i)) {
				((ResourceGR)getGraphicalRepresentation(r)).setLocation(new FGEPoint(i*width/maxOrder,j*height/resources.get(i).size()));
				j++;
			}
		}
