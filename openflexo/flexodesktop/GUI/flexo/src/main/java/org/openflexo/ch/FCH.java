...
/**
 * Utility class provinding facilities to handle contextual help
 * Note: FCH stands for FlexoContextualHelp (ou bien "Fuck cette Connerie de Help")
 *
 * @author sguerin
 */
public class FCH
{
    ...
    private static synchronized DocItem createCHEntry (JComponent component, String identifier, DocItem parentItem)
    {
        ...
        DocItem newEntry = DocItem.createDocItem(identifier,"",parentItem.getFolder(),DocResourceManager.instance().getDocResourceCenter(),false);
    ...
    public static DocItem getDocItemFor(FlexoModule module)
    {
        ...
        if (DocResourceManager.getDocItem(identifier) == null) {
            DocItemFolder newModuleFolder = DocItemFolder.createDocItemFolder(identifier,"",DocResourceManager.instance().getFlexoToolSetItem().getFolder(),drc);
    ...
    public static DocItem getModuleViewItemFor(FlexoModule module, ModuleView view)
    {
        ...
        if (view.getPerspective() != null) {
            String perspectiveIdentifier = view.getPerspective().getName();
             if (DocResourceManager.getDocItem(perspectiveIdentifier) == null) {
                DocItemFolder perspectiveFolder = DocItemFolder.createDocItemFolder(perspectiveIdentifier,"Documentation on that perspective",mainPaneItem.getFolder(),drc);
        ...
        if (DocResourceManager.getDocItem(moduleViewIdentifier) == null) {
            DocItemFolder moduleViewFolder = DocItemFolder.createDocItemFolder(moduleViewIdentifier,"Documentation on that view",folder,drc);
