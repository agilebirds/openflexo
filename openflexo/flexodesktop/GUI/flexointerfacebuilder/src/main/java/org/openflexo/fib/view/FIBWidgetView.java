 ...
 * @author sylvain
 */
public abstract class FIBWidgetView<M extends FIBWidget, J extends JComponent, T> extends FIBView<M,J> 
implements FocusListener, Observer, PropertyChangeListener
{

    private static final Logger logger = Logger
            .getLogger(FIBWidgetView.class.getPackage().getName());
    ...
    {
    	if (getController().getFocusedWidget() != null 
    			&& getController().getFocusedWidget()._hasFocus == true) 
    ...
    public final boolean isComponentEnabled()
    {
    	...
    	if (getComponent().getReadOnly()) return false;
    	if (getComponent().getEnable() != null && getComponent().getEnable().isValid()) {
    		...
    		if (isEnabled instanceof Boolean) componentEnabled = (Boolean)isEnabled;
    	}
