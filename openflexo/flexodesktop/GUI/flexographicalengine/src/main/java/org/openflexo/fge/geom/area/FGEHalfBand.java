...
public class FGEHalfBand implements FGEArea {
	...
	{
		if (halfLine1.overlap(halfLine2)) throw new IllegalArgumentException("lines are overlaping halfLine1="+halfLine1+" halfLine2="+halfLine2);
		if (!halfLine1.isParallelTo(halfLine2)) throw new IllegalArgumentException("lines are not parallel: "+halfLine1+" and "+halfLine2);
	...
	{
		if (!embeddingBand.containsPoint(p)) return false;
	...
	{
		if (!embeddingBand.containsLine(l)) return false;
	...
	public FGEArea transform(AffineTransform t)
	{
		...
		if (l1.overlap(l2)) return l1.intersect(l2);
		else return new FGEHalfBand(l1,l2);
	}
	...
	protected FGEArea computeHalfPlaneIntersection(FGEHalfPlane anHalfPlane) 
	{
		...
		if (anHalfPlane.containsArea(this)) return clone();
		...
		if (returned instanceof FGEHalfBand) {
			...
			if (hb.halfLine1.containsPoint(pp1) && hb.halfLine2.containsPoint(pp2)) {
				FGEArea returnThis = FGEPolygon.makeArea(Filling.FILLED,
						hb.halfLine1.getLimit(),
						hb.halfLine2.getLimit(),
						pp2,
						pp1);
				return returnThis;
			}
				...
				if (halfPlane.containsLine(hb.halfLine1) && halfPlane.containsLine(hb.halfLine2)) return hb;
				return new FGEEmptyArea();
			}
		}
	...
	public FGEPoint getNearestPoint(FGEPoint aPoint)
	{
		...
		else return FGEPoint.getNearestPoint(aPoint, halfLine1.getNearestPoint(aPoint), halfLine2.getNearestPoint(aPoint));
	}
	...
	public FGEArea getOrthogonalPerspectiveArea(SimplifiedCardinalDirection orientation)
	{
		if (orientation == SimplifiedCardinalDirection.NORTH) {
			if (halfLine1.isVertical()) return clone(); 
			else {
			...
			if (halfLine1.isHorizontal()) return clone(); 
			else {
