...
public class FGESegment extends FGEAbstractLine<FGESegment> implements FGEGeneralShape.GeneralShapePathElement<FGESegment> {
	...
	/**
	 * Return flag indicating if intersection of this segment and supplied
	 * line occurs somewhere INSIDE this segment
	 * @param line
	 * @return
	 */
	...
	/**
	 * Return flag indicating if intersection of this segment and supplied
	 * line occurs somewhere INSIDE this segment. If insideOnly set to true
	 * and intersection is one of extremities return false
	 * @param line
	 * @return
	 */
	public boolean intersectsInsideSegment(FGEAbstractLine line, boolean insideOnly)
	{
		...
		if (!(contains(intersection) && line.contains(intersection))) return false;
		if (insideOnly) {
			if (intersection.equals(getP1()) 
					|| intersection.equals(getP2())
					|| ((line instanceof FGESegment) && (intersection.equals(line.getP1()) || intersection.equals(line.getP2())))) return false;
		}
		return true;
	}
	...
	/**
	 * Return flag indicating if intersection of supplied segment and supplied
	 * line occurs somewhere INSIDE supplied segment
	 * @param line
	 * @return
	 */
	 ...
	 * 
	 * If orthogonal projection of supplied point on segment is inside the segment,
	 * return this projection.
	 * Otherwise, return adequate segment extremity
	 * 
	 * @param p
	 * @return
	 */
	...
	public boolean equals(Object obj)
	{
		if (obj instanceof FGESegment) {
			...
			return ((getP1().equals(s.getP1()) || getP1().equals(s.getP2()))
					&& (getP2().equals(s.getP1()) || getP2().equals(s.getP2())));
		}
		return false;
	}
		...
		if (s1.containsPoint(s2.getP1())) {
			if (s1.containsPoint(s2.getP2())) {
				...
				if (s2.containsPoint(s1.getP1())) {
					if (s1.getP1().equals(s2.getP1())) return s1.getP1().clone();
					return new FGESegment(s1.getP1(),s2.getP1());
				}
				else {
					if (s2.getP1().equals(s1.getP2())) return s2.getP1().clone();
			...
			if (s1.containsPoint(s2.getP2())) {
				if (s2.containsPoint(s1.getP1())) {
					if (s1.getP1().equals(s2.getP2())) return s1.getP1().clone();
					return new FGESegment(s1.getP1(),s2.getP2());
				}
				else {
					if (s2.getP2().equals(s1.getP2())) return s2.getP2().clone();
					return new FGESegment(s2.getP2(),s1.getP2());
				}
			}
			else {
				if (s2.containsPoint(s1.getP1()) && s2.containsPoint(s1.getP2())) return s1.clone();
	...
	public FGEArea getOrthogonalPerspectiveArea(SimplifiedCardinalDirection orientation)
	{
		...
		if (orientation == SimplifiedCardinalDirection.NORTH) {
			...
			if (Math.abs(ps1.x-ps2.x) < EPSILON) return hl1;
		}
			...
			if (Math.abs(ps1.y-ps2.y) < EPSILON) return hl1;
		}
		...
		try {
			...
			logger.warning("Could not obtain resulting half-band, hl1="+hl1+" hl2="+hl2+" orientation="+orientation+" ps1="+ps1+" ps2="+ps2);
			return new FGEEmptyArea();
		}
	}
	...
	public double getRelativeLocation(FGEPoint p)
	{
		...
		if (Math.abs(getP2().x-getP1().x) < EPSILON) return (proj.y-getP1().y)/(getP2().y-getP1().y);
		else return (proj.x-getP1().x)/(getP2().x-getP1().x);
	}
	...
	public SimplifiedCardinalDirection getOrientation()
	{
		...
		/**
		 * GPO:
		 * Angle will be between -PI/2 and 3PI/2
		 * 0-->WEST
		 * PI/2--> SOUTH
		 * -PI or PI-->EAST (Almost sure that -PI will never come but it does not cost too much so let's do it)
		 * -PI/2 or 3PI/2-->NORTH
		 */
		...
		else if (Math.abs(angle-3*Math.PI/2) < EPSILON
				||Math.abs(angle+Math.PI/2) < EPSILON) return SimplifiedCardinalDirection.NORTH;
		else return null;
	}
	...
	public final FGEArea union(FGEArea area)
	{
		...
		if (area instanceof FGESegment) {
			...
			if (s.containsLine(this)) return s;
			if (containsLine(s)) return clone();
			if (overlap(s)) {
				if (containsPoint(s.getP1())) {
					// Doesn't contains P2, otherwise all contained, see above
					if (s.containsPoint(getP1())) return new FGESegment(getP2(),s.getP2());
					if (s.containsPoint(getP2())) return new FGESegment(getP1(),s.getP2());
				}
				if (containsPoint(s.getP2())) {
					// Doesn't contains P2, otherwise all contained, see above
					if (s.containsPoint(getP1())) return new FGESegment(getP2(),s.getP1());
					if (s.containsPoint(getP2())) return new FGESegment(getP1(),s.getP1());
				}
			}
			if (getP1().equals(s.getP2())) return new FGEPolylin(getP2(),getP1(),s.getP1());
			if (getP1().equals(s.getP1())) return new FGEPolylin(getP2(),getP1(),s.getP2());
			if (getP2().equals(s.getP2())) return new FGEPolylin(getP1(),getP2(),s.getP1());
			if (getP2().equals(s.getP1())) return new FGEPolylin(getP1(),getP2(),s.getP2());
