...
public class ShapeGraphicalRepresentation<O> extends GraphicalRepresentation<O> implements Observer {
	...
	{
		/**
		 * Shape is freely relocatable in parent global bounds
		 * (rectangular bounds, don't care about borders nor shape of parent)
		 */
		FREELY_MOVABLE,
		/**
		 * Shape is freely relocatable in parent exact bounds
		 * (shape of this GR must be fully located INSIDE parent GR outline)
		 */
		CONTAINED_IN_SHAPE,
		/**
	...
	public ShapeGraphicalRepresentation(ShapeType shapeType, O aDrawable, Drawing<?> aDrawing)
	{
		...
		addToMouseDragControls(MouseDragControl.makeMouseShiftDragControl("Rectangle selection",MouseButton.LEFT,MouseDragControlActionType.RECTANGLE_SELECTING));
	}
	...
	public void delete()
	{
		...
		if (shadowStyle != null) shadowStyle.deleteObserver(this);
		super.delete();
	}
	...
	public void update(Observable observable, Object notification)
	{
		...
		if (observeParentGRBecauseMyLocationReferToIt && observable == getContainerGraphicalRepresentation()) {
			if ((notification instanceof ObjectWillMove)
					...
					|| (notification instanceof ObjectHasMoved)
					|| (notification instanceof ObjectHasResized)
					|| (notification instanceof ObjectMove)
	...
	public void setLocation(FGEPoint newLocation)
	{
		...
		if (!newLocation.equals(oldLocation)) {
			...
			if (!isFullyContainedInContainer()) {
				if (logger.isLoggable(Level.FINE))logger.fine("setLocation() lead shape going outside it's parent view");
			}
		}
	}
	...
	private void updateRequiredBoundsForChildGRLocation (ShapeGraphicalRepresentation<?> child, FGEPoint newChildLocation)
	{
		...
		for (GraphicalRepresentation<?> gr : getContainedGraphicalRepresentations()) {
			if (gr instanceof ShapeGraphicalRepresentation) {
				...
				if (shapeGR == child) shapeGR.setLocationNoCheckNorNotification(newChildLocation.transform(translation));
				else {
					//FGEPoint oldLocationInParent = new FGEPoint(oldBounds.x+shapeGR.getLocation().x,oldBounds.y+shapeGR.getLocation().y);
					//FGEPoint newLocationInParent = new FGEPoint(newBounds.x+shapeGR.getLocation().transform(translation).x,newBounds.y+shapeGR.getLocation().transform(translation).y);
	...
	private FGERectangle getRequiredBoundsForChildGRLocation (ShapeGraphicalRepresentation<?> child, FGEPoint newChildLocation)
	{
		...
		for (GraphicalRepresentation<?> gr : getContainedGraphicalRepresentations()) {
			if (gr instanceof ShapeGraphicalRepresentation) {
				...
				if (requiredBounds == null) requiredBounds = bounds;
				else {
	...
	public void setLocationConstraints(LocationConstraints locationConstraints)
	{
		FGENotification notification = requireChange(
				Parameters.locationConstraints, locationConstraints);
	...
	{
		FGENotification notification = requireChange(
				Parameters.locationConstrainedArea, locationConstrainedArea);
		if (notification != null) {
	...
	/**
	 * Calling this method forces FGE to check (and eventually update)
	 * location of current graphical representation according defined
	...
	{
		if (isParentLayoutedAsContainer()) return newLocation;
		...
		if (getLocationConstraints() == LocationConstraints.CONTAINED_IN_SHAPE) {
			...
			if (parent instanceof ShapeGraphicalRepresentation) {
				...
				return new FGEPoint(
						center.x+(newLocation.x-center.x)*authorizedRatio,
						center.y+(newLocation.y-center.y)*authorizedRatio);
			}
		}
	...
	public double getMoveAuthorizedRatio(FGEPoint desiredLocation,FGEPoint initialLocation)
	{
		...
		FGERectangle containerViewArea = new FGERectangle(0,0,
				getContainerGraphicalRepresentation().getViewWidth(1),
		...
		for (ControlPoint cp : getShape().getControlPoints()) {
			Point currentCPInContainerView = convertLocalNormalizedPointToRemoteViewCoordinates(
					cp.getPoint(),
					getContainerGraphicalRepresentation(),
					1);
			FGEPoint initialCPInContainerView = new FGEPoint(
					(int)(currentCPInContainerView.x+initialLocation.x-getX()),
					(int)(currentCPInContainerView.y+initialLocation.y-getY()));
			FGEPoint desiredCPInContainerView = new FGEPoint(
					(int)(currentCPInContainerView.x+desiredLocation.x-getX()),
			...
			if (!containerViewArea.containsPoint(desiredCPInContainerView)) {
				...
				FGESegment segment = new FGESegment(
						initialCPInContainerView,
						desiredCPInContainerView);
				...
				if (intersection instanceof FGEPoint) {
					...
					if (Math.abs(desiredCPInContainerView.x-initialCPInContainerView.x) > FGEGeometricObject.EPSILON) {
						currentRatio = (intersect.x - initialCPInContainerView.x)/(desiredCPInContainerView.x-initialCPInContainerView.x)-FGEGeometricObject.EPSILON;
					}
					else if (Math.abs(desiredCPInContainerView.y-initialCPInContainerView.y) > FGEGeometricObject.EPSILON) {
						currentRatio = (intersect.y - initialCPInContainerView.y)/(desiredCPInContainerView.y-initialCPInContainerView.y)-FGEGeometricObject.EPSILON;
					}
					else {
		...
		if (logger.isLoggable(Level.FINE))
			logger.fine("getMoveAuthorizedRatio() initial="+initialLocation+" desired="+desiredLocation+" return "+returnedAuthorizedRatio);
		if (returnedAuthorizedRatio < 0) returnedAuthorizedRatio = 0;
	...
	public void setAdjustMinimalWidthToLabelWidth(
			boolean adjustMinimalWidthToLabelWidth)
	{
		FGENotification notification = requireChange(
				Parameters.adjustMinimalWidthToLabelWidth,
				adjustMinimalWidthToLabelWidth);
		if (notification != null) {
	...
	public void setAdjustMinimalHeightToLabelHeight(
			boolean adjustMinimalHeightToLabelHeight)
	{
		FGENotification notification = requireChange(
				Parameters.adjustMinimalHeightToLabelHeight,
				adjustMinimalHeightToLabelHeight);
		if (notification != null) {
	...
	public void setAdjustMaximalWidthToLabelWidth(
			boolean adjustMaximalWidthToLabelWidth)
	{
		FGENotification notification = requireChange(
				Parameters.adjustMaximalWidthToLabelWidth,
				adjustMaximalWidthToLabelWidth);
		if (notification != null) {
	...
	public void setAdjustMaximalHeightToLabelHeight(
			boolean adjustMaximalHeightToLabelHeight)
	{
		FGENotification notification = requireChange(
				Parameters.adjustMaximalHeightToLabelHeight,
				adjustMaximalHeightToLabelHeight);
		if (notification != null) {
	...
	{
		FGENotification notification = requireChange(Parameters.minimalWidth,
				minimalWidth);
		if (notification != null) {
	...
	{
		FGENotification notification = requireChange(Parameters.minimalHeight,
				minimalHeight);
		if (notification != null) {
	...
	{
		FGENotification notification = requireChange(Parameters.maximalHeight,
				maximalHeight);
		if (notification != null) {
	...
	{
		FGENotification notification = requireChange(Parameters.maximalWidth,
				maximalWidth);
		if (notification != null) {
	...
	{
		FGENotification notification = requireChange(Parameters.allowToLeaveBounds,
				allowToLeaveBounds);
		if (notification != null) {
	...
	public DimensionConstraints getDimensionConstraints()
	{
		...
		if (shape != null && shape.areDimensionConstrained()) {
			if (dimensionConstraints == DimensionConstraints.CONSTRAINED_DIMENSIONS) return DimensionConstraints.CONSTRAINED_DIMENSIONS;
			if (dimensionConstraints == DimensionConstraints.FREELY_RESIZABLE) return DimensionConstraints.CONSTRAINED_DIMENSIONS;
	...
	{
		FGENotification notification = requireChange(
				Parameters.dimensionConstraints, dimensionConstraints);
		if (notification != null && getShape() != null) {
	...
	{
		FGENotification notification = requireChange(
				Parameters.dimensionConstraintStep, dimensionConstraintStep);
		if (notification != null) {
	...
	/**
	 * Calling this method forces FGE to check (and eventually update)
	 * dimension of current graphical representation according defined
	...
	/**
	 * Return required width of shape, giving computed width of current label
	 * (usefull for auto-layout, when <pre>adjustMinimalWidthToLabelWidth</pre> is set to true).
	...
	/**
	 * Return required height of shape, giving computed height of current label
	 * (usefull for auto-layout, when <pre>adjustMinimalHeightToLabelHeight</pre> is set to true).
	 * Override this method to implement custom layout
	 * @param labelHeight
	 * @return
	 */
	...
	private void checkAndUpdateDimensionBoundsIfRequired()
	{
		...
		if (isCheckingDimensionConstraints) return;
		...
		try {
			...
			boolean useStepDimensionConstraints = getDimensionConstraints()==DimensionConstraints.STEP_CONSTRAINED && getDimensionConstraintStep()!=null;
	...
	public static BindingDefinition WIDTH_CONSTRAINTS = new BindingDefinition("widthConstraints", Double.class, BindingDefinitionType.GET, false);
	public static BindingDefinition HEIGHT_CONSTRAINTS = new BindingDefinition("heightConstraints", Double.class, BindingDefinitionType.GET, false);
	...
	{
		if (xConstraints == null) xConstraints = new DataBinding(this,Parameters.xConstraints,X_CONSTRAINTS);
		return xConstraints;
	}
	...
	{
		FGENotification notification = requireChange(
				Parameters.xConstraints, xConstraints);
		if (notification != null) {
	...
	{
		if (yConstraints == null) yConstraints = new DataBinding(this,Parameters.yConstraints,Y_CONSTRAINTS);
		return yConstraints;
	}
	...
	{
		FGENotification notification = requireChange(
				Parameters.yConstraints, yConstraints);
		if (notification != null) {
	...
	{
		if (widthConstraints == null) widthConstraints = new DataBinding(this,Parameters.widthConstraints,WIDTH_CONSTRAINTS);
	...
	{
		FGENotification notification = requireChange(
				Parameters.widthConstraints, widthConstraints);
		if (notification != null) {
	...
	{
		if (heightConstraints == null) heightConstraints = new DataBinding(this,Parameters.heightConstraints,HEIGHT_CONSTRAINTS);
	...
	{
		FGENotification notification = requireChange(
				Parameters.heightConstraints, heightConstraints);
		if (notification != null) {
	...
	protected void computeNewConstraint(ConstraintDependancy dependancy)
	{
		...
		else if (dependancy.requiringParameter == Parameters.heightConstraints && heightConstraints != null && heightConstraints.isValid()) {
	...
	public void constraintChanged(DataBinding constraint)
	{
		...
		else if (constraint.getBindingAttribute() == Parameters.widthConstraints && widthConstraints != null && widthConstraints.isValid()) {
			updateWidthPosition();
		}
		else if (constraint.getBindingAttribute() == Parameters.heightConstraints  && heightConstraints != null && heightConstraints.isValid()) {
			updateHeightPosition();
		}
	}
	...
	{
		FGENotification notification = requireChange(Parameters.shadowStyle,
				aShadowStyle);
		if (notification != null) {
			if (shadowStyle != null) shadowStyle.deleteObserver(this);
			this.shadowStyle = aShadowStyle;
			if (aShadowStyle != null) aShadowStyle.addObserver(this);
			hasChanged(notification);
		}
	}
	...
	public void notifyObjectMoved()
	{
		...
		if (mustNotify) notifyObjectWillMove();
		notifyObjectMoved(null);
		if (mustNotify) notifyObjectHasMoved();
	}
	...
	{
		if (getShape() != null) getShape().updateShape();
		setChanged();
	...
	/**
	 * Notify that the object resizing has finished
	 * (take care that this just notify END of resize, this
	 * should NOT be used to notify a resizing: use notifyObjectResize() instead)
	 */
	...
	{
		FGENotification notification = requireChange(
				Parameters.isFloatingLabel, isFloatingLabel);
		if (notification != null) {
	...
	public FGERectangle getBounds()
	{
		return new FGERectangle(
				getX(),getY(),
				getWidth()+(getBorder()!=null?getBorder().left+getBorder().right:0),
	...
	public void paint(Graphics g, DrawingController<?> controller)
	{
		...
		if (FGEConstants.DEBUG) {
			if (getBorder() != null) {
				...
				g2.drawRect(0,0,
						(getViewWidth(controller.getScale()))-1,
						(getViewHeight(controller.getScale()))-1);
				g2.setColor(Color.BLUE);
				g2.drawRect((int)(getBorder().left*controller.getScale()),
						(int)(getBorder().top*controller.getScale()),
						(int)(getWidth()*controller.getScale())-1,
						(int)(getHeight()*controller.getScale())-1);
			}
			else {
				g2.setColor(Color.BLUE);
		...
		if (shape != null && getShadowStyle() != null) {
			if (getShadowStyle().getDrawShadow()) shape.paintShadow(graphics);
	...
	/**
	 * Override this if you want to use this feature
	 * Default implementation return always false
	 * @return
	 */
	 ...
	 * return boolean indicating if drag was sucessfully performed
	 */
