...
public class FGESubstractionArea extends FGEOperationArea {
	...
	/**
	 * Build a new FGESubstractionArea given container area and substracted area.
	 * Really build this operation area without trying to compute or simplify it.
	 * 
	 * @param containerArea 	container area
	 * @param substractedArea	area to substract to container area
	...
	/**
	 * Build a new Area given container area and substracted area.
	 * Try to compute, and simplify resulting area
	 * 
	 * @param containerArea 	container area
	 * @param substractedArea	area to substract to container area
	 * @param isStrict 			boolean indicating if a point located in the border
	 ...
	 *              			of substracted area should be consider inside resulting area or not
	 *               			If <pre>isStrict</pre> is true, point should NOT be considered.
	 * @param checkNonNullIntersection indicates if non-null intersection should be checked (take care about infinite loop !)
	 * @return
	 */
	protected static FGEArea makeSubstraction(FGEArea containerArea, FGEArea substractedArea, boolean isStrict, boolean checkNonNullIntersection)
	{
		if (containerArea instanceof FGEEmptyArea) return new FGEEmptyArea();
		if (substractedArea instanceof FGEEmptyArea) return containerArea.clone();
		if (checkNonNullIntersection && (containerArea.intersect(substractedArea) instanceof FGEEmptyArea)) return containerArea.clone();
		if (substractedArea.containsArea(containerArea)) return new FGEEmptyArea();
	...
	public boolean containsPoint(FGEPoint p)
	{
		...
		&& (!substractedArea.containsPoint(p) 
				|| (!isStrict() && isPointLocatedOnSubstractedAreaBorder(p)));
	}
	...
	{
		if (!substractedArea.containsPoint(testPoint)) return false;
	...
	public FGEPoint getNearestPoint(FGEPoint aPoint)
	{
		...
		if (!substractedArea.containsPoint(returned)) return returned;
		...
		if (substractedArea instanceof FGEShape && !isStrict()) {
			...
			if (containsPoint(outlinePoint)) return outlinePoint;
		}
	...
	public boolean equals(Object obj)
	{
		if (obj instanceof FGESubstractionArea) {
			...
			return sub.getContainerArea().equals(getContainerArea()) 
			&& sub.getSubstractedArea().equals(getSubstractedArea())
	...
	/**
	 * Return a boolean indicating if a point located in the border of substracted 
	 * area should be consider inside resulting area or not. If <pre>isStrict</pre> is true,
