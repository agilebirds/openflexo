...
public class FGEShapeGraphicsImpl extends FGEGraphicsImpl implements FGEShapeGraphics {
	...
	private static final Logger logger = Logger.getLogger(FGEShapeGraphicsImpl.class.getPackage().getName());

	public FGEShapeGraphicsImpl(ShapeNode<?> node) {
		super(node);
	}
	...
	protected void applyCurrentBackgroundStyle() {
		...
		if (currentBackground instanceof BackgroundImageBackgroundStyle
				&& ((BackgroundImageBackgroundStyle) currentBackground).getFitToShape()) {
