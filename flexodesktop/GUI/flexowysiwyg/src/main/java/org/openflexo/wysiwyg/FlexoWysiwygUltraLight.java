...
public abstract class FlexoWysiwygUltraLight extends FlexoWysiwygLight {
	...
	/**
	 * Creates the ultra light" version of wysiwyg component 
	 * with only one toolbar and a set of the 10 most useful options.
	 * This class must implement <code>textChanged(String htmlText)</code> to be concrete.
	 * @param isUltraLight if true, will create ultra light version of the wysiwyg.
	 */
	...
	public FlexoWysiwygUltraLight(String htmlContent, boolean isViewSourceAvailable) {
		...
		for (Component c : editingToolbar.getComponents()) {
			if (c instanceof JButton) {
				...
				if (currentIcon.getToolTipText().equals("undo") 
				 || currentIcon.getToolTipText().equals("redo")
		...
		{
			JButton switchViewButton = createMenuButton(getFormattingToolBar(), "switch view", "switchView", getSharedIcon("page-properties"));
	...
	public void switchView() {
		...
		if (isSourceEditorVisible()) {// switch to classic view
			for (Component c : getMainTabbedPane().getComponents()) { //...get the editor tab content by browsing the content of the mainTabbedPane
			...
			for (Component c : getMainTabbedPane().getComponents()) { //...get the source html tab content by browsing the content of the mainTabbedPane
