...
public class QuadCurveGraphicalRepresentation extends GeometricObjectGraphicalRepresentation<FGEQuadCurve,QuadCurve> implements XMLSerializable
{
	// Called for LOAD
	...
	protected List<ControlPoint> buildControlPointsForCurve(FGEQuadCurve curve)
	{
		...
		if (curveConstruction instanceof QuadCurveWithThreePointsConstruction) {
			...
			if (startPointConstruction instanceof ExplicitPointConstruction) {
				returned.add(new DraggableControlPoint<FGEQuadCurve>(this,"p1",curve.getP1(),(ExplicitPointConstruction)startPointConstruction) {
					@Override
			...
			if (controlPointConstruction instanceof ExplicitPointConstruction) {
				returned.add(new DraggableControlPoint<FGEQuadCurve>(this,"cp",curve.getCtrlPoint(),(ExplicitPointConstruction)controlPointConstruction) {
					@Override
					public boolean dragToPoint(FGEPoint newRelativePoint, FGEPoint pointRelativeToInitialConfiguration, FGEPoint newAbsolutePoint, FGEPoint initialPoint, MouseEvent event)
					{
			...
			if (endPointConstruction instanceof ExplicitPointConstruction) {
				returned.add(new DraggableControlPoint<FGEQuadCurve>(this,"p2",curve.getP2(),(ExplicitPointConstruction)endPointConstruction) {
					@Override
					public boolean dragToPoint(FGEPoint newRelativePoint, FGEPoint pointRelativeToInitialConfiguration, FGEPoint newAbsolutePoint, FGEPoint initialPoint, MouseEvent event)
					{
