...
public class TestFGEDeserializationAndDisplay {

	public static EventProcessor eventProcessor;
	public static DrawingEditorFactory FACTORY;
		...
		try {
			SwingUtilities.invokeAndWait(new Runnable() {
				...
				public void run() {
					...
					JFrame frame = new JFrame("TestFGEDeserializationAndDisplay");
					frame.setLayout(new BorderLayout());
					frame.setSize(new Dimension(1024, 768));
					frame.setLocationRelativeTo(null);
					JButton myButton;
					frame.add(myButton = new JButton("I take the control"));
	...
	private MyDrawing deserialize(File file) {
		final MyDrawing drawing = MyDrawingImpl.load(file, FACTORY);
		System.out.println("Deserialized:");
		System.out.println(FACTORY.getSerializer().serializeAsString(drawing));
		try {
			SwingUtilities.invokeAndWait(new Runnable() {
				@Override
				public void run() {
					tabbedPane.add(drawing.getTitle(), new JScrollPane(drawing.getEditedDrawing().getController().getDrawingView()));
	...
	public static void waitGUI() {
		if (dontDestroyMe) {
			while (true) {
				try {
					synchronized (TestFGEDeserializationAndDisplay.class) {
						TestFGEDeserializationAndDisplay.class.wait();
					}
				} catch (InterruptedException e) {
					e.printStackTrace();
				}
