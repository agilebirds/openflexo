...
public class FGEUnionArea extends FGEOperationArea {
	...
	private static FGEArea makeUnion(List<? extends FGEArea> objects, boolean tryToReduceUnionByConcatenation)
	{
		...
		if (tryToReduceUnionByConcatenation) {
			...
			if (concatenedObjects.size() == 0) return new FGEEmptyArea();
			else if (concatenedObjects.size() == 1) return concatenedObjects.get(0).clone();
		...
		for (int i=0; i<objects.size(); i++) {
			...
			if (o instanceof FGEEmptyArea) {
				...
				for (FGEArea a : objectsToTakeUnderAccount) {
					if (a.containsArea(o)) isAlreadyContained = true;
				}
				if (!isAlreadyContained) {
					Vector<FGEArea> noMoreNecessaryObjects = new Vector<FGEArea>();
					for (FGEArea a2 : objectsToTakeUnderAccount) {
						if (o.containsArea(a2)) noMoreNecessaryObjects.add(a2);
					}
					for (FGEArea removeThat : noMoreNecessaryObjects) objectsToTakeUnderAccount.remove(removeThat);
	...
	public boolean containsPoint(FGEPoint p)
	{
		for (FGEArea a : _objects) {
			if (a.containsPoint(p)) return true;
		}
		return false;
	}
	...
	public boolean containsLine(FGEAbstractLine l)
	{
		...
		for (FGEArea a : _objects) {
			if (a.containsLine(l)) return true;
		}
		return false;
	}
	...
	public boolean containsPolylin(FGEPolylin p)
	{
		for (FGESegment s : p.getSegments()) {
			if (!containsLine(s)) return false;
		}
		return true;
	}
	...
	{
		for (FGEArea a : _objects) a.paint(g);
	}
	
	public void addArea(FGEArea area)
	{
		if (area instanceof FGEEmptyArea) return;
		
		if (containsArea(area)) return;
		...
		for (FGEArea a : _objects) {
			if (area.containsArea(a)) uselessObjects.add(a);
		}
		
		for (FGEArea a : uselessObjects) _objects.remove(a);
	...
	public boolean containsArea(FGEArea a)
	{
		for (FGEArea o : _objects) {
			if (o.containsArea(a)) return true;
		}
		...
		if (a instanceof FGEPolylin) return containsPolylin((FGEPolylin)a);
		if (a instanceof FGEPolygon) return containsPolygon((FGEPolygon)a);
	...
	public <T> boolean isUnionOf(Class<T> aClass) 
	{
		...
		for (FGEArea a : _objects) {
			if (!aClass.isAssignableFrom(a.getClass())) return false;
		}
		return true;
	}
	...
	public boolean equals(Object obj)
	{
		if (obj instanceof FGEUnionArea) {
			...
			if (getObjects().size() != u.getObjects().size()) return false;
				for (int i=0; i<getObjects().size(); i++) {
					FGEArea a = getObjects().get(i);
					// Equals even if not same order
					if (u.getObjects().indexOf(a) == -1) return false;
			}
			return true;
		}
	...
	public FGEArea getAnchorAreaFrom(SimplifiedCardinalDirection direction)
	{
		if (isUnionOfSegments()) {
			...
			for (FGEArea s : getObjects()) segments.add((FGESegment)s);
	 ...
	 * An union area is finite if and only if all areas are finite
	 * @return
	 */
	@Override
