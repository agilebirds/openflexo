...
public class AdjustableIntermediateControlPoint extends RectPolylinAdjustableControlPoint
{
	...
	/**
	 * This method is internally called to notify that intermediate control point at
	 * supplied index has just been moved.
	 *
	 * Previous and next point are updated accordingly
	 *
	 * Return boolean indicating if dragging lead to major layout modification
	 * where current dragged control point was suppressed:
	 * - When true: control point still exist
	 * - When false: control point has disappeared and dragging should stop
	 *
	 * @param index
	 */
	private boolean movedIntermediateCP(int index, FGEPoint oldCPLocation, boolean simplifyLayout)
	{
		if (RectPolylinConnector.logger.isLoggable(Level.FINE)) RectPolylinConnector.logger.fine("Moved intermediate point at index: "+index);
		...
		if (simplifyLayout) {
			...
			if (!intermediateCPStartSegmentHalfPlane.containsPoint(newCPLocation)) {
				if (logger.isLoggable(Level.INFO)) logger.info("Two points will be removed (pattern 1) at index="+(index-1));
			...
			if (index > 2) {
				...
				FGEHalfPlane intermediateCPBeforeStartSegmentHalfPlane = new FGEHalfPlane(intermediateCPBeforeStartSegment,previousCPOldLocation);
				if (!intermediateCPBeforeStartSegmentHalfPlane.containsPoint(previousCPNewLocation)) {
					if (logger.isLoggable(Level.INFO)) logger.info("Two points will be removed (pattern 2) at index="+(index-2));
		...
		if (simplifyLayout) {
			...
			if (!intermediateCPEndSegmentHalfPlane.containsPoint(newCPLocation)) {
				if (logger.isLoggable(Level.INFO)) logger.info("Two points will be removed (pattern 3) at index="+index);
			...
			if (getPolylin().getSegmentNb() > index+2) {
				...
				if (!intermediateCPAfterEndSegmentHalfPlane.containsPoint(nextCPNewLocation)) {
					if (logger.isLoggable(Level.INFO)) logger.info("Two points will be removed (pattern 4) at index="+(index+1));
					getConnector()._simplifyLayoutOfCurrentPolylinByDeletingTwoPoints(index+1, newCPLocation);
					/*
				getPolylin().removePointAtIndex(index+1);
				getPolylin().removePointAtIndex(index+1);
