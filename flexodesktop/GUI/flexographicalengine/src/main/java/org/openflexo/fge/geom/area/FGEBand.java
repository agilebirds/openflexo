...
public class FGEBand implements FGEArea {
	...
	public FGEBand(FGELine aLine1, FGELine aLine2) 
	{
		...
		if (!line1.isParallelTo(line2)) throw new IllegalArgumentException("lines are not parallel");
		computeTestPoint();
	}
	...
	protected void computeTestPoint()
	{
		...
		while (!found) {
			...
			if (!line1.contains(randPoint) && !line2.contains(randPoint)) found = true;
		}
		FGESegment segment = new FGESegment(
				line1.getProjection(randPoint),
				line2.getProjection(randPoint));
	...
	{
		if (line1.contains(p)) return true;
		if (line2.contains(p)) return true;
	...
	{
		if (!containsLineIgnoreBounds(l)) return false;
		
		if (l instanceof FGEHalfLine) return l.isParallelTo(line1);
		if (l instanceof FGESegment) return true;
		if (l instanceof FGELine) return l.isParallelTo(line1);
	...
	public boolean containsArea(FGEArea a)
	{
		...
		if (a instanceof FGEHalfBand) return containsLine(((FGEHalfBand)a).halfLine1) && containsLine(((FGEHalfBand)a).halfLine2);
		if (a instanceof FGEBand) return containsLine(((FGEBand)a).line1) && containsLine(((FGEBand)a).line2);
		return false;
	}
	...
	public FGEArea intersect(FGEArea area)
	{
		...
		if (area instanceof FGEPolygon) return area.intersect(this);
		if (area instanceof FGEArc) return area.intersect(this);
	...
	public FGEArea transform(AffineTransform t)
	{
		...
		if (l1.overlap(l2)) return l1;
		else return new FGEBand(l1,l2);
	}
	...
	{
		if (hp.containsArea(this)) return clone();
		...
		if (hp.line.isParallelTo(line1)) {
			...
			try {
				...
				if (!hp.line.containsPoint(line1.getP1())) {
					...
					else pp1 = FGEAbstractLine.getOppositePoint(line1.getP1(), i1);
				}			
				else {
					if (hp.containsPoint(line1.getP2())) {
						pp1 = line1.getP2();
					}
					else pp1 = FGEAbstractLine.getOppositePoint(line1.getP2(), i1);
				}
				...
				if (!hp.line.containsPoint(line2.getP1())) {
					...
					else pp2 = FGEAbstractLine.getOppositePoint(line2.getP1(), i2);
				}
				else {
					if (hp.containsPoint(line2.getP2())) {
						pp2 = line2.getP2();
					}
					else pp2 = FGEAbstractLine.getOppositePoint(line2.getP2(), i2);
				}
			
				FGEHalfLine hl1 = new FGEHalfLine(i1,pp1);
				FGEHalfLine hl2 = new FGEHalfLine(i2,pp2);
				
				if (hl1.overlap(hl2)) return new FGEEmptyArea();
				else return new FGEHalfBand(hl1,hl2);
	...
	protected FGEArea computeBandIntersection(FGEBand band)
	{
		...
		if (!line1.isParallelTo(band.line1)) {
			try {
				...
				else returned = FGEPolygon.makeArea(
						Filling.FILLED,
						line1.getLineIntersection(band.line1),
						line1.getLineIntersection(band.line2),
						line2.getLineIntersection(band.line2),
		...
		if (returned == null) return new FGEEmptyArea();
	...
	private FGEArea computeLineIntersection(FGEAbstractLine aLine)
	{
		...
		if (a1 instanceof FGEEmptyArea) return a1;
	...
	public FGEPoint getNearestPoint(FGEPoint aPoint)
	{
		...
		else return FGEPoint.getNearestPoint(aPoint, line1.getProjection(aPoint), line2.getProjection(aPoint));
	}
	...
	public boolean equals(Object obj)
	{
		if (obj instanceof FGEBand) {
			...
			return ((line1.overlap(b.line1) || line1.overlap(b.line2)) 
					&& (line2.overlap(b.line1) || line2.overlap(b.line2)));
		}
		return false;
	}
	...
	public FGEArea getOrthogonalPerspectiveArea(SimplifiedCardinalDirection orientation)
	{
		if (orientation == SimplifiedCardinalDirection.NORTH) {
			if (line1.isVertical()) return clone(); 
			else return FGEAbstractLine.getNorthernLine(line1,line2);
		}
		else if (orientation == SimplifiedCardinalDirection.SOUTH) {
			if (line1.isVertical()) return clone(); 
			else return FGEAbstractLine.getSouthernLine(line1,line2);
		}
		else if (orientation == SimplifiedCardinalDirection.EAST) {
			if (line1.isHorizontal()) return clone(); 
			else return FGEAbstractLine.getEasternLine(line1,line2);
		}
		else if (orientation == SimplifiedCardinalDirection.WEST) {
			if (line1.isHorizontal()) return clone(); 
			else return FGEAbstractLine.getWesternLine(line1,line2);
		}
		return null;
	}
