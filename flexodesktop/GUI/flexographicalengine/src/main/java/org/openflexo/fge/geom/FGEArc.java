...
public class FGEArc extends Arc2D.Double implements FGEGeometricObject<FGEArc>, FGEShape<FGEArc> {
	 ...
	 * @param w
	 * @param h
	 * @param start angle in degree
	 * @param extent angle in degree
	 * @param type
	 */
	public FGEArc(double x, double y, double w, double h,
			double start, double extent, ArcType type) 
	{
			...
			double start, double extent) 
	{
		...
		if (extent >= 360) logger.warning("Arc created instead of ellips or circle (extent >=360) ");
	}
	...
	{
		if (!filled) setFGEArcType(ArcType.OPEN);
		else setFGEArcType(ArcType.PIE);
	}
	...
	{
		if (getArcType() == ArcType.OPEN.ordinal()) return  ArcType.OPEN;
		else if (getArcType() == ArcType.CHORD.ordinal()) return  ArcType.CHORD;
		else if (getArcType() == ArcType.PIE.ordinal()) return  ArcType.PIE;
		return null;
	}

	public void setFGEArcType(ArcType arcType)
	{
		if (arcType == ArcType.OPEN) setArcType(ArcType.OPEN.ordinal());
		else if (arcType == ArcType.CHORD) setArcType(ArcType.CHORD.ordinal());
		else if (arcType == ArcType.PIE) setArcType(ArcType.PIE.ordinal());
	}
	...
	public static void main(String[] args)
	{
		...
		FGEHalfLine hl = new FGEHalfLine(new FGEPoint(0.11718749999999999,0.22277227722772278),new FGEPoint(0.11718749999999999,1.2227722772277227));
	...
	/**
	 * Returns the angular extent of the arc, expressed in radians,
	...
	/**
	 * Returns the starting angle of the arc expressed in radians,
	 * in the range -PI / PI
	...
	/**
	 * Returns the ending angle of the arc expressed in radians,
	 * in the range -PI / PI
	 * @return a double value that represents the ending angle 
	 * of the arc in degrees.
	 */
	...
	/**
	 * Returns the supplied angle of the arc expressed in radians,
	 * in the range -PI / PI
	...
	{
		while (radianAngle <= -Math.PI) radianAngle += 2*Math.PI;
		while (radianAngle > Math.PI) radianAngle -= 2*Math.PI;
		return radianAngle;
	}
	 ...
	 * 
	 * @return A <CODE>FGEPoint</CODE> object representing the 
	...
	/**
	 * Returns boolean indicating if supplied angle (expressed in radians)
	 * in contained in this arc
	 * 
	 * @param radianAngle
	 * @return
	 */
	...
	{
		while (radianAngle < -Math.PI-EPSILON) radianAngle += 2*Math.PI;
		while (radianAngle > Math.PI+EPSILON) radianAngle -= 2*Math.PI;
		...
		if (angle_extent > 0) {
			return ((radianAngle >= angle_start-EPSILON && radianAngle <= angle_start+angle_extent+EPSILON)
					|| (radianAngle+2*Math.PI >= angle_start-EPSILON && radianAngle+2*Math.PI <= angle_start+angle_extent+EPSILON)
					|| (radianAngle-2*Math.PI >= angle_start-EPSILON && radianAngle-2*Math.PI <= angle_start+angle_extent+EPSILON));
		}
	...
	private FGEArea computeLineIntersection(FGELine line)
	{
		...
		if (result.solutionType == org.openflexo.fge.geom.FGEArc.LineIntersectionResult.SolutionType.NO_SOLUTION) {
			...
			// One solution, with angle alpha
			return new FGEPoint(width/2.0*Math.cos(result.alpha1)+x+width/2.0,height/2.0*Math.sin(result.alpha1)+y+height/2.0);
		}

		else if (result.solutionType == org.openflexo.fge.geom.FGEArc.LineIntersectionResult.SolutionType.TWO_SOLUTIONS) {
			FGEPoint p1 = new FGEPoint(width/2.0*Math.cos(-result.alpha1)+x+width/2.0,height/2.0*Math.sin(-result.alpha1)+y+height/2.0);
			FGEPoint p2 = new FGEPoint(width/2.0*Math.cos(-result.alpha2)+x+width/2.0,height/2.0*Math.sin(-result.alpha2)+y+height/2.0);
			...
			if (includeP1) {
				if (getFGEArcType() == ArcType.OPEN) {
					...
					logger.warning("Unexpected situation encoutered here: arc="+this+" line="+line+" pp1="+pp1+" pp2="+pp2+" p1="+p1+" p2="+p2);
				}				
			}
			else if (includeP2) {
				if (getFGEArcType() == ArcType.OPEN) {
	...
	public FGEPoint nearestOutlinePoint(FGEPoint aPoint) 
	{
		...
		if (!includeAngle(angle)) {
			...
			FGEPoint returned = FGEPoint.getNearestPoint(aPoint,
					getPointAtRadianAngle(getRadianAngleStart()),
		...
		if (getFGEArcType() == ArcType.OPEN
				|| getFGEArcType() == ArcType.CHORD) {
			if (FGEPoint.distanceSq(aPoint,getStartPoint())
					< FGEPoint.distanceSq(aPoint,getEndPoint()))
				return getStartPoint();
			else return getEndPoint();
		}
		...
		// NOTE: CHORD not well handled here: also handle segment [getStartPoint();getEndPoint()]
			...
			if (FGEPoint.distanceSq(aPoint,pp1)
					< FGEPoint.distanceSq(aPoint,pp2))
				return pp1;
			else return pp2;
		}
	}
	...
	public FGEPoint nearestPointFrom(FGEPoint from, SimplifiedCardinalDirection orientation) 
	{
		...
		if (intersect instanceof FGEEmptyArea) return null;

		if (intersect instanceof FGEPoint) return (FGEPoint)intersect;
	...
	public FGEArea intersect(FGEArea area)
	{
		...
		if (area instanceof FGEAbstractLine) return computeIntersection((FGEAbstractLine)area);
	...
	{
		if (!getFGEBounds().containsPoint(p)) return false;
		
		double radianAngle = angleForPoint(p);
		
		if (!includeAngle(radianAngle)) return false;
		...
		if (getFGEArcType() == ArcType.OPEN) {
			...
			if (getFGEArcType() == ArcType.PIE) return true;
	...
	{
		return (containsPoint(l.getP1()) 
				&& containsPoint(l.getP2())
				&& (l instanceof FGESegment));
	}
	...
	public boolean containsArea(FGEArea a)
	{
		if (a instanceof FGEArc && ((FGEArc)a).overlap(this)) {
			...
			if (arc1.containsArea(arc2)) return true;
	...
	public String toString()
	{
		return "FGEArc: ("+x+","+y+","+width+","+height+","+getAngleStart()+","+getAngleExtent()+" type="+getFGEArcType()+")";
	}
	...
	public FGERectangle getBoundingBox()
	{
		...
		if (includeAngle(0)) angles.add(0.0);
		if (includeAngle(Math.PI/2)) angles.add(Math.PI/2);
		if (includeAngle(Math.PI)) angles.add(Math.PI);
		if (includeAngle(3*Math.PI/2)) angles.add(3*Math.PI/2);
	...
	public boolean overlap(FGEArc arc)
	{
		...
		return ((Math.abs(getX()-arc.getX()) <= EPSILON) &&
				(Math.abs(getY()-arc.getY()) <= EPSILON) &&
				(Math.abs(getWidth()-arc.getWidth()) <= EPSILON) &&
				(Math.abs(getHeight()-arc.getHeight()) <= EPSILON));
	}

	@Override
	public final FGEArea getOrthogonalPerspectiveArea(SimplifiedCardinalDirection orientation)
	{
		if (orientation == null) return new FGEEmptyArea();
		FGERectangle boundingBox;
		FGEArea anchorArea = getAnchorAreaFrom(orientation);
		if (anchorArea instanceof FGEArc) boundingBox = ((FGEArc)anchorArea).getBoundingBox();
		else boundingBox = getBoundingBox();
	 ...
	 * 
	 * @param startAngle, expressed in radians
	 * @param endAngle, expressed in radians
	...
	public static double angleExtent(double startAngle, double endAngle)
	{
		...
		else return end+2*Math.PI-start;
	}
	
	/**
	 * Compute an equivalent for an arc in the 1-D model
	 * Angles are normalized and a segment (or a union of 2
	 * segments mapping 2.PI-modulo is retrieved )
	 * 
	 * @param anArc
	 * @return
	 */
	private static FGEArea _equivalentForArcIn1D(FGEArc anArc, boolean forIntersection)
	{
		...
		if (anArc instanceof FGEEllips 
				|| anArc.getRadianAngleExtent() >= 2*Math.PI
				|| anArc.getRadianAngleExtent() <= -2*Math.PI) {
			return new FGEUnionArea(
					new FGESegment(new FGEPoint(-Math.PI,0),new FGEPoint(Math.PI,0)),
					new FGESegment(new FGEPoint(Math.PI,0),new FGEPoint(3*Math.PI,0)));
		}
		...
		if (anArc.includeAngle(Math.PI)) {
			if (forIntersection) {
				return new FGEUnionArea(segment,
						new FGESegment(new FGEPoint(startAngle+2*Math.PI,0),new FGEPoint(endAngle+2*Math.PI,0)));
			}
		}
	...
	/**
	 * Compute an equivalent for an area in the 1-D model
	 * to arc model
	 * 
	 * @param anArc
	 * @return
	...
	{
		if (anArea instanceof FGEEmptyArea) return new FGEEmptyArea();
		else if (anArea instanceof FGESegment) {
			FGESegment s = (FGESegment)anArea;
			if (s.getP2().x>=s.getP1().x+2*Math.PI
					|| s.getP2().x<=s.getP1().x-2*Math.PI) {
				return new FGEEllips(x,y,width,height,getIsFilled()?Filling.FILLED:Filling.NOT_FILLED);
			}		
			return new FGEArc(x,y,width,height,
					Math.toDegrees(normalizeRadianAngle(s.getP1().x)),
					Math.toDegrees(s.getP2().x-s.getP1().x),
					getFGEArcType());
		}
			...
			//if (u.isUnionOfSegments() || u.isUnionOfPoints()) {
				...
				if (returned.size() == 1) return returned.firstElement();
				if (returned.size() == 2 
						&& returned.firstElement() instanceof FGEArc
						&& returned.elementAt(1) instanceof FGEArc) {
					...
					if (arc1EndToPI && arc2StartFromPI) {
						return _equivalentToArcIn1D(
								new FGESegment(new FGEPoint(arc1.getRadianAngleStart(),0),new FGEPoint(arc2.getRadianAngleEnd()+2*Math.PI,0)));
					}
					if (arc2EndToPI && arc1StartFromPI) {
						return _equivalentToArcIn1D(
								new FGESegment(new FGEPoint(arc2.getRadianAngleStart(),0),new FGEPoint(arc1.getRadianAngleEnd()+2*Math.PI,0)));
					}
				}
				...
				if (union.getObjects().size() == 1) return union.getObjects().firstElement();
				return union;
			//}
		}
	...
	/**
	 * Compute arc union, asserting that this arc and supplied arc
	 * differs only because of their start angles and/or angle extent
	 * 
	 * @param anArc
	 * @return
	 */
	protected FGEArea computeArcUnion(FGEArc anArc)
	{
		if (anArc instanceof FGEEllips 
				|| anArc.getRadianAngleExtent() >= 2*Math.PI
				|| anArc.getRadianAngleExtent() <= -2*Math.PI
				|| this instanceof FGEEllips 
				|| getRadianAngleExtent() >= 2*Math.PI
				|| getRadianAngleExtent() <= -2*Math.PI) {
			// Trivial case
			return new FGEEllips(x,y,width,height,getIsFilled()?Filling.FILLED:Filling.NOT_FILLED);
		}
	...
	/**
	 * Compute arc intersection, asserting that this arc and supplied arc
	 * differs only because of their start angles and/or angle extent
	 * 
	 * @param anArc
	 * @return
	...
	{
		return new FGEPoint(
				getCenter().x+getWidth()/2*Math.cos(radianAngle),
	...
	public double angleForPoint(FGEPoint p)
	{
		...
		if (result.solutionType == org.openflexo.fge.geom.FGEArc.LineIntersectionResult.SolutionType.NO_SOLUTION) {
			...
			FGEPoint p1 = new FGEPoint(width/2.0*Math.cos(-result.alpha1)+x+width/2.0,height/2.0*Math.sin(-result.alpha1)+y+height/2.0);
			FGEPoint p2 = new FGEPoint(width/2.0*Math.cos(-result.alpha2)+x+width/2.0,height/2.0*Math.sin(-result.alpha2)+y+height/2.0);
			if (hl.containsPoint(p1)) return result.alpha1;
			else /*if (hl.containsPoint(p2))*/ return result.alpha2;
		}
