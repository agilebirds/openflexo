...
public class FGERoundRectangle extends RoundRectangle2D.Double implements FGEGeometricObject<FGERoundRectangle>, FGEShape<FGERoundRectangle> {
	...
	private FGEArea computeRectangleIntersection(FGERoundRectangle rect)
	{
		...
		else if (pts.size() == 2) return new FGESegment(pts.firstElement(),pts.elementAt(1));
	...
	private FGEArea computeHalfPlaneIntersection(FGEHalfPlane hp)
	{
		...
		if (computeLineIntersection(hp.line) instanceof FGEEmptyArea) {
			...
			if (getIsFilled()) return FGEPolygon.makeArea(Filling.FILLED,pts);
			else return new FGEUnionArea(pts);
		}
	}
	...
	private FGEArea computeLineIntersection(FGEAbstractLine line)
	{
		...
		if (returned instanceof FGEEmptyArea) {
			...
			if (getNorthEastRoundBounds().containsPoint(p)) return getNorthEastRound().intersect(line);
			if (getSouthEastRoundBounds().containsPoint(p)) return getSouthEastRound().intersect(line);
			if (getNorthWestRoundBounds().containsPoint(p)) return getNorthWestRound().intersect(line);
			if (getSouthWestRoundBounds().containsPoint(p)) return getSouthWestRound().intersect(line);
			 return new FGEEmptyArea();
		}
		else {
			FGEPoint p1;
			...
			if (returned instanceof FGEUnionArea && ((FGEUnionArea)returned).isUnionOfPoints()) {
				...
				if (containsPoint(p1) && containsPoint(p2)) return returned;
			}
			...
			FGEArea newP1 = p1;
			FGEArea newP2 = p2;
			if (getNorthEastRoundBounds().containsPoint(p1)) newP1 = getNorthEastRound().intersect(line);
			if (getSouthEastRoundBounds().containsPoint(p1)) newP1 = getSouthEastRound().intersect(line);
			if (getNorthWestRoundBounds().containsPoint(p1)) newP1 = getNorthWestRound().intersect(line);
			if (getSouthWestRoundBounds().containsPoint(p1)) newP1 = getSouthWestRound().intersect(line);
			if (getNorthEastRoundBounds().containsPoint(p2)) newP2 = getNorthEastRound().intersect(line);
			if (getSouthEastRoundBounds().containsPoint(p2)) newP2 = getSouthEastRound().intersect(line);
			if (getNorthWestRoundBounds().containsPoint(p2)) newP2 = getNorthWestRound().intersect(line);
			if (getSouthWestRoundBounds().containsPoint(p2)) newP2 = getSouthWestRound().intersect(line);

			if (newP1 instanceof FGEPoint) {
				if (newP2 instanceof FGEPoint) {
					if (returned instanceof FGEUnionArea) return new FGEUnionArea(newP1,newP2);
					else if (returned instanceof FGESegment) return new FGESegment((FGEPoint)newP1,(FGEPoint)newP2);
				}
				else if (newP2 instanceof FGEEmptyArea) return newP1;
			}
			else if (newP1 instanceof FGEEmptyArea) {
				if (newP2 instanceof FGEPoint) return newP2;
				else return new FGEEmptyArea();
			}
			else if (newP1 instanceof FGEUnionArea
					&& ((FGEUnionArea)newP1).isUnionOfPoints()
					&& newP2 instanceof FGEUnionArea
					&& ((FGEUnionArea)newP2).isUnionOfPoints()
					&& newP1.equals(newP2)) {
				return newP1;
			}
	...
	public FGEArea intersect(FGEArea area)
	{
		...
		if (area instanceof FGERoundRectangle) return computeRectangleIntersection((FGERoundRectangle)area);
	...
	public FGEArea transform(AffineTransform t)
	{
		...
		return new FGERoundRectangle(
				...
				Math.abs(p1.x-p2.x),
				Math.abs(p1.y-p2.y),
				arcwidth*t.getScaleX(),archeight*t.getScaleY(),
				_filling);
	}
