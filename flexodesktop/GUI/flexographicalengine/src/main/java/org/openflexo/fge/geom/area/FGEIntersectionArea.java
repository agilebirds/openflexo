...
public class FGEIntersectionArea extends FGEOperationArea {
	...
	public static FGEArea makeIntersection(List<? extends FGEArea> objects)
	{
		...
		for (int i=0; i<objects.size(); i++) {
			...
			if (a1 instanceof FGEEmptyArea) return new FGEEmptyArea();
			...
			if (!isAlreadyContained) {
				...
				for (FGEArea a2 : nonEmbeddedObjects) {
					if (a1.containsArea(a2)) noMoreNecessaryObjects.add(a2);
				}
				for (FGEArea removeThat : noMoreNecessaryObjects) nonEmbeddedObjects.remove(removeThat);
	...
	public void setObjects(Vector<FGEArea> objects)
	{
		...
		else _objects = new Vector<FGEArea>();
		for (FGEArea o : objects) {
			_objects.add(o.clone());
		}
	}
	...
	public boolean containsPoint(FGEPoint p)
	{
		...
		for (FGEArea a : _objects) {
			if (!a.containsPoint(p)) return false;
		}
		return true;
	}
	...
	public boolean containsLine(FGEAbstractLine l)
	{
		if (_objects.size() == 0) return false;
		for (FGEArea a : _objects) {
			if (!a.containsLine(l)) return false;
		}
		return true;
	}
	...
	public FGEArea transform(AffineTransform t)
	{
		...
		for (int i=0; i<_objects.size(); i++) all[i] = _objects.get(i).transform(t);
	...
	public FGEPoint getNearestPoint(FGEPoint aPoint)
	{
		...
		for (int i=0; i<getObjects().size(); i++) {
			...
			if (tryThis != null) potentialPoints.add(tryThis);
		}

		if (potentialPoints.size() == 0) {
			logger.warning("getNearestPoint() called for "+this+": Not implemented yet (tried to compute heuristic, but failing to obtain a result)");
			return null;
		}
	...
	/**
	 * Little heuristic to find nearest point for a formal intersection
	 * (not working in all cases !)
	 * 
	 * @param aPoint
	...
	private FGEPoint _getApproximatedNearestPoint(FGEPoint aPoint, int firstTriedObjectIndex)
	{
		...
		while (!containsPoint(returned) && tries < MAX_TRIES) {
			...
			if (firstTriedObjectIndex >= getObjects().size()) firstTriedObjectIndex = 0;
	...
	public boolean equals(Object obj)
	{
		if (obj instanceof FGEIntersectionArea) {
			...
			if (getObjects().size() != inters.getObjects().size()) return false;
			for (int i=0; i<getObjects().size(); i++) {
				FGEArea a = getObjects().get(i);
				// Equals even if not same order
				if (inters.getObjects().indexOf(a) == -1) return false;
			}
			return true;
		}
	 ...
	 * An union area is finite if and only if at least on area
	 * is finite
	 * @return
	 */
	@Override
	 ...
	 * For non-finite areas (if this area contains a least one non-finite area), return null
	 * 
	 * @return
	 */
	@Override
	...
	{
		if (getObjects().size() <= 1) return false;
	...
	private static FGEUnionArea _findFirstUnionArea(FGEIntersectionArea intersection)
	{
		for (FGEArea o : intersection.getObjects()) {
			if (o instanceof FGEUnionArea) return (FGEUnionArea)o;
		}
	...
	{
		for (FGEArea o : intersection.getObjects()) {
			if (o instanceof FGESubstractionArea) return (FGESubstractionArea)o;
		}
	...
	public FGEArea makeDevelopped()
	{
		...
		if (returned instanceof FGEIntersectionArea) {
			...
			if (!intersection.isDevelopable()) return clone();
	...
	{
		if (_findFirstUnionArea(this) == null) return clone();
		...
		if (area == null) {
			...
			if (others.size() > 0) {
				...
				if (result.isDevelopable()) return result.developUnions();
			}
	...
	{
		if (_findFirstSubstractionArea(this) == null) return clone();
		...
		if (area == null) {
			...
			if (others.size() > 0) {
				others.add(developedArea);
				FGEIntersectionArea result = new FGEIntersectionArea(others);
				if (result.isDevelopable()) return result.developSubstractions();
			}
	...
	{
		return FGESubstractionArea.makeSubstraction(sub.getContainerArea().intersect(area),sub.getSubstractedArea().intersect(area),sub.isStrict(),false);
	}
