...
public class FGERectPolylin extends FGEPolylin {
     ...
     * @param overlap
     */
    public static FGERectPolylin makeShortestRectPolylin(FGEArea startArea, FGEArea endArea, boolean straightWhenPossible,
     ...
     */
    public static FGERectPolylin makeShortestRectPolylin(FGEArea startArea, FGEArea endArea, boolean straightWhenPossible,
                                                         double overlapX, double overlapY, SimplifiedCardinalDirection startOrientation,
                                                         SimplifiedCardinalDirection endOrientation) {
        return makeShortestRectPolylin(startArea, endArea, straightWhenPossible, overlapX, overlapY,
                SimplifiedCardinalDirection.allDirectionsExcept(startOrientation), SimplifiedCardinalDirection.allDirectionsExcept
                (endOrientation));
    }
     ...
     */
    public static FGERectPolylin makeShortestRectPolylin(FGEArea startArea, FGEArea endArea, boolean straightWhenPossible,
                                                         double overlapX, double overlapY, FGEPoint minimizeDistanceToThisPoint,
                                                         Vector<SimplifiedCardinalDirection> excludedStartOrientations,
                                                         Vector<SimplifiedCardinalDirection> excludedEndOrientations) {
        FGERectPolylin returned = null;
    ...
    public static FGERectPolylin makeRectPolylinCrossingPoint(FGEArea startArea, FGEArea endArea, FGEPoint crossedPoint,
                                                              ...
                                                              SimplifiedCardinalDirection endOrientation, boolean straightWhenPossible,
                                                              double overlapX, double overlapY) {
    ...
    public static FGERectPolylin makeRectPolylinCrossingPoint(FGEArea startArea, FGEArea endArea, FGEPoint crossedPoint,
        ...
        for (SimplifiedCardinalDirection orientation1 : SimplifiedCardinalDirection.values()) {
            FGERectPolylin polylin = makeShortestRectPolylin(startArea, crossedPoint, true, overlapX, overlapY,
    ...
    private void restoreDefaultLayout() {
        ...
        if (intersect instanceof FGEPoint) {
            ...
            FGEPoint p_start = (startOrientation.isHorizontal() ? nearestPointOnHorizontalLine(p,
                    startArea) : nearestPointOnVerticalLine(p, startArea));
            ...
            FGEPoint p_start = (startOrientation.isHorizontal() ? nearestPointOnHorizontalLine(center,
                    startArea) : nearestPointOnVerticalLine(center, startArea));

            FGEPoint p_end = (endOrientation.isHorizontal() ? nearestPointOnHorizontalLine(center,
                    endArea) : nearestPointOnVerticalLine(center, endArea));
        ...
        if (startOrientation == SimplifiedCardinalDirection.EAST && endOrientation == SimplifiedCardinalDirection.EAST) {
            ...
            computeAs(new FGERectPolylin(endArea, endOrientation, startArea, startOrientation, straightWhenPossible, overlapX,
                    overlapY));
    ...
    private void restoreDefaultLayoutForSouthNorth() {
        ...
        if (getMaxYFor(startAnchorArea) > getMinYFor(endAnchorArea)) {
            ...
            double middleX = (
                    significativeStartLocation.x <= significativeEndLocation.x ?
                            (getMaxXFor(startArea) + getMinXFor(endArea)) / 2 : (getMinXFor(startArea) + getMaxXFor(endArea)) / 2);
    ...
    private void restoreDefaultLayoutForEastNorth() {
        ...
        if (quadrant == CardinalQuadrant.SOUTH_EAST) {
            ...
            alternativeLine1 = FGELine.makeVerticalLine(new FGEPoint((significativeStartLocation.x + significativeEndLocation.x) / 2,
                    0));
            ...
            alternativeLine2 = FGELine.makeHorizontalLine(new FGEPoint(0, (significativeStartLocation.y + significativeEndLocation.y) /
                                                                          2));
    ...
    protected FGEArc getRoundedArc(FGESegment s1, FGESegment s2, double arcWidth, double arcHeight) {
        ...
        if (orientation1 == SimplifiedCardinalDirection.NORTH) {
            if (orientation2 == SimplifiedCardinalDirection.EAST) {
                return new FGEArc(new FGEPoint(s1.x2 + arcWidth, s1.y2 + arcHeight), new FGEDimension(
                        arcWidth * 2, arcHeight * 2), 180, -90);
            }
            if (orientation2 == SimplifiedCardinalDirection.WEST) {
                return new FGEArc(new FGEPoint(s1.x2 - arcWidth, s1.y2 + arcHeight), new FGEDimension(arcWidth * 2, arcHeight * 2), 0,
                        90);
            }
            return null;
        }
    ...
    /**
     * Return a flag indicating wether this polylin is normalized or not A rect polylin is normalized when: - all segments are rect
     * (their
     * orientations matches cardinal orientation NORTH, EAST, SOUTH, WEST) - no extra points are found (no point located inside a
     * segment,
    ...
    public FGERectPolylin makeNormalizedRectPolylin() {
        ...
        for (int i = 1; i < getPointsNb() / 2; i++) {
            ...
            SimplifiedCardinalDirection orientation2 = updatedPolylin.getApproximatedOrientationOfSegment(
                    updatedPolylin.getPointsNb() - i - 1);
        ...
        // Finish updating
        if (logger.isLoggable(Level.FINEST)) {
            logger.finest("Still have to finish updating, currentStartIndex=" + currentStartIndex + " currentEndIndex=" +
                          currentEndIndex);
        }
        ...
        if (currentPointEndingSide.equals(currentPointStartingSide)
            ...
            if (currentEndIndex - currentStartIndex == 1) {
                ...
                missingPath = makeRectPolylinCrossingPoint(currentPointStartingSide, currentPointEndingSide, updatedPolylin.getPointAt(
                        currentStartIndex + 1), true, overlapX, overlapY, excludedStartOrientations, excludedEndOrientations);
