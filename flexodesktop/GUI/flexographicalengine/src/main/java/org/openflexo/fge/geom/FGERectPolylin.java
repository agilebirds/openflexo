...
public class FGERectPolylin extends FGEPolylin {
	...
	private static List<FGEPoint> makeList(FGEPoint... points)
	{
		...
		for (FGEPoint pt : points) returned.add(pt);
		return returned;
	}
	...
	/** 
	 * Build and return a FGERectPolylin given supplied start and end orientation,
	...
	public FGERectPolylin(
			FGEArea aStartArea, 
			SimplifiedCardinalDirection startOrientation, 
			FGEArea anEndArea, 
			...
			double overlapY)
	{
		...
		if (getOrientationOfSegment(0) != startOrientation 
				&& overlapX > 0 
				&& overlapY > 0 
				&& getFirstSegment() != null
		...
		if (endOrientation != null 
				&& getOrientationOfSegment(getSegmentNb()-1) != endOrientation.getOpposite() 
				&& overlapX > 0 
				&& overlapY > 0 
				&& getLastSegment() != null
				&& getLastSegment().getLength() > 0) {
			if (getOrientationOfSegment(getSegmentNb()-1) != null)
				logger.info("Inconsistant end orientation invariant... "+getOrientationOfSegment(getSegmentNb()-1)+" != "+endOrientation.getOpposite());
			if (logger.isLoggable(Level.FINE)) {
				logger.info("startArea="+startArea);
				logger.info("startOrientation="+startOrientation);
				logger.info("endArea="+endArea);
	...
	/**
	 * Return and build a FGERectPolylin, given some parameters.
	 * All orientation solutions are examined, and best solution is returned:
	 * - 1st: try to minimize number of points
	 * - 2nd: try to minimize total length of path
	 * 
	 * @param startArea
	 * @param endArea
	 * @param straightWhenPossible
	 * @param overlap
	 */
	public static FGERectPolylin makeShortestRectPolylin (
			FGEArea startArea, 
			FGEArea endArea, 
			boolean straightWhenPossible,
			double overlapX,
			double overlapY)
	{
		return makeShortestRectPolylin(startArea, endArea, straightWhenPossible, 
				overlapX, overlapY,
				(Vector<SimplifiedCardinalDirection>)null,
				(Vector<SimplifiedCardinalDirection>)null);
	}

	/**
	 * Return and build a FGERectPolylin, given some parameters.
	 * All orientation solutions (except those supplied as to be excluded) 
	 * are examined, and best solution is returned:
	 * - 1st: try to minimize number of points
	 * - 2nd: try to minimize total length of path
	 * 
	 * @param startArea
			...
			if (excludedStartOrientations == null || !excludedStartOrientations.contains(startOrientation)) {
				for (SimplifiedCardinalDirection endOrientation : SimplifiedCardinalDirection.values()) {
					if (excludedEndOrientations == null || !excludedEndOrientations.contains(endOrientation)) {
						FGERectPolylin tried = new FGERectPolylin(startArea,startOrientation,endArea,endOrientation,straightWhenPossible,overlapX,overlapY);
	 ...
	 * Return and build a FGERectPolylin, given some parameters.
	 * All orientation solutions (except those supplied as to be excluded)
	 * are examined, and best solution is returned regarding
	 * distance between returned polylin and supplied point
	 * 
	 * @param startArea
	 * @param endArea
	 * @param straightWhenPossible
	...
	public static FGERectPolylin makeShortestRectPolylin (
			...
			double overlapX,
			double overlapY,
			FGEPoint minimizeDistanceToThisPoint,
			Vector<SimplifiedCardinalDirection> excludedStartOrientations,
			Vector<SimplifiedCardinalDirection> excludedEndOrientations)
	{
		FGERectPolylin returned = null;
		...
		for (SimplifiedCardinalDirection startOrientation : SimplifiedCardinalDirection.values()) {
			if (excludedStartOrientations == null || !excludedStartOrientations.contains(startOrientation)) {
				for (SimplifiedCardinalDirection endOrientation : SimplifiedCardinalDirection.values()) {
					if (excludedEndOrientations == null || !excludedEndOrientations.contains(endOrientation)) {
						FGERectPolylin tried = new FGERectPolylin(startArea,startOrientation,endArea,endOrientation,straightWhenPossible,overlapX,overlapY);
						double distance = FGEPoint.distance(tried.getNearestPoint(minimizeDistanceToThisPoint),minimizeDistanceToThisPoint);
	...
	/**
	 * Return and build a FGERectPolylin linking a start and an end area,
	 * and crossing supplied point.
	 * All orientation solutions are examined, and best solution is returned regarding
	 * distance between returned polylin and supplied point
	 * 
	 * @param startArea
	 * @param endArea
	...
	/**
	 * Return and build a FGERectPolylin linking a start and an end area,
	 * and crossing supplied point, using supplied start and end orientation
	 * 
	 * @param startArea
	 * @param endArea
	 * @param straightWhenPossible
	...
	public static FGERectPolylin makeRectPolylinCrossingPoint (
			...
			double overlapY)
	{
		return makeRectPolylinCrossingPoint(
				startArea, endArea, crossedPoint, straightWhenPossible, overlapX, overlapY,
				SimplifiedCardinalDirection.allDirectionsExcept(startOrientation),
				SimplifiedCardinalDirection.allDirectionsExcept(endOrientation));
	}
			...
			Vector<SimplifiedCardinalDirection> excludedEndOrientations)
	{
		...
		Hashtable<SimplifiedCardinalDirection,FGERectPolylin> polylins1 
		= new Hashtable<SimplifiedCardinalDirection,FGERectPolylin>();
		Hashtable<SimplifiedCardinalDirection,FGERectPolylin> polylins2 
		= new Hashtable<SimplifiedCardinalDirection,FGERectPolylin>();
		...
		for (SimplifiedCardinalDirection orientation1 : SimplifiedCardinalDirection.values()) {
			for (SimplifiedCardinalDirection orientation2 : SimplifiedCardinalDirection.values()) {
				if (orientation1 != orientation2) {
					...
					if (polylin1 != null 
							&& polylin1.doesRespectAllConstraints() 
							&& polylin2 != null
	...
	public boolean crossedItSelf()
	{
		for (FGESegment s1 : getSegments()) {
			for (FGESegment s2 : getSegments()) {
				if (s1 != s2 
						&& (s1.overlap(s2)
								|| (s1.intersectsInsideSegment(s2)
										&& !s1.getLineIntersection(s2).equals(s1.getP1())
										&& !s1.getLineIntersection(s2).equals(s1.getP2())
										&& !s1.getLineIntersection(s2).equals(s2.getP1())
										&& !s1.getLineIntersection(s2).equals(s2.getP2())))) return true;
			}
		}
		return false;
	}
	...
	/**
	 * Merge polylin given supplied polylin and indexes.
	 * Ignore extra points...
	 * 
	 * @param p1
	 * @param startIndex1
	 ...
	 * @return
	 */
	public static FGERectPolylin mergePolylins(FGERectPolylin p1, int startIndex1, int endIndex1,
			FGERectPolylin p2, int startIndex2, int endIndex2) 
	{
	...
	public void debugPaint(FGEGraphics g)
	{
		...
		if (resultingStartArea != null) resultingStartArea.paint(g);
	...
	private void restoreDefaultLayout()
	{
		...
		if (intersect instanceof FGEPoint) 
		{
			...
			FGEPoint p_start = (startOrientation.isHorizontal() ?
					nearestPointOnHorizontalLine(p, startArea) :
						nearestPointOnVerticalLine(p, startArea));

			FGEPoint p_end = (endOrientation.isHorizontal() ?
					nearestPointOnHorizontalLine(p, endArea) :
		...
		else if (intersect instanceof FGESegment) 
		{
			...
			FGEPoint p_start = (startOrientation.isHorizontal() ?
					nearestPointOnHorizontalLine(center, startArea) :
						nearestPointOnVerticalLine(center, startArea));

			FGEPoint p_end = (endOrientation.isHorizontal() ?
					nearestPointOnHorizontalLine(center, endArea) :
						nearestPointOnVerticalLine(center, endArea));

			//FGEPoint p_start = FGEPoint.getNearestPoint(center,startArea.nearestPointFrom(center, startOrientation.getOpposite()),startArea.nearestPointFrom(center, startOrientation));
			//FGEPoint p_end =  FGEPoint.getNearestPoint(center,endArea.nearestPointFrom(center, endOrientation.getOpposite()),endArea.nearestPointFrom(center, endOrientation));
		...
		if (startOrientation == SimplifiedCardinalDirection.EAST 
				&& endOrientation == SimplifiedCardinalDirection.EAST) {
			restoreDefaultLayoutForEastEast();
		}
		else if (startOrientation == SimplifiedCardinalDirection.WEST 
				&& endOrientation == SimplifiedCardinalDirection.WEST) {
			restoreDefaultLayoutForWestWest();
		}
		else if (startOrientation == SimplifiedCardinalDirection.NORTH 
				&& endOrientation == SimplifiedCardinalDirection.NORTH) {
			restoreDefaultLayoutForNorthNorth();
		}
		else if (startOrientation == SimplifiedCardinalDirection.SOUTH 
				&& endOrientation == SimplifiedCardinalDirection.SOUTH) {
			restoreDefaultLayoutForSouthSouth();
		}
		else if (startOrientation == SimplifiedCardinalDirection.EAST 
				&& endOrientation == SimplifiedCardinalDirection.WEST) {
	...
	{
		for (int i=poly.getPointsNb()-1; i >= 0; i--) addToPoints(poly.getPointAt(i));
	}

	private FGEPoint nearestPointOnHorizontalLine(FGEPoint p, FGEArea area)
	{
		FGEPoint returned = FGEPoint.getNearestPoint(
				p,
				area.nearestPointFrom(p, SimplifiedCardinalDirection.EAST),
	...
	private FGEPoint nearestPointOnVerticalLine(FGEPoint p, FGEArea area)
	{
		FGEPoint returned = FGEPoint.getNearestPoint(
				p,
				area.nearestPointFrom(p, SimplifiedCardinalDirection.NORTH),
	...
	{
		if (logger.isLoggable(Level.FINE)) logger.fine("restoreDefaultForEastEast()");
	...
	{
		if (logger.isLoggable(Level.FINE)) logger.fine("restoreDefaultForWestWest()");
	...
	{
		if (logger.isLoggable(Level.FINE)) logger.fine("restoreDefaultForNorthNorth()");
	...
	{
		if (logger.isLoggable(Level.FINE)) logger.fine("restoreDefaultForSouthSouth()");
	...
	private void restoreDefaultLayoutForEastWest()
	{
		...
		if (getMaxXFor(startAnchorArea) > getMinXFor(endAnchorArea)) {
			...
			double middleY = (significativeStartLocation.y <= significativeEndLocation.y ?
					(getMaxYFor(startArea)+getMinYFor(endArea))/2 :
			...
			FGELine line = FGELine.makeVerticalLine(new FGEPoint(
					(significativeStartLocation.x <= significativeEndLocation.x ?
							(getMaxXFor(startArea)+getMinXFor(endArea))/2 :
	...
	{
		if (logger.isLoggable(Level.FINE)) logger.fine("restoreDefaultForSouthNorth()");
		...
		if (getMaxYFor(startAnchorArea) > getMinYFor(endAnchorArea)) {
			...
			double middleX = (significativeStartLocation.x <= significativeEndLocation.x ?
					(getMaxXFor(startArea)+getMinXFor(endArea))/2 :
			...
			FGELine line = FGELine.makeHorizontalLine(new FGEPoint(0,
					(significativeStartLocation.y <= significativeEndLocation.y ?
							(getMaxYFor(startArea)+getMinYFor(endArea))/2 :
								(getMinYFor(startArea)+getMaxYFor(endArea))/2)));				
			FGELine alternativeLine = FGELine.makeHorizontalLine(new FGEPoint(0,(significativeStartLocation.y+significativeEndLocation.y)/2));

			FGEArea i1 = resultingStartArea.intersect(line);
			FGEArea i2 = resultingEndArea.intersect(line);

			if (i1 instanceof FGEEmptyArea) i1 = resultingStartArea.intersect(alternativeLine);
			if (i2 instanceof FGEEmptyArea) i2 = resultingEndArea.intersect(alternativeLine);
	...
	{
		if (logger.isLoggable(Level.FINE)) logger.fine("restoreDefaultForEastNorth()");
		...
		if (useAlternativeLine1) line1 = alternativeLine1;
		if (useAlternativeLine2) line2 = alternativeLine2;
	...
	{
		if (logger.isLoggable(Level.FINE)) logger.fine("restoreDefaultForEastSouth()");
	...
	{
		if (logger.isLoggable(Level.FINE)) logger.fine("restoreDefaultForWestNorth()");
	...
	{
		if (logger.isLoggable(Level.FINE)) logger.fine("restoreDefaultForWestSouth()");
	...
	private FGEPoint getLocationFor(FGEArea a)
	{
		...
		if (a instanceof FGEPoint) return ((FGEPoint)a).clone();
	...
	private FGEPoint getSignificativeAnchorAreaLocationFor(FGEArea a, SimplifiedCardinalDirection direction)
	{
		...
		if (anchorArea instanceof FGEPoint) return ((FGEPoint)anchorArea).clone();
		...
		if (anchorArea instanceof FGEShape) return getSignificativeAnchorAreaLocationFor(((FGEShape)anchorArea).getBoundingBox(),direction);
	...
	{
		if (a instanceof FGEPoint) return ((FGEPoint)a).x;
		if (a instanceof FGESegment) return Math.max(((FGESegment)a).getP1().x,((FGESegment)a).getP2().x);
		if (a instanceof FGEShape) return ((FGEShape)a).getBoundingBox().x+((FGEShape)a).getBoundingBox().width;
		if (a instanceof FGEPolylin) return ((FGEPolylin)a).getBoundingBox().x+((FGEPolylin)a).getBoundingBox().width;
		if (a instanceof FGEArc) return ((FGEArc)a).getBoundingBox().x+((FGEArc)a).getBoundingBox().width;
		if (a instanceof FGEUnionArea && ((FGEUnionArea)a).isUnionOfPoints()) {
			double returned = Double.NEGATIVE_INFINITY;
			for (FGEArea p :  ((FGEUnionArea)a).getObjects()) {
				FGEPoint pt = (FGEPoint)p;
				if (pt.x > returned) returned = pt.x;
			}
			return returned;
		}
		if (a instanceof FGEUnionArea && ((FGEUnionArea)a).isUnionOfFiniteGeometricObjects()) {
	...
	{
		if (a instanceof FGEPoint) return ((FGEPoint)a).x;
		if (a instanceof FGESegment) return Math.min(((FGESegment)a).getP1().x,((FGESegment)a).getP2().x);
		if (a instanceof FGEShape) return ((FGEShape)a).getBoundingBox().x;
		if (a instanceof FGEPolylin) return ((FGEPolylin)a).getBoundingBox().x;
		if (a instanceof FGEArc) return ((FGEArc)a).getBoundingBox().x;
		if (a instanceof FGEUnionArea && ((FGEUnionArea)a).isUnionOfPoints()) {
	...
	{
		if (a instanceof FGEPoint) return ((FGEPoint)a).y;
		if (a instanceof FGESegment) return Math.max(((FGESegment)a).getP1().y,((FGESegment)a).getP2().y);
		if (a instanceof FGEShape) return ((FGEShape)a).getBoundingBox().y+((FGEShape)a).getBoundingBox().height;
		if (a instanceof FGEPolylin) return ((FGEPolylin)a).getBoundingBox().y+((FGEPolylin)a).getBoundingBox().height;
		if (a instanceof FGEArc) return ((FGEArc)a).getBoundingBox().y+((FGEArc)a).getBoundingBox().height;
		if (a instanceof FGEUnionArea && ((FGEUnionArea)a).isUnionOfPoints()) {
			double returned = Double.NEGATIVE_INFINITY;
			for (FGEArea p :  ((FGEUnionArea)a).getObjects()) {
				FGEPoint pt = (FGEPoint)p;
				if (pt.y > returned) returned = pt.y;
			}
			return returned;
		}
		if (a instanceof FGEUnionArea && ((FGEUnionArea)a).isUnionOfFiniteGeometricObjects()) {
	...
	{
		if (a instanceof FGEPoint) return ((FGEPoint)a).y;
		if (a instanceof FGESegment) return Math.min(((FGESegment)a).getP1().y,((FGESegment)a).getP2().y);
		if (a instanceof FGEShape) return ((FGEShape)a).getBoundingBox().y;
		if (a instanceof FGEPolylin) return ((FGEPolylin)a).getBoundingBox().y;
		if (a instanceof FGEArc) return ((FGEArc)a).getBoundingBox().y+((FGEArc)a).getBoundingBox().height;
		if (a instanceof FGEUnionArea && ((FGEUnionArea)a).isUnionOfPoints()) {
			double returned = Double.POSITIVE_INFINITY;
			for (FGEArea p :  ((FGEUnionArea)a).getObjects()) {
	...
	public FGESegment getMiddleSegment()
	{
		...
		for (FGESegment s : getSegments()) {
			if ((distance <= middleDistancePath) 
					&& (distance+s.getLength() >= middleDistancePath)) {
				return s;
			}
	...
	public FGEArc getArcForNearestPointLocatedOnRoundedRepresentation(FGEPoint p, double arcWidth, double arcHeight)
	{
		...
		if (orientation == SimplifiedCardinalDirection.NORTH) {
			if (segment.y1-point.y < arcHeight) arc = startArc;
			if (point.y-segment.y2 < arcHeight) arc = endArc;
		}
		if (orientation == SimplifiedCardinalDirection.SOUTH) {
			if (point.y-segment.y1 < arcHeight) arc = startArc;
			if (segment.y2-point.y < arcHeight) arc = endArc;
		}
		if (orientation == SimplifiedCardinalDirection.WEST) {
			if (segment.x1-point.x < arcWidth) arc = startArc;
			if (point.x-segment.x2 < arcWidth) arc = endArc;
		}
		if (orientation == SimplifiedCardinalDirection.EAST) {
			if (point.x-segment.x1 < arcWidth) arc = startArc;
			if (segment.x2-point.x < arcWidth) arc = endArc;
		}
		return arc;
	}
	...
	protected FGEArc getRoundedArc (FGESegment s1, FGESegment s2, double arcWidth, double arcHeight)
	{
		...
		if (orientation1 == SimplifiedCardinalDirection.NORTH) {
			if (orientation2 == SimplifiedCardinalDirection.EAST) 
				return new FGEArc(new FGEPoint(s1.x2+arcWidth,s1.y2+arcHeight),new FGEDimension(arcWidth*2,arcHeight*2),180,-90);
	...
	/**
	 * Return the nearest segment (if any) on which supplied
	 * point p might be projected
	...
	/**
	 * Return a flag indicating wether this polylin is normalized or not
	 * A rect polylin is normalized when:
	 * - all segments are rect (their orientations matches cardinal
	 *  orientation NORTH, EAST, SOUTH, WEST)
	 * - no extra points are found (no point located inside a segment, 
	...
	/**
	 * Return a flag indicating wether this polylin has non-rect segment
	 * (their orientations doesn't matches cardinal
	 *  orientation NORTH, EAST, SOUTH, WEST)
	 * 
	 * @return
	 */
	public boolean hasNonRectSegments()
	{
		for (int i=0; i<getSegmentNb(); i++) {
			if (getOrientationOfSegment(i) == null) return true;
		}
		return false;
	}

	/**
	 * Return a flag indicating wether this polylin has extra point
	 * (no point located inside a segment, 
	 * which means that no colinear adjascent segment were found)
	 * 
	...
	private boolean hasEqualsAdjascentPoints()
	{
		FGEPoint previous = null;
		for (FGEPoint p : getPoints()) {
			if (previous != null && previous.equals(p)) return true;
			previous = p;
		}
		return false;
	}
	...
	private boolean hasColinearAdjascentSegments()
	{
		...
		for (FGESegment s : getSegments()) {
			if (previous_s != null && previous_s.getApproximatedOrientation().equals(s.getApproximatedOrientation())) return true;
			previous_s = s;
		}
		return false;
	}
	...
	public FGERectPolylin makeNormalizedRectPolylin()
	{
		...
		{
			// Starting side
			if (logger.isLoggable(Level.FINEST)) logger.finest("Checking point at "+i);
			currentStartIndex = i;
			...
			if (orientation.isVertical()) {
				if (currentPointStartingSide.x != previousPointStartingSide.x) {
					if (logger.isLoggable(Level.FINEST)) logger.finest("Updating point at "+i+" for vertical orientation");
				...
				if (currentPointStartingSide.y != previousPointStartingSide.y) {
					if (logger.isLoggable(Level.FINEST)) logger.finest("Updating point at "+i+" for horizontal orientation");
			...
			// Ending side
			if (logger.isLoggable(Level.FINEST)) logger.finest("Checking point at "+(updatedPolylin.getPointsNb()-i-1));
			...
			SimplifiedCardinalDirection orientation2 = updatedPolylin.getApproximatedOrientationOfSegment(updatedPolylin.getPointsNb()-i-1);
			if (orientation2.isVertical()) {
				if (currentPointEndingSide.x != previousPointEndingSide.x) {
					if (logger.isLoggable(Level.FINEST)) logger.finest("Updating point at "+(updatedPolylin.getPointsNb()-i-1)+" for vertical orientation");
				...
				if (currentPointEndingSide.y != previousPointEndingSide.y) {
					if (logger.isLoggable(Level.FINEST)) logger.finest("Updating point at "+(updatedPolylin.getPointsNb()-i-1)+" for horizontal orientation");
		...
		// Finish updating
		if (logger.isLoggable(Level.FINEST)) logger.finest("Still have to finish updating, currentStartIndex="+currentStartIndex+" currentEndIndex="+currentEndIndex);
		...
		if (currentPointEndingSide.equals(currentPointStartingSide)
			...
			if (currentEndIndex-currentStartIndex == 1) {
				/*FGERectPolylin*/ missingPath = makeShortestRectPolylin(currentPointStartingSide, currentPointEndingSide, true, overlapX, overlapY, excludedStartOrientations, excludedEndOrientations);
				...
				/*FGERectPolylin*/ //missingPath = makeShortestRectPolylin(currentPointStartingSide, currentPointEndingSide, true, overlap, updatedPolylin.getPointAt(currentStartIndex+1), excludedStartOrientations, excludedEndOrientations);
				missingPath = makeRectPolylinCrossingPoint(currentPointStartingSide, currentPointEndingSide, updatedPolylin.getPointAt(currentStartIndex+1), true, overlapX, overlapY,  excludedStartOrientations, excludedEndOrientations);
	...
	public FGEArea getEndArea()
	{
		...
		if (endArea == null && getPointsNb() > 0) return getPointAt(getPointsNb()-1);
		return endArea;
	}
	...
	{
		// if no start area defined, return start point
		if (startArea == null && getPointsNb() > 0) return getPointAt(0);
		return startArea;
	}
