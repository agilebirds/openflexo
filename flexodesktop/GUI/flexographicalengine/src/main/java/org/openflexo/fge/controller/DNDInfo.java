...
public class DNDInfo
{
	...
	DNDInfo(MoveAction moveAction, ShapeGraphicalRepresentation<?> gr, DrawingController<?> controller, final MouseEvent initialEvent)
	{
		...
		this.dragSource.startDrag(dge, MoveAction.dropKO, new MoveAction.ShapeGraphicalRepresentationTransferable(gr,initialPoint), dsListener);
	...
	protected void enableDragging()
	{
		...
		if (dropTargets != null) dropTargets.clear();
		...
		for (GraphicalRepresentation<?> gr : _controller.getDrawingView().getContents().keySet()) {
			...
			((Component)view).setDropTarget(
					new DropTarget(
							shapeView, 
							DnDConstants.ACTION_MOVE, 
							new MoveActionDropListener((JComponent)view),
							true));
		}
	}
	...
	class DGListener implements DragGestureListener
	{
		...
		public void dragGestureRecognized(DragGestureEvent e)
		{
			...
			ShapeGraphicalRepresentationTransferable transferable = new MoveAction.ShapeGraphicalRepresentationTransferable(null,e.getDragOrigin());
	...
	public class MoveActionDropListener implements DropTargetListener
	{
		...
		private DataFlavor chooseDropFlavor(DropTargetDropEvent e)
		{
			if (e.isLocalTransfer() == true 
					&& e.isDataFlavorSupported(ShapeGraphicalRepresentationTransferable.defaultFlavor())) {
