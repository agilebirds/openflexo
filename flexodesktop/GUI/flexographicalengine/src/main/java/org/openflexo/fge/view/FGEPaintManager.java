...
public class FGEPaintManager {
	...
	public FGEPaintManager(DrawingView<?> drawingView)
	{
		...
		if (ENABLE_CACHE_BY_DEFAULT) enablePaintingCache();
		else disablePaintingCache();
	}
	...
	public boolean containsTemporaryObject(GraphicalRepresentation<?> gr)
	{
		...
		if (isTemporaryObject(gr)) return true;
		if (gr.getContainedGraphicalRepresentations() == null) return false;
		for (GraphicalRepresentation<?> child : gr.getContainedGraphicalRepresentations()) {
			if (containsTemporaryObject(child)) return true;
		}
		return false;
	}
	...
	{
		if (!_drawingView.contains(view)) return;
		...
		if (view instanceof ShapeView /*&& isPaintingCacheEnabled()*/) {
			if (((Component)view).getParent()==null) {
				if (logger.isLoggable(Level.WARNING))
					logger.warning("Parent view to repaint is null: "+(view.getGraphicalRepresentation()!=null?view.getModel():view));
				return;
			}
			...
			if (gr.getBorder().top*view.getScale() < requiredControlPointSpace) {
				Rectangle repaintAlsoThis = new Rectangle(-requiredControlPointSpace,-requiredControlPointSpace,((Component)view).getWidth()+requiredControlPointSpace*2,requiredControlPointSpace*2);
			...
			if (gr.getBorder().bottom*view.getScale() < requiredControlPointSpace) {
				Rectangle repaintAlsoThis = new Rectangle(-requiredControlPointSpace,((Component)view).getHeight()-requiredControlPointSpace,((Component)view).getWidth()+requiredControlPointSpace*2,requiredControlPointSpace*2);
				repaintAlsoThis = SwingUtilities.convertRectangle((Component)view, repaintAlsoThis, ((Component)view).getParent());
				((Component)view).getParent().repaint(repaintAlsoThis.x,repaintAlsoThis.y,repaintAlsoThis.width,repaintAlsoThis.height);
				//System.out.println("Repaint "+repaintAlsoThis+" for "+((Component)view).getParent());
			}
			if (gr.getBorder().left*view.getScale() < requiredControlPointSpace) {
				Rectangle repaintAlsoThis = new Rectangle(-requiredControlPointSpace,-requiredControlPointSpace,requiredControlPointSpace*2,((Component)view).getHeight()+requiredControlPointSpace*2);
				repaintAlsoThis = SwingUtilities.convertRectangle((Component)view, repaintAlsoThis, ((Component)view).getParent());
				((Component)view).getParent().repaint(repaintAlsoThis.x,repaintAlsoThis.y,repaintAlsoThis.width,repaintAlsoThis.height);
				//System.out.println("Repaint "+repaintAlsoThis+" for "+((Component)view).getParent());
			}
			if (gr.getBorder().right*view.getScale() < requiredControlPointSpace) {
				Rectangle repaintAlsoThis = new Rectangle(((Component)view).getWidth()-requiredControlPointSpace,-requiredControlPointSpace,requiredControlPointSpace*2,((Component)view).getHeight()+requiredControlPointSpace*2);
				repaintAlsoThis = SwingUtilities.convertRectangle((Component)view, repaintAlsoThis, ((Component)view).getParent());
				((Component)view).getParent().repaint(repaintAlsoThis.x,repaintAlsoThis.y,repaintAlsoThis.width,repaintAlsoThis.height);
				//System.out.println("Repaint "+repaintAlsoThis+" for "+((Component)view).getParent());
			}
	...
	public BufferedImage getScreenshot(GraphicalRepresentation<?> gr)
	{
		...
		Rectangle rect = new Rectangle(((JComponent)v).getX(), ((JComponent)v).getY(),((JComponent)v).getWidth(), ((JComponent)v).getHeight());
		...
		public synchronized void addTemporaryRepaintArea(Rectangle r, JComponent view)
		{
			if (MANAGE_DIRTY_REGIONS) {
				...
				if (paintRequestLogger.isLoggable(Level.FINER))
					paintRequestLogger.finer("addTemporaryRepaintArea("+r+") for "+view.getClass().getSimpleName()+" temporaryRepaintAreas size="+allRect.size());
			}
		}
	...
	protected boolean renderUsingBuffer(Graphics2D g, Rectangle renderingBounds, GraphicalRepresentation gr, double scale)
	{
		...
		Rectangle viewBoundsInDrawingView = GraphicalRepresentation.convertRectangle(gr, renderingBounds, gr.getDrawingGraphicalRepresentation(), scale);
		...
		if ((sp1.x < 0)
				|| (sp1.x > buffer.getWidth())
				|| (sp1.y < 0)
				|| (sp1.y > buffer.getHeight())
				|| (sp2.x < 0)
				|| (sp2.x > buffer.getWidth())
				|| (sp2.y < 0)
				|| (sp2.y > buffer.getHeight())) {
			// We have here a request for render outside cached image
			// We cannot do that, so skip buffer use and do normal painting
			if (FGEPaintManager.paintPrimitiveLogger.isLoggable(Level.FINE))
				FGEPaintManager.paintPrimitiveLogger.fine("GraphicalRepresentation:"+gr+" / request to render outside image buffer, use normal rendering clip="+renderingBounds);
