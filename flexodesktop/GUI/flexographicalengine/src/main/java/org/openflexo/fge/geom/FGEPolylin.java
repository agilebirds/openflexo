...
/**
 * The <code>FGEPolylin</code> class encapsulates a description of an
 * open path, defined as an arbitrary number of line segments
 *
 * @author sylvain
 *
 */
public class FGEPolylin implements FGEGeometricObject<FGEPolylin> {
	...
	public void removePointAtIndex(int index)
	{
		...
		if (index == getSegmentNb()) {
			...
			if (index < _segments.size()) _segments.remove(index);
			if (index >= 1
					&& index-1 < _segments.size()
					&& index < _points.size()) {
	...
	public int getPointIndex(FGEPoint point)
	{
		...
		for (FGEPoint p : getPoints()) {
			if (p.equals(point)) return index;
			index++;
		}
		return -1;
	}
	...
	public int getSegmentIndex(FGESegment segment)
	{
		...
		for (FGESegment s : getSegments()) {
			if (s.equals(segment)) return index;
			index++;
		}
		return -1;
	}
	...
	private void reCalculateBounds()
	{
		...
		bounds = new FGERectangle(boundsMinX, boundsMinY,
				...
				boundsMaxY - boundsMinY,Filling.FILLED);
		boundsChanged = false;
	}

	public FGERectangle getBoundingBox()
	{
		if (boundsChanged || bounds == null) reCalculateBounds();
		return bounds;
	}
	...
	public boolean containsLine(FGEAbstractLine l)
	{
		if (l instanceof FGESegment) {
			for (FGESegment s : _segments) {
				if (s.containsLine(l)) return true;
			}
		}
		return false;
	}
	...
	public double getRelativeLocation(FGEPoint p)
	{
		...
		for (int i=0; i<index; i++) cumulated += getSegmentAt(i).getLength();
	...
	private FGEArea computePolylinIntersection(FGEPolylin polylin)
	{
		...
		for (FGESegment s1 : getSegments()) {
			for (FGESegment s2 : polylin.getSegments()) {
				...
				if (!(i instanceof FGEEmptyArea)) unionAreas.add(i);
			}
		}
	...
	public FGEArea intersect(FGEArea area)
	{
		...
		if (area instanceof FGEPolylin) return computePolylinIntersection((FGEPolylin)area);
	...
	public final FGEArea union(FGEArea area)
	{
		...
		if (area instanceof FGEPolylin) {
			...
			if (returned instanceof FGEUnionArea) return new FGEUnionArea(this,area);
			return returned;
		}
	...
	{
		if (a instanceof FGESegment && _segments.contains(a)) return true;
		...
		for (FGESegment s : _segments) {
			if (s.containsArea(a)) return true;
		}
	...
	{
		double returned = 0;
		for (FGESegment s : getSegments()) returned += s.getLength();
		return returned;
	}
