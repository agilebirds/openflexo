...
public class MoveInfo
{
	...
	MoveInfo(ShapeGraphicalRepresentation graphicalRepresentation,DrawingController<?> controller)
	{
		...
		startMovingLocationInDrawingView = 
			GraphicalRepresentation.convertPointFromDrawableToDrawing(
					...
					new Point(graphicalRepresentation.getViewX(controller.getScale()),graphicalRepresentation.getViewY(controller.getScale())), 
					controller.getScale());
		...
		for (GraphicalRepresentation d : controller.getSelectedObjects()) {
			if (d != graphicalRepresentation 
					...
					&& ((ShapeGraphicalRepresentation)d).getContainer() == graphicalRepresentation.getContainer()
					&& !d.getIsReadOnly()
	...
	void moveTo(Point newLocationInDrawingView) 
	{
		...
		if (!moveHasStarted) startDragging();
	...
	{
		if (movedObjects.size() != 1) return false;
		...
		if (movedObject.getContainerGraphicalRepresentation() instanceof ShapeGraphicalRepresentation) {
			...
			FGERectangle bounds = new FGERectangle(0,0,container.getWidth()-movedObject.getWidth(),container.getHeight()-movedObject.getHeight(),Filling.FILLED);
			FGEPoint nearestPoint = bounds.getNearestPoint(desiredLocation);
			Point p1 = GraphicalRepresentation.convertPointFromDrawableToDrawing(
					movedObject.getContainerGraphicalRepresentation(), 
					desiredLocation.toPoint(),
					view.getScale());
			Point p2 = GraphicalRepresentation.convertPointFromDrawableToDrawing(
					movedObject.getContainerGraphicalRepresentation(), 
