...
public abstract class FGEGraphics {
	...
	/**
	 * Creates a new <code>Graphics2D</code> object that is 
	 * a copy of current <code>Graphics2D</code> object.
	 * 
	...
	protected String debugForegroundStyle()
	{
		...
		return "FS-(color="+g2d.getColor()+")-(with="+stroke.getLineWidth()+")-(transp="+g2d.getComposite()+")-(join="+stroke.getLineJoin()+")-(cap="+stroke.getEndCap()+")-(dash="+stroke.getDashPhase()+")";
	}
	...
	{
		if (currentForeground.getNoStroke()) return null;
		Point p = convertNormalizedPointToViewCoordinates(x,y);
		p.x -= size;
		p.y -= size;
		g2d.fillRect(p.x,p.y,
				(size*2),
				(size*2));		
	...
	{
		if (currentForeground.getNoStroke()) return;
		Point p1 = convertNormalizedPointToViewCoordinates(x,y);
	...
	{
		if (currentBackground instanceof BackgroundStyle.None) return;
	...
	/**
	 * This method is used to paint an image or a portion of an image into a supplied
	 * shape. Background properties are used, and transparency managed here.
	 * 
	 * @param aShape
	 */
	private void fillInShapeWithImage(Shape aShape)
	{
		...
		if (currentBackground instanceof BackgroundStyle.BackgroundImage) {
			at.concatenate(AffineTransform.getTranslateInstance(
					((BackgroundStyle.BackgroundImage)currentBackground).getDeltaX(), 
	...
	public void drawCurve(FGECubicCurve curve)
	{
		...
		g2d.draw(awtCurve);
		if (logger.isLoggable(Level.FINE))
			logger.fine("drawCurve("+p1.x+","+p1.y+","+ctrl_p1.x+","+ctrl_p1.y+","+p2.x+ctrl_p2.x+","+ctrl_p2.y+","+p2.x+","+p2.y+")");
	}
	...
	public FGERectangle drawString(String text, double x, double y, int orientation, TextAlignment alignment)
	{
		...
		AffineTransform at = AffineTransform.getScaleInstance(getScale(), getScale());
		if (orientation != 0) at.concatenate(AffineTransform.getRotateInstance(Math.toRadians(orientation)));
		...
		g2d.setFont(oldFont);
		return convertViewCoordinatesToNormalizedRectangle(
				(int)(bounds.getX()+p.x-bounds.getWidth()/2),
				(int)(bounds.getY()+p.y+bounds.getHeight()/2),
				(int)bounds.getWidth(),(int)bounds.getHeight());
	}
