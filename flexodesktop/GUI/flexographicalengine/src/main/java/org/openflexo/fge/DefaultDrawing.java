...
/**
 * This class is the default implementation for all objects representing a graphical drawing,
...
public abstract class DefaultDrawing<M> extends Observable implements Drawing<M> {
	...
	public final void updateGraphicalObjectsHierarchy()
	{
		...
		if (isUpdatingObjectHierarchy) // Already doing it.
			return;
	...
	public void printGraphicalObjectHierarchy()
	{
		...
		else logger.info(" > Root node is null !");
	}

	private void _printGraphicalObjectHierarchy(DrawingTreeNode<?> dtn, int level)
	{
		logger.info(buildWhiteSpaceIndentation(level*5)+" > "+(dtn.graphicalRepresentation != null ? dtn.graphicalRepresentation.getClass().getSimpleName()+" "+Integer.toHexString(dtn.graphicalRepresentation.hashCode()):" null ")+" object="+dtn.drawable);
		if (dtn.childNodes != null) for (DrawingTreeNode<?> child : dtn.childNodes) _printGraphicalObjectHierarchy(child,level+1);
	}
	...
	protected void beginUpdateObjectHierarchy()
	{
		...
		while (allNodes.hasMoreElements()) {
			...
			if (next.graphicalRepresentation != null) {
				//logger.info("What about "+next.getClass().getSimpleName());
				next.graphicalRepresentation.notifyObjectHierarchyWillBeUpdated();
			}
		...
		for (DrawingTreeNode n : nodesToUpdate) {
			if (n.graphicalRepresentation instanceof ConnectorGraphicalRepresentation) {
				ConnectorGraphicalRepresentation connector = (ConnectorGraphicalRepresentation)n.graphicalRepresentation;
		...
		for (DrawingTreeNode n : nodesToUpdate) n.beginUpdateObjectHierarchy();
	...
	protected void endUpdateObjectHierarchy()
	{
		...
		for (DrawingTreeNode n : nodesToUpdate) n.endUpdateObjectHierarchy();
	...
	private class DrawingTreeNode<O>
	{
		...
		{
			isInvalidated = true;
			for (DrawingTreeNode<?> dtn : childNodes) dtn.invalidate();
		}
		...
		protected void delete()
		{
			...
			if (parentNode != null && parentNode.childNodes.contains(this)) parentNode.removeChild(this);
			...
			if (childs != null) childs.removeAllElements();
			childs = null;

			if (_hashMap != null && drawable != null) _hashMap.remove(drawable);
		...
		protected void beginUpdateObjectHierarchy()
		{
			...
			if (childNodes != null) {
				for (DrawingTreeNode n : (new Vector<DrawingTreeNode<?>>(childNodes))) {
					if (n.isInvalidated) removeDrawable(n.drawable, drawable);
				}
			}
	...
	public List<?> getContainedObjects(Object aDrawable)
	{
		...
		if (treeNode != null) // Bug 1006529 Fix
	...
	public <O> GraphicalRepresentation<O> getGraphicalRepresentation(O aDrawable)
	{
		...
		if (treeNode != null)
			return treeNode.graphicalRepresentation;
		return null;
	}
	...
	public void delete()
	{
		...
		if (_hashMap != null) {
			...
			for (Object drawable : drawableList) {
				...
				if (dtn != null) {
					if (dtn.graphicalRepresentation != null) dtn.graphicalRepresentation.delete();
					dtn.delete();
				}
			}
			_hashMap.clear();
