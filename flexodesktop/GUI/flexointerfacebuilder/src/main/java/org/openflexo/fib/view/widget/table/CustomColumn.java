...
public class CustomColumn<T extends Object> extends AbstractColumn<T> implements EditableColumn<T>, ApplyCancelListener
{
   ...
   private FIBCustomColumn _customColumn;
    ...
    public CustomColumn(FIBCustomColumn columnModel, FIBTableModel tableModel, FIBController controller)
    {
        ...
        _viewCustomWidget = makeCustomComponent((Class<FIBCustomComponent<T,?>>)columnModel.getComponentClass(),(Class<T>)columnModel.getDataClass());
        _editCustomWidget = makeCustomComponent((Class<FIBCustomComponent<T,?>>)columnModel.getComponentClass(),(Class<T>)columnModel.getDataClass());
        if (_editCustomWidget != null) _editCustomWidget.addApplyCancelListener(this);
...
public Class<T> getValueClass()
    ...
    protected class CustomCellEditor extends AbstractCellEditor implements TableCellEditor, ActionListener
    {
        ...
        {
        	if (_customWidget == null) return null;
	...
	{
		if (variable.getVariableName().equals(FIBCustom.COMPONENT_NAME)) return _editCustomWidget;
		else return super.getValue(variable);
	}
