 ...
 *
 */
public class AbstractActivityNodeElement extends BrowserElement implements
		ExpansionSynchronizedElement {

	private static final Logger logger = Logger
			.getLogger(AbstractActivityNodeElement.class.getPackage().getName());

	public AbstractActivityNodeElement(AbstractActivityNode node,
			BrowserElementType elementType, ProjectBrowser browser, BrowserElement parent) {
    ...
    private void addObserver()
    {
        ...
        if (getAbstractActivityNode().getOperationPetriGraph() != null) {
            ...
            isObserving = true;
        } else if (getAbstractActivityNode() instanceof SelfExecutableNode && ((SelfExecutableNode)getAbstractActivityNode()).getExecutionPetriGraph()!=null) {
...
*/
		...
		for (Enumeration e = getAbstractActivityNode().getPreConditions()
				.elements(); e.hasMoreElements();) {
			addToChilds((FlexoPreCondition) e.nextElement());
		}
		// We add post conditions
		for (Enumeration e = getAbstractActivityNode()
				.getOutgoingPostConditions().elements(); e.hasMoreElements();) {
	...
	public boolean isExpansionSynchronizedWithData() {
		if (_browser.getSelectionManager() != null) {
			return (getAbstractActivityNode().getProcess() == _browser
	...
	public boolean isExpanded() {
		if (getAbstractActivityNode() instanceof SelfExecutableNode) {
    		return ((SelfExecutableNode)getAbstractActivityNode()).hasExecutionPetriGraph() && ((SelfExecutableNode)getAbstractActivityNode()).getExecutionPetriGraph().getIsVisible();
    	}
		if (getAbstractActivityNode().hasContainedPetriGraph()) {
			return getAbstractActivityNode().getOperationPetriGraph()
					.getIsVisible();
		}
		return false;
	}
	...
	public void expand() {
		if (getAbstractActivityNode() instanceof SelfExecutableNode) {
    		if (((SelfExecutableNode)getAbstractActivityNode()).hasExecutionPetriGraph() && !isExpanded()) {
    			OpenExecutionPetriGraph.actionType.makeNewAction(getAbstractActivityNode(), null, getProjectBrowser().getEditor()).doAction();
    		}
