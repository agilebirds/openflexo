...
public class FlexoAutoSaveThread extends Thread
{
    ...
    /**
     * The root directory for the specified project containing all the
     * intermediate project save. For example, you will have
     * tempDirectory that is "C:\Documents and Settings\UserName\Local Settings\Temp\A Flexo Temp Directory"
     * and the <code>projects</code> list will contain files like:<ul>
    ...
    private void initFromFile()
    {
        try {
            ...
            if (!tempDirectory.exists() || (!content.startsWith(System.getProperty("java.io.tmpdir"))&& !content.startsWith(new File(System.getProperty("java.io.tmpdir")).getCanonicalPath())))
        ...
        Collections.sort(projects, new Comparator<FlexoAutoSaveFile>(){ // This comparator will make oldest files first and newer ones last in the queue
            /**
             * Overrides compare
    ...
    public void run()
    {
        while(run) {
            ...
            if (projects.size()==0)
	            ...
	            Enumeration<FlexoResource<FlexoResourceData>> en = new Vector<FlexoResource<FlexoResourceData>>(project.getResources().values()).elements();
    ...
    public static class AutoSaveActionFailed implements Runnable {
		...
		public void run()
        {
            FlexoController.showError(FlexoLocalization.localizedForKey("auto_save_action_failed"), FlexoLocalization.localizedForKey("auto_save_action_could_not_be_performed")+"\n"+FlexoLocalization.localizedForKey("verify_that_your_disk_is_not_full_and_that_you_can_write_in_the_temp_directory."));
        }
    }
    ...
    public void showTimeTravelerDialog() {
        ...
        final FlexoDialog dialog = new FlexoDialog(FlexoModule.getActiveModule()!=null?FlexoModule.getActiveModule().getFlexoFrame():null,true);
        ...
        parameters[1] = new PropertyListParameter<FlexoAutoSaveFile>("backUps", FlexoLocalization.localizedForKey("back-ups"),projects,20,12);
        ...
        JLabel label = new JLabel("<html>"+FlexoLocalization.localizedForKey("time_travel_info")+"</html>",IconLibrary.TIME_TRAVEL_ICON,SwingConstants.LEFT);
        north.add(label);
                ...
                if (autoSaveFile!=null) {
                    if (FlexoController.confirm(FlexoLocalization.localizedForKey("are_you_sure_that_you_want_to_revert_to_that_version?"))) {
                        try {
                            ...
                            e1.printStackTrace();
                            FlexoController.showError(FlexoLocalization.localizedForKey("an_error_occured_while_trying_to_restore_your_project")+"\n"+project.getProjectDirectory().getAbsolutePath());
                        }
                    } else {
                        ModuleLoader.startAutoSaveThread();
                    }
