...
public class BrowserFooter extends JPanel implements MouseListener, WindowListener
{
    ...
    public BrowserFooter(BrowserView browserView)
    {
        ...
        if (logger.isLoggable(Level.FINE)) {
			logger.fine("Browser " + browserView.getBrowser() + " has " + browserView.getBrowser().getConfigurableElementTypeFilters().size()
                            + " filters");
		}
   ...
   /* protected void elementTypeFilterChanged()
    ...
    {
        if (logger.isLoggable(Level.FINE)) {
			logger.fine("Browser " + _browserView.getBrowser() + " has now "
            		+ (_browserView.getBrowser().getConfigurableElementTypeFilters().size()+_browserView.getBrowser().getCustomFilters().size())
                    + " filters");
		}
    ...
    void plusPressed()
    {
        ...
        actionType.actionPerformed(new ActionEvent(new BrowserActionSource(_browserView.getBrowser(), getFocusedObject(), null, _browserView.getEditor()), 1,
                "BrowserFooter-PlusPressed"));
    }
                ...
                menuItem.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent e)
                    {
                        action.actionPerformed(new ActionEvent(
                                new BrowserActionSource(_browserView.getBrowser(), getFocusedObject(), null, _browserView.getEditor()), 1,
                                "BrowserFooter-PlusPressed"));
                    }
                });
    ...
    void minusPressed()
    {
        ...
        actionType.actionPerformed(new ActionEvent(new BrowserActionSource(_browserView.getBrowser(), _browserView.getBrowser().getFocusedObject(), _browserView.getBrowser().getSelection(), _browserView.getEditor()), 2,
                "BrowserFooter-MinusPressed"));
    }
    ...
    /**
     * Returns focused object, considering focused object is the last selected
     * object. If none object are selected, return null.
     */
