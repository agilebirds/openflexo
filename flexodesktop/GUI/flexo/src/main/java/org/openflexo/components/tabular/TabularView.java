 ...
 * 
 */
public abstract class TabularView extends JPanel 
implements TableModelListener, ListSelectionListener, SelectionSynchronizedComponent, GraphicalFlexoObserver
{
	...
	public void valueChanged(ListSelectionEvent e)
    {
        ...
        if (e.getValueIsAdjusting())
            return;
        if (logger.isLoggable(Level.FINE)) logger.fine("valueChanged() ListSelectionEvent=" + e + " ListSelectionModel=" + _listSelectionModel.toString());
        ...
        */
        for (int i = beginIndex; i <= endIndex; i++) {
             if (_listSelectionModel.isSelectedIndex(i) 
                    != _controller.getSelectionManager().selectionContains(_model.elementAt(i))) {
                 if (logger.isLoggable(Level.FINE)) logger.fine("Selection status for object "+_model.elementAt(i)+" at index "+i+" has changed");
                if (_listSelectionModel.isSelectedIndex(i)) {
                    // Change for addition
...
*/
    ...
    {
    	if (_table.isEditing() && _table.getCellEditor()!=null && getModel().getModel()!=null && getModel().getModel()!=newModel && newModel!=null)
    		_table.getCellEditor().stopCellEditing();
    ...
    protected void refreshNow() {
        ...
        // After updating the content of the table, we set the selection back to its previous state (content has change but not the selection)
...
public void fireObjectDeselected(FlexoModelObject object)
    {
            ...
            if ((getMasterTabularView() != null) && (parent != null)) {
                ...
                if ((getMasterTabularView().getSelectedObjects().contains(parent)
                        && (!getSelectionManager().selectionContains(parent)))) {
           ...
           if ((getSlaveTabularViews().size() > 0) 
