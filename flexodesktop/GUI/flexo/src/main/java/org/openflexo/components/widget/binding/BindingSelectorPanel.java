...
class BindingSelectorPanel extends BindingSelector.AbstractBindingSelectorPanel implements ListSelectionListener
	{
		...
		private BindingColumnElement findElementMatching(ListModel listModel, String subPartialPath, Vector<Integer> pathElementIndex)
		{
			for(int i=0;i<listModel.getSize();i++){
				if((listModel.getElementAt(i) instanceof BindingColumnElement) && ((BindingColumnElement)listModel.getElementAt(i)).getLabel().startsWith(subPartialPath)){
		...
		public void addNewEntryCreationPanel()
		{
			ActionListener actionListener = new ActionListener() {
				public void actionPerformed(ActionEvent e)
				{
					...
					if ((BindingSelectorPanel.this._bindingSelector.getEditedObject() != null) && (BindingSelectorPanel.this._bindingSelector.getEditedObject() instanceof BindingValue)) {
						BindingValue bv = (BindingValue)BindingSelectorPanel.this._bindingSelector.getEditedObject();
						if ((bv.getBindingVariable() != null)
								&& (bv.getBindingVariable().getType() != null)
							...
							for (BindingPathElement bpe : bv.getBindingPath()) {
								if ((bpe.getType() != null)
										&& (bpe.getType().getBaseEntity() != null)
					...
					final StaticDropDownParameter<String> kindOfPropertyParam
					= new StaticDropDownParameter<String>("kindOfProperty","kind_of_property",kindOfProperty,(currentEntity instanceof DMEOEntity?EO_ATTRIBUTE:BASIC_PROPERTY));
					kindOfPropertyParam.setShowReset(false);

					if (parentEntities.size() == 1) {
						parentEntity = new ReadOnlyTextFieldParameter("parentEntity","creates_entry_in_entity",parentEntities.firstElement().getName());
					}
					else {
						parentEntity = new DynamicDropDownParameter<DMEntity>("parentEntity","creates_entry_in_entity",parentEntities,currentEntity) {
							@Override
					...
					ChoiceListParameter<DMPropertyImplementationType> newPropertyImplementationType
					= new ChoiceListParameter<DMPropertyImplementationType>(
							"implementation",
							"implementation",
							currentEntity.getPropertyDefaultImplementationType());
					...
					final DynamicDropDownParameter<DMEOPrototype> prototypeParam = new DynamicDropDownParameter<DMEOPrototype>(
							"prototype", "prototype", allEOPrototypes, null);
					...
					AskParametersDialog dialog = AskParametersDialog.createAskParametersDialog(
							currentEntity.getProject(), owner,
								...
								public boolean isValid(ParametersModel model) {
									...
									if ((model.objectForKey("propertyName") == null)
										...
										if (!DMRegExp.ENTITY_NAME_PATTERN.matcher((CharSequence) model.objectForKey("propertyName")).matches()) {
											errorMessage = FlexoLocalization.localizedForKey("property_name_must_start_with_a_letter_and_be_followed_by_digits_or_letters");
											return false;
										}
										if ((entity != null) && (entity.getDeclaredProperty((String)model.objectForKey("propertyName")) != null)) {
											errorMessage = FlexoLocalization.localizedForKey("a_property_with_this_name_is_already_declared_in_this_entity");
											return false;
										}
					...
					if (dialog.getStatus() == AskParametersDialog.VALIDATE) {
						...
						if ((selectionContainsEOEntity && kindOfPropertyParam.getValue().equals(BASIC_PROPERTY))
								|| (!selectionContainsEOEntity)) {
							newProperty = BindingSelectorPanel.this._bindingSelector.createsNewEntry(newPropertyNameParam.getValue(), newPropertyTypeParam.getValue(), newPropertyImplementationType.getValue(),currentEntity);
						}
						else {
							try {
								newProperty = DMEOAttribute.createsNewDMEOAttribute(currentEntity.getDMModel(),(DMEOEntity)currentEntity, newPropertyNameParam.getValue(), false, true, DMPropertyImplementationType.PUBLIC_ACCESSORS_ONLY);
						...
						if ((BindingSelectorPanel.this._bindingSelector.getBindingDefinition() != null) && (BindingSelectorPanel.this._bindingSelector.getBindingDefinition().getType() != null)) {
							if (BindingSelectorPanel.this._bindingSelector.getBindingDefinition().getType().isAssignableFrom(newBindingValue.getAccessedType(),true)) {
...
//                        }
		...
		protected class TranstyperPanel extends JPanel
		{
			...
			protected TranstyperPanel()
			{
				...
				useTranstyperCB.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						if (useTranstyperCB.isSelected()) {
							BindingSelectorPanel.this._bindingSelector._editedObject = new TranstypedBinding(BindingSelectorPanel.this._bindingSelector.getBindingDefinition(),(FlexoModelObject)BindingSelectorPanel.this._bindingSelector.getBindable());
							((TranstypedBinding)BindingSelectorPanel.this._bindingSelector._editedObject).setTranstyper((DMTranstyper)transtyperCB.getSelectedItem());
				...
				transtyperCB.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						TranstypedBinding newBinding = new TranstypedBinding(BindingSelectorPanel.this._bindingSelector.getBindingDefinition(),(FlexoModelObject)BindingSelectorPanel.this._bindingSelector.getBindable());
			...
			void updateTranstyperPanel()
			{
				...
				if (isVisible) {
					...
					for (DMTranstyper tt : BindingSelectorPanel.this._bindingSelector.getProject().getDataModel().getDMTranstypers(BindingSelectorPanel.this._bindingSelector.getBindingDefinition().getType())) {
					...
					if ((BindingSelectorPanel.this._bindingSelector.getEditedObject() instanceof TranstypedBinding)
						...
						availableTranstypers.setSelectedItem(((TranstypedBinding)BindingSelectorPanel.this._bindingSelector.getEditedObject()).getTranstyper());
					}
				}
				...
				_expressionButton = _controlPanel.addButton("expression",new ActionListener() {
					public void actionPerformed(ActionEvent e)
					{
						if (BindingSelectorPanel.this._bindingSelector.getEditedObject() != null) {
							BindingSelectorPanel.this._bindingSelector.activateBindingExpressionMode(new BindingExpression(BindingSelectorPanel.this._bindingSelector.getBindingDefinition(), (FlexoModelObject) BindingSelectorPanel.this._bindingSelector.getBindable(), BindingSelectorPanel.this._bindingSelector.getEditedObject()));
						}
						else {
		...
		protected void updateSearchedTypeLabel()
		{
			if ((_bindingSelector.getEditedObject() != null)
					&& (_bindingSelector.getEditedObject() instanceof TranstypedBinding)
					&& (((TranstypedBinding)_bindingSelector.getEditedObject()).getTranstyper() != null)) {
				searchedTypeLabel.setText(((TranstypedBinding)_bindingSelector.getEditedObject()).getTranstyper().getTranstyperStringRepresentation());
			}
			else if (_bindingSelector.getBindingDefinition() != null) {
				if (BindingSelector.logger.isLoggable(Level.FINE)) {
					BindingSelector.logger.fine("updateSearchedTypeLabel() with "+_bindingSelector.getBindingDefinition().getTypeStringRepresentation());
				}
			...
			newList.addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e)
				{
					if (e.getClickCount() == 2) {
						if ((BindingSelectorPanel.this._bindingSelector.getEditedObject() != null) && ((BindingSelectorPanel.this._bindingSelector.getEditedObject()).isBindingValid())) {
							BindingSelectorPanel.this._bindingSelector.apply();
						}
					}
					...
					if (e.getKeyChar() == '\n') {
						...
						if (bindingValue instanceof BindingValue) {
							...
							if((i>-1) && (i<_lists.size()) &&
								...
								((BindingValue)bindingValue).setBindingPathElementAtIndex((BindingPathElement)listAtIndex(i+1).getModel().getElementAt(0).getElement(), i);
		...
		protected void resetMethodCallPanel()
		{
			if ((_bindingSelector.getEditedObject() == null)
					...
					|| ((_bindingSelector.getEditedObject() instanceof BindingValue) && (((BindingValue)_bindingSelector.getEditedObject()).getBindingPath().size() == 0))) {
		...
		{
			if (BindingSelector.logger.isLoggable(Level.FINE)) {
				BindingSelector.logger.fine("updateMethodCallPanel with "+_bindingSelector.editionMode+" binding="+_bindingSelector.getEditedObject()+" _selectedPathElementIndex="+_selectedPathElementIndex);
			}
		...
		{
			if (BindingSelector.logger.isLoggable(Level.FINE)) {
				BindingSelector.logger.fine("fireBindingDefinitionChanged / Setting new binding definition: "+_bindingSelector.getBindingDefinition());
			}
		...
		protected void update()
		{
			...
			if ((binding == null) || (binding instanceof StaticBinding)) {
				...
				if (bindingValue.getBindingVariable() != null) {
					...
					BindingColumnElement elementToSelect
					= listAtIndex(0).getModel().getElementFor(bindingValue.getBindingVariable());
					...
					for (int i = 0; i < bindingValue.getBindingPath().size(); i++) {
						...
						if (pathElement instanceof DMProperty) {
							...
							= listAtIndex(i+1).getModel().getElementFor(((MethodCall)pathElement).getMethod());
				...
				if ((columnElement.getElement() != null) && (columnElement.getElement() instanceof BindingVariable)) {
					...
					if ((binding != null) && (binding instanceof BindingValue)) {
						...
						if (bindingValue.isConnected()
								&& (bindingValue.isLastBindingPathElement(property, getIndexOfList(this) - 1))) {
							...
							if (columnElement.getResultingType().getKindOfType() == KindOfType.RESOLVED) {
								if (columnElement.getResultingType().getBaseEntity() == null) {
									BindingSelector.logger.warning("Unexpected type: "+property.getType()+" kind: "+property.getType().getKindOfType());
								}
									...
									if ((BindingSelectorPanel.this._bindingSelector.getBindingDefinition() != null)
											...
											&& (!BindingSelectorPanel.this._bindingSelector.getBindingDefinition().getType().isAssignableFrom(columnElement.getResultingType(),true))) {
										return true;
									}
									if ((BindingSelectorPanel.this._bindingSelector.getBindingDefinition() != null)
					...
					if (binding instanceof BindingValue) {
						...
						if ((bindingValue.isConnected())
								&& (bindingValue.isLastBindingPathElement(bpe, getIndexOfList(this) - 1))
							...
							if (columnElement.getResultingType().getBaseEntity().getAccessibleProperties().size()
								...
								if ((BindingSelectorPanel.this._bindingSelector.getBindingDefinition() != null) && ((BindingSelectorPanel.this._bindingSelector.getBindingDefinition().getType() != null) && (BindingSelectorPanel.this._bindingSelector.getBindingDefinition().getType().getBaseEntity()!=null))
										&& (!BindingSelectorPanel.this._bindingSelector.getBindingDefinition().getType().getBaseEntity().isAncestorOf(columnElement.getResultingType().getBaseEntity()))) {
									return true;
								}
							}
						}
		...
		private class RootBindingColumnListModel extends BindingColumnListModel
		{
			...
			public void updateValues()
			{
				...
				for (int i=0; i<_myBindingModel.getBindingVariablesCount(); i++) {
					_elements.add(new BindingColumnElement(_myBindingModel.getBindingVariableAt(i),_myBindingModel.getBindingVariableAt(i).getType()));
				}
			}
					...
					if (bce instanceof BindingColumnElement) {
						...
						if ((columnElement.getElement() != null) && (columnElement.getElement() instanceof BindingVariable)) {
							...
							if ((binding != null) && (binding instanceof BindingValue)) {
								BindingValue bindingValue = (BindingValue)binding;
								if (bindingValue.isConnected()
										&& (bindingValue.isLastBindingPathElement(property, _lists.indexOf(list) - 1))) {
							...
							if ((binding != null) && (binding instanceof BindingValue)) {
								...
								if ((bindingValue.isConnected())
										&& (bindingValue.isLastBindingPathElement(bpe, _lists.indexOf(list) - 1))
										&& ((bpe instanceof MethodCall) && (((MethodCall)bpe).getMethod() == method))) {
									setIcon(label, FIBIconLibrary.CONNECTED_ICON, list);
								} else if ((columnElement.getResultingType() != null) && (columnElement.getResultingType().getBaseEntity() != null)) {
									if (columnElement.getResultingType().getBaseEntity().getAccessibleProperties().size()
...
//					fireEditedObjectChanged();
		...
		protected class CompletionInfo
		{
			...
			@Override
			public String toString() {
				return "CompletionInfo, completionInitPath="+completionInitPath+" validPath="+validPath+" commonBeginningPath="+commonBeginningPath+" matchingElements="+matchingElements;
			}
		...
		protected void processBackspace()
		{
			...
			if(!suppressSelection()){
				if(_bindingSelector.getTextField().getText().length()>0){
					_bindingSelector.getTextField().setText(_bindingSelector.getTextField().getText().substring(0,_bindingSelector.getTextField().getText().length()-1));
		...
		private boolean suppressSelection()
		{
			if(_bindingSelector.getTextField().getText().length()>0){
				if((_bindingSelector.getTextField().getSelectedText()!=null) && (_bindingSelector.getTextField().getSelectedText().length()>0)){
					...
					_bindingSelector.getTextField().setText(_bindingSelector.getTextField().getText().substring(0,begin)+_bindingSelector.getTextField().getText().substring(end,_bindingSelector.getTextField().getText().length()));
					return true;
				}
			}
			return false;
		...
		boolean isKeyPathFromPanelValid()
		{
			...
			if (listAtIndex(i-1).getSelectedValue() instanceof BindingColumnElement){
				if ((_bindingSelector.getBindingDefinition().getType() == null)
						|| _bindingSelector.getBindingDefinition().getType().isAssignableFrom(((BindingColumnElement)listAtIndex(i-1).getSelectedValue()).getResultingType(),true)) {
					return true;
				}
			}
			return false;
