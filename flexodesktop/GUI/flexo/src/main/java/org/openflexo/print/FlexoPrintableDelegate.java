...
public class FlexoPrintableDelegate implements Printable, Pageable {
    ...
    private enum PaintMode { PREVIEW, PRINTING };
    ...
    public PaintParameters paintPrelude(Graphics2D g2)
    {
        ...
        if (_paintMode == PaintMode.PREVIEW) {
            ...
            returned.marginScale = Math.min((double)imageablePageBounds.width/(double)pageBounds.width, (double)imageablePageBounds.height/(double)pageBounds.height);
    ...
    private void showPages(Graphics2D g2) 
    {
       ...
       for (int i=0; i<widthPageNb; i++) {
            for (int j=0; j<heightPageNb; j++) {
                ...
                if (_paintMode == PaintMode.PREVIEW) {
                    g2.drawString("Page: "+(pageIndex+1)+"/"+getNumberOfPages(),
                            pageBounds.width/2-35 + i*pageBounds.width, 
    ...
    private void showTitles(Graphics2D g2) 
    {
        ...
        for (int i=0; i<widthPageNb; i++) {
            for (int j=0; j<heightPageNb; j++) {
                //int pageIndex =  j*widthPageNb+i;
                if (_paintMode == PaintMode.PREVIEW) {
                    g2.drawString(getPrintTitle(),
                            (int)(pageBounds.width/2-titleBounds.getWidth()/2) + i*pageBounds.width, 
   ...
   public double getPreviewScale() 
    ...
    public void fitToPage()
    {
        if (_pageFormat != null) {
            ...
            if (scaleX < scaleY) setScale(scaleX); else setScale(scaleY);
        }
    }
    ...
    {
        Rectangle pageBounds = getPageRect(_pageFormat);
        Dimension viewSize = new Dimension((int)(widthPageNb*pageBounds.width*previewScale),(int)(heightPageNb*pageBounds.height*previewScale));
        if (logger.isLoggable(Level.FINER))
