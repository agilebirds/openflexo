...
*/
    ...
    {
        super(makeBrowserConfiguration(controller.getProject(),DocResourceManager.instance().getDocResourceCenter()), controller.getSelectionManager() /* Remove this parameter if you don't want browser synchronized with selection */);
          ...
          protected class DREBrowserElementFactory implements BrowserElementFactory
        {
           /* public BrowserElementType DOC_ITEM_FOLDER = new DocItemFolderElementType();
            ...
            public void configure(ProjectBrowser browser)
            {
                ...
                browser.setFilterStatus(BrowserElementType.AVAILABLE_NEW_VERSION_PENDING_DOC_ITEM,BrowserFilterStatus.OPTIONAL_INITIALLY_SHOWN);
...
*/
		...
		public BrowserElement makeNewElement(FlexoModelObject object, ProjectBrowser browser, BrowserElement parent)
                ...
                if (object instanceof DocItemFolder) {
                	...
                	int status = ((DocItem)object).getStatusForLanguage(currentLanguage);
                     if (status == DocItem.NO_DOCUMENTED) {
                        ...
                        return new DocItemElement((DocItem)object,BrowserElementType.AVAILABLE_NEW_VERSION_PENDING_DOC_ITEM,browser,parent);
                    }
            ...
            {
                public AbstractDREElement(DRMObject object, BrowserElementType elementType,
				...
				public void update(FlexoObservable observable, DataModification dataModification)
                {
                    ...
                    if ((_browser != null)
                            && (dataModification instanceof DRMDataModification)) {
                        refreshWhenPossible();
                    }
                    else {
           ...
           private class DocItemFolderElement extends AbstractDREElement
            {
				...
				protected void buildChildrenVector()
                {
                    ...
                    for (Enumeration en=getDocItemFolder().getOrderedItems().elements(); en.hasMoreElements();) {
                        ...
                        if ((next.getEmbeddingParentItem() == null)
                                || (next.getEmbeddingParentItem().getFolder() != next.getFolder())) {
                            addToChilds(next);
                        }
                    }
                 }
	...
	{
		if (_availableLanguages != null) return _availableLanguages.getValue();
		else return DocResourceManager.instance().getLanguage(GeneralPreferences.getLanguage());
	}
}
