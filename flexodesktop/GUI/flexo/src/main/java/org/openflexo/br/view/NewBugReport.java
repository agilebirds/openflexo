...
public class NewBugReport extends FlexoDialog
{
    ...
    private static String sendToDLPM(BugReport bugReport, int timeoutInSeconds, int numberOfTries)
    {
        ...
        bugReport.description = bugReport.description == null || bugReport.description.trim().length() == 0 ? null : bugReport.description.trim();
...
/*        param.put("foundInReleaseID", FlexoCst.DLPM_RELEASE_VERSION_ID);
        ...
        while(t.isAlive() && tries<numberOfTries) {
			...
			if (t.isAlive()) {
				if (tries<numberOfTries) {
					if(FlexoController.confirm(FlexoLocalization.localizedForKey("could_not_send_incident_so_far_keep_trying")+"? ("+tries+")"))
						continue;
				}
	...
	protected static String sendRequest(Hashtable<String, String> param) {
        ...
        try {
        	...
        	return FlexoLocalization.localizedForKey("host")+" \""+e.getMessage()+"\" "+FlexoLocalization.localizedForKey("cannot_be_reached");
        } catch (IOException e) {
            if (logger.isLoggable(Level.SEVERE)) {
                logger.severe(url);
                logger.log(Level.SEVERE, "Error while sending bug to url "+url, e);
    ...
    {
        return FlexoLocalization.localizedForKey("An error has occured !")+((ModuleLoader.getUserType() == UserType.DEVELOPER || ModuleLoader.getUserType() == UserType.MAINTAINER)?response.response:"");
    }
