...
public class BindingExpressionPanel extends JPanel implements FocusListener {
	...
	protected BindingExpressionStringConverter converter;
		...
		if (_bindingExpression.getExpression() == null) {
			...
			try {
				message = FlexoLocalization.localizedForKey(UNDEFINED_OPERAND_FOR_OPERATOR) + " " + undefinedOperator.getLocalizedName()
						+ " [" + pp.getSymbol(undefinedOperator) + "]";
			...
			try {
				...
				if (_bindingExpression.getBindingDefinition() != null && _bindingExpression.getBindingDefinition().getType() != null) {
					...
					if (wantedEvaluationType == EvaluationType.LITERAL || evaluationType == wantedEvaluationType
						...
						message = FlexoLocalization.localizedForKeyWithParams(VALID_EXPRESSION_BUT_MISMATCH_TYPE,
	...
	private void init() {
		...
		showEvaluationButton.setNormalIcon(IconLibrary.TOGGLE_ARROW_BOTTOM_ICON);
		showEvaluationButton.setMouseOverIcon(IconLibrary.TOGGLE_ARROW_BOTTOM_SELECTED_ICON);
		...
		showEvaluationButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (!evaluationPanel.isVisible()) {
					showEvaluationButton.setNormalIcon(IconLibrary.TOGGLE_ARROW_TOP_ICON);
					showEvaluationButton.setMouseOverIcon(IconLibrary.TOGGLE_ARROW_TOP_SELECTED_ICON);
		...
		moreButton.setNormalIcon(IconLibrary.TOGGLE_ARROW_BOTTOM_ICON);
		moreButton.setMouseOverIcon(IconLibrary.TOGGLE_ARROW_BOTTOM_SELECTED_ICON);
		moreButton.setToolTipText(FlexoLocalization.localizedForKey("show_more_operators"));
		moreButton.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				if (!mathControls.isVisible()) {
					moreButton.setNormalIcon(IconLibrary.TOGGLE_ARROW_TOP_ICON);
					moreButton.setMouseOverIcon(IconLibrary.TOGGLE_ARROW_TOP_SELECTED_ICON);
	...
	private JPanel createOperatorGroupPanel(String title, Operator... operators) {
		...
		returned.setBorder(BorderFactory.createTitledBorder(null, FlexoLocalization.localizedForKey(title), TitledBorder.CENTER,
				TitledBorder.TOP, new Font("SansSerif", Font.ITALIC, 8)));
		return returned;
	}
		...
		private void update() {
			...
			/*else if (_representedExpression instanceof Variable || _representedExpression instanceof Constant) {
				...
				if (binding != null) {
					...
					_bindingSelector.setBindingDefinition(new BindingDefinition("common", DMType.makeObjectDMType(getProject()),
							_bindingExpression, BindingDefinitionType.GET, true));
				}
	...
	public FlexoProject getProject() {
		if (_bindingExpression != null) {
			return _bindingExpression.getProject();
		}
		return null;
	}
