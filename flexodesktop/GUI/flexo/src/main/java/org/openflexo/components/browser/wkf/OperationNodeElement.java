...
public class OperationNodeElement extends BrowserElement implements ExpansionSynchronizedElement {
	...
	protected void buildChildrenVector() {
		...
		*/
		...
		for (Enumeration<FlexoPostCondition<AbstractNode, AbstractNode>> e = getOperationNode().getOutgoingPostConditions().elements(); e
				.hasMoreElements();) {
			addToChilds(e.nextElement());
		}
	...
	public boolean isExpanded() {
		if (getOperationNode() instanceof SelfExecutableNode) {
			return ((SelfExecutableNode) getOperationNode()).hasExecutionPetriGraph()
					&& ((SelfExecutableNode) getOperationNode()).getExecutionPetriGraph().getIsVisible();
		}
	...
	public void expand() {
		...
		} else if (getOperationNode().hasContainedPetriGraph() && !getOperationNode().isBeginNode() && !getOperationNode().isEndNode()
				&& !isExpanded()) {
