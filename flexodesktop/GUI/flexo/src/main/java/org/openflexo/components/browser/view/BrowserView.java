 ...
 * @author sguerin
 */
public abstract class BrowserView extends JPanel implements FlexoActionSource,ProjectBrowserListener, TreeSelectionListener, TreeExpansionListener
{
    ...
    {
    	/**
    	 * This policy indicates that selections performed by this browser are
    	 * repercuted to current selection. An object addition or removing are
    	 * notified to Selection Manager which handle it. If Selection Manager
    	 * contains an object that this browser cannot represent, this object
    	 * is still maintained by Selection Manager
    	 * With this policy, a selection could be represented by many objects
    	 * in many browsers
    	 */
    	ParticipateToSelection,
    	/**
    	 * This policy indicates that selection maintained by Selection Manager
    	 * is performed only by this browser. Any selection modification performed
    	 * in this browser are repercuted into Selection Manager which takes
    	 * as selection all but only all objects selected in this browser.
    	 */
    	ForceSelection
    }
    ...
    {
    	if(!_browser.handlesControlPanel())return null;
        if (_browser.getSelectionManager() != null)
	...
	public void treeExpanded(TreeExpansionEvent event)
    {
        ...
        if (_browser.isExpansionSynchronizedElement(element) && element.isSynchronizeExpansionEnabled()) {
            ...
            if (superviseExpansion) {
            	...
            	// GPO: If we come here, it means that we have been notified by the selection manager and we want to know if the expansion caused by
    			// the new selection (selected objects in JTree are automatically shown (thus all its parents are automatically expanded)) must be
            	...
            	if(_browser.getSelectionManager()!=null)
            		for (FlexoModelObject o:_browser.getSelectionManager().getSelection())
            			if (element.contains(o))// If the expanded element contains object from the selection, then we synchronize the browser
    ...
    public class FlexoJTree extends JTree implements Autoscroll
    {
        ...
        public void addSelectionPaths(TreePath[] paths)
        {
            ...
            if (GeneralPreferences.getSynchronizedBrowser() && paths != null && paths.length > 0) {
				...
				while(treePath!=null) {
					if (!treeView.isExpanded(treePath.getParentPath()))// If my parent is not expanded, then I'm not visible
        ...
        public void addTreeSelectionListener(TreeSelectionListener tsl) {
        	/**
        	 * GPO: The code hereunder is here to prevent a same tree selection listener from
        	 * being added twice to the same view making all further add/remove (treeSelectionListener()) call
        	 * completely useless. This was called "Bug critique difficilement reproductible" reported by FVA.
        	 */
        ...
        public void removeTreeSelectionListener(TreeSelectionListener tsl) {
        	/**
        	 * GPO: The code hereunder is here to track when we are not really removing
        	 * <code>BrowserView.this</code> from the tree selection listeners because another action has already
        	 * performed this. This trace is very interesting to find which event is starting the other
        	 * one. You should see next a log "Adding twice the same tsl: " following this log.
        	 * This was called "Bug critique difficilement reproductible" reported by FVA.
        	 */
        ...
        public void captureDraggedNode(TreePath path, Point offset)
        {
            ...
            JComponent lbl = (JComponent)getCellRenderer().getTreeCellRendererComponent(this, path.getLastPathComponent(), false , isExpanded(path),getModel().isLeaf(path.getLastPathComponent()), 0,false);//returning the label
            lbl.setBounds(pathBounds);//setting bounds to lbl
            capturedDraggedNodeImage = new BufferedImage(lbl.getWidth(), lbl.getHeight(), java.awt.image.BufferedImage.TYPE_INT_ARGB_PRE);//buffered image reference passing the label's ht and width
            Graphics2D graphics = capturedDraggedNodeImage.createGraphics();//creating the graphics for buffered image
            graphics.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.7f));   //Sets the Composite for the Graphics2D context
            lbl.setOpaque(false);
