...
public class DescriptionInspectorWidget extends CustomInspectorWidget<FlexoModelObject>
{
    ...
    protected class DescriptionPanel extends JPanel implements FlexoObserver
    {
        ...
        public DescriptionPanel()
        {
            ...
            boolean showViewSourceButtonInWysiwyg = getController().getConfiguration()!=null && getController().getConfiguration().showViewSourceButtonInWysiwyg();
       ...
       public void updateWidgetFromModel()
		...
		private void snapBottomPanelToMinimalSize() {
			if (splitPane.getHeight()>0)
				splitPane.setDividerLocation(splitPane.getHeight() - DescriptionPanel.this.getPreferredSize().height-splitPane.getDividerSize());
		}
    ...
    public FlexoModelObject getObject()
    {
            ...
            for (int i = 0; i < s.length - 1; i++) {
                ...
                if (!(returned instanceof KVCObject)) {
                    if (logger.isLoggable(Level.SEVERE))
                        logger.severe("Someting is wrong with an inspector. Inspected property path contains object that are not KVCObject " + getPropertyModel().name);
                    return null;
                } else
        ...
        if (o instanceof FlexoModelObject)
            ...
            logger.severe("Object at the end of property path is not a FlexoModelObject: " + getPropertyModel().name+ " this is a "+o+" of "+o.getClass().getSimpleName());
        return null;
    }
    ...
    {
    	if (FocusManager.getCurrentManager().getFocusOwner()!=null && getDynamicComponent()!=null && SwingUtilities.isDescendingFrom(FocusManager.getCurrentManager().getFocusOwner(),getDynamicComponent())) {
    		// If we have the focus and we switch the observed object, we first set the last values on the previous object (so that data does not get lost)
    		updateModelFromWidget();
    	}
        super.setModel(value);
        if (value != null) {
        	if (value  instanceof FlexoModelObject && ((FlexoModelObject)value).getProject()!=null) {
        		defaultWidget.getDynamicComponent().addSupportForInsertedObjects(((FlexoModelObject)value).getProject().getImportedImagesDir());
	...
	protected boolean useUltraLightWysiwyg() {
		return getPropertyModel().hasValueForParameter("useUltraLightWysiwyg") && getPropertyModel().getBooleanValueForParameter("useUltraLightWysiwyg");
	}
