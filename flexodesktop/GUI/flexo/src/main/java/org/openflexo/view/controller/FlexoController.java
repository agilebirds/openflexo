...
public abstract class FlexoController implements InspectorNotFoundHandler, InspectorExceptionHandler
{
	...
	public static boolean confirmWithWarning(String msg) throws HeadlessException
	{
		return showOptionDialog(getActiveFrame(), msg, FlexoLocalization.localizedForKey("information"), JOptionPane.YES_NO_OPTION,
				JOptionPane.WARNING_MESSAGE, null, new Object[] { FlexoLocalization.localizedForKey("yes"),
	...
	public static int confirmYesNoCancel(String localizedMessage) {
		return showOptionDialog(getActiveFrame(), localizedMessage, localizedMessage, JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, null, null, null);
	}
	...
	/**
	 * The selection manager manages the inspector, but we can "force" the
	 * inspector with this method
	 *
	 * @deprecated
	 */
	/*
	 ...
	 *            properly; non-<code>String</code> objects are rendered
	 *            using their <code>toString</code> methods; if this parameter
	 *            is <code>null</code>, the options are determined by the
	 *            Look and Feel
	 * @param initialValue
	...
	private static synchronized int showOptionDialog(Component parentComponent, Object message, String title, int optionType,
		...
		if (optionType == JOptionPane.OK_CANCEL_OPTION && options == null) {
			...
			availableOptions = new Object[] { FlexoLocalization.localizedForKey("yes"), FlexoLocalization.localizedForKey("no"), FlexoLocalization.localizedForKey("cancel") };
			pane = new JOptionPane(message, messageType, optionType, icon, availableOptions, availableOptions[0]) {
				@Override
				public int getMaxCharactersPerLineCount() {
					return FlexoController.getMaxCharactersPerLine(parent, this);
		...
		if (window!=null && window.isVisible()) {
			maxDim = new Dimension(Math.min(dialog.getWidth(),window.getGraphicsConfiguration().getDevice().getDefaultConfiguration().getBounds().width),
					Math.min(dialog.getHeight(),window.getGraphicsConfiguration().getDevice().getDefaultConfiguration().getBounds().height));
		} else {
		...
		if (window!=null && window.isVisible()){
			...
			Dimension dim = new Dimension(locationOnScreen.x+window.getWidth()/2,
					locationOnScreen.y+window.getHeight()/2);
	...
	public void switchToPerspective(FlexoPerspective perspective)
	{
		...
		if (_currentPerspective != perspective)
		{
			...
			logger.fine("switchToPerspective "+perspective+" with object "+newEditedObject+(newEditedObject != null ? " of "+newEditedObject.getClass().getSimpleName() : ""));
	...
	/**
	 * Internal method to set the current perspective. Can possibly be used to
	 * notify views for perspective change. For the rest of the application, the
	 * method {@link #switchToPerspective(FlexoPerspective)} can be used.
	 *
	 * @param perspective
	 */
	 ...
	 * indicates if this view must be build if not already existant.
	 *
	 * @param object
		...
		try {
			...
			if (getLoadedViewsForCurrentPerspective().get(object) == null) {
				if (recalculateViewIfRequired) {
					ModuleView<? extends O> view = createModuleViewForObjectAndPerspective(object, (FlexoPerspective<O>)_currentPerspective);
					if (view != null) {
	...
	/**
	 * Returns an initialized view (build and initialize a new one, or return
	 * the stored one) representing supplied object.If not already existant,
	 * build the view.
	 *
	 * @param object
	 * @return an initialized ModuleView instance
	...
	/**
	 * Creates a new view dor supplied object, or null if this object is not
	 ...
	 */
	public final <O extends FlexoModelObject> ModuleView<? extends O> createModuleViewForObjectAndPerspective(O object, FlexoPerspective<O> perspective)
	{
		try {
			if (perspective == null) {
			...
			logger.warning("Perspective "+perspective+" is not supposed to represent objects of class "+object.getClass().getSimpleName()+"\n"+e.getMessage());
			// Check the method hasModuleViewForObject(FlexoModelObject) in the perspective, it is more than probably the cause of the ClassCast
			e.printStackTrace();
			return null;
		}
	}
	...
	@SuppressWarnings("unchecked")
	public final <O extends FlexoModelObject> boolean hasViewForObjectAndPerspective(FlexoModelObject object, FlexoPerspective<O> perspective)
	{
		if (object == null) {
			return false;
		}
		try {
			...
			if (logger.isLoggable(Level.FINE)) {
				logger.fine("Perspective "+perspective+" is not supposed to represent objects of class "+object.getClass().getSimpleName());
			}
			return false;
		}
	}
	...
	public void removeModuleView(ModuleView aView)
	{
		if (aView.getRepresentedObject() != null) {
			if (getLoadedViewsForCurrentPerspective().get(aView.getRepresentedObject()) == aView) {// Let's make sure we remove the proper view!
				getLoadedViewsForCurrentPerspective().remove(aView.getRepresentedObject());
			}
			if (aView.getPerspective()!=null && getLoadedViewsForPerspective(aView.getPerspective()).get(aView.getRepresentedObject()) == aView) {
	...
	/**
	 * Return hashtable where are stored all the loaded view, and where the key
	 * is the related FlexoModelObject itself
	 *
	...
	public class FlexoControllerInspectorDelegate implements InspectorDelegate
	{
		 ...
		 *
		 * @see org.openflexo.inspector.InspectorDelegate#performAction(java.lang.String,
		 *      java.lang.Object)
		 */
		@Override
	...
	public void handleWSException(RemoteException e) {
		...
		if (e instanceof PPMWebServiceAuthentificationException) {
			...
			FlexoController.notify(FlexoLocalization.localizedForKey("connection_error")+": "+FlexoLocalization.localizedForKey(ioEx.getClass().getSimpleName())+" "+ioEx.getMessage());
		} else {
			if(e.getMessage()!=null && e.getMessage().indexOf("Content is not allowed in prolog")>-1){
				FlexoController.notify("Check your connection url in FlexoPreferences > Advanced.\n It seems wrong.\nsee logs for details.");
			} else {
				FlexoController.notify(FlexoLocalization.localizedForKey("webservice_remote_error")+" \n"+(e.getMessage()==null || "java.lang.NullPointerException".equals(e.getMessage())?"Check your connection parameters.\nThe service may be temporary unavailable.":e.getMessage()));
			}
		}
	}
	...
	public void handleWSException(PPMWebServiceAuthentificationException e) {
		FlexoController.notify(FlexoLocalization.localizedForKey("webservice_authentification_failed")+": "+FlexoLocalization.localizedForKey(e.getMessage1()));
	}
	...
	public FlexoProgress willLoad(File fibFile)
	{
		...
		if (!FIBLibrary.instance().componentIsLoaded(fibFile)) {
			FlexoProgress progress = getEditor().getFlexoProgressFactory().makeFlexoProgress(FlexoLocalization.localizedForKey("loading_interface..."), 3);
