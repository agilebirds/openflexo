...
public class FullInteractiveProjectLoadingHandler extends InteractiveProjectLoadingHandler {
			...
			throws ProjectLoadingCancelledException 
	{
		...
		if (alwaysUpgradeResourceToLatestVersion) return true;
		...
		int choice = FlexoController.selectOption(
				...
				+FlexoLocalization.localizedForKey("this_resource_has_been_serialized_with_an_older_version_than_the_one_declared_as_current_application_version")+"<br>"
				+FlexoLocalization.localizedForKey("should_i_convert_this_resource_to_latest_version_(recommanded_choice)")+"<br>"
				+FlexoLocalization.localizedForKey("current_version")+" : <b>"+resource.getXmlVersion()+"</b><br>"
				+FlexoLocalization.localizedForKey("will_be_converted_to_version")+" : <b>"+resource.latestVersion()+"</b><br></html>",
				CONVERT_ALL, CONVERT_ALL, CONVERT, DONT_CONVERT, CANCEL);
			...
			throws ProjectLoadingCancelledException
	{
		...
		if (useOlderMappingHashtable.get(resource) != null) return useOlderMappingHashtable.get(resource);
		...
		int choice = FlexoController.selectOption(
				...
				+FlexoLocalization.localizedForKey("should_i_try_to_recover_by_using_older_versions")+"<br>"
				+"<i>"+FlexoLocalization.localizedForKey("you_may_loose_some_informations")+"</i><br></html>",
	...
	private static class ProjectConversionDialog extends FlexoDialog
	{
		...
		{
			super((JFrame)null,
					FlexoLocalization.localizedForKey("project_conversion"),true);
			LabelParameter infoLabel = new LabelParameter("info","info",
					"<html><center>"+CGIconLibrary.UNFIXABLE_WARNING_ICON.getHTMLImg()+"<b>&nbsp;"+FlexoLocalization.localizedForKey("warning")+"</b></center><br>"
					+"<center>"+project.getProjectDirectory().getName()+"</center><br>"
	    			+FlexoLocalization.localizedForKey("this_project_seems_to_have_been_created_with_an_older_version_of_flexo")+"<br>"
					+FlexoLocalization.localizedForKey("would_you_like_to_convert_entire_project_to_new_version_of_flexo_(recommanded)")+"<br>"
					+"<i>"+FlexoLocalization.localizedForKey("following_resources_will_be_converted")+"</i>"+"</html>", false);
			final PropertyListParameter<ResourceToConvert> resourcesParam 
			= new PropertyListParameter<ResourceToConvert>("resources",
