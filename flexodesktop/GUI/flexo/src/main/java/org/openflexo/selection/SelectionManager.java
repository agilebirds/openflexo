...
/**
 * Abstract selection manager: there is one SelectionManager for each FlexoModule.
 * This manager is responsible of:
 * <UL>
 * <LI>Selection management: it holds a list of all currently selected
 * objects, as FlexoModelObject instances</LI>
 * <LI>Synchonization of all components synchronized with itself
 * objects, as FlexoModelObject instances</LI>
...
public abstract class SelectionManager extends Observable
{
    ...
    {
        if(!_selectionListeners.contains(listener))_selectionListeners.add(listener);
    }
     ...
     *
     * @param objects: objects to add to selection, as a Vector of FlexoModelObject
     */
     ...
     *
     * @param objects: the object to set for current selection, as a Vector of FlexoModelObject
     */
    ...
    /**
     * Request to all registered SelectionListener of this SelectionManager to be
    ...
    /**
     * override this method in your Module's selectionManager
     * if you want to specify another inspector for an object.
     */
     ...
     private void setCurrentInspectedObject(InspectableObject inspectable)
    {
    	...
    	if(!isInspectable(inspectable)) return;
        if (logger.isLoggable(Level.FINE))
    ...
    {
        if (!(object instanceof FlexoModelObject)) return true;
