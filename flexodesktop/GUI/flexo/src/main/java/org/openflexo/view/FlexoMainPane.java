...
public abstract class FlexoMainPane extends JPanel implements GraphicalFlexoObserver
{
    ...
    public FlexoMainPane(ModuleView moduleView, FlexoFrame mainFrame, FlexoController controller)
    {
     ...
     public FlexoMainPane(ModuleView moduleView, FlexoFrame mainFrame, FlexoController controller, boolean rightPaneIsSplitPane, boolean verticalOrientation)
    {
    ...
    public class ControlPanel extends JPanel
    {
        ...
        public ControlPanel()
        {
            ...
            availableObjectsActionListener = new ActionListener() {
                ...
                {
                    switchToModuleViewForObjectAndPerspective((FlexoModelObject) availableObjects.getSelectedItem(), _controller
            ...
            availableObjects.setRenderer(new DefaultListCellRenderer() {
				...
				public Component getListCellRendererComponent(JList list, Object value, int index, boolean isSelected, boolean cellHasFocus)
                {
                    ...
                    if ((value != null) && (!((FlexoModelObject) value).isDeleted())) {
                    	...
                    	if (title == null) {
                    		logger.warning("Unexpected object "+value+" asked for controller "+_controller+" perspective="+_controller.getCurrentPerspective());
                    		title = "???";
                    	}
                        returned.setText(title);
                        returned.setPreferredSize(new Dimension(returned.getFontMetrics(returned.getFont()).stringWidth(title)+15,returned.getFontMetrics(returned.getFont()).getHeight()));
                    } else {
                    	if(value==null) {
           ...
           if (_controller.getPerspectives().size() > 0) {
            ...
            backwardButton.setSize(IconLibrary.NAVIGATION_BACKWARD_ICON.getIconWidth(), IconLibrary.NAVIGATION_BACKWARD_ICON.getIconHeight());
	...
	protected void updateControlsForObjectRemovedFromHistory(FlexoModelObject removedObject) {
		if (logger.isLoggable(Level.INFO)) {
			logger.info("Synchronize control panel for Object deletion with " + removedObject + " of "
