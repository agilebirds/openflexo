			...
			public void valueChanged(ListSelectionEvent e)
            {
                ...
                if (lsm.isSelectionEmpty()) {
                    ...
                    if (_consistencyCheckDialog.getController() instanceof SelectionManagingController) {
                        ((SelectionManagingController) _consistencyCheckDialog.getController()).selectAndFocusObject(issue.getSelectableObject());
                    }
...
//            }
    ...
    protected void disableRule(ValidationIssue issue){
    	...
    	disableRuleButton.setText(issue.getCause().getIsEnabled()?FlexoLocalization.localizedForKey("disableRule", disableRuleButton):FlexoLocalization.localizedForKey("enableRule", disableRuleButton));
    }
    ...
    protected void applyFixProposal(FixProposal fixProposal)
    {
        if (fixProposal != null) {
            if (fixProposal instanceof ParameteredFixProposal) {
                ...
                AskParametersDialog askParams = AskParametersDialog.createAskParametersDialog(
                        fixProposal.getProject(),
                        null, FlexoLocalization.localizedForKey("validation_error_fixing"), fixProposal.getLocalizedMessage(), ((ParameteredFixProposal) fixProposal).getParameters());
