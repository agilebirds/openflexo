...
public abstract class BrowserElement implements TreeNode, FlexoObserver
{
    ...
    static final Logger logger = Logger.getLogger(BrowserElement.class.getPackage()
            .getName());
    ...
    public BrowserElement(FlexoModelObject object, BrowserElementType elementType,
            ProjectBrowser browser, BrowserElement parent)
    {
        ...
        if (object == null) {
            ...
            if (BrowserElementType.getBrowserElementTypeForClassName(object.getClass().getName())!=_elementType) {
            	if (logger.isLoggable(Level.WARNING))
					logger.warning("Element type do not match: "+BrowserElementType.getBrowserElementTypeForClassName(object.getClass().getName())+"!="+_elementType);
            }
    ...
    private void recursiveDeleteChilds()
    {
        if (_childs != null) {
            ...
            _childs.clear();// To be sure, but normally, they are all gone since delete() invokes parent.removeFromChilds()! Beware of concurrent modification of the _childs Vector
        }
    }
   ...
   protected abstract void buildChildrenVector();
    ...
    protected final Icon decorateIcon(Icon returned)
    {
    	if(AdvancedPrefs.getHightlightUncommentedItem() &&
    			getObject()!=null &&
    			getObject().isDescriptionImportant() &&
    ...
    protected ElementTypeBrowserFilter newBrowserFilter(BrowserFilterStatus initialFilterStatus)
    {
    	...
    	// can be overridden. Am I wrong?
    		...
    		/*return new ElementTypeBrowserFilter(getFilteredElementType().getName(), getFilteredElementType()
                .getIcon());*/
    	if (initialFilterStatus == null) initialFilterStatus = BrowserFilterStatus.SHOW;
    ...
    protected void addToChilds(FlexoModelObject modelObject)
    {
    	...
    	if (!matchAnyCustomFilter(modelObject)) return;
        if (modelObject != null) {
    ...
    private boolean matchAnyCustomFilter(FlexoModelObject modelObject)
    {
    	...
    	if (_browser.getCustomFilters().size() == 0) return true;
    	for (CustomBrowserFilter filter : _browser.getCustomFilters()) {
    		if (filter.getStatus() == BrowserFilterStatus.SHOW && filter.accept(modelObject)) return true;
    	}
    	return false;
    }
	...
	public void update(FlexoObservable observable, DataModification dataModification)
    {
        ...
        if (logger.isLoggable(Level.FINE))
            logger.fine(getClass().getName() + " receive DataModification "
                    + dataModification.getClass().getName());
        if (_browser != null) {
            if (((dataModification instanceof WKFDataModification)
            		|| (dataModification instanceof IEDataModification)
            		|| (dataModification instanceof DKVDataModification)
            		|| (dataModification instanceof DMDataModification)
            		|| (dataModification instanceof WSDataModification)
            		|| (dataModification instanceof OEDataModification)
            		|| (dataModification instanceof CGDataModification)
    ...
    */
    ...
    void rebuildChildren()
    {
        ...
        try {
            ...
            buildChildrenVector();
            _browser.reload(this);
            _browser.updateSelection(); // Resynch this project browser with the Selection manager-->Jtree will then be resynched with this browser
	...
	public TreeNode getChildAt(int childIndex)
    {
        if (childIndex < getChildCount()) {
            ...
            if (logger.isLoggable(Level.WARNING))
                logger.warning("Element " + getClass().getName() + " request for child at index "
                        + childIndex + " out of bounds (" + getChildCount() + ")");
            return null;
        }
    }
    ...
    protected BrowserElement findNearestAncestor(BrowserElementType... types)
    {
    	...
    	for (BrowserElementType t : types) {
     		if (_parent._elementType == t) return _parent;
    	}
