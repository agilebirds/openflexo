...
public abstract class FlexoModuleTestCase extends FlexoTestCase {
	...
	public static class InteractiveFlexoTestEditor extends InteractiveFlexoEditor {
		...
		// @Override
		// public <A extends FlexoAction<?, T1, T2>, T1 extends FlexoModelObject, T2 extends FlexoModelObject> FlexoActionInitializer<?
		// super A> getInitializerFor(FlexoActionType<A, T1, T2> actionType) {
		// FlexoActionInitializer<A> init = new FlexoActionInitializer<A>() {
		//
		// @Override
		// public boolean run(ActionEvent event, A action) {
		// boolean reply = action.getActionType().isEnabled(action.getFocusedObject(),
		// action.getGlobalSelection(),InteractiveFlexoTestEditor.this);
		// if(!reply){
