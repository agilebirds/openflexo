...
//			    		comparePanel._buttonForChanges.get(changesList.getSelectedValue()).getPopupMenu().show(mouseEvent.getComponent(), mouseEvent.getX()+10, mouseEvent.getY());
//			    }				  
...
//		}
	...
	public class ComparePanel extends JPanel
	{
		...
		ComparePanel()
		{
	       ...
	       c.fill = GridBagConstraints.BOTH;
			...
			add(_scrollPane = new JScrollPane(centerPane,ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED,ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER),BorderLayout.CENTER);
		...
		private class SelectChange implements ActionListener
		{
			...
			SelectChange(MergeChange change) { this.change=change; }
			@Override
			public void actionPerformed(ActionEvent e) { MergePanelElements.this.selectChange(change); }
		}
		...
		protected class MergeChangeButton extends JButton
		{
			...
			protected MergeChangeButton(MergeChange c)
			{
				...
				if ((change.getMergeChangeAction() == MergeChangeAction.AutomaticMergeResolving)
						&& change.isResolved()) {
	...
	static Icon iconForChangeCategory(MergeChange.ChangeCategory cat){
		switch(cat){
			case LEFT_ADDITION : return UtilsIconLibrary.LEFT_ADDITION_ICON;
			case LEFT_MODIFICATION : return UtilsIconLibrary.LEFT_MODIFICATION_ICON;
			case LEFT_REMOVAL : return UtilsIconLibrary.LEFT_REMOVAL_ICON;
			case SMART_CONFLICT_RESOLVED : return UtilsIconLibrary.SMART_CONFLICT_RESOLVED_ICON;
			case SMART_CONFLICT_UNRESOLVED :  return UtilsIconLibrary.SMART_CONFLICT_UNRESOLVED_ICON;
			case CUSTOM_EDITING_RESOLVED : return UtilsIconLibrary.CUSTOM_EDITING_RESOLVED_ICON;
			case CUSTOM_EDITING_UNRESOLVED : return UtilsIconLibrary.CUSTOM_EDITING_UNRESOLVED_ICON;
			case ADD_CONFLICT_RESOLVED : return UtilsIconLibrary.ADD_CONFLICT_RESOLVED_ICON;
			case ADD_CONFLICT_UNRESOLVED : return UtilsIconLibrary.ADD_CONFLICT_UNRESOLVED_ICON;
			case CONFLICT_RESOLVED : return UtilsIconLibrary.CONFLICT_RESOLVED_ICON;
			case CONFLICT_UNRESOLVED : return UtilsIconLibrary.CONFLICT_UNRESOLVED_ICON;
			case DEL_CONFLICT_RESOLVED : return UtilsIconLibrary.DEL_CONFLICT_RESOLVED_ICON;
			case DEL_CONFLICT_UNRESOLVED : return UtilsIconLibrary.DEL_CONFLICT_UNRESOLVED_ICON;
			case RIGHT_ADDITION : return UtilsIconLibrary.RIGHT_ADDITION_ICON;
			case RIGHT_MODIFICATION : return UtilsIconLibrary.RIGHT_MODIFICATION_ICON;
			case RIGHT_REMOVAL : return UtilsIconLibrary.RIGHT_REMOVAL_ICON;
		}
		return null;
	}
...
//		}
	...
	private Hashtable<MergeChange,Hashtable<MergeChangeAction,ChangeAction>> storedActions
	= new Hashtable<MergeChange,Hashtable<MergeChangeAction,ChangeAction>>();
		...
		private void initFilteredList(String noChangeLabel){
			if (_merge.getChanges().size() > 0) {
				...
				_filteredList.addMouseListener(new MouseAdapter() {
				    ...
				    {
				    	if (mouseEvent.isPopupTrigger() 
				    			&& _filteredList.getSelectedValue() != null
				    		...
				    		comparePanel._buttonForChanges.get(_filteredList.getSelectedValue()).getPopupMenu().show(mouseEvent.getComponent(), mouseEvent.getX()+10, mouseEvent.getY());
				    }				  
				    @Override
					public void mouseReleased(MouseEvent mouseEvent)
				    {
				...
				public ButtonFilter(MergeChange.ChangeCategory[] categories, boolean isSelected, Icon icon){
					...
					addActionListener(new ActionListener(){
						...
						public void actionPerformed(ActionEvent e) {
							...
							if(_isSelected)addAllFilter(_categories);
							else removeAllFilter(_categories);
							setBorder(_isSelected?BorderFactory.createEtchedBorder(EtchedBorder.LOWERED):BorderFactory.createEmptyBorder());
