...
import org.openflexo.foundation.ptoc.PSlideType;
...
public class AddPSlideInitializer extends ActionInitializer {

	private static final Logger logger = Logger.getLogger(ControllerActionInitializer.class.getPackage().getName());

    public AddPSlideInitializer(ControllerActionInitializer actionInitializer)
	{
		super(AddPSlide.actionType,actionInitializer);
	}

	@Override
	protected FlexoActionInitializer<AddPSlide> getDefaultInitializer() 
	{
		return new FlexoActionInitializer<AddPSlide>() {
			@Override
			public boolean run(ActionEvent e, AddPSlide action)
			{
				final Vector<PSlideType> staticSlideTypes = new Vector<PSlideType>();
				for (PSlideType type : PSlideType.values()) {
					//if (action.getRepository().getPTOCEntryWithID(docSection)==null)
						staticSlideTypes.add(type);
				}
				
				//Vector<FlexoProcess> processSections = new Vector<FlexoProcess>();
//				for (FlexoProcess process : getProject().getAllLocalFlexoProcesses()) {
//					if (action.getRepository().getPTOCEntryForObject(process)==null)
//						processSections.add(process);
//				}
				
				ParameterDefinition[] def = new ParameterDefinition[4];
				def[0] = new TextFieldParameter("title","toc_entry_title",null);
				def[1] = new EnumDropDownParameter<PSlideLayout>("slideLayout","sectionType",PSlideLayout.TitleOnly,PSlideLayout.values()){
					@Override
					public boolean accept(PSlideLayout value) {
						switch (value) {
						case TitleOnly:
							return true;
						case TitleAndContent:
							return true;
						case TitleImageAndDescription:
							return false;
//						case ROLE:
//							return true;
//						case ENTITY:
...
//							break;
				...
				def[2] = new EnumDropDownParameter<PSlideType>("slideType","section",staticSlideTypes.size()>0?staticSlideTypes.firstElement():null,staticSlideTypes.toArray(new PSlideType[staticSlideTypes.size()])){
//					@Override
//					public boolean accept(PSlideType value) {
//						switch (value) {
//							case ER_DIAGRAM:
//								return false;
//							default:
...
//				def[8].addParameter("format", "name");
				...
				if (dialog.getStatus()==AskParametersDialog.VALIDATE) {
					action.setPSlideTitle((String) def[0].getValue());
//					if(def[1].getValue()==SectionTypeList.PRE_DEFINED) {
//							if(def[2].getValue()==null)
//								return false;
							action.setLayout((PSlideLayout) def[1].getValue());
//							return true;
//					}
							action.setSlideType((PSlideType) def[2].getValue());
//					if(def[1].getValue()==SectionTypeList.PROCESS) {
//						if(def[3].getValue()==null)
//							return false;
					if(def[3].getValue()!=null){
						action.setModelObject((FlexoModelObject) def[3].getValue());
						action.setLayout(PSlideLayout.TitleImageAndDescription);
					}
...
//					return true;
	...
	@Override
	protected FlexoActionFinalizer<AddPSlide> getDefaultFinalizer() 
	{
		return new FlexoActionFinalizer<AddPSlide>() {
			@Override
			public boolean run(ActionEvent e, AddPSlide action)
			{
				return true;
	...
	@Override
	protected FlexoExceptionHandler<? super AddPSlide> getDefaultExceptionHandler() {
		return new FlexoExceptionHandler<AddPSlide>(){
