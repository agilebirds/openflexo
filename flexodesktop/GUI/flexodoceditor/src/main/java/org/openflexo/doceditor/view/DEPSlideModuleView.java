 ...
 * @author MOSTAFA
 */
public class DEPSlideModuleView extends JPanel implements ModuleView<PSlide>, FlexoActionSource, FlexoObserver
{

    private Logger logger = FlexoLogger.getLogger(DETOCEntryModuleView.class.getPackage().getName());

    protected PSlide _slide;
    ...
    public DEPSlideModuleView(PSlide slide, DEController controller, FlexoPerspective perspective)
    {
    ...
    private void rebuildView() {
    	...
    	if (!_slide.isReadOnly()){
			...
			entryEditor = new FlexoWysiwygLight(_slide.getContent(), getController().getProject().getDocumentationCssResource().getFile(), showViewSourceButtonInWysiwyg) {

				@Override
				public void notifyTextChanged() {
					try {
						updatingModel = true;
						_slide.setContent(entryEditor.getBodyContent());
	    	...
	    	entryEditor.addSupportForInsertedObjects(_slide.getProject().getImportedImagesDir());
...
//    		textarea.setText(FlexoLocalization.localizedForKey(key, textarea));
	...
	public void update(FlexoObservable observable, DataModification dataModification)
    {
    	...
    	if ("content".equals(dataModification.propertyName())) {
	        ...
	        if (entryEditor!=null)
	        	entryEditor.setContent(_slide.getContent());
    	}
    }
    ...
    @Override
	public PSlide getRepresentedObject()
    {
        return _slide;
    }
	...
	protected class ViewHeader extends JPanel
    {
        ...
        protected ViewHeader()
        {
            ...
            title = new JLabel();
            title.setText(_slide.getTitle()!=null?_slide.getTitle():FlexoLocalization.localizedForKey("untitled",title));
