		...
		return new FlexoActionInitializer<AddDocType>() {
			...
			public boolean run(EventObject e, AddDocType action) {
				...
				AskParametersDialog d = AskParametersDialog.createAskParametersDialog(null, null,
						FlexoLocalization.localizedForKey("add_doc_type"), FlexoLocalization.localizedForKey("enter_name_and_folder"), pd);
		...
		return new FlexoExceptionHandler<AddDocType>() {
			...
			public boolean handleException(FlexoException exception, AddDocType action) {
				if (exception instanceof DuplicateDocTypeException) {
					FlexoController.notify(FlexoLocalization.localizedForKey("a_doc_type_with_a_similar_name_"
							+ ((DuplicateDocTypeException) exception).getDocType().getName()
							+ FlexoLocalization.localizedForKey("_already_exists")));
					return true;
				}
				return false;
			}
