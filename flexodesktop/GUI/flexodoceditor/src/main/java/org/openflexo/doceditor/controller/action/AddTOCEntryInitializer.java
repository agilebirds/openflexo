...
public class AddTOCEntryInitializer extends ActionInitializer {
	...
	protected FlexoActionInitializer<AddTOCEntry> getDefaultInitializer() {
		return new FlexoActionInitializer<AddTOCEntry>() {
			...
			public boolean run(ActionEvent e, AddTOCEntry action) {
				...
				def[2] = new EnumDropDownParameter<DocSection>("section", "section",
						staticSections.size() > 0 ? staticSections.firstElement() : null,
						staticSections.toArray(new DocSection[staticSections.size()])) {
					@Override
				...
				if (dialog.getStatus() == AskParametersDialog.VALIDATE) {
					...
					if (def[1].getValue() == SectionTypeList.PROCESS) {
						...
						action.setSubType(def[7].getValue().equals(SectionSubType.Doc) ? ProcessDocSectionSubType.Doc : (def[7].getValue()
								...
								SectionSubType.SipocLevel2) ? ProcessDocSectionSubType.SIPOCLevel2 : (def[7].getValue().equals(
								SectionSubType.SipocLevel3) ? ProcessDocSectionSubType.SIPOCLevel3
	...
	enum SectionTypeList {
		CUSTOM_SECTION, PRE_DEFINED, PROCESS, ROLE, ENTITY, OPERATIONCOMPONENT, ER_DIAGRAM
	}
