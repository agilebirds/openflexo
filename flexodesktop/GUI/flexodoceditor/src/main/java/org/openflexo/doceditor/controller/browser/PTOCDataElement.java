...
public class PTOCDataElement extends DEBrowserElement
{
	public PTOCDataElement(PTOCData tocData, ProjectBrowser browser, BrowserElement parent)
	{
		super(tocData, BrowserElementType.PTOC_DATA, browser,parent);
        tocData.getProject().addObserver(this);
	}

	@Override
	public void delete() {
		getPTOCData().getProject().deleteObserver(this);
		super.delete();
	}
	...
	{
		for (PTOCRepository repository : getPTOCData().getRepositories()) {
			addToChilds(repository);
		}
        for (DocType dt : getPTOCData().getProject().getDocTypes()) {
            addToChilds(dt);
        }
	}
