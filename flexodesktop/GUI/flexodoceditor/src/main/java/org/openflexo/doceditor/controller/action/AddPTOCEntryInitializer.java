...
public class AddPTOCEntryInitializer extends ActionInitializer {

	private static final Logger logger = Logger.getLogger(ControllerActionInitializer.class.getPackage().getName());

    public AddPTOCEntryInitializer(ControllerActionInitializer actionInitializer)
	{
		super(AddPTOCEntry.actionType,actionInitializer);
	}

	@Override
	protected FlexoActionInitializer<AddPTOCEntry> getDefaultInitializer() 
	{
		return new FlexoActionInitializer<AddPTOCEntry>() {
			@Override
			public boolean run(ActionEvent e, AddPTOCEntry action)
			{
...
//				def[8].addParameter("format", "name");
				...
				AskParametersDialog dialog = AskParametersDialog.createAskParametersDialog(getProject(), getController().getFlexoFrame(), FlexoLocalization.localizedForKey("toc_entry_creation"), FlexoLocalization.localizedForKey("enter_new_title"), def[0]);
				if (dialog.getStatus()==AskParametersDialog.VALIDATE) {
					action.setPTocEntryTitle((String) def[0].getValue());
...
//					}
	...
	@Override
	protected FlexoActionFinalizer<AddPTOCEntry> getDefaultFinalizer() 
	{
		return new FlexoActionFinalizer<AddPTOCEntry>() {
			@Override
			public boolean run(ActionEvent e, AddPTOCEntry action)
			{
				return true;
	...
	@Override
	protected FlexoExceptionHandler<? super AddPTOCEntry> getDefaultExceptionHandler() {
		return new FlexoExceptionHandler<AddPTOCEntry>(){
