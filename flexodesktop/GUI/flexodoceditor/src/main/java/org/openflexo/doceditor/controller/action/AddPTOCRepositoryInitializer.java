...
public class AddPTOCRepositoryInitializer extends ActionInitializer {

	public AddPTOCRepositoryInitializer(DEControllerActionInitializer controllerActionInitializer) {
		super(AddPTOCRepository.actionType, controllerActionInitializer);
	}

	@Override
	protected FlexoActionInitializer<? super AddPTOCRepository> getDefaultInitializer() {
		return new FlexoActionInitializer<AddPTOCRepository>() {

			@Override
			public boolean run(ActionEvent event, AddPTOCRepository action) {
				...
				File tocTemplateDirectory = new FileResource("Config/PTOCTemplates");
				...
				AskParametersDialog dialog = AskParametersDialog.createAskParametersDialog(getProject(), getController().getFlexoFrame(),
						...
						FlexoLocalization.localizedForKey("enter_parameters_for_table_of_content"), new ValidationCondition() {
							...
							public boolean isValid(ParametersModel model) {
								...
								if (getProject().getPTOCData().getRepositoryWithTitle((String) model.parameterForKey("name").getValue()) != null) {
	...
	//TODO_MOS Remove this method
	public static void main(String[] args){
	
	     
		File tocTemplateFile = new File("C:/Users/MOSTAFA/Git/openflexo/flexodesktop/model/flexofoundation/src/main/resources/Config/PTOCTemplates/BRS.xml");
		
		PTOCRepository tocTemplate = null;
		try {
			tocTemplate = (PTOCRepository) XMLDecoder.decodeObjectWithMappingFile(new FileInputStream(
					tocTemplateFile), new File("C:/Users/MOSTAFA/Git/openflexo/flexodesktop/model/flexofoundation/src/main/resources/Models/PTOCModel/toc_template_0.1.xml"), new FlexoPTOCBuilder(null));
		...
		if(tocTemplate != null){
			for(PTOCUnit unit : tocTemplate.getPtocUnits()){
				
				System.out.println("l'uint est de type :"+unit.getClass().getName() +" Le titre de "+unit.getTitle());
				if(unit instanceof PTOCEntry){
					for(PTOCUnit u : ((PTOCEntry)unit).getPtocUnits()){
						
						System.out.println("--->le fils est de type :"+u.getClass().getName() +" Le titre de "+u.getTitle());
					}
				}
			}
	...
	@Override
	protected FlexoActionFinalizer<? super AddPTOCRepository> getDefaultFinalizer() {

		return new FlexoActionFinalizer<AddPTOCRepository>() {
