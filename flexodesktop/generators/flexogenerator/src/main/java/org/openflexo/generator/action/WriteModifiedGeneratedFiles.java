...
public class WriteModifiedGeneratedFiles extends MultipleFileGCAction<WriteModifiedGeneratedFiles> {
	...
	public static final MultipleFileGCActionType<WriteModifiedGeneratedFiles> actionType = new MultipleFileGCActionType<WriteModifiedGeneratedFiles>(
			"write_to_disk", GENERATE_MENU, WRITE_GROUP, FlexoActionType.NORMAL_ACTION_TYPE) {
		/**
		 * Factory method
		 */
		@Override
	...
	protected void doImpl(Object context) throws GenerationException, SaveResourceException, FlexoException {
		...
		makeFlexoProgress(
				FlexoLocalization.localizedForKey("write") + " " + getFilesToWrite().size() + " "
		...
		for (AbstractCGFile file : getFilesToWrite()) {
			...
			// GPO: The above code has been commented. Unless we find a very good reason for calling the code above, we should not do this.
			// Why?
			// Well because! No, because when we call generate(false), we may trigger the generator to run again. So far it ain't too bad,
			// except that when the generator is done, it sends a notification CGContentRegenerated and it causes the flag "mark as merged"
			// to
			// go back to false (making it impossible to write it down!). In conclusion, if you decide to uncomment the block above, then
			// you
