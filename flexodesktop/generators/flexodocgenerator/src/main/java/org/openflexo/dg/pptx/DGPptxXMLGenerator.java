...
public class DGPptxXMLGenerator<T extends FlexoModelObject> extends Generator<T, DGRepository> implements IFlexoResourceGenerator
{
	...
	protected static final Logger logger = FlexoLogger.getLogger(DGPptxXMLGenerator.class.getPackage().getName());

	private String identifier;
	private LinkedHashMap<PptxXmlFileResource<DGPptxXMLGenerator<T>>, PptxTemplatesEnum> pptxResources;

	public DGPptxXMLGenerator(ProjectDocPptxGenerator projectGenerator, T source, String identifier)
	{
		super(projectGenerator, source);
		this.pptxResources = new LinkedHashMap<PptxXmlFileResource<DGPptxXMLGenerator<T>>, PptxTemplatesEnum>();
		this.identifier = identifier;
	}

	public CGSymbolicDirectory getSymbolicDirectory(DGRepository repository)
	{
		return repository.getPptxSymbolicDirectory();
	}

	@Override
	public GeneratedPptxXmlResource getGeneratedCode()
	{
		if (generatedCode == null)
		{
			GeneratedPptxXmlResource generatedCodeTmp = new GeneratedPptxXmlResource(getIdentifier());
			for(PptxXmlFileResource<DGPptxXMLGenerator<T>> resource : pptxResources.keySet())
			{
				...
				generatedCodeTmp.addCode(getPptxTemplateForResource(resource), resource.getASCIIFile().getLastAcceptedContent());
			}

			generatedCode = generatedCodeTmp;
		}
		return (GeneratedPptxXmlResource)generatedCode;
	}
	...
	public void addPptxResource(PptxXmlFileResource<DGPptxXMLGenerator<T>> PptxResource, PptxTemplatesEnum PptxTemplate)
	{
		pptxResources.put(PptxResource, PptxTemplate);
	}

	public PptxTemplatesEnum getPptxTemplateForResource(PptxXmlFileResource<DGPptxXMLGenerator<T>> PptxResource)
	{
		return pptxResources.get(PptxResource);
	}

	public String getGenerationResultKeyForResource(PptxXmlFileResource<DGPptxXMLGenerator<T>> PptxResource)
	{
		return pptxResources.get(PptxResource).toString();
	}

	public String getFileNameForResource(PptxXmlFileResource<DGPptxXMLGenerator<T>> PptxResource)
	{
		return pptxResources.get(PptxResource).getFilePath();
	}

	public String getTemplateNameForResource(PptxXmlFileResource<DGPptxXMLGenerator<T>> PptxResource)
	{
		return pptxResources.get(PptxResource).getTemplatePath();
	}
