...
public class CGWebServerImageFile extends AbstractCGFile implements GenerationAvailableFile, GeneratedResourceData {
	...
	public void writeToFile(File file) throws FlexoException {
		if (getFlexoResource() instanceof FlexoCopyOfFlexoResource) {
			try {
				FileUtils.copyFileToFile(((FlexoCopyOfFlexoResource) getFlexoResource()).getResourceToCopy().getFile(), file);
			} catch (IOException e) {
				throw new FlexoException(e.getMessage(), e);
			}
		} else if (getFlexoResource() instanceof FlexoCopyOfFileResource) {
			try {
