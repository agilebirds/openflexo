   ...
   /**
	...
	public void buildResourcesAndSetGenerators(CGRepository repository, Vector<CGRepositoryFileResource> resources) {
		...
		if(settingsGenerator==null){
			...
			settingsGenerator = new PackagedResourceToCopyGenerator<CGRepository>(getProjectGenerator(),
					FileFormat.UNKNOWN_BINARY_FILE,
					ResourceType.COPIED_FILE,
					orig, getRepository().getProjectSymbolicDirectory(),SETTINGS_RELATIVE_PATH);
	...
	private PrototypeProcessBusinessDataSamplesGenerator getGeneratorForPrototypeProcessSample(String businessDataKey)
    {
        if (_prototypeProcessSamplesGenerator.get(businessDataKey)==null) {
        	_prototypeProcessSamplesGenerator.put(businessDataKey, new PrototypeProcessBusinessDataSamplesGenerator(getProjectGenerator(),businessDataKey, getSamplesCreator()));
        }
	...
	private PackagedResourceToCopyGenerator<CGRepository> getGenerator(FileResource lg)
    {
        if (_driverGenerator.get(lg)==null) {
        	_driverGenerator.put(lg, new PackagedResourceToCopyGenerator<CGRepository>(getProjectGenerator(),FileFormat.UNKNOWN_BINARY_FILE,ResourceType.COPIED_FILE,lg,getProjectGenerator().getRepository().getLibSymbolicDirectory(),""));
        }
	...
	private PackagedResourceToCopyGenerator<CGRepository> getGeneratorForEclipseResource(FileResource lg)
    {
        if (_eclipseFilesGenerator.get(lg)==null) {
        	_eclipseFilesGenerator.put(lg, new PackagedResourceToCopyGenerator<CGRepository>(getProjectGenerator(),FileFormat.UNKNOWN_BINARY_FILE,ResourceType.COPIED_FILE,lg,getProjectGenerator().getRepository().getProjectSymbolicDirectory(),"woproject"));
        }
