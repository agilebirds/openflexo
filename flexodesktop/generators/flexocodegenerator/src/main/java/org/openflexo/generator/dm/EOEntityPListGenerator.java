...
public class EOEntityPListGenerator extends MetaFileGenerator {
	...
	public EOEntityPListGenerator(ProjectGenerator projectGenerator, DMEOEntity eoEntity) {
		super(projectGenerator, FileFormat.PLIST, ResourceType.PLIST_FILE, eoEntity.getName() + ".plist", "PLIST."
				+ eoEntity.getFullyQualifiedName());
	...
	public void generate(boolean forceRegenerate) {
		...
		try {
			...
			cre.printStackTrace();
			try {
				generatedCode = new GeneratedTextResource(getEntity().getName(),
						FileUtils.fileContents(getEntity().getEOEntity().getFile()));
