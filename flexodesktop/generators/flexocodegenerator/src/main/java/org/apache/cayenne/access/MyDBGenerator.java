...
public class MyDBGenerator {
     ...
     */
    public MyDBGenerator(DbAdapter _adapter, DataMap _map, Collection excludedEntities,
            DataDomain _domain, Comparator entitySorter) {
        // sanity check
        if (_adapter == null) {
        ...
        while (it.hasNext()) {
            ...
            while (nextDbAtributes.hasNext()) {
                ...
                if (attr.getType() == TypesMapping.NOT_DEFINED) {
                    logger.info("Skipping entity, attribute type is undefined: "
                            + nextEntity.getName()
                            + "."
                            + attr.getName());
            ...
            if (pkAttributes.size() == 1) {
                ...
                while (i.hasNext()) {
                    ...
                    switch (att.getType()) {
                    case Types.INTEGER:case Types.NUMERIC:case Types.BIGINT:case Types.TINYINT:case Types.SMALLINT:
                        needAutoPK=true;
                        break;
                    default:
                        break;
