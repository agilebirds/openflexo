...
public abstract class AgileBirdsFlexoController extends FlexoController {
		...
		if (optionType == JOptionPane.OK_CANCEL_OPTION && options == null) {
			...
			pane = new JOptionPane(message, messageType, optionType, icon, availableOptions, availableOptions[0]) {
				...
				public int getMaxCharactersPerLineCount() {
					return AgileBirdsFlexoController.getMaxCharactersPerLine(parent, this);
				}
			};
			isLocalized = true;
	...
	private boolean _handleWSException(Throwable e) {
		...
		if (e.getMessage() != null && e.getMessage().startsWith("redirect")) {
			...
			AgileBirdsFlexoController.notify(FlexoLocalization.localizedForKey("could_not_connect_to_web_sevice") + ": "
		...
		if (e instanceof WebApplicationException) {
			...
			case 500:
				return AgileBirdsFlexoController.confirm(FlexoLocalization.localizedForKey("webservice_remote_error") + entity + "\n"
						+ FlexoLocalization.localizedForKey("would_you_like_to_try_again?"));
			case 401:
				return AgileBirdsFlexoController.confirm(FlexoLocalization.localizedForKey("unauthorized_action_on_the_server") + entity
						+ "\n" + FlexoLocalization.localizedForKey("would_you_like_to_try_again?"));
			default:
				if (entity != null) {
					return AgileBirdsFlexoController.confirm(entity.toString() + "\n"
							+ FlexoLocalization.localizedForKey("would_you_like_to_try_again?"));
				} else {
					return AgileBirdsFlexoController.confirm(FlexoLocalization
							.localizedForKey("unexpected_error_occured_while_connecting_to_server")
							+ "\n"
							+ FlexoLocalization.localizedForKey("would_you_like_to_try_again?"));
				}
			}
		...
		if (e.getCause() instanceof ConnectException) {
			return AgileBirdsFlexoController.confirm(FlexoLocalization.localizedForKey("connection_error")
			...
			return AgileBirdsFlexoController.confirm(FlexoLocalization.localizedForKey("could_not_connect_to_web_sevice") + ": "
					+ FlexoLocalization.localizedForKey("the_url_seems_incorrect") + "\n"
					+ FlexoLocalization.localizedForKey("would_you_like_to_try_again?"));
		} else if (e.toString() != null && e.toString().startsWith("javax.net.ssl.SSLHandshakeException")) {
			return AgileBirdsFlexoController.confirm(FlexoLocalization.localizedForKey("connection_error") + ": " + e + "\n"
					+ FlexoLocalization.localizedForKey("would_you_like_to_try_again?"));
		} else if (e instanceof SocketTimeoutException) {
			return AgileBirdsFlexoController.confirm(FlexoLocalization.localizedForKey("connection_timeout") + "\n"
					+ FlexoLocalization.localizedForKey("would_you_like_to_try_again?"));
