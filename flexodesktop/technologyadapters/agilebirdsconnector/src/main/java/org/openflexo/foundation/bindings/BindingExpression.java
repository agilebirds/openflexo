...
public class BindingExpression extends AbstractBinding {
	...
	public BindingExpression(BindingDefinition bindingDefinition, AgileBirdsObject owner) {
		super(bindingDefinition, owner);
	}

	public BindingExpression(BindingDefinition bindingDefinition, AgileBirdsObject owner, AbstractBinding abstractBinding) {
		super(bindingDefinition, owner);
	...
	public BindingExpression(BindingDefinition bindingDefinition, AgileBirdsObject owner, Expression anExpression) {
		super(bindingDefinition, owner);
	...
	public static BindingExpression makeBindingExpression(String value, AgileBirdsObject owner) {
	...
	public static class BindingValueConstant extends Expression {
		...
		public void setConstant(Constant aConstant) {
			...
			if (_bindable != null) {
				bd = new BindingDefinition("constant", DMType.makeObjectDMType(((AgileBirdsObject) _bindable).getProject()),
						(AgileBirdsObject) _bindable, BindingDefinitionType.GET, true);
			}
			if (constant == Constant.BooleanConstant.TRUE) {
				staticBinding = new BooleanStaticBinding(bd, (AgileBirdsObject) _bindable, true);
			} else if (constant == Constant.BooleanConstant.FALSE) {
				staticBinding = new BooleanStaticBinding(bd, (AgileBirdsObject) _bindable, false);
			} else if (constant instanceof Constant.IntegerConstant) {
				staticBinding = new IntegerStaticBinding(bd, (AgileBirdsObject) _bindable, ((Constant.IntegerConstant) constant).getValue());
			} else if (constant instanceof Constant.FloatConstant) {
				staticBinding = new FloatStaticBinding(bd, (AgileBirdsObject) _bindable, ((Constant.FloatConstant) constant).getValue());
			} else if (constant instanceof Constant.StringConstant) {
				staticBinding = new StringStaticBinding(bd, (AgileBirdsObject) _bindable, ((Constant.StringConstant) constant).getValue());
			} else if (constant instanceof Constant.DateConstant) {
				staticBinding = new DateStaticBinding(bd, (AgileBirdsObject) _bindable, ((Constant.DateConstant) constant).getDate());
			} else if (constant instanceof Constant.DurationConstant) {
				staticBinding = new DurationStaticBinding(bd, (AgileBirdsObject) _bindable,
				...
				if (_bindable != null && ((Constant.EnumConstant) constant).getName().startsWith(DMType.DKV_PREFIX)) {
					...
					if (st.hasMoreTokens()) {
						...
						Domain domain = ((AgileBirdsObject) _bindable).getProject().getDKVModel().getDomainNamed(domainName);
						if (domain != null && st.hasMoreTokens()) {
							key = domain.getKeyNamed(st.nextToken());
						}
					}
				}
				staticBinding = new DKVBinding(bd, (AgileBirdsObject) _bindable, key);
			}
			if (logger.isLoggable(Level.FINE)) {
	...
	public static class BindingValueVariable extends Expression {
		...
		public BindingValueVariable(String variableName, Bindable bindable) {
			this(new Variable(variableName), bindable, bindable != null ? new BindingDefinition("object",
					DMType.makeObjectDMType(((AgileBirdsObject) bindable).getProject()), (AgileBirdsObject) bindable,
		...
		public BindingValueVariable(Variable aVariable, Bindable bindable, BindingDefinition bd) {
			...
			if (bindable != null) {
				BindingValueStringConverter converter = ((AgileBirdsObject) bindable).getProject().getBindingValueConverter();
			...
			if (bindingValue == null) {
				bindingValue = new BindingValue(null, (AgileBirdsObject) bindable);
