...
public class EntityFromWidgets {
	...
	public EntityFromWidgets(FlexoProject project, List<AgileBirdsObject> widgets, boolean useDMEOEntity) {
		super();
		_project = project;
	...
	private Map<IEWidget, PropertyProposal> extractRelevantWidgets(List<AgileBirdsObject> widgets, boolean useDMEOEntity) {
		Map<IEWidget, PropertyProposal> reply = new Hashtable<IEWidget, PropertyProposal>();
		Iterator<AgileBirdsObject> it = widgets.iterator();
		AgileBirdsObject modelObject = null;
		while (it.hasNext()) {
			modelObject = it.next();
