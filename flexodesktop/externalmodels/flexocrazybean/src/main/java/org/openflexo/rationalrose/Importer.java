...
public class Importer implements DataImporter
{
    ...
    private static final String[] PROPERTIES_TO_IGNORE = new String[]{QUID_PROPERTY_NAME,QUIDU_PROPERTY_NAME,ATTRIBUTES_PROPERTY_NAME,OPERATIONS_PROPERTY_NAME,PARAMETERS_PROPERTY_NAME,ROLES_PROPERTY_NAME, DOCUMENTATION_PROPERTY_NAME,CLASS_ATTRIBUTES_PROPERTY_NAME,SUPER_CLASSES_PROPERTY_NAME};
	...
	public Object importInProject (FlexoProject project, File importedFile, Object[] parameters)
    {
        ...
        flexoAction = (parameters.length >= 3 ? (FlexoAction<?, ? extends FlexoModelObject, ? extends FlexoModelObject>)parameters[2] : null);
        this.project = project;
        
        String[] args = new String[]{importedFile.getAbsolutePath()};
        if (flexoAction != null) flexoAction.setProgress(FlexoLocalization
                .localizedForKey("parsing")+ " " + importedFile.getName());
    ...
    private void extractData()
    {
        ...
        while(en.hasMoreElements()){
        	try {
	            ...
	            String className = EOModelExtractor.getLabel(currentClass);
	            if (flexoAction != null) {
	            	flexoAction.setSecondaryProgress(FlexoLocalization.localizedForKey("extracting_attributes_operations_and_relationships_for")+" "+className);
	            }
    ...
    private DMProperty createRelationship(Role role, DMEntity from, DMEntity to)
    {
        ...
        DMProperty newProperty = new DMProperty(from.getDMModel(), EOModelExtractor.getLabel(role), DMType.makeResolvedDMType(to), /*isRoleToOne?*/DMCardinality.SINGLE/*:DMCardinality.VECTOR*/,
                from.getIsReadOnly(), true, DMPropertyImplementationType.PUBLIC_ACCESSORS_PRIVATE_FIELD);
    ...
    private DMProperty createDMProperty(DMEntity entity, String newPropertyName)
    {
	    ...
	    DMProperty newProperty = new DMProperty(entity.getDMModel(), /*entity,*/ newPropertyName, DMType.makeResolvedDMType(defaultType), DMCardinality.SINGLE,
                entity.getIsReadOnly(), true, DMPropertyImplementationType.PUBLIC_ACCESSORS_PRIVATE_FIELD);
