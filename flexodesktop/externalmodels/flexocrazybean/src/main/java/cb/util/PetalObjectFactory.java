...
/**
 * Create empty Petal objects with just some initial properties (the quid in particular) set up. The user can then add them to model, i.e.
 * PetalFile object. Some objects are created by reading serialized templates from the templates directory. init() is called on them usually
 * automagically when they're added to the model.
 * 
 * @see PetalFile
...
public class PetalObjectFactory {
	...
	/**
	 * Create InheritanceRelationship between two classes, this method is called by Class.addSuperClass(). Shouldn't be called directly I
	 * think. If you really want to, don't forget to call init().
	 * 
	...
	/**
	 * Create RealizeRelationship between class and an interface, called by Class.addImplementedInterface(). Shouldn't be called directly I
	 * think. If you really want to, don't forget to call init().
	 * 
	 * @see cb.petal.Class#addImplementedInterface(cb.petal.Class)
	 */
	public RealizeRelationship createRealizeRelationship(cb.petal.Class clazz, cb.petal.Class inter) {
	...
	/**
	 * Create UsesRelationship between class and an interface, called by Class.addUsedClass(). Shouldn't be called directly I think. If you
	 * really want to, don't forget to call init().
	 * 
	 * @see cb.petal.Class#addUsedClass(cb.petal.Class)
	 */
	public UsesRelationship createUsesRelationship(cb.petal.Class clazz, cb.petal.Class clazz2) {
	...
	/**
	 * Create Association between two classes. Cardinality and other properties can be configured in the roles of the returned object.
	 * Classes must have already been added to the model.
	 */
	public Association createAssociation(String name, cb.petal.Class clazz1, cb.petal.Class clazz2) {
	 ...
	 */
	public Association createAssociation(cb.petal.Class clazz1, cb.petal.Class clazz2) {
	...
	/**
	 * Creates class view for given class and sets the qualified name and quidu for the referenced class accordingly. The tag, i.e., the
	 * index in the views list (@12, e.g.), is set when the view is added to the model. It is also set them for the ItemLabel objects
	...
	/**
	 * @return list of inherit view objects since a class may extend multiple classes (not in Java, I know). The client and supplier tags
	 *         (@5, e.g. which are references to the corresponding class view indexes) are set when the views are added to the model.
	 * 
	...
	public java.util.List createInheritViews(Inheritable clazz) {
		...
		for (Iterator i = clazz.getSuperclassList().getElements().iterator(); i.hasNext();) {
	...
	public java.util.List createRealizeViews(cb.petal.Class clazz) {
		...
		for (Iterator i = clazz.getRealizedInterfacesList().getElements().iterator(); i.hasNext();) {
	...
	/**
	 * @return list of uses view objects. The client and supplier tags (@5, e.g. which are references to the corresponding class view
	 *         indexes) are set when the views are added to the model.
	 * 
	...
	public java.util.List createUsesViews(cb.petal.Class clazz) {
		...
		for (Iterator i = clazz.getUsedClassesList().getElements().iterator(); i.hasNext();) {
	...
	/**
	 * Creates use case view for given class and sets the qualified name and quidu for the referenced class accordingly. The tag, i.e., the
	 * index in the views list (@12, e.g.), is set when the view is added to the model. It is also set them for the ItemLabel objects
	 * associated with the view.
	 * 
