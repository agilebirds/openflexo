...
public class SendTokenToPrecondition extends ControlGraphBuilder {
	...
	public static ControlGraphBuilder getSendTokenToPreconditionBuilder (FlexoPreCondition precondition, FlexoPostCondition<?, ?> edge) throws NotSupportedException, InvalidModelException
	{
	 ...
	 */
	public static ControlGraph sendTokenToPrecondition (FlexoPreCondition precondition, FlexoPostCondition<?, ?> edge,boolean interprocedural) throws NotSupportedException, InvalidModelException
	{
		...
		if (interprocedural) {
			...
			returned.appendHeaderComment("Precondition "+precondition.getName()+" receive a new token from node "+(edgeOrigin!=null?edgeOrigin.getName():"null"),true);
			return returned;
		}
		else {
	...
	protected String getProcedureComment()
	{
		...
		if (getPrecondition().getAttachedBeginNode() != null)
			returned.append(FlexoLocalization.localizedForKeyWithParams("this_method_represents_code_to_be_executed_when_precondition_($0)_receive_a_new_token",getPrecondition().getName()));
		else 
			returned.append(FlexoLocalization.localizedForKeyWithParams("this_method_represents_code_to_be_executed_when_precondition_of_node_($0)_receive_a_new_token",getPrecondition().getAttachedNode().getName()));
	...
	{
		if (tokenIncrementVariable == null) tokenIncrementVariable = new Variable("tokenIncrement");
	...
	private ControlGraph makeControlGraphForTokenIncrementSetting()
	{
		...
		for (FlexoPostCondition edge : getPrecondition().getIncomingPostConditions()) {
			...
			Conditional currentConditional = new Conditional(
					condition,
					new Assignment(getTokenIncrementVariable(),new Constant.IntegerConstant((edge instanceof TokenEdge ? ((TokenEdge)edge).getTokenIncrem() : 1))),
	...
	protected ControlGraph makeControlGraph(boolean interprocedural) throws InvalidModelException, NotSupportedException 
	{
		...
		Expression storedTokensOnPreconditionIncrementedWithCurrentTokenIncrement
		= new BinaryOperatorExpression(
				ArithmeticBinaryOperator.ADDITION,
				storedTokensOnPrecondition,
				tokenIncrement);
		...
		if (precondition.getAttachedNode() instanceof ActionNode 
				&& ! (precondition.getAttachedNode() instanceof SelfExecutableActionNode) 
			...
			if (precondition.getIncomingPostConditions().size() == 1 
					&& (precondition.getIncomingPostConditions().firstElement() == edge)
					&& (tokenIncrement instanceof Constant.IntegerConstant && ((Constant.IntegerConstant)tokenIncrement).getValue() == 1)
					&& (precondition.getInitTokenNbr() == 0)
					&& (precondition.getTriggerLevel() == 1)) {
			...
			else return makeSequentialControlGraph(
					setTokenIncrement,
					new Conditional(
							condition,
							NodeActivation.activateNode(precondition.getAttachedNode(),precondition,interprocedural),
							STORE_TOKEN,
							"Test if precondition"+(precondition.getAttachedBeginNode()!=null?" "+precondition.getName():"")+" of node "+precondition.getAttachedNode().getName()+" is triggering"));
	...
	protected String getProcedureName()
	{
		if (getPrecondition() != null && getPrecondition().getAttachedNode() != null) {
			if (getPrecondition().getAttachedBeginNode() != null) {
				return "precondition_"+ToolBox.capitalize(ToolBox.getJavaName(getPrecondition().getName()))+"_of_"+ToolBox.capitalize(ToolBox.getJavaName(getPrecondition().getAttachedNode().getName()))+"_"+getPrecondition().getFlexoID()+"_receiveNewToken";	
			}
			else {
				return "precondition_"+ToolBox.capitalize(ToolBox.getJavaName(getPrecondition().getAttachedNode().getName()))+"_"+getPrecondition().getFlexoID()+"_receiveNewToken";	
			}
		}
		else if (getPrecondition() != null) {
			return "precondition_"+ToolBox.capitalize(ToolBox.getJavaName(getPrecondition().getName()))+"_of_???_"+getPrecondition().getFlexoID()+"_receiveNewToken";			
		}
		else return "precondition_???_receiveNewToken";			
	}
