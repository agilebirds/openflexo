...
/*
   ...
   */
    	...
    	schemaTypeSystem =(SchemaTypeSystem) XmlBeans.loadXsd(new XmlObject[] { XmlObject.Factory.parse(schema) });
    //schemaTypeSystem = (SchemaTypeSystem)schemaTypeLoader;
    	*/
    	 
    	/** this is the API CORRECT WAY... but the types are not java-ized ! */
    //	schemaTypeSystem = XmlBeans.compileXsd(new XmlObject[]{ XmlObject.Factory.parse(schema) },XmlBeans.getBuiltinTypeSystem(),null);
   
    /** this is another CORRECT but from modified source way to do it WITH java-ized types**/
    	 schemaTypeSystem = MySchemaTypeSystemCompiler.compile(null,null,new XmlObject[]{ XmlObject.Factory.parse(schema) },null,XmlBeans.getBuiltinTypeSystem(),null,null);
    	
    //	 schemaTypeSystem =  (SchemaTypeSystem)_compilationMethod.invoke(null, new Object[]{null,null,new XmlObject[]{ XmlObject.Factory.parse(schema) },null,XmlBeans.getBuiltinTypeSystem(),null,null});
