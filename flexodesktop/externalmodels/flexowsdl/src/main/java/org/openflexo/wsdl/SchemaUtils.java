...
public class SchemaUtils
{
   ...
   public static SchemaTypeLoader loadSchemaTypes(List schemas) throws Exception
   {
      ...
      try
		{
			return XmlBeans.loadXsd(
					(XmlObject[])schemas.toArray(new XmlObject[schemas.size()]), options);
		}
		catch (Exception e)
		{
   ...
   /**
    * Fixes relative xsd imports in the specified schema. The schemaLocation of an 
    * xsd import must be an URI which is not the case if the loaded 
    * wsdl is loaded with the "file" schema (ie from the file system) and an import is relative. 
    */
   
	private static void fixRelativeFileImports(XmlObject xmlObject) throws Exception
	{
		 XmlObject[] imports = xmlObject
       	.selectPath("declare namespace s='http://www.w3.org/2001/XMLSchema' .//s:import");
