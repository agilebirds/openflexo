...
public class Importer implements DataImporter
{
    ...
    private void addTypeToMessageDef(MessageDefinition def, SchemaTypeExtractor extractor, Part messagePart, FlexoProcess newProcess) throws FlexoException
    {
        ...
        if (dmType==null || dmType.getName()==null){
        	...
        	//throw new FlexoException("The specified data type \""+typeString+"\" for message "+def.getName()+" could not be matched to any type in the DataModel.");
        }
    ...
    private DMProperty createProperty(DMEntity type, String newPropertyName, QName newPropertyType, boolean isVector)
    {
        ...
        DMProperty newProperty = new DMProperty(type.getDMModel(), /* entity, */newPropertyName, defaultType, (isVector?DMCardinality.VECTOR:DMCardinality.SINGLE),
        		type.getIsReadOnly(), true, DMPropertyImplementationType.PUBLIC_ACCESSORS_PRIVATE_FIELD);
    ...
    private void treatType(SchemaType t, Hashtable ht, WSDLRepository repository) {
    	if (treatedTypes.contains(t)) return;
    	if (t.isBuiltinType()) {
    		// do nothing
    	}
    	if (t.isSimpleType()) {
    		...
    		//if(!t.isAnonymousType()) System.out.println("Adding simpleType : "+t.getName().getLocalPart() +" of type : "+t.getBaseType().getName().getLocalPart());
...
*/
     ...
     */
    private void treatProperty(Hashtable ht, WSDLRepository repository, DMEntity entity, String propName, SchemaType propType, boolean isVector) {
    	// create the property
