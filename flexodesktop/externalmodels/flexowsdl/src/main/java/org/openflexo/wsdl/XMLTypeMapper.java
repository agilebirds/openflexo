...
public class XMLTypeMapper {
	...
	public static String[] getFullJavaNameForType(SchemaType type, SchemaTypeExtractor extractor, Hashtable simpleTypes) {
		...
		// Test an Array
		if(type==null){
			if (logger.isLoggable(Level.WARNING)) logger.warning("Type is NULL. Cannot find java Name for a NULL Type");
	...
	public static String getArrayType( SchemaType type, SchemaTypeExtractor extractor, Hashtable simpleTypes) 
	{
		try{
			...
			for (int i = 0; i < schemas.length; i++)
			{
				...
				if(useSchemaFromExtractor)wsdlNS = "http://www.apache.org/internal/xmlbeans/wsdlsubst";
				
				
				// Find Elements of with correct name
				if(type.isAnonymousType()||type.isDocumentType()){
					attributes= schema.selectPath("declare namespace s='http://www.w3.org/2001/XMLSchema' declare namespace wsdl='"+wsdlNS+"' .//s:element[@name='"+typeName+"']//s:attribute[@wsdl:arrayType]");
				}
				// Or Find ComplexTypes with correct name
				else{
					attributes= schema.selectPath("declare namespace s='http://www.w3.org/2001/XMLSchema' declare namespace wsdl='"+wsdlNS+"' .//s:complexType[@name='"+typeName+"']//s:attribute[@wsdl:arrayType]");
