...
public class CVSFile extends CVSAbstractFile
{
    ...
    /**
     * This method returns the last modified date for this local file but
     * milliseconds are truncated from that time.
     * 
     * @return the last modified date for this local file but milliseconds are
     *         truncated from that time
     */
    ...
    {
        if (_contentOnRepository == null 
        		&& _contentOnRepositoryRevisionRetriever == null
        		&& !getSharedProject().isSynchronizing()) {
    ...
    private class RevisionRetriever implements FlexoRunnable
    {
		...
		public void run()
        {
            ...
            if (_listener != null) _listener.notifyRevisionRetrieved(CVSFile.this, revisionToRetrieve);
        }
    ...
    {
    	if (getContentOnDisk() == null) return;
    	if (getOriginalContent() == null) return;
    	if (getContentOnRepository() == null) return;
...
//    }
    ...
    private synchronized void rebuildXMLDiff3()
    {
    	...
    	_xmlDiff3 = new XMLDiff3(docSource,v1,v2,getResourceType().getMapping(getSharedProject().getFlexoXMLMappings()),getResourceType().getMergedDocumentType());
    }
