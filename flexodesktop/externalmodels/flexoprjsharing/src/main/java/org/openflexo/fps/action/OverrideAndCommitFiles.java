...
public class OverrideAndCommitFiles extends MultipleFileCVSAction<OverrideAndCommitFiles> implements UpdateListener, CommitListener {
	...
	public static MultipleFileCVSActionType<OverrideAndCommitFiles> actionType = new MultipleFileCVSActionType<OverrideAndCommitFiles>(
	...
	static {
		FlexoObject.addActionForClass(OverrideAndCommitFiles.actionType, CVSAbstractFile.class);
	}
	...
	protected void doAction(Object context) {
		...
		Hashtable<CVSContainer, Vector<CVSFile>> filesToMerge = new Hashtable<CVSContainer, Vector<CVSFile>>();
		for (CVSFile f : selectedFiles) {
	...
	private void waitCommitResponses() {
		...
		while (committingFilesToWait > 0 && System.currentTimeMillis() - lastCommitReception < TIME_OUT) {
			try {
				Thread.sleep(100);
			} catch (InterruptedException e) {
				e.printStackTrace();
