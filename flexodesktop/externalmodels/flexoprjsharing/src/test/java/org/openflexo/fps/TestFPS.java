...
public class TestFPS extends FPSTestCase  {
	...
	public void test5CheckLocalSynchronization()
	{
		...
		assertAllFilesAreUpToDateOrCVSIgnoredExcept(_project2,
				_project2.getCVSFile(_projectIdentifier+".rmxml"),
	...
	public void test7ResynchronizeFirstProject()
	{
		...
		assertAllFilesAreUpToDateOrCVSIgnoredExcept(_project1,
				_project1.getCVSFile(_projectIdentifier+".rmxml"),
		...
		assertEquals(_project1.getCVSFile("DataModel"+File.separator+_projectIdentifier+".dm").getStatus(), CVSStatus.RemotelyModified);
		assertEquals(_project1.getCVSFile("Workflow"+File.separator+_projectIdentifier+".wkf").getStatus(), CVSStatus.RemotelyModified);
		assertEquals(_project1.getCVSFile("Workflow"+File.separator+_projectIdentifier+".xml").getStatus(), CVSStatus.RemotelyModified);
	...
	public void test8UpdateFiles()
	{
		...
		// Test that checkouted project is still readable by Flexo
		try {
			assertNotNull(_initialProjectEditor = FlexoResourceManager.initializeExistingProject(_project1.getModuleDirectory(),EDITOR_FACTORY,null));
			_initialProject = _initialProjectEditor.getProject();
		} catch (ProjectInitializerException e) {
			e.printStackTrace();
			fail();
	...
	public void test9PerformSomeConcurrentModifications()
	{
		...
		assertAllFilesAreUpToDateOrCVSIgnoredExcept(_project1,
				...
				_project1.getCVSFile("removeThisFileLater3"),
				_project1.getCVSFile("removeThisFileLater2"));
		...
		assertEquals(_project1.getCVSFile("Components"+File.separator+OPERATION_COMPONENT_1+".woxml").getStatus(), CVSStatus.RemotelyModified);
		assertEquals(_project1.getCVSFile("Components"+File.separator+OPERATION_COMPONENT_2+".woxml").getStatus(), CVSStatus.LocallyModified);
