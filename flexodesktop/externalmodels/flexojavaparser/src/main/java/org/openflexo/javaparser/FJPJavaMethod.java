...
public class FJPJavaMethod extends FJPJavaEntity implements ParsedJavaMethod {
	...
	{
		String returned = 
			(withModifiers?getModifiersAsString():"")
			+getReturnAsString()
			+" "+getName()
			+"("+getParametersAsString()+")"
			+(getExceptions().length>0?" throws "+getExceptionsAsString():"");
		return returned;
	}
	...
	public String getSimplifiedParametersTypesAsString() 
	{
		if (simplifiedParametersTypesAsString == null) {
			...
			StringBuffer sb = new StringBuffer();
			for (FJPJavaParameter p : getParameters()) {
				sb.append((isFirst?"":",")+getNonQualifiedName(p.getType())+(p.isVarArgs()?"...":"")+(p.isArray()?"[]":""));
				isFirst = false;
			}
	...
	{
		if (getReturns() != null) return getReturns().toString();
		return "void";
	}
	...
	private static String unqualifiedType(String qualifiedType)
	{
		...
		if (qualifiedType.indexOf("<") > -1 
				&& qualifiedType.indexOf(">") > -1
				&& qualifiedType.indexOf("<") < qualifiedType.indexOf(">")) 
	...
	{
		if (aSignature.indexOf("(") > -1 
				&& aSignature.indexOf(")") > -1
				&& aSignature.indexOf("(") < aSignature.indexOf(")")) 
