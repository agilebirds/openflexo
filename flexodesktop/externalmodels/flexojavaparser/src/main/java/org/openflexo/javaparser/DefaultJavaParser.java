...
public class DefaultJavaParser implements JavaClassParser, JavaMethodParser, JavaFieldParser {
	...
	{
		try {
			String parsedString = "public class TemporaryClass {"+StringUtils.LINE_SEPARATOR
			+methodCode+StringUtils.LINE_SEPARATOR+"}"+StringUtils.LINE_SEPARATOR;
			...
			FJPJavaClass parsedClass = source.getRootClass();
			if (parsedClass.getMethods().length == 0) return null;
	...
	public ParsedJavaField parseField(String fieldCode, DMModel dataModel) throws JavaParseException
	{
		...
		try {
			parsedString = "public class TemporaryClass {"+StringUtils.LINE_SEPARATOR
			+fieldCode+StringUtils.LINE_SEPARATOR+"}"+StringUtils.LINE_SEPARATOR;
			...
			FJPJavaSource source = new FJPJavaSource(sourceName,parsedString,dataModel.getClassLibrary());
			FJPJavaClass parsedClass = source.getRootClass();
			if (parsedClass.getFields().length == 0) return null;
	...
	public static void main(String[] args)
	{
		...
		String codeToParse = 
			...
			+" * description first line"+StringUtils.LINE_SEPARATOR
			+" * description second line"+StringUtils.LINE_SEPARATOR
			+" * description third line"+StringUtils.LINE_SEPARATOR
			+" * "+StringUtils.LINE_SEPARATOR
			+" * @doc UserManual a description for user manual"+StringUtils.LINE_SEPARATOR
			+" * @doc Technical a technical description"+StringUtils.LINE_SEPARATOR
			+" * @param param1 param1 description"+StringUtils.LINE_SEPARATOR
			+" * @param param2 param2 description"+StringUtils.LINE_SEPARATOR
			+" * @param param3 param3 description"+StringUtils.LINE_SEPARATOR
			+" * @return foo"+StringUtils.LINE_SEPARATOR
			+" */"+StringUtils.LINE_SEPARATOR
			+"public a() {}";
		try {
	...
	public void updateWith(DMMethod method, ParsedJavaMethod javaMethod) throws DuplicateMethodSignatureException 
	{
			...
			if (logger.isLoggable(Level.FINE)) logger.fine("Try to update method "+method+" from parsed method");
			try {
				DMMethod updatedMethod = FJPDMMapper.makeMethod(parsedClass,parsedMethod.getCallSignature(),method.getDMModel(),null,source,false);
			...
			if (logger.isLoggable(Level.FINE)) logger.fine ("Try to update method "+method+" from parsed method: DONE ");
	}

	@Override
	public void updateGetterWith(DMProperty property, ParsedJavaMethod javaMethod) throws DuplicateMethodSignatureException
	{
		if (logger.isLoggable(Level.FINE)) logger.fine("Try to update property "+property+" from parsed getter method");
	...
	{	
		if (logger.isLoggable(Level.FINE)) logger.fine("Try to update property "+property+" from parsed setter method");
	...
	{	
		if (logger.isLoggable(Level.FINE)) logger.fine("Try to update property "+property+" from parsed addition method");
	...
	{	
		if (logger.isLoggable(Level.FINE)) logger.fine("Try to update property "+property+" from parsed removal method");
	...
	{
		if (logger.isLoggable(Level.FINE)) logger.fine("Try to update property "+property+" from parsed field");
		// TODO implement this
		// TODO: handle 'static' here
	}
