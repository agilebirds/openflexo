...
/**
 * Utility class used to perform mapping between FJP model and Flexo DM model
 * (maps source code on DM-model, ie DMEntity, DMProperty, ...)
 * 
 * @author sylvain
 *
 */
public class FJPDMMapper {
	...
	public static Vector<DMMethod> searchForMethods(
			FJPJavaClass aClass, 
			DMModel dataModel, 
			FJPDMSet context, 
			...
			Vector<String> excludedSignatures)
	{
		...
		try {
			...
			for (FJPJavaMethod method : declaredMethods) {
				...
				if (newMethod != null && 
						(excludedSignatures == null
						|| !excludedSignatures.contains(newMethod.getSignature()))) {
	...
	public static Vector<DMProperty> searchForProperties(
			FJPJavaClass aClass, 
			DMModel dataModel, 
			FJPDMSet context, 
			FJPJavaSource source, 
			...
			Vector<String> excludedSignatures)
	{
		...
		for (FJPJavaMethod method : declaredMethods) {
			...
			try {
				newProperty = makeProperty(method,dataModel,context,source,includesGetOnlyProperties,importReferencedEntities,excludedSignatures);
		...
		for (int i = 0; i < declaredFields.length; i++) {
			...
			if (newProperty != null) returned.add(newProperty);
			...
			boolean importReferencedEntities) throws FJPTypeResolver.CrossReferencedEntitiesException
	{
		...
		if (method.isPublic()) visibility = DMVisibilityType.PUBLIC;
		else if (method.isProtected()) visibility = DMVisibilityType.PROTECTED;
		else if (method.isPrivate()) visibility = DMVisibilityType.PRIVATE;
		...
		if (parameters != null) {
			for (FJPJavaParameter parameter : parameters) {
				...
				if (FJPTypeResolver.isResolvable(parameter.getType(), dataModel, context, source)) {
					/*typeEntity =*/ FJPTypeResolver.resolveEntity(parameter.getType(),dataModel,context, source,importReferencedEntities);
		...
		try {
			newMethod.getSourceCode().setCode((method.getJavadoc()!=null?method.getJavadoc().toString():"")+method.getSourceCode(),false);
	 ...
	 */
	static DMProperty makeProperty(
			FJPJavaClass aClass, 
			String propertyName, 			
			DMModel dataModel, 
			FJPDMSet context,
			...
			boolean includesGetOnlyProperties, 
			boolean importReferencedEntities, 
			Vector<String> excludedSignatures) throws FJPTypeResolver.CrossReferencedEntitiesException
	{
		if (logger.isLoggable(Level.FINE))
		...
		if (field == null) field = aClass.getFieldByName("_"+propertyName);
		if (field != null) return makeProperty(
				field,
				dataModel,
				context,
				source,
			...
			Vector<String> excludedSignatures) throws FJPTypeResolver.CrossReferencedEntitiesException
	{
				...
				&& parameters.size() == 0) 
		{
			...
			if (cardinality == DMCardinality.VECTOR) {
				...
				if (addToMethods.size() > 0) additionMethod = addToMethods.first().method;
				if (removeFromMethods.size() > 0) removalMethod = removeFromMethods.first().method;
			...
			if (cardinality == DMCardinality.HASHTABLE) {
				...
				if (setMethods.size() > 0) additionMethod = setMethods.first().method;
				if (removeMethods.size() > 0) removalMethod = removeMethods.first().method;
				returnType = propertyType;
				if (returnType == null) {
					returnType = DMType.makeResolvedDMType(dataModel.getDMEntity(Object.class));
				}
			...
			if (includesGetOnlyProperties || isSettable) {
				...
				try {
					newProperty.getGetterSourceCode().setCode((method.getJavadoc()!=null?method.getJavadoc().toString():"")+method.getSourceCode(),false);
					if (setMethod != null)
						newProperty.getSetterSourceCode().setCode((setMethod.getJavadoc()!=null?setMethod.getJavadoc().toString():"")+setMethod.getSourceCode(),false);
					if (additionMethod != null)
						newProperty.getAdditionSourceCode().setCode((additionMethod.getJavadoc()!=null?additionMethod.getJavadoc().toString():"")+additionMethod.getSourceCode(),false);
					if (removalMethod != null)
						newProperty.getRemovalSourceCode().setCode((removalMethod.getJavadoc()!=null?removalMethod.getJavadoc().toString():"")+removalMethod.getSourceCode(),false);
				} catch (ParserNotInstalledException e) {
					e.printStackTrace();
				} catch (DuplicateMethodSignatureException e) {
					e.printStackTrace();
				...
				if (isSettable) {
					if (setMethod != null 
							&& setMethod.getParameters() != null
							&& setMethod.getParameters().size() == 1)
						newProperty.setSetterParamName(setMethod.getParameters().firstElement().getName());
				}
				
				if (cardinality.isMultiple()) {
					if (additionMethod != null 
							&& additionMethod.getParameters() != null
							&& additionMethod.getParameters().size() == 1)
						newProperty.setAdditionAccessorParamName(additionMethod.getParameters().firstElement().getName());
					if (removalMethod != null 
							&& removalMethod.getParameters() != null
							&& removalMethod.getParameters().size() == 1)
	 ...
	 */
	private static FJPJavaMethod searchMatchingMethod(FJPJavaClass aClass, String signature, DMModel dataModel, FJPDMSet context, FJPJavaSource source)
	{	
		if (logger.isLoggable(Level.FINE)) logger.fine("Searching "+signature);
		for (FJPJavaMethod method : aClass.getMethods()) {
			if (logger.isLoggable(Level.FINE)) logger.fine("Try "+getFullQualifiedCallSignature(method,dataModel,context,source));
	...
	private static DMCardinality getCardinality(Type aType)
	{
		...
		if (aType.isA(NSARRAY_TYPE)) return DMCardinality.VECTOR;
		Type NSARRAY_TYPE_FQ = new Type("com.webobjects.foundation.NSArray");
		if (aType.isA(NSARRAY_TYPE_FQ)) return DMCardinality.VECTOR;
		
		Type VECTOR_TYPE = new Type(java.util.Vector.class.getCanonicalName());
		if (aType.isA(VECTOR_TYPE)) return DMCardinality.VECTOR;
		Type HASHTABLE_TYPE = new Type(java.util.Hashtable.class.getCanonicalName());
		if (aType.isA(HASHTABLE_TYPE)) return DMCardinality.HASHTABLE;
