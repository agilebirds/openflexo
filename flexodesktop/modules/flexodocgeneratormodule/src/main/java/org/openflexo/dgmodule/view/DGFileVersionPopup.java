 ...
 * @author sylvain
 */
public class DGFileVersionPopup extends FlexoDialog
{
	private final Logger logger = FlexoLogger.getLogger(DGFileVersionPopup.class.getPackage().getName());
	...
	public DGFileVersionPopup (CGFile file, ContentSource contentSource, DGController controller)
	{
	...
	public class CGFileVersionPanel extends JPanel implements FlexoObserver
	{
		...
		{
			if ((generationStatus == GenerationStatus.Unknown)
					|| (generationStatus != _cgFile.getGenerationStatus())
					|| (generationStatus == GenerationStatus.GenerationError)
					|| (isEdited != _cgFile.isEdited())) {
				logger.info("CGFileVersionPanel :"+_cgFile.getFileName()+" rebuild view for new status "+_cgFile.getGenerationStatus());
				rebuildView();
				revalidate();
				repaint();
			}
	...
	{
		return file.getResource().getFile().getName()
		+" ["+contentSource.getStringRepresentation()+"]";
	}
