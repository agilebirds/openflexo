        ...
        return new FlexoActionInitializer<GeneratePDF>() {
			...
			public boolean run(ActionEvent e, GeneratePDF action)
            {
                ...
                if (!action.getRepository().getPostBuildRepository().getDirectory().exists()) {
                    if (FlexoController.confirm(FlexoLocalization.localizedForKey("directory")+" "+action.getRepository().getPostBuildRepository().getDirectory().getAbsolutePath()+" "+FlexoLocalization.localizedForKey("does_not_exist")+"\n"+FlexoLocalization.localizedForKey("would_you_like_to_create_it_and_continue?"))) {
                ...
                if (DGPreferences.getLatexCommand() == null || DGPreferences.getLatexCommand().trim().length() == 0) {
                    ...
                    if (DGPreferences.getLatexCommand() == null || DGPreferences.getLatexCommand().trim().length() == 0) {
                        ...
                        FileSelectorParameter file = new FileSelectorParameter("file", "file",
                                ToolBox.getPLATFORM() == ToolBox.WINDOWS ? new File(System.getenv("ProgramFiles")) : new File(System
                        ...
                        AskParametersDialog ask = AskParametersDialog.createAskParametersDialog(getProject(), null,
                                FlexoLocalization
								                .localizedForKey("select_latex_command"), FlexoLocalization.localizedForKey("choose_command_to_execute"), radio, command, file);
                        if (ask.getStatus() == AskParametersDialog.VALIDATE) {
                            ...
                            if (LatexUtils.testLatexCommand(DGPreferences.getLatexCommand())) {
                                ...
                                if (FlexoController.confirm(FlexoLocalization.localizedForKey("PDF_generation_is_based_on_code_generated_on_disk._Is_your_code_on_disk_ready_to_be_compiled?"))) {
                                    return true;
                                } else
                                    return false;
                            }
                    ...
                    if (FlexoController
						...
						if (action.getRepository().getPostBuildFile() != null && action.getRepository().getPostBuildFile().exists()) {
							...
							try {
								...
								if (FlexoController.confirm(FlexoLocalization.localizedForKey("target_pdf_file_seems_to_be_locked.continue_anyway")))
									return true;
								else
									return false;
							} finally {
        ...
        return new FlexoExceptionHandler<GeneratePDF>() {
			...
			public boolean handleException(FlexoException exception, GeneratePDF action)
            {
                ...
                if (exception instanceof IOExceptionOccuredException) {
                    FlexoController.notify(FlexoLocalization.localizedForKey("generation_failed") + ":\n"+FlexoLocalization.localizedForKey("could_not_rename_output_to")+" "+action.getRepository().getPostBuildFile().getAbsolutePath()+"\n"+FlexoLocalization.localizedForKey("verify_that_you_have_write_permissions,_that the file is not used by another application"));
                    return true;
                }
