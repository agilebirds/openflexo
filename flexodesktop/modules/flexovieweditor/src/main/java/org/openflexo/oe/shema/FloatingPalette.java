...
import org.openflexo.foundation.ontology.action.AddConnector;
...
import org.openflexo.foundation.ontology.calc.AddConcept;
...
public class FloatingPalette extends ControlArea<FGERoundRectangle> implements Observer {
	...
	protected OEShapeGR to = null;
	protected GraphicalRepresentation<?> focusedGR;
	private OEShemaController controller;
	private FGEPoint normalizedStartPoint;

	private Rectangle previousRectangle;
	private Mode mode;

	public void paint(Graphics g, OEShemaController controller)
	{
	...
	public void startDragging(DrawingController<?> controller, FGEPoint startPoint) {
		...
		if (mode!=null) {
			...
			this.controller = (OEShemaController)controller;
			((OEShemaView)controller.getDrawingView()).setFloatingPalette(this);
		} else {
			drawEdge = false;
		}
	}
	...
	public boolean dragToPoint(FGEPoint newRelativePoint, FGEPoint pointRelativeToInitialConfiguration, FGEPoint newAbsolutePoint,
		...
		if (drawEdge) {
			...
			if (focusedGR instanceof OEShapeGR && focusedGR != shapeGR) {
				to = (OEShapeGR)focusedGR;
			} else {
				to = null;
			}
	...
	public void stopDragging(DrawingController<?> controller) {
		if (drawEdge && currentDraggingLocationInDrawingView!=null && isDnd) {
			try {
				...
				resetVariables();
				((OEShemaView)controller.getDrawingView()).resetFloatingPalette();
				DrawingView<?> drawingView = controller.getDrawingView();
				FGEPaintManager paintManager = drawingView.getPaintManager();
	...
	private static FGERoundRectangle makeRoundRect(OEShapeGR shapeGR, SimplifiedCardinalDirection orientation)
	{
