...
public class DrawEdgeControl extends MouseDragControl {
	...
	protected OEShapeGR fromShape = null;
	protected OEShapeGR toShape = null;
	...
	protected class DrawEdgeAction extends CustomDragControlAction
	{
		...
		{
			if (graphicalRepresentation instanceof OEShapeGR) {
				drawEdge = true;
				fromShape = (OEShapeGR)graphicalRepresentation;
				((OEShemaView)controller.getDrawingView()).setDrawEdgeAction(this);
				return true;
			}
			return false;
		}
		...
		public boolean handleMouseReleased(GraphicalRepresentation<?> graphicalRepresentation, final DrawingController<?> controller, MouseEvent event, boolean isSignificativeDrag)
		{
			if (drawEdge) {
				...
				if (fromShape != null && toShape != null) {
					...
					if (availableConnectors.size()>0) {
							...
							menuItem.addActionListener(new ActionListener() {
								...
								public void actionPerformed(ActionEvent e)
								{
									...
									LinkSchemeAction action = LinkSchemeAction.actionType.makeNewAction(
											from.getShema(), null, ((OEShemaController)controller).getOEController().getEditor());
						...
						menuItem.addActionListener(new ActionListener() {
							...
							public void actionPerformed(ActionEvent e)
							{
								AddConnector action = AddConnector.actionType.makeNewAction(from, null, ((OEShemaController) controller)
				...
				fromShape = null;
				toShape = null;
				((OEShemaView)controller.getDrawingView()).setDrawEdgeAction(null);
				return true;
			}
			return false;
		}
		...
		public boolean handleMouseDragged(GraphicalRepresentation<?> graphicalRepresentation, DrawingController<?> controller, MouseEvent event)
		{
			if (drawEdge) {
				...
				if (gr instanceof OEShapeGR
						&& gr != fromShape
						&& !fromShape.getAncestors().contains(gr.getDrawable())) {
					toShape = (OEShapeGR)gr;
				}
				else {
					toShape = null;
				}
