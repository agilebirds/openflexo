...
public class CVSFileModuleView extends JPanel implements ModuleView<CVSFile>, FlexoObserver, FlexoActionSource, FileContentEditor, Observer
{
	...
	protected class ViewHeader extends JPanel
	{
		...
		protected ViewHeader()
		{
			...
			if (isEdited) {
				FlexoActionButton saveAction = new FlexoActionButton(SaveCVSFile.actionType,"save",CVSFileModuleView.this,_controller.getEditor());
				FlexoActionButton revertToSavedAction = new FlexoActionButton(RevertToSavedCVSFile.actionType,"revert_to_saved",CVSFileModuleView.this,_controller.getEditor());
				...
				FlexoActionButton editFileAction = new FlexoActionButton(EditCVSFile.actionType,"edit",CVSFileModuleView.this,_controller.getEditor());
				actionButtons.add(editFileAction);
				controlPanel.add(editFileAction);
				
				if (_cvsFile.getStatus().isConflicting()) {
					FlexoActionButton markAsMergedFileAction = new FlexoActionButton(MarkAsMergedFiles.actionType,"mark_as_merged",CVSFileModuleView.this,_controller.getEditor());
				...
				if (_cvsFile.getStatus().isLocallyModified() && !_cvsFile.getStatus().isConflicting()) {
					FlexoActionButton commitFileAction = new FlexoActionButton(CommitFiles.actionType,"commit",CVSFileModuleView.this,_controller.getEditor());
				...
				if (_cvsFile.getStatus().isRemotelyModified() && !_cvsFile.getStatus().isConflicting()) {
					FlexoActionButton updateFileAction = new FlexoActionButton(UpdateFiles.actionType,"update",CVSFileModuleView.this,_controller.getEditor());
		...
		private String mergeInfoForFile()
		{
			if (_cvsFile.getStatus().isConflicting()) {
				...
				return "<html>"
				+rightChangesIconHTML+" "+_cvsFile.getMerge().getRightChangeCount()+" &nbsp; "
				+leftChangesIconHTML+" "+_cvsFile.getMerge().getLeftChangeCount()+" &nbsp; "
				+conflictIconHTML+" "+_cvsFile.getMerge().getConflictsChangeCount()+" "
				+" ("+_cvsFile.getMerge().getResolvedConflictsChangeCount()+"/"+_cvsFile.getMerge().getConflictsChangeCount()
				+" "+FlexoLocalization.localizedForKey("resolved_plural")+")"
				+"</html>";
			}
				...
				return "<html>"
				+leftModificationIconHTML+" "+modificationAsString+" &nbsp; "
				+leftAdditionIconHTML+" "+additionAsString+" &nbsp; "
				+leftRemovalIconHTML+" "+removalAsString
				+"</html>";
			}
				...
				return "<html>"
				+rightModificationIconHTML+" "+modificationAsString+" &nbsp; "
				+rightAdditionIconHTML+" "+additionAsString+" &nbsp; "
				+rightRemovalIconHTML+" "+removalAsString
				+"</html>";
			}
			else {
				return "";
		...
		if (isEdited) {
			...
			if (_cvsFile.getStatus().isConflicting()) {
				...
				observedMerged.addObserver(this);
					...
					|| _cvsFile.getStatus().isRemotelyModified()) {
