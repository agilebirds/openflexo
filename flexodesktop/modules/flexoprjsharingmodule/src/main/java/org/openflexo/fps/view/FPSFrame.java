...
public class FPSFrame extends FlexoFrame {
	 ...
	 */
	public FPSFrame(String title, FPSController controller, FPSKeyEventListener wkfKeyEventListener, FPSMenuBar menuBar)
	...
	public void updateTitle() {
		if (getController().getCurrentModuleView() != null) {
			setTitle(FlexoCst.BUSINESS_APPLICATION_VERSION_NAME
					+ (getModule() != null ? " - " + getModule().getName() : "")
					+ " : "
					+ getViewTitle()
					+ (getFPSController().getSharedProject() != null ? " - "
							+ getFPSController().getSharedProject().getCVSModule().getModuleName() + " - "
