...
//				}
	...
	protected class XMLStorageResourceMergeResultDisplayer extends JPanel implements MergeCodeDisplayerComponent, Observer
	{
		...
		public void update()
		{
			...
			_editor = new DiffPanel(ComputeDiff.diff(getCVSFile().getMerge().getLeftSource(),getCVSFile().getMerge().getMergedSource()),false);
	...
	protected class ASCIIFileMergeCodeDisplayer extends JPanel implements MergeCodeDisplayerComponent, Observer
	{
		...
		public void update()
		{
			...
			_editor = new CVSMergeEditor(getCVSFile().getMerge(),
					getTokenMarkerStyle(),
					FlexoLocalization.localizedForKey("content_on_disk")
					+" ("+FlexoLocalization.localizedForKey("based_on_revision")
					+" "+getCVSFile().getRevision()+")",
					FlexoLocalization.localizedForKey("content_on_repository")
					+" ("+FlexoLocalization.localizedForKey("revision")
	...
	protected class XMLStorageResourceMergeCodeDisplayer extends JPanel implements MergeCodeDisplayerComponent, Observer
	{
		...
		public void update()
		{
			...
			_editor = new XMLDiff3MergeEditor((XMLDiff3)getCVSFile().getMerge(),
					FlexoLocalization.localizedForKey("content_on_disk")
					+" ("+FlexoLocalization.localizedForKey("based_on_revision")
					+" "+getCVSFile().getRevision()+")",
					FlexoLocalization.localizedForKey("content_on_repository")
