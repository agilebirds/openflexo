		...
		return new FlexoActionInitializer<ShareProject>() {
			...
			public boolean run(ActionEvent e, ShareProject action)
            {
                if (action.getProjectDirectory() == null) {
                	...
                	if (newProjectDirectory == null) return false;
                ...
                while (repository == null) {
                	int returned = FlexoController.selectOption(
                			FlexoLocalization.localizedForKey("you_must_now_define_a_cvs_repository_location"), 
                	...
                	if (returned == 0) { // Create new repository
                		AddCVSRepository createNewRepository = AddCVSRepository.actionType.makeNewAction(CVSAction.getRepositoryList(action.getFocusedObject()), null, action.getEditor());
                		createNewRepository.doAction();
                		repository = createNewRepository.getNewCVSRepository();
                		returned = 1;
                	}
                	if (returned == 1) {
                		InfoLabelParameter infoLabel = new InfoLabelParameter("info","info",FlexoLocalization.localizedForKey("share_project_description"),false,5,40);
                		 CVSRepositoryParameter repositoryChoice 
                		= new CVSRepositoryParameter("CVSRepository", "cvs_repository",repository);
                        final TextFieldParameter moduleNameChoice = new TextFieldParameter(
                        		"moduleName", "project_will_be_imported_as_module",action.getModuleName(),40);

                		AskParametersDialog dialog = AskParametersDialog.createAskParametersDialog(
                				getProject(), null, 
                				FlexoLocalization
                				.localizedForKey("share_project"),FlexoLocalization
                				.localizedForKey("please_supply_import_parameters"),infoLabel, repositoryChoice, moduleNameChoice);
                	...
                	else if (returned == 2) return false;
                }
        		...
        		if (!mainCVSIgnoreFile.exists()) {
        			action.setCvsIgnorize(FlexoController.confirm(FlexoLocalization.localizedForKey("would_you_like_to_instrumentalize_your_project_with_required_cvs_ignore_files")));
        		}

        		if (FlexoProject.searchCVSDirs(action.getProjectDirectory())) {
        			action.setRemoveExistingCVSDirectories(FlexoController.confirm(FlexoLocalization.localizedForKey("your_project_seems_to_be_already_under_cvs_remove_all_cvs_informations")));
        		}

    			TextFieldParameter vendorTag 
       			= new TextFieldParameter("vendorTag", "vendor_tag",action.getVendorTag());
       			TextFieldParameter releaseTag 
       			= new TextFieldParameter("releaseTag", "release_tag",action.getReleaseTag());
     			TextAreaParameter commitMessage 
       			= new TextAreaParameter("logMessage", "log_message",action.getLogMessage(),40,10);
    			CheckboxParameter cvsIgnorise 
       			= new CheckboxParameter("cvsIgnorise", "create_cvs_ignore_files",action.getCvsIgnorize());
                     
    			AskParametersDialog dialog = AskParametersDialog.createAskParametersDialog(
    					getProject(), 
    					null, 
    					action.getLocalizedName(),
    					FlexoLocalization.localizedForKey("please_supply_import_parameters"),
    					vendorTag,
    					releaseTag, commitMessage, cvsIgnorise);
    			if (dialog.getStatus() == AskParametersDialog.VALIDATE) {
