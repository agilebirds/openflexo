...
public class WKFSetPropertyInitializer extends ActionInitializer {
	...
	protected FlexoActionInitializer<SetPropertyAction> getDefaultInitializer()
	{
		return new FlexoActionInitializer<SetPropertyAction>() {
			...
			{
				if (action.getFocusedObject() == null) return false;
				...
				if (action.getFocusedObject() instanceof FlexoProcess
						&& action.getKey().equals("parentProcess")) {
		            ...
		            return FlexoController.confirm(FlexoLocalization.localizedForKeyWithParams(message,movedProcess.getName(),(newParentProcess!=null?newParentProcess.getName():null)));
				}
			...
			public boolean run(ActionEvent e, SetPropertyAction action)
			{
				if (action.getFocusedObject() instanceof FlexoProcess
                   ...
                   ((WKFController)getController()).getWorkflowBrowser().focusOn(movedProcess);
					...
					if (action.getPreviousValue() instanceof FlexoProcess && ((FlexoProcess)action.getPreviousValue()).isImported() && !(action.getValue() instanceof FlexoProcess && ((FlexoProcess)action.getValue()).isImported())) {
						if ((action.getFocusedObject() instanceof SingleInstanceSubProcessNode
							...
							if (((SubProcessNode)action.getFocusedObject()).getPreConditions().size()>0) {
								WKFDelete delete = WKFDelete.actionType.makeNewAction(null, new Vector<WKFObject>(((SubProcessNode)action.getFocusedObject()).getPreConditions()), getEditor());
								delete.setNoConfirmation(true);
								delete.doAction();
							}
							if (((SubProcessNode)action.getFocusedObject()).getOutgoingPostConditions().size()>0) {
								WKFDelete delete = WKFDelete.actionType.makeNewAction(null, new Vector<WKFObject>(((SubProcessNode)action.getFocusedObject()).getOutgoingPostConditions()), getEditor());
								delete.setNoConfirmation(true);
								delete.doAction();
							}
						}
		...
		return new FlexoExceptionHandler<SetPropertyAction>() {
			...
			public boolean handleException(FlexoException exception, final SetPropertyAction action)
			{
				if (action.getFocusedObject() instanceof OperationNode
						&& exception instanceof OperationAssociatedWithComponentSuccessfully) {
					WKFExceptionHandler.handleAssociation((OperationAssociatedWithComponentSuccessfully)exception, (WKFController)getController());
					return true;
					...
					if(action.getKey().equals("parentProcess")) {
						if (exception instanceof InvalidParentProcessException) {
		                    ...
		                    FlexoController.showError(FlexoLocalization.localizedForKey("could_not_move_process_inconsistent_process_hierarchy"));
		                    return true;
		                } else if (exception instanceof InvalidProcessReferencesException) {
		                	if (action.getPerformValidate()) {
			                    logger.info("InvalidProcessReferencesException: launching CheckConsistency dialog");
			                    ((InvalidProcessReferencesException)exception).report.setLocalizedTitle(FlexoLocalization.localizedForKey("refactoring_has_generated_following_inconsistency"));
			                    PartialConsistencyCheckDialog pccd = new PartialConsistencyCheckDialog(FlexoLocalization
			                            .localizedForKey("moving_process"), (WKFController)getController(), ((InvalidProcessReferencesException)exception).report);
			                    pccd.show();
		                	}
		                    return true;
		                }
						...
						SwingUtilities.invokeLater(new Runnable(){
							...
							public void run() {
								FlexoController.notify(FlexoLocalization.localizedForKey("could_not_rename_process_to")+" "+
										(action.getValue()==null||action.getValue().equals("")?FlexoLocalization.localizedForKey("empty_value"):action.getValue())+" "+FlexoLocalization.localizedForKey("because_there_is_already_a_process_with_that_name"));
							}
						});
		                return true;
					}
