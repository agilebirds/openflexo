...
public class FloatingPalette extends ControlArea<FGERoundRectangle> implements Observer {
	...
	public void paint(Graphics g, RoleEditorController controller)
	{
		if (drawEdge && currentDraggingLocationInDrawingView != null) {
			...
			FGEPoint nearestOnOutline = fgeShape.getNearestPoint(drawingGR.convertLocalViewCoordinatesToRemoteNormalizedPoint(currentDraggingLocationInDrawingView, roleGR, scale));
	...
	public boolean dragToPoint(FGEPoint newRelativePoint, FGEPoint pointRelativeToInitialConfiguration, FGEPoint newAbsolutePoint,
		...
		if (drawEdge) {
			...
			if (!isDnd) {
				isDnd = roleGR.convertLocalNormalizedPointToRemoteViewCoordinates(normalizedStartPoint, controller.getDrawingGraphicalRepresentation(), controller.getScale()).distance(currentDraggingLocationInDrawingView)>5;
			}
	...
	public void stopDragging(DrawingController<?> controller) {
		if (drawEdge && currentDraggingLocationInDrawingView!=null && isDnd) {
			try {
				...
				SimplifiedCardinalDirection direction = FGEPoint.getSimplifiedOrientation(new FGEPoint(roleGR.convertLocalNormalizedPointToRemoteViewCoordinates(this.normalizedStartPoint, controller.getDrawingGraphicalRepresentation(), controller.getScale())), new FGEPoint(currentDraggingLocationInDrawingView));
				...
				AddRoleSpecialization addRoleSpecializationAction = AddRoleSpecialization.actionType.makeNewAction(roleGR.getDrawable(), null,
	...
	private Role createRole(FGEPoint dropLocation, RoleList container, SimplifiedCardinalDirection direction) 
	{
		...
		if (addRole.getNewRole()!=null) {
			ShapeGraphicalRepresentation<?> gr=(ShapeGraphicalRepresentation<?>) controller.getGraphicalRepresentation(addRole.getNewRole());
	...
	public Rectangle paint(FGEGraphics drawingGraphics)
	{
		...
		if (roleGR.getIsSelected() && !roleGR.getIsFocused()) return null;
	...
	private void updateElements(SimplifiedCardinalDirection orientation) 
	{
		...
		case EAST:
			roleRect = (FGERoundRectangle) new FGERoundRectangle(
					roleGR.getWidth()+SPACING+(PALETTE_WIDTH-ELEMENTS_WIDTH)/2+0.5,
					(roleGR.getHeight()-PALETTE_HEIGHT)/2+SPACING,
					ELEMENTS_WIDTH,
					ELEMENTS_HEIGHT,
					2,2,Filling.FILLED).transform(at);
			edgeRect = (FGERectangle) new FGERectangle(
					roleGR.getWidth()+SPACING+(PALETTE_WIDTH-ELEMENTS_WIDTH)/2,
					(roleGR.getHeight()-PALETTE_HEIGHT)/2+SPACING+(SPACING+ELEMENTS_HEIGHT),
					ELEMENTS_WIDTH,
					ELEMENTS_HEIGHT,
		...
		case NORTH:
			roleRect = (FGERoundRectangle) new FGERoundRectangle(
					(roleGR.getWidth()-PALETTE_WIDTH)/2+SPACING,
					-SPACING-ELEMENTS_HEIGHT,
					ELEMENTS_WIDTH,
					ELEMENTS_HEIGHT,
					2,2,Filling.FILLED).transform(at);
			edgeRect = (FGERectangle) new FGERectangle(
					(roleGR.getWidth()-PALETTE_WIDTH)/2+SPACING+(SPACING+ELEMENTS_WIDTH),
					-SPACING-ELEMENTS_HEIGHT,
					ELEMENTS_WIDTH,
					ELEMENTS_HEIGHT,
					Filling.FILLED).transform(at);
		...
		case SOUTH:
			roleRect = (FGERoundRectangle) new FGERoundRectangle(
					(roleGR.getWidth()-PALETTE_WIDTH)/2+SPACING,
					roleGR.getHeight()+SPACING+(PALETTE_HEIGHT-ELEMENTS_HEIGHT)/2+0.5,
					ELEMENTS_WIDTH,
					ELEMENTS_HEIGHT,
					2,2,Filling.FILLED).transform(at);
			edgeRect = (FGERectangle) new FGERectangle(
	...
	private static FGERoundRectangle makeRoundRect(RoleGR roleGR, SimplifiedCardinalDirection orientation)
	{
		...
		case EAST:
			...
			return new FGERoundRectangle(
					x/roleGR.getWidth(),
					y/roleGR.getHeight(),
					width/roleGR.getWidth(),
					height/roleGR.getHeight(),
					13.0/roleGR.getWidth(),
					13.0/roleGR.getHeight(),
					Filling.FILLED);
		case WEST:
