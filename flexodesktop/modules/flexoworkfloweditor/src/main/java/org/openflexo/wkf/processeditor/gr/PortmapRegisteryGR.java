...
public class PortmapRegisteryGR extends WKFObjectGR<PortMapRegistery> {
	...
	public PortmapRegisteryGR(PortMapRegistery portmapRegistery, ProcessRepresentation aDrawing) {
		...
		background = BackgroundStyleImpl.makeColoredBackground(Color.ORANGE);

		setForeground(foreground);
		setBackground(background);

		setBorder(new ShapeGraphicalRepresentationImpl.ShapeBorderImpl(0, 0, 0, 0));
	...
	public void refreshOrientation() {
		...
		if (!FGEUtils.areElementsConnectedInGraphicalHierarchy(this, subProcessNodeGR)) {
			// Might happen when big restructuration (eg multiple deletion, see Bug 1005566)
			return;
		}
		FGEPoint locationInSubProcessNode = FGEUtils.convertNormalizedPoint(this, new FGEPoint(0.5, 0.5),
				subProcessNodeGR);
