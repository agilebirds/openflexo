		...
		return new FlexoActionInitializer<AddRoleSpecialization>() {
			...
			public boolean run(ActionEvent e, AddRoleSpecialization action)
			{
                ...
                if (action.getRoleSpecializationAutomaticallyCreated()) {
                	if (availableRoles.contains(action.getNewParentRole())) return true;
                	else {
                		// This parent is not conform
                		if (action.getFocusedObject().isSpecializingRole(action.getNewParentRole())) {
                			FlexoController.notify(FlexoLocalization.localizedForKeyWithParams("role_($0)_is_already_specialized_by_role_($1)", action.getNewParentRole().getName(), action.getFocusedObject().getName()));
                		}
                		else if (action.getFocusedObject().isTransitivelySpecializingRole(action.getNewParentRole())) {
                			FlexoController.notify(FlexoLocalization.localizedForKeyWithParams("role_($0)_is_already_transitively_specialized_by_role_($1)", action.getNewParentRole().getName(), action.getFocusedObject().getName()));
                		}
                		else if (action.getNewParentRole().isTransitivelySpecializingRole(action.getFocusedObject())) {
                			FlexoController.notify(FlexoLocalization.localizedForKeyWithParams("role_($0)_cannot_specialize_role_($1)_which_already_specialize_this_role_(forbidden_cycles)", action.getFocusedObject().getName(), action.getNewParentRole().getName()));
                		}
                		else {
				...
				if (availableRoles.size() == 0) {
          			FlexoController.notify(FlexoLocalization.localizedForKeyWithParams("found_no_role_that_could_be_specialized_by_role_($0)", action.getFocusedObject().getName()));
          			return false;
				}
