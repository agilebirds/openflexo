...
public class AddSubProcessInitializer extends ActionInitializer {
	...
	protected FlexoActionInitializer<AddSubProcess> getDefaultInitializer()
	{
		return new FlexoActionInitializer<AddSubProcess>() {
			...
			public boolean run(ActionEvent e, AddSubProcess action)
            {
                ...
                parameters[1] = new RadioButtonListParameter<String>("context", "process_context", process==null?NO_CONTEXT:UNDER_PROCESS, contexts);
                ...
                AskParametersDialog dialog = AskParametersDialog.createAskParametersDialog(getProject(), null, FlexoLocalization
								                .localizedForKey("create_new_sub_process"), FlexoLocalization
                  .localizedForKey("enter_parameters_for_the_new_sub_process"), new ValidationCondition(){
                	@Override
	...
	protected FlexoActionFinalizer<AddSubProcess> getDefaultFinalizer()
	{
		return new FlexoActionFinalizer<AddSubProcess>() {
            ...
            {
            	boolean res = true;
            	if (action.getNewProcess()!=null && action.getFocusedObject() instanceof ProcessFolder
            			&& action.getNewProcess().getProcessNode().getFatherProcessNode()==((ProcessFolder)action.getFocusedObject()).getProcessNode()) {
            		AddToProcessFolder add = AddToProcessFolder.actionType.makeNewAction(action.getNewProcess(), null, getController().getEditor());
