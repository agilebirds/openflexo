		...
		return new FlexoActionInitializer<WKFDelete>() {
			...
			public boolean run(ActionEvent e, WKFDelete action)
            {
                ...
                if (v.size() == 1 && v.firstElement() instanceof FlexoProcess) {
            		...
            		if (action.getInvoker() instanceof FlexoMenuItem
            				|| action.getInvoker() instanceof WKFKeyEventListener) {
                	...
                	if (process.getAllProcessInstances().size() > 0) {
                		...
                		for (SubProcessNode node : process.getAllProcessInstances()) {
                			sb.append("\n").append("* ").append(node.getName()).append(
                					FlexoLocalization.localizedForKey("located_within_the_process_") + node.getProcess().getName());
                		}
                            ...
                            if (o instanceof FlexoProcess) {
                                if (((FlexoProcess) o).isRootProcess()) {
                                    ...
                                    if (process.getAllProcessInstances().size() > 0) {
                                        ...
                                        for (SubProcessNode node : process.getAllProcessInstances()) {
                                            sb.append("\n").append("* ").append(node.getName()).append(
                                                    FlexoLocalization.localizedForKey("located_within_the_process_")
                                    ...
                                    action.setDeletionContextForObject(new Boolean(FlexoController
                                            .confirmWithWarning(FlexoLocalization.localizedForKey("would_you_like_to_delete_the_process")
                                                    + ((FlexoProcess) o).getName() + " ?")), process);
                                }
                            }
                        }
                        doIt = true;
	...
	protected FlexoActionFinalizer<WKFDelete> getDefaultFinalizer() 
	{
     ...
     protected FlexoActionEnableCondition getEnableCondition()
 	{
 		return new FlexoActionEnableCondition<WKFDelete,WKFObject,WKFObject>() {
			@Override
			public boolean isEnabled(FlexoActionType<WKFDelete, WKFObject, WKFObject> actionType, WKFObject object, Vector<WKFObject> globalSelection, FlexoEditor editor) {
