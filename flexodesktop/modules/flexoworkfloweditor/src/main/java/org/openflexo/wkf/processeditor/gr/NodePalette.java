...
public class NodePalette extends ControlArea<FGERoundRectangle> implements ProcessEditorConstants, Observer {
			...
			try {
				...
				case DATA_SOURCE:
					if (target.getLevel()==FlexoLevel.ACTIVITY)
						...
						return ;
					to = createElement(((ProcessEditorController)controller).getArtefactPalette().getDataSource(), dropLocation, target, direction);
					break;
				case DATA_OBJECT:
					/*if (target.getLevel()==FlexoLevel.ACTIVITY)
						...
						return ;*/
					to = createElement(((ProcessEditorController)controller).getArtefactPalette().getDataFile(), dropLocation, target, direction);
					break;
	...
	private WKFNode createElement(WKFPaletteElement element, FGEPoint dropLocation, FlexoPetriGraph container, SimplifiedCardinalDirection direction) 
	{
	...
	public Rectangle paint(FGEGraphics drawingGraphics)
	{
		...
		if (nodeGR.getIsSelected() && !nodeGR.getIsFocused()) return null;
