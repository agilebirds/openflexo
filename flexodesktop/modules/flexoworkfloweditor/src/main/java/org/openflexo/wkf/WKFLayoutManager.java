...
public abstract class WKFLayoutManager
{
	...
	private void buildNodeMap(FlexoProgress progress)
	{
		...
		for (PetriGraphNode n : nodeMap.keySet()) {
			...
			if (progress!=null) progress.setSecondaryProgress(FlexoLocalization.localizedForKey("current_node")+" "+aln.node.getName());
			aln.addEdges(n);
	...
	private void buildSecondaryPaths(FlexoProgress progress)
	{
		...
		while (forgottenNodes.size() > 0) {
			...
			Vector<AutoLayoutNodePath> potentialSubPaths
			= _buildAllSubPathForForForgottenNodes(forgottenNodes,mainPath,secondaryPaths);
			...
			for (AutoLayoutNode node : bestSecondaryPath) {
				if (forgottenNodes.contains(node)) {
					if (progress!=null)
						progress.setSecondaryProgress(FlexoLocalization.localizedForKey("from_node")+" "+bestSecondaryPath.firstElement().node.getName());
	...
	private Vector<AutoLayoutNodePath> _buildAllSubPathForForForgottenNodes(
			Vector<AutoLayoutNode> forgottenNodes,
			AutoLayoutNodePath mainPath,
			Vector<AutoLayoutNodePath> secondaryPaths)
	{
		...
		for (AutoLayoutNodePath subPath : secondaryPaths) alreadyVisited.addAll(subPath);
		...
		Collections.sort(returned, new Comparator<AutoLayoutNodePath>() {
			...
			{
				if (p1.size() != p2.size()) return p2.size()-p1.size();
				int lp1 = (p1.startPath!=null?1:0)+(p1.endPath!=null?1:0);
				int lp2 = (p2.startPath!=null?1:0)+(p2.endPath!=null?1:0);
				if (lp1 != lp2) return lp2-lp1;
				if (lp1 == 2) {
					if (p1.startPath == p1.endPath) return -1;
					else if (p2.startPath == p2.endPath) return 1;
				}
				return -1;
			}
		});
	...
	private void _findLoopingAndShortcutEdges(AutoLayoutNodePath path, Vector<FlexoPostCondition<AbstractNode, AbstractNode>> loopingEdgesVector, Vector<FlexoPostCondition<AbstractNode, AbstractNode>> shortcutEdgesVector)
	{
		/*boolean debug = false;
			...
			for (FlexoPostCondition<AbstractNode, AbstractNode> post : node.followingNodes.keySet()) {
				...
				if (!currentPath.contains(n)) {
					if (path.contains(n) && !(path.indexOf(n)==path.indexOf(node)+1)) {
						...
						if (!shortcutEdgesVector.contains(post)) shortcutEdgesVector.add(post);
					}
					else {
						...
						for (AutoLayoutNodePath subPath : subPaths) {
							...
							if (!subPath.isTerminal()) {
								...
								if (lastPost != null && !loopingEdgesVector.contains(lastPost)) loopingEdgesVector.add(lastPost);
							}
						}
					}
				}
	...
	public class AutoLayoutNode
	{
		...
		{
			System.out.println("\nNode: "+node);
			for (AutoLayoutNode aln : followingNodes.values()) System.out.println("> "+aln.node.getName());
			for (AutoLayoutNode aln : previousNodes.values()) System.out.println("< "+aln.node.getName());
		}
	...
	public class AutoLayoutNodePath extends Vector<AutoLayoutNode>
	{
		...
		{
			super();
			for (AutoLayoutNode n : nodes) add(n);
		}
		...
		{
			return name+": "+super.toString()+" start_from:"+(startPath!=null?startPath.name:null)+" ends_to:"+(endPath!=null?endPath.name:null);
		}
	}
