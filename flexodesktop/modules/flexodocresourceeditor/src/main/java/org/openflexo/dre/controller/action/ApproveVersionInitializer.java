	...
	protected FlexoActionInitializer<ApproveVersion> getDefaultInitializer() 
	{
		return new FlexoActionInitializer<ApproveVersion>() {
			...
			public boolean run(ActionEvent e, final ApproveVersion action)
            {
                ...
                if (availableVersions.size() == 0) return false;
                ...
                AskParametersDialog dialog = AskParametersDialog.createAskParametersDialog(getProject(),
                        null, FlexoLocalization.localizedForKey("approve_version"), FlexoLocalization.localizedForKey("define_approval_parameters"), params);
