	...
	protected FlexoActionFinalizer<SetPropertyAction> getDefaultFinalizer()
	{
		return new FlexoActionFinalizer<SetPropertyAction>() {
			...
			/*	if (!updatedMethod.isResolvable()) {
				...
				if (action.getFocusedObject() instanceof WORepository
						&& action.getKey().equals(WORepository.CUSTOM_COMPONENT_ENTITY_KEY)) {
					...
					{
						final PropertyListParameter<DMEntity> concernedComponentsParam
						= new PropertyListParameter<DMEntity>("concerned_component",
								"concerned_component",
								nullParentTypeEntities,
								20,10);
						...
						AskParametersDialog dialog = AskParametersDialog.createAskParametersDialog(
								...
								null,
								FlexoLocalization.localizedForKey("selecting_default_component_entity"), "<html><center>"+CGIconLibrary.UNFIXABLE_WARNING_ICON.getHTMLImg()+"<b>&nbsp;"+FlexoLocalization.localizedForKey("warning")+"</b></center><br>"
								+FlexoLocalization.localizedForKey("following_components_dont_declare_any_parent_component")+"<br>"
								+FlexoLocalization.localizedForKey("would_you_like_to_set_this_component_as_parent_component_(recommanded)")+"</html>", concernedComponentsParam);
						if (dialog.getStatus() == AskParametersDialog.VALIDATE) {
							for (DMEntity componentEntity : nullParentTypeEntities) {
								componentEntity.setParentType(DMType.makeResolvedDMType(((WORepository)action.getFocusedObject()).getCustomComponentEntity()), true);
							}
						}
					}
			...
			public boolean handleException(final FlexoException exception, final SetPropertyAction action)
			{
				...
				if (action.getFocusedObject() instanceof DMMethod) {
					if (exception instanceof ParserNotInstalledException) {
						...
						FlexoController.showError(FlexoLocalization.localizedForKey("sorry_this_code_could_not_be_parsed_as_valid_java_code"));
						return true;
					}
