	...
	protected FlexoActionInitializer<CreateComponentFromEntity> getDefaultInitializer() {
		return new FlexoActionInitializer<CreateComponentFromEntity>() {
			...
			public boolean run(EventObject event, CreateComponentFromEntity action) {
				...
				final TextFieldParameter componentNameParam = new TextFieldParameter("componentName",
						FlexoLocalization.localizedForKey("component_name"), ToolBox.capitalize(action.getVariableName(), true)
								+ ToolBox.capitalize(FlexoLocalization.localizedForKey("edition")));
				AskParametersDialog dialog = AskParametersDialog.createAskParametersDialog(getProject(), getController().getFlexoFrame(),
						FlexoLocalization.localizedForKey("create_component_from_entity"), "",
				...
				if (dialog.getStatus() == AskParametersDialog.VALIDATE) {
					SelectPropertiesAndMethodsPopup popup = new SelectPropertiesAndMethodsPopup(
							FlexoLocalization.localizedForKey("attributes"),
							FlexoLocalization.localizedForKey("select_properties_and_methods_description"), "finish",
							action.getFocusedObject(), getProject(), (DMController) getController());
