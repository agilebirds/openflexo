...
public class CreateDMTranstyperInitializer extends ActionInitializer {
	...
	protected FlexoActionInitializer<CreateDMTranstyper> getDefaultInitializer() 
	{
		return new FlexoActionInitializer<CreateDMTranstyper>() {
            ...
            {
            	ReadOnlyTextFieldParameter declaringEntityParam = new ReadOnlyTextFieldParameter("declaringEntity","declaring_entity",action.getEntity().getName());
            	TextFieldParameter nameParam = new TextFieldParameter("transtyperName","transtyper_name",action.getEntity().getDMModel().getNextDefautTranstyperName(action.getEntity()));
            	DMTypeParameter typeParam = new DMTypeParameter("returnedType","returned_type",DMType.makeResolvedDMType(action.getEntity()));
  				

            	final TranstyperEntryInfoParameter entriesParameters = new TranstyperEntryInfoParameter("entries","required_entries_for_this_new_transtyper",action.getFocusedObject(),20,8);
		    	...
		    	PropertyListColumn typeColumn = entriesParameters.addCustomColumn("type", "entry_type","org.openflexo.components.widget.DMTypeInspectorWidget",180,true);
		    	...
		    	typeParam.addValueListener(new ParameterDefinition.ValueListener<DMType>(){
					...
					public void newValueWasSet(ParameterDefinition param, DMType oldValue, DMType newValue) {
	          			allowsMappingDefinitionParam.setValue(
	          					DMTranstyper.allowsMappingDefinitionForType(newValue)?ALLOWS_MAPPING:REFUSE_MAPPING);
	          		}
	          	});
				...
				final RadioButtonListParameter<String> transtyperTypeChoiceParam 
				= new RadioButtonListParameter<String>("transtyperType","please_select_a_choice",DEFINE_MAPPING,relationshipTypeChoices);
