...
public class ImportTemplatesInitializer extends ActionInitializer {
	...
	ImportTemplatesInitializer(GeneratorControllerActionInitializer actionInitializer)
	{
	...
	protected FlexoActionInitializer<ImportTemplates> getDefaultInitializer() 
	{
		return new FlexoActionInitializer<ImportTemplates>() {
			...
			public boolean run(ActionEvent e, ImportTemplates action)
			{
				...
				DirectoryParameter paramDir = new DirectoryParameter("directory", "source_directory", new File(System.getProperty("user.home"),getProject().getProjectName()));
				...
				RadioButtonListParameter<String> repChoiceParam = new RadioButtonListParameter<String>("repChoice","select_a_choice",NEW,repChoice);
				DynamicDropDownParameter<CustomCGTemplateRepository> repositoryParam = new DynamicDropDownParameter<CustomCGTemplateRepository>("repository","template_repository",templates.getCustomCodeRepositoriesVector(),hasTemplateRepositories?action.getFocusedObject().getProject().getGeneratedCode().getTemplates().getCustomCodeRepositoriesVector().firstElement():null);
				...
				AskParametersDialog dialog = AskParametersDialog.createAskParametersDialog(getProject(), null, 
						...
						FlexoLocalization
						.localizedForKey("enter_new_repository_name"),
						params);
				if (dialog.getStatus() == AskParametersDialog.VALIDATE) {
					if(paramDir.getValue()==null || !paramDir.getValue().isDirectory())return false;
					if(newRepositoryNameParam.getValue()==null || newRepositoryNameParam.getValue().length()==0)return false;
					...
					if (!hasTemplateRepositories || repChoiceParam.getValue().equals(NEW)) {
						if (hasTemplateRepositories) {
							if (templates.getCustomCGTemplateRepositoryForName(newRepositoryNameParam.getValue())!=null) {
								FlexoController.notify(FlexoLocalization.localizedForKey("template_repository")+" "+newRepositoryNameParam.getValue()+" "+FlexoLocalization.localizedForKey("already_exists"));
								return false;
							}
						}
						...
						if (!addDirectory.hasActionExecutionSucceeded()) {
							FlexoController.notify(FlexoLocalization.localizedForKey("could_not_create_repository")+" "+newRepositoryNameParam.getValue());
							return false;
						}
