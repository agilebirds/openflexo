...
import org.openflexo.generator.action.GoToCorrespondingWO;
...
public class CGFileModuleView extends JPanel implements ModuleView<CGFile>, FlexoObserver, FlexoActionSource, FileContentEditor
{
	...
	public CGFileModuleView(CGFile cgFile, GeneratorController controller)
	{
	...
	protected class ViewHeader extends JPanel implements Observer
	{
		...
		protected ViewHeader()
		{
			...
			if (isEdited) {
				FlexoActionButton saveAction = new FlexoActionButton(SaveGeneratedFile.actionType,"save",CGFileModuleView.this,getController().getEditor());
				FlexoActionButton revertToSavedAction = new FlexoActionButton(RevertToSavedGeneratedFile.actionType,"revert_to_saved",CGFileModuleView.this,getController().getEditor());
				...
				FlexoActionButton editFileAction = new FlexoActionButton(EditGeneratedFile.actionType,"edit",CGFileModuleView.this,getController().getEditor());
				if(!_cgFile.getMarkedAsDoNotGenerate()) {
					actionButtons.add(editFileAction);
				}

				FlexoActionButton generateAction = new FlexoActionButton(GenerateSourceCode.actionType,"regenerate",CGFileModuleView.this,getController().getEditor());
				if(!_cgFile.getMarkedAsDoNotGenerate()) {
					actionButtons.add(generateAction);
				}

				FlexoActionButton writeFileAction = new FlexoActionButton(WriteModifiedGeneratedFiles.actionType,"write_file",CGFileModuleView.this,getController().getEditor());
				if(!_cgFile.getMarkedAsDoNotGenerate()) {
					actionButtons.add(writeFileAction);
				}
				
				FlexoActionButton dismissUnchangedFileAction = new FlexoActionButton(DismissUnchangedGeneratedFiles.actionType,"dismiss_as_unchanged",CGFileModuleView.this,getController().getEditor());
				if(!_cgFile.getMarkedAsDoNotGenerate()) {
					actionButtons.add(dismissUnchangedFileAction);
				}
				
				FlexoActionButton forceRegenerateAction = new FlexoActionButton(ForceRegenerateSourceCode.actionType,"force_regenerate",CGFileModuleView.this,getController().getEditor());
				if(!_cgFile.getMarkedAsDoNotGenerate()) {
					actionButtons.add(forceRegenerateAction);
				}
				
				FlexoActionButton acceptDiskVersionAction = new FlexoActionButton(AcceptDiskUpdate.actionType,"accept_disk_version",CGFileModuleView.this,getController().getEditor());
				if(!_cgFile.getMarkedAsDoNotGenerate()) {
					actionButtons.add(acceptDiskVersionAction);
				}
				
				FlexoActionButton markAsMergedAction = new FlexoActionButton(MarkAsMerged.actionType,"mark_as_merged",CGFileModuleView.this,getController().getEditor());
				if(!_cgFile.getMarkedAsDoNotGenerate()) {
					actionButtons.add(markAsMergedAction);
				}
				
				FlexoActionButton cancelOverridingAction = new FlexoActionButton(CancelOverrideWithVersion.actionType,"cancel_override",CGFileModuleView.this,getController().getEditor());
				if(!_cgFile.getMarkedAsDoNotGenerate()) {
					actionButtons.add(cancelOverridingAction);
				}
				
                FlexoActionButton goToCorrespondingWO = null;
                if ((_cgFile instanceof CGJavaFile) && (_cgFile.getResource() instanceof ComponentJavaFileResource)) {
					goToCorrespondingWO = new FlexoActionButton(GoToCorrespondingWO.actionType,"go_to_wo",CGFileModuleView.this,getController().getEditor());
				}
                if(!_cgFile.getMarkedAsDoNotGenerate() && (goToCorrespondingWO!=null)) {
					actionButtons.add(goToCorrespondingWO);
				}
                FlexoActionButton goToCorrespondingJava = null;
                if (_cgFile instanceof CGWOFile) {
					goToCorrespondingJava= new FlexoActionButton(GoToCorrespondingJava.actionType,"go_to_java",CGFileModuleView.this,getController().getEditor());
				}
                if(!_cgFile.getMarkedAsDoNotGenerate() && (goToCorrespondingJava!=null)) {
					actionButtons.add(goToCorrespondingJava);
				}
                ...
                if (goToCorrespondingJava!=null) {
					controlPanel.add(goToCorrespondingJava);
				} else if (goToCorrespondingWO!=null) {
					controlPanel.add(goToCorrespondingWO);
				}
			...
			if (!isEdited) {
				...
				if (_cgFile.getResource() instanceof GenerationAvailableFileResource) {
					...
					if (resource.getGenerator() instanceof FlexoResourceGenerator) {
						...
						if (generator != null) {
							...
							if (generator.hasAppendingException()) {
								addInfoPanel(CGIconLibrary.UNFIXABLE_WARNING_ICON, 
										FlexoLocalization.localizedForKey("java_appending_warning_title"),
										FlexoLocalization.localizedForKey("java_appending_warning_description"));
							}
						}
					}
				}			
				...
				if (_cgFile.needsMemoryGeneration()) {
					addInfoPanel(GeneratorIconLibrary.NEEDS_REGENERATE_ICON, 
							FlexoLocalization.localizedForKey("needs_regenerate_warning"),
				...
				if (_cgFile.hasGenerationErrors()) {
					addInfoPanel(CGIconLibrary.UNFIXABLE_ERROR_ICON, 
							FlexoLocalization.localizedForKey("generation_error"),
		...
		private String subTitleForFile()
		{
			...
			if (isEdited) {
				...
				if (!generationStatus.isAbnormal() 
						&& (generationStatus != GenerationStatus.CodeGenerationNotSynchronized)) {
					if (_cgFile.needsMemoryGeneration()) {
	...
	private void rebuildView()
	{
		...
		if(_cgFile.getMarkedAsDoNotGenerate()){
        	add(new JLabel(FlexoLocalization.localizedForKey("file_marked_as_do_not_generate"), SwingConstants.CENTER),
                    BorderLayout.CENTER);
            validate();
            return;
			...
			if (_cgFile.getGenerationStatus() == GenerationStatus.CodeGenerationNotAvailable) {
				add(new JLabel(FlexoLocalization.localizedForKey("sorry_code_generator_not_available_int_this_version"),SwingConstants.CENTER), BorderLayout.CENTER);
                validate();
				return;
			}
			else if (_cgFile.getGenerationStatus() == GenerationStatus.CodeGenerationNotSynchronized) {
				FlexoActionButton syncButton = new FlexoActionButton(SynchronizeRepositoryCodeGeneration.actionType,"synchronize_now",new FlexoActionSource(){
					@Override
				...
				add(new JLabel(FlexoLocalization.localizedForKey("code_has_not_been_generated"),SwingConstants.CENTER), BorderLayout.CENTER);
				return;
			}

			if (_cgFile instanceof CGWebServerImageFile) {
    ...
    public class ScreenshotDisplayer extends JPanel
    {
        ...
        public void update()
        {
            ...
            CGRepositoryFileResource<? extends GeneratedResourceData, ? extends IFlexoResourceGenerator, ? extends CGFile> resource = screenshotFile.getResource();
