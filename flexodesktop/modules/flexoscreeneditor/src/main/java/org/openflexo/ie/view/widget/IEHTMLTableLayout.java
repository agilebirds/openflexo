...
public class IEHTMLTableLayout implements LayoutManager2
{
    ...
    /**
     * Returns the total width percentage from the cell <code>from</code> to
     * the cell <code>to</code> (included). If you want to know the width
     * percentage of a cell, <code>from</code> and <code>to</code> are equal
     * to the xLocation of that cell.
     * 
     * @param from -
     *            the starting cell
     * @param to -
     *            the ending cell
     * @return the total width percentage between these two cells (both
     *         included)
     */
    ...
    /**
     * The maximum number of grid positions (both horizontally and vertically)
     * that can be laid out by the grid bag layout.
     */
     ...
     * <code>IEHTMLTableConstraints</code>.
     * 
     * @serial
     ...
     * <code>layoutInfo</code> is <code>null</code> this indicates that
     * there are no components in the gridbag or if there are components, they
     ...
     * is non-<code>null</code> the values are applied to the gridbag after
     * all of the minimum columns widths have been calculated. If columnWidths
     * has more elements than the number of columns, columns are added to the
     ...
     * is non-</code>null</code> the values are applied to the gridbag after
     * all of the minimum row heights have been calculated. If <code>rowHeights</code>
     * has more elements than the number of rows, rowa are added to the gridbag
     * to match the number of elements in <code>rowHeights</code>.
     * 
     * @serial
     * @see #getLayoutDimensions()
     */
     ...
     * non-<code>null</code> the values are applied to the gridbag after all
     * of the columns weights have been calculated. If
     * <code>columnWeights[i]</code> &gt; weight for column i, then column i
     * is assigned the weight in <code>columnWeights[i]</code>. If
     * <code>columnWeights</code> has more elements than the number of
     * columns, the excess elements are ignored - they do not cause more columns
     * to be created.
     * 
     * @serial
     */
    ...
    /**
     * This field holds the overrides to the row weights. If this field is non-</code>null</code>
     * the values are applied to the gridbag after all of the rows weights have
     * been calculated. If <code>rowWeights[i]</code> &gt; weight for row i,
     * then row i is assigned the weight in <code>rowWeights[i]</code>. If
     * <code>rowWeights</code> has more elements than the number of rows, the
     * excess elements are ignored - they do not cause more rows to be created.
     * 
     * @serial
     */
     ...
     * <code>IEHTMLTableConstraints</code> object is returned.
     * 
     * @param comp
     ...
     * is not a copy, but is the actual <code>IEHTMLTableConstraints</code>
     ...
     * new code should call {@link #getLayoutInfo(Container, int) getLayoutInfo}
     * instead.
     */
    protected GridBagLayoutInfo GetLayoutInfo(Container parent, int sizeflag)
    {
        synchronized (parent.getTreeLock()) {
            ...
            for (compindex = 0; compindex < components.length; compindex++) {
                ...
                px = curX +curWidth;
                if(px>r.width)r.width = px;
                //for (py = curY + curHeight; r.height < py; r.height++);
                py = curY + curHeight;
                if(py>r.height)r.height=py;
     ...
     * depending on the constraint geometry and pads.
     * 
     * @param constraints
     *            the constraints to be applied
     * @param r
    ...
    /**
     * This method is obsolete and supplied for backwards compatability only;
     * new code should call {@link #getMinSize(Container, GridBagLayoutInfo)
     * getMinSize} instead.
     */
    protected Dimension GetMinSize(Container parent, GridBagLayoutInfo info)
    {
