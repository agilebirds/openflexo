...
public class IEHyperlinkWidgetView extends AbstractInnerTableWidgetView<IEHyperlinkWidget> implements DoubleClickResponder, LabeledWidget
{
            	...
            	if (getModel().isCustomButton()) {
	                ...
	                if (s == null)
	                    ...
	                    return new Dimension(
	                            (int) (getFontMetrics(getFont()).getStringBounds(s, getGraphics()).getWidth() + 32), 15);
	                }
            	} else
	...
	private static final String SPAN_OPEN = "<html><body><div><FONT FACE=\"Verdana, Arial, Helvetica, sans-serif\" SIZE=3>";//SIZE is not pixel for the FONT tag
    ...
    public Dimension getPreferredSize()
    {
        ...
        Dimension d;
        if (getModel().isCustomButton())
        	d = new Dimension((int) (labelEditing?_jLabelTextField.getPreferredSize().getWidth():_jLabel.getPreferredSize().getWidth())+2,17);
        else 
