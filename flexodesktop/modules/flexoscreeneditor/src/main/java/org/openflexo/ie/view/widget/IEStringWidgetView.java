 ...
 */
public class IEStringWidgetView extends AbstractInnerTableWidgetView<IEStringWidget> implements DoubleClickResponder, DisplayableBindingValue, LabeledWidget
{
    ...
    public IEStringWidgetView(IEController ieController, IEStringWidget model, boolean addDnDSupport, IEWOComponentView view)
    {
        ...
        jLabel.setFont(getModel().getTextCSSClass() == null ? TextCSSClass.BLOC_BODY_CONTENT.font() : getModel()
                .getTextCSSClass().font());
    ...
    private static final String SPAN_OPEN = "<html><FONT FACE=\"Verdana, Arial, Helvetica, sans-serif\" SIZE=2>";//SIZE is not pixel for the FONT tag
    ...
    {
        return !IEPreferences.getDisplayBindingValue() && getModel().getIsHTML() && (getModel().getFieldType()==null || getModel().getFieldType()==TextFieldType.TEXT);
    }
	...
	public void performDoubleClick(JComponent clickedContainer, Point clickedPoint, boolean isShiftDown)
    {
        if (getModel().isDKVField() || getModel().isStatusField())
            return;
        if(IEPreferences.getDisplayBindingValue())return;
        editLabel();
    }
