...
public class JHIndexer {
    ...
    private File writeConfigFile(String prjDir) 
    {
      ...
      try
      {
        ...
        String attribute = "IndexRemove " + prjDir + File.separator +
  lineSeparator;
    ...
    /**
     * create a list of files and directories that are targets for a
     * full text search
     */
        ...
        for(int i=0; i<files.length; i++) {
                  ...
                  fName.endsWith(".html"))
          {
            if( !fName.endsWith("index.htm") &&
                !fName.endsWith("toc.htm") &&
                !fName.endsWith("images") &&
