   ...
   public class HSMapEntry extends FlexoObject implements XMLSerializable
   {
       ...
       public void generate()
       {
           if (docItem != null) {
               ...
               while (inheritanceParent != null) {
                   if (!inheritanceParent.isPublished()) {
                       if (inheritanceParent.getLastApprovedActionForLanguage(_language) != null) {
                           inheritanceText = inheritanceParent.getLastApprovedActionForLanguage(_language).getVersion().getFullHTMLDescription()+"<br>"+inheritanceText;
                       }
                   }
            	   ...
            	   for (Enumeration en=currentDocItem.getEmbeddingChildItems().elements(); en.hasMoreElements();) {
                	   ...
                	   if (next.isIncluded(configuration)) {
                		   if (next.getIsEmbedded() && next.isPublished()) {
                			   ...
                			   if (next.getLastApprovedActionForLanguage(_language) != null) {
                				   embeddedItemsAtThisLevel.append(next.getLastApprovedActionForLanguage(_language).getVersion().getFullHTMLDescription());
                			   }/*
                			   else {
            	   ...
            	   if (hasEmbeddedItemsToInclude) {
            		   if (currentDocItem == docItem) { // We are at this level
            			   if (embeddedItemsAtThisLevel.length()>0) {
	              			   contents.append("<H2>"+FlexoLocalization.localizedForKeyAndLanguage("declared_attributes",lang)+"</H2>\n");
            			   ...
            			   if (embeddedItemsAtThisLevel.length()>0) {
	            			   ...
	            			   contents.append("<H2>"+FlexoLocalization.localizedForKeyAndLanguage("attributes_inherited_from",lang)+" "+inheritedItemsTitle+"</H2>\n");
	             			   contents.append(embeddedItemsAtThisLevel.toString());
            			   }
            		   }
            	   }
               ...
               if (inheritanceParentHTMLFragment != null) contents.append(inheritanceParentHTMLFragment);
               String embeddingParentHTMLFragment = getEmbeddingParentHTMLFragment();
               if (embeddingParentHTMLFragment != null) contents.append(embeddingParentHTMLFragment);
               String inheritanceChildsHTMLFragment = getInheritanceChildsHTMLFragment();
               if (inheritanceChildsHTMLFragment != null) contents.append(inheritanceChildsHTMLFragment);
               String embeddingChildsHTMLFragment = getEmbeddingChildsHTMLFragment();
               if (embeddingChildsHTMLFragment != null) contents.append(embeddingChildsHTMLFragment);
               String relatedToItemsHTMLFragment = getRelatedToItemsHTMLFragment();
               if (relatedToItemsHTMLFragment != null) contents.append(relatedToItemsHTMLFragment);
               ...
               try {
                   ...
                   logger.warning ("Exception raised: "+e.getClass().getName()+" when writing "+fileToGenerate.getAbsolutePath()+". See console for details.");
                   e.printStackTrace();
               }
           }
       }
       ...
       {
           if (firstNonHiddenParent == null) return null;
       ...
       {
           if (docItem.getEmbeddingParentItem() == null) return null;
           StringBuffer returned = new StringBuffer();
           returned.append("<p>");
       ...
       private String getInheritanceChildsHTMLFragment()
       {
           ...
           for (DocItem aDocItem : docItem.getInheritanceChildItems()) {
               if (aDocItem.isIncluded(configuration)) inheritanceChildItemCount++;
           }
           if (inheritanceChildItemCount == 0) return null;
           StringBuffer returned = new StringBuffer();
           returned.append("<p>");
       ...
       private String getEmbeddingChildsHTMLFragment()
       {
           ...
           for (DocItem next : docItem.getEmbeddingChildItems()) {
               if (!next.getIsEmbedded()) embeddedChilds.add(next);
           }
           if (embeddedChilds.size() == 0) return null;
           StringBuffer returned = new StringBuffer();
           returned.append("<p>");
       ...
       {
           if (docItem.getRelatedToItems().size() == 0) return null;
           StringBuffer returned = new StringBuffer();
           returned.append("<p>");
