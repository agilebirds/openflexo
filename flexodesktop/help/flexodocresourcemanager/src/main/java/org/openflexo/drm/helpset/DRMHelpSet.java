...
public class DRMHelpSet extends FlexoObject implements XMLSerializable {
    ...
    public DRMHelpSet (DocResourceCenter drc, File directory, String baseName, HelpSetConfiguration config)
    {
        ...
        presentations = new Vector();
         toc = new View("TOC",FlexoLocalization.localizedForKeyAndLanguage("table_of_content",lang),"javax.help.TOCView",new View.ViewData(getTocFile().getName(),null));
         ...
         search = new View("Search",FlexoLocalization.localizedForKeyAndLanguage("search",lang),"javax.help.SearchView",new View.ViewData("JavaHelpSearch","com.sun.java.help.search.DefaultSearchEngine"));
        views.add(toc);
        views.add(index);
        views.add(search);

        Presentation primaryPresentation = new Presentation("primary",new Presentation.Size(800,600),new Presentation.Location(100,100),title,TOP_LEVEL_FOLDER);
        ...
        Presentation mainPresentation = new Presentation("main",new Presentation.Size(800,600),new Presentation.Location(100,100),title,null);
    ...
    {
        return FlexoLocalization.localizedForKey("help_set_for_distribution")+" "
        + configuration.getDistributionName()+" "
        + FlexoLocalization.localizedForKey("and_language")+" "
    ...
    public void generate(FlexoProgress progress)
    {
        ...
        try {
            FileUtils.copyFileToDir(new FileResource("Resources/FlexoHelpMasterStyle.css"), new File(getHelpSetDirectory(),"HTML/"+_drc.getRootFolder().getIdentifier()+"/"));
        } catch (IOException e) {
            e.printStackTrace();
        }
        if (progress != null) {
        ...
        for (File f : _drc.getRootFolder().getDirectory().listFiles()) {
            if (f.isDirectory()) {
                try {
                    ...
                    if (!f.getCanonicalFile().equals(_drc.getFTSFolder().getDirectory().getCanonicalFile()) && !f.getCanonicalFile().equals(_drc.getModelFolder().getDirectory().getCanonicalFile()))
	...
	private void scanForImages(File[] files, DocItemFolder f, Vector<String> visitedDirectories, int recursiveDepth) throws IOException {
		...
		for (File file : files) {
			if (file.isDirectory() && recursiveDepth>0 && !visitedDirectories.contains(file.getCanonicalPath())) {
				...
				if (file.getName().toLowerCase().endsWith(".jpg")||file.getName().toLowerCase().endsWith(".jpeg") || file.getName().toLowerCase().endsWith(".png") || file.getName().toLowerCase().endsWith(".gif")) {
					try {
						FileUtils.copyFileToFile(file, new File(getHelpSetDirectory(),"HTML/"+_drc.getRootFolder().getIdentifier()+"/"+file.getAbsolutePath().substring(_drc.getRootFolder().getDirectory().getAbsolutePath().length())));
					} catch (IOException e) {
						e.printStackTrace();
					}
				}
