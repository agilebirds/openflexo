...
public class BindingValue extends AbstractBinding
{
	...
	{   	
		...
		if (logger.isLoggable(Level.FINEST)) {
			...
			logger.finest("_bindingDefinition.getType().isAssignableFrom(getAccessedType())="+TypeUtils.isTypeAssignableFrom(getBindingDefinition().getType(),getAccessedType(),true));
		}
	...
	{   	
		...
		logger.info("_bindingDefinition.getType().isAssignableFrom(getAccessedType())="+TypeUtils.isTypeAssignableFrom(getBindingDefinition().getType(),getAccessedType(),true));
		...
		logger.info("Invalid binding because types are not matching searched "+getBindingDefinition().getType()+" having "+getAccessedType());
		return false;
	...
	{
		if (getOwner() != null) return getOwner().getBindingFactory().getBindingValueFactory();
		return null;
	...
	public class BindingPath extends Vector<BindingPathElement>
	{
		...
		private void setResultingTypeAtIndex (int index)
		{
			if (_bindingVariable != null) {
				...
				if (type == null) {
					logger.warning("Cannot find resulting type for bindingPathElement "+bpe.getSerializationRepresentation()+ " in path :"+getStringRepresentation());
					return;
				}
	...
	public BindingPathElement findFirstBindingPathElementOfType(Class aClass)
	{
		...
		while(en.hasMoreElements()){
			...
			if(TypeUtils.getBaseClass(reply.getType()).equals(aClass))return reply;
		}
		return null;
	}
	...
	public Object getBindingValue(BindingEvaluationContext context)
	{
		...
		try {
			...
			logger.warning("InvalidObjectSpecificationException raised while evaluating GET "+getStringRepresentation()+" : "+e.getMessage());
			return null;
		}
	...
	{
		if (!isBindingValid()) return EMPTY_LIST;
		if (!isSettable()) return EMPTY_LIST;
	...
	public List<TargetObject> getTargetObjects(BindingEvaluationContext context)
	{
		...
		if (current == null) return returned;
	...
	public void setBindingValue(Object value, BindingEvaluationContext context)
	{
		...
		try {
			...
			logger.warning("InvalidObjectSpecificationException raised while evaluating SET "+getStringRepresentation()+" : "+e.getMessage());
			//System.out.println("returned="+returned);
