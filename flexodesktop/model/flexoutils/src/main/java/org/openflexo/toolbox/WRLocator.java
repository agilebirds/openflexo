...
public class WRLocator
{
    ...
    private static File styledImageDir(String css)
    {
        ...
        if(!reply.exists())reply = new File(imageDir,"Contento");
        return reply;
    }

    /**
     * Returns the specificButtons directory. If it does not exist, it is then
     * created.
     * 
     * @return
     */
...
//    }
    ...
    public static File[] listSmallButtons(String css)
    {
        ...
        FileFilter styledIconFilter = new FileFilter() {
			...
			public boolean accept(File aFile)
            {
                return aFile.getName().indexOf("_Icon_") > 0 
                    || aFile.getName().indexOf("_BigIcon_") > 0
                    || aFile.getName().indexOf("_ProgressIcon_") > 0;
            }
        };
