...
public class MergeChange
{
	...
	public static final ChangeCategory[] CONFLICT_RESOLVED_CATEGORIES = new ChangeCategory[]{ChangeCategory.SMART_CONFLICT_RESOLVED,ChangeCategory.CUSTOM_EDITING_RESOLVED,ChangeCategory.ADD_CONFLICT_RESOLVED,ChangeCategory.CONFLICT_RESOLVED,ChangeCategory.DEL_CONFLICT_RESOLVED};
	public static final ChangeCategory[] CONFLICT_UNRESOLVED_CATEGORIES = new ChangeCategory[]{ChangeCategory.SMART_CONFLICT_UNRESOLVED,ChangeCategory.CUSTOM_EDITING_UNRESOLVED,ChangeCategory.ADD_CONFLICT_UNRESOLVED,ChangeCategory.CONFLICT_UNRESOLVED,ChangeCategory.DEL_CONFLICT_UNRESOLVED};
	public static final ChangeCategory[] LEFT_CATEGORIES = new ChangeCategory[]{ChangeCategory.LEFT_ADDITION,ChangeCategory.LEFT_MODIFICATION,ChangeCategory.LEFT_REMOVAL};
	public static final ChangeCategory[] RIGHT_CATEGORIES = new ChangeCategory[]{ChangeCategory.RIGHT_ADDITION,ChangeCategory.RIGHT_MODIFICATION,ChangeCategory.RIGHT_REMOVAL};
	...
	protected static MergeChange makeRightMergeChange(DiffChange aChange, int leftToOriginal) 
	{
		...
		if (aChange instanceof ModificationChange) returned.mergeChangeType = MergeChangeType.Modification;
		if (aChange instanceof AdditionChange) returned.mergeChangeType = MergeChangeType.Addition;
		if (aChange instanceof RemovalChange) returned.mergeChangeType = MergeChangeType.Removal;
	...
	protected static MergeChange makeLeftMergeChange(DiffChange aChange, int originalToRight) 
	{
		...
		if (aChange instanceof ModificationChange) returned.mergeChangeType = MergeChangeType.Modification;
		if (aChange instanceof AdditionChange) returned.mergeChangeType = MergeChangeType.Removal;
		if (aChange instanceof RemovalChange) returned.mergeChangeType = MergeChangeType.Addition;
	...
	protected static MergeChange makeLeftMergeChange(Vector<DiffChange> changes, int originalToRight, Vector<DiffChange> oppositeChanges, int leftToOriginal, MergeChangeAction defaultAction)
	{
		...
		if ((oppositeMerge.getFirst1() >= returned.getFirst1())
				&& (oppositeMerge.getLast1() <= returned.getLast1())) {
			int d = (oppositeMerge.getLast2()-oppositeMerge.getFirst2()-oppositeMerge.getLast1()+oppositeMerge.getFirst1());
			if (Merge.debug) System.out.println("Including case from left d="+d);
	...
	protected static MergeChange makeRightMergeChange(Vector<DiffChange> changes, int leftToOriginal, Vector<DiffChange> oppositeChanges, int originalToRight, MergeChangeAction defaultAction)
	{
		Vector<MergeChange> changesToMerge = new Vector<MergeChange>();
		for (DiffChange diffChange : changes) {
			MergeChange change = makeRightMergeChange(diffChange, leftToOriginal);
		...
		if ((oppositeMerge.getFirst1() >= returned.getFirst1())
			...
			if (Merge.debug) System.out.println("Including case from right d="+d);
	...
	{
		return getMergeChangeTypeAsString()+" "+getMergeChangeSourceAsString()+" "+first0+","+last0+"/"+first1+","+last1+"/"+first2+","+last2;
	}
	...
	{
		if (Merge.debug) return toDebugString();
		return toNiceString();
	}
	...
	public MergeChangeResult getMergeChangeResult()
	{
		if (mergeChangeSource == MergeChangeSource.Left) {
			...
			if (mergeChangeAction == MergeChangeAction.ChooseLeft) {
				...
				if (customHandEdition != null) return customHandEdition;
				return new MergeChangeResult("",0);
			}
			else if (mergeChangeAction == MergeChangeAction.AutomaticMergeResolving) {
				if (getAutomaticResolvedMerge() != null) return getAutomaticResolvedMerge();
	...
	{
		if (!triedToAutomaticallyResolve 
				&& getMerge().getDocumentType().getAutomaticMergeResolvingModel() != null) {
	...
	{
		if ((mergeChangeSource == MergeChangeSource.Left)
				|| (mergeChangeSource == MergeChangeSource.Right)) return true;
		// else conflict
		if (mergeChangeAction == MergeChangeAction.Undecided) return false;
	...
	public ChangeCategory category()
	{
		if (getMergeChangeSource() == MergeChange.MergeChangeSource.Left) {
			...
			if (getMergeChangeAction() == MergeChangeAction.AutomaticMergeResolving) {
				if (isResolved()) return ChangeCategory.SMART_CONFLICT_RESOLVED;
				else return ChangeCategory.SMART_CONFLICT_UNRESOLVED;
			}
			else if (getMergeChangeAction() == MergeChangeAction.CustomEditing) {
				if (isResolved()) return ChangeCategory.CUSTOM_EDITING_RESOLVED;
				else return ChangeCategory.CUSTOM_EDITING_UNRESOLVED;
			}
			if (getMergeChangeType() == MergeChange.MergeChangeType.Addition) {
				if (isResolved()) return ChangeCategory.ADD_CONFLICT_RESOLVED;
				else return ChangeCategory.ADD_CONFLICT_UNRESOLVED;
			}
			else if (getMergeChangeType() == MergeChange.MergeChangeType.Modification) {
				if (isResolved()) return ChangeCategory.CONFLICT_RESOLVED;
				else return ChangeCategory.CONFLICT_UNRESOLVED;
			}
			else if (getMergeChangeType() == MergeChange.MergeChangeType.Removal) {
				if (isResolved()) return ChangeCategory.DEL_CONFLICT_RESOLVED;
				else return ChangeCategory.DEL_CONFLICT_UNRESOLVED;
			}
		}
