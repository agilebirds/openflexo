...
/**
 * This class is used to encode all log records of a session (current or
 * expired) of Flexo. An instance of LogRecords can be represented in a
 * FlexoLoggingViewerWindow.
 * 
 * @author sguerin
 */
public class LogRecords extends KVCObject implements XMLSerializable, TableModel
{
    ...
    public void add(LogRecord record)
    {
    	synchronized (records) {
				if (FlexoLoggingManager.getLogCount() > -1 && 
						records.size() > FlexoLoggingManager.getLogCount())
					records.remove(0);
				records.add(record);
    	}
