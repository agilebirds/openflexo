   ...
   public IntegerStaticBinding(BindingDefinition bindingDefinition, Bindable owner, long aValue)
	...
	protected boolean _areTypesMatching()
	{
		...
		if (getBindingDefinition().getType().equals(Double.class)
				...
				|| getBindingDefinition().getType().equals(Float.TYPE)
				|| getBindingDefinition().getType().equals(Long.class)
				|| getBindingDefinition().getType().equals(Long.TYPE)) return true;


		if (getBindingDefinition().getType().equals(Integer.class)
				|| getBindingDefinition().getType().equals(Integer.TYPE)) 
			return (getValue() >= Integer.MIN_VALUE && getValue() <= Integer.MAX_VALUE);

		if (getBindingDefinition().getType().equals(Short.class)
				|| getBindingDefinition().getType().equals(Short.TYPE)) 
			return (getValue() >= Short.MIN_VALUE && getValue() <= Short.MAX_VALUE);

		if (getBindingDefinition().getType().equals(Byte.class)
				|| getBindingDefinition().getType().equals(Byte.TYPE)) 
			return (getValue() >= Byte.MIN_VALUE && getValue() <= Byte.MAX_VALUE);

		if (getBindingDefinition().getType().equals(Character.class)
				|| getBindingDefinition().getType().equals(Character.TYPE)) 
