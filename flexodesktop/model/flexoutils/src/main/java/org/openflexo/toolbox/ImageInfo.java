...
/**
 * Get file format, image resolution, number of bits per pixel and optionally number of images, comments and physical resolution from JPEG,
 * GIF, BMP, PCX, PNG, IFF, RAS, PBM, PGM, PPM and PSD files (or input streams).
 * <p>
 ...
 * Known limitations:
 * <ul>
 * <li>When the determination of the number of images is turned off, GIF bits per pixel are only read from the global header. For some GIFs,
 * local palettes change this to a typically larger value. To be certain to get the correct color depth, call setDetermineImageNumber(true)
 * before calling check(). The complete scan over the GIF file will take additional time.</li>
 * <li>Transparency information is not included in the bits per pixel count. Actually, it was my decision not to include those bits, so it's
 * a feature! ;-)</li>
 * </ul>
 * <p>
 * Requirements:
 ...
 * <li><strong>2002-01-24</strong> Added support for file formats Portable Bitmap / Graymap / Pixmap (PBM, PGM, PPM) and Adobe Photoshop
 * (PSD). Added new method getMimeType() to return the MIME type associated with a particular file format.</li>
 * <li><strong>2002-03-15</strong> Added support to recognize number of images in file. Only works with GIF. Use
 * {@link #setDetermineImageNumber} with <code>true</code> as argument to identify animated GIFs ({@link #getNumberOfImages()} will return a
 * value larger than <code>1</code>).</li>
 ...
 * Thanks to Marcelo P. Lima for sending in the bug report. Released as 1.1.1.</li>
 * <li><strong>2002-04-18</strong> Added {@link #setCollectComments(boolean)}. That new method lets the user specify whether textual
 * comments are to be stored in an internal list when encountered in an input image file / stream. Added two methods to return the physical
 * width and height of the image in dpi: {@link #getPhysicalWidthDpi()} and {@link #getPhysicalHeightDpi()}. If the physical resolution
 * could not be retrieved, these methods return <code>-1</code>.</li>
 ...
 * <li><strong>2002-06-17</strong> Added support for SWF, sent in by Michael Aird. Changed checkJpeg() so that other APP markers than APP0
 * will not lead to a failure anymore. Released as 1.3.</li>
 * <li><strong>2003-07-28</strong> Bug fix - skip method now takes return values into consideration. Less bytes than necessary may have been
 * skipped, leading to flaws in the retrieved information in some cases. Thanks to Bernard Bernstein for pointing that out. Released as 1.4.
 * </li>
 * <li><strong>2004-02-29</strong> Added support for recognizing progressive JPEG and interlaced PNG and GIF. A new method
 * {@link #isProgressive()} returns whether ImageInfo has found that the storage type is progressive (or interlaced). Thanks to Joe Germuska
 * for suggesting the feature. Bug fix: BMP physical resolution is now correctly determined. Released as 1.5.</li>
 * <li><strong>2004-11-30</strong> Bug fix: recognizing progressive GIFs (interlaced in GIF terminology) did not work (thanks to Franz
 * Jeitler for pointing this out). Now it should work, but only if the number of images is determined. This is because information on
 * interlacing is stored in a local image header. In theory, different images could be stored interlaced and non-interlaced in one file.
 * However, I think that's unlikely. Right now, the last image in the GIF file that is examined by ImageInfo is used for the "progressive"
 * status.</li>
 * <li><strong>2005-01-02</strong> Some code clean up (unused methods and variables commented out, missing javadoc comments, etc.). Thanks
 * to George Sexton for a long list. Removed usage of Boolean.toString because it's a Java 1.4+ feature (thanks to Gregor Dupont). Changed
 * delimiter character in compact output from semicolon to tabulator (for better integration with cut(1) and other Unix tools). Added some
 * points to the <a href="http://schmidt.devlib.org/image-info/index.html#knownissues">'Known issues' section of the website</a>. Released
 * as 1.6.</li>
 * <li><strong>2005-07-26</strong> Removed code to identify Flash (SWF) files. Has repeatedly led to problems and support requests, and I
 * don't know the format and don't have the time and interest to fix it myself. I repeatedly included fixes by others which didn't work for
 * some people. I give up on SWF. Please do not contact me about it anymore. Set package of ImageInfo class to org.devlib.schmidt.imageinfo
 * (a package was repeatedly requested by some users). Released as 1.7.</li>
 * <li><strong>2006-02-23</strong> Removed Flash helper methods which weren't used elsewhere. Updated skip method which tries "read"
 * whenever "skip(Bytes)" returns a result of 0. The old method didn't work with certain input stream types on truncated data streams.
 * Thanks to Martin Leidig for reporting this and sending in code. Released as 1.8.</li>
 * </li>
 * <li><strong>2006-11-13</strong> Removed check that made ImageInfo report JPEG APPx markers smaller than 14 bytes as files in unknown
 * format. Such JPEGs seem to be generated by Google's Picasa application. First reported with fix by Karl von Randow. Released as 1.9.</li>
 * </ul>
 * 
...
public class ImageInfo {
	/**
	 * Return value of {@link #getFormat()} for JPEG streams. ImageInfo can extract physical resolution and comments from JPEGs (only from
	 * APP0 headers). Only one image can be stored in a file. It is determined whether the JPEG stream is progressive (see
	 * {@link #isProgressive()}).
	 */
	public static final int FORMAT_JPEG = 0;

	/**
	 * Return value of {@link #getFormat()} for GIF streams. ImageInfo can extract comments from GIFs and count the number of images (GIFs
	 * with more than one image are animations). It is determined whether the GIF stream is interlaced (see {@link #isProgressive()}).
	 */
	public static final int FORMAT_GIF = 1;

	/**
	 * Return value of {@link #getFormat()} for PNG streams. PNG only supports one image per file. Both physical resolution and comments can
	 * be stored with PNG, but ImageInfo is currently not able to extract those. It is determined whether the PNG stream is interlaced (see
	 * {@link #isProgressive()}).
	 */
	public static final int FORMAT_PNG = 2;

	/**
	 * Return value of {@link #getFormat()} for BMP streams. BMP only supports one image per file. BMP does not allow for comments. The
	 * physical resolution can be stored.
	 */
	public static final int FORMAT_BMP = 3;

	/**
	 * Return value of {@link #getFormat()} for PCX streams. PCX does not allow for comments or more than one image per file. However, the
	 * physical resolution can be stored.
	 */
	...
	/**
	 * Call this method after you have provided an input stream or file using {@link #setInput(InputStream)} or {@link #setInput(DataInput)}
	 * . If true is returned, the file format was known and information on the file's content can be retrieved using the various getXyz
	 * methods.
	 * 
	...
	/**
	 * If {@link #check()} was successful, returns the image's number of bits per pixel. Does not include transparency information like the
	 * alpha channel.
	 * 
	...
	/**
	 * If {@link #check()} was successful and {@link #setCollectComments(boolean)} was called with <code>true</code> as argument, returns
	 * the number of comments retrieved from the input image stream / file. Any number &gt;= 0 and smaller than this number of comments is
	 * then a valid argument for the {@link #getComment(int)} method.
	 * 
	...
	/**
	 * Returns the number of images in the examined file. Assumes that <code>setDetermineImageNumber(true);</code> was called before a
	 * successful call to {@link #check()}. This value can currently be only different from <code>1</code> for GIF images.
	 * 
	...
	/**
	 * To use this class as a command line application, give it either some file names as parameters (information on them will be printed to
	 * standard output, one line per file) or call it with no parameters. It will then check data given to it via standard input.
	 * 
	 * @param args
	...
	private static void printCompact(String sourceName, ImageInfo imageInfo) {
		final String SEP = "\t";
		System.out.println(sourceName + SEP + imageInfo.getFormatName() + SEP + imageInfo.getMimeType() + SEP + imageInfo.getWidth() + SEP
				+ imageInfo.getHeight() + SEP + imageInfo.getBitsPerPixel() + SEP + imageInfo.getNumberOfImages() + SEP
				+ imageInfo.getPhysicalWidthDpi() + SEP + imageInfo.getPhysicalHeightDpi() + SEP + imageInfo.getPhysicalWidthInch() + SEP
	...
	/**
	 * Specify whether textual comments are supposed to be extracted from input. Default is <code>false</code>. If enabled, comments will be
	 * added to an internal list.
	 * 
	 * @param newValue
	...
	/**
	 * Specify whether the number of images in a file is to be determined - default is <code>false</code>. This is a special option because
	 * some file formats require running over the entire file to find out the number of images, a rather time-consuming task. Not all file
	 * formats support more than one image. If this method is called with <code>true</code> as argument, the actual number of images can be
	 * queried via {@link #getNumberOfImages()} after a successful call to {@link #check()}.
	 * 
	 * @param newValue
