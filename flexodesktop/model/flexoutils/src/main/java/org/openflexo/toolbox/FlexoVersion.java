...
public class FlexoVersion implements StringConvertable
{
   public static final StringEncoder.Converter<FlexoVersion> converter = new Converter<FlexoVersion>(FlexoVersion.class) {
        	...
        	try {
        		minor = (new Integer(next)).intValue();
               	...
               	try {
              		...
              		minor = 0;
            		(new NumberFormatException("Invalid minor number: " + next + " is not valid (" + versionAsString + ")")).printStackTrace();
            	}
       	}
        }
        if (st.hasMoreTokens()) {
    ...
    private int parseAlphaBetaRC(String versionAsString, String token) throws NumberFormatException
    {
        ...
        if (token.toLowerCase().indexOf("rc") > -1) {
            ...
            try {
                ...
                rc = -1;
            	throw new NumberFormatException("Invalid RC number: " + token.substring(token.toLowerCase().indexOf("rc") + 2) + " is not valid ("
                            + versionAsString + ")");
             }
        } else {
    ...
    /**
     * Version have the following format MAJOR.MINOR.PATCH(RCX|alpha|beta)
     * members in parenthesis are optional and are called additional members
     * Priority of members is this (bigger the greater, of course):
     * 1) MAJOR
     * 2) MINOR
     * 3) PATCH
     * 4) NO additional member
     * 5) beta
     * 6) alpha
     * 7) RC (RC2 is bigger than RC1 of course)
     *
     * @author gpolet
     *
     */
    ...
    {
    	String[] s = { "0.9.0", "0.9.1", "0.9.0RC1", "0.9.0RC2","0.9.0alpha","0.9.0beta", "0.10.0", "0.10.1", "0.10.0RC1", "0.10.0RC2","0.10.0alpha","0.10.0beta","0.9","1.0alpha","1.1alpha","1.1RC8" };
