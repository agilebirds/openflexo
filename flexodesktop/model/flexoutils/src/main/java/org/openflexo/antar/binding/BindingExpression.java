...
public class BindingExpression extends AbstractBinding {
    ...
    {
		if (getOwner() != null) return getOwner().getBindingFactory().getBindingExpressionFactory();
		return null;
	...
	public static class BindingValueVariable extends Expression
	{
		...
		public BindingValueVariable(String variableName, Bindable bindable)
		{
			this(new Variable(variableName),
					bindable,
					(bindable!=null?new BindingDefinition("object",Object.class,BindingDefinitionType.GET,true):null));
		}
	...
	{
		try {
			if (getEvaluationType() == EvaluationType.LITERAL) return Object.class;
			if (getEvaluationType() == EvaluationType.ARITHMETIC_INTEGER) return Long.class;
			if (getEvaluationType() == EvaluationType.ARITHMETIC_FLOAT) return Float.class;
			if (getEvaluationType() == EvaluationType.BOOLEAN) return Boolean.class;
			if (getEvaluationType() == EvaluationType.STRING) return String.class;
	...
	public boolean debugIsBindingValid()
    {
    	...
    	logger.info("Binding "+this+" not valid because types are not matching: searched: "+getBindingDefinition().getType()+" have: "+getAccessedType());
   		return false;
	...
	public Object getBindingValue(final BindingEvaluationContext context)
    {
		...
		if (evaluatedExpression instanceof Constant) return ((Constant)evaluatedExpression).getValue();
		...
		= new EvaluationContext(
				...
				new VariableFactory() {				
					...
					public Expression makeVariable(Word value) {
						...
						if (list != null) returned.addAll(list);
						return constantFactory.makeConstant(Value.createConstantValue(evaluatedVariable));
					}
				},
				new FunctionFactory() {
