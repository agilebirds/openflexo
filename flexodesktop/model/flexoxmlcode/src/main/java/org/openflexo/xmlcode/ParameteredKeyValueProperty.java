...
public class ParameteredKeyValueProperty extends SingleKeyValueProperty {
	 ...
	 *                if an error occurs
	 */
	public ParameteredKeyValueProperty(Class anObjectClass, String propertyName, boolean setMethodIsMandatory)
			...
			for (Method m : getMethods) {
				try {
					...
					for (int i = 0; i < arguments.size(); i++) {
						parameters[i] = _translateValueForAssignability(m.getParameterTypes()[i],
								arguments.get(i).getObjectValue(initialObject));
						// debug = debug + (i>0?",":"") +
						// parameters[i]+"/"+(parameters[i]!=null?parameters[i].getClass().getSimpleName():"null");
					}
		...
		if (object == null) {
			...
			for (Method m : setMethods) {
				try {
					...
					// String debug = ""+aValue;
					for (int i = 0; i < arguments.size(); i++) {
						parameters[i + 1] = _translateValueForAssignability(m.getParameterTypes()[i + 1],
								arguments.get(i).getObjectValue(initialObject));
						// debug = debug + "," +
						// parameters[i+1]+"/"+(parameters[i+1]!=null?parameters[i+1].getClass().getSimpleName():"null");
					}
					// System.out.println("Trying to invoke on object "+currentObject+" method "+m+" with "+debug);
			...
			for (int i = 1; i < getMethods.size(); i++) {
				if (!type.equals(getMethods.get(i).getReturnType())) {
					throw new InvalidKeyValuePropertyException(
							"InvalidKeyValuePropertyException: Ambigous methods found with different return types for property " + name);
				}
			}
		}
	...
	private Vector<KeyValueProperty> parsePathElementArguments(String pathElement) {
		...
		while (t.hasMoreTokens()) {
			...
			if (isParsableAsADouble) {
				...
				newProperty = new ConstantKeyValueProperty.StringConstantKeyValueProperty(getObjectClass(), next.substring(1,
						next.length() - 1));
			} else if (next.startsWith("'") && next.endsWith("'")) {
				newProperty = new ConstantKeyValueProperty.StringConstantKeyValueProperty(getObjectClass(), next.substring(1,
						next.length() - 1));
