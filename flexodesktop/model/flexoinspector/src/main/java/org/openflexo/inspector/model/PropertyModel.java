...
public class PropertyModel extends ParametersContainerModelObject implements InnerTabWidget/*,PropertyParameter<Object>*/
{
   ...
   public boolean hasStaticList()
            ...
            try {
                ...
                if (currentObject instanceof Vector) {
                    ...
                    if (logger.isLoggable(Level.WARNING))
                        logger.warning("Succeeded access to " + listAccessor + " but answer is not a Vector but a :" + currentObject.getClass().getName()+" value="+currentObject);
                    return new Vector();
                }
            } catch (Exception e) {
                if (logger.isLoggable(Level.WARNING))
                    logger.warning("getDynamicList() failed for property " + name + " for object " + object + " : exception " + e.getMessage());
                e.printStackTrace();
                return new Vector();
            }
        } else {
    ...
    public static Object getObjectForMultipleAccessors(KeyValueCoding object, String listAccessor)
    {
    	...
    	if (listAccessor != null && listAccessor.equals("this")) return object;
        ...
        if (hasFormatter()) {
            try {
                ...
                if (currentObject instanceof String) {
                    if (logger.isLoggable(Level.FINE))
                        logger.fine("Format object " + object + " with format " + getValueForParameter("format") + " returns "+currentObject);
                    return (String) currentObject;
                } else if (currentObject == null) {
                    return "";
                } else {
                    if (logger.isLoggable(Level.WARNING))
                        logger.warning("Property " + name + ": succeeded acces to " + listAccessor + " but answer is not a String "+currentObject+" of "+currentObject.getClass().getName());
                    return null;
                }
            } catch (Exception e) {
                if (logger.isLoggable(Level.WARNING))
    ...
    public boolean isEditable(Object object) {
    	if (hasValueForParameter("isEditable")&&(object instanceof KeyValueCoding)) {
    		...
    		if (logger.isLoggable(Level.WARNING))
				logger.warning("There is an error in model: parameter 'isEditable' seems to be defined but not on a boolean: "+getValueForParameter("isEditable"));
    	}
    	return true;
    }
    ...
    public static KeyValueCoding getTargetObject(KeyValueCoding object, String keyPath)
    {
        ...
        String accessor;
        Object currentObject = object;
        while (strTok.hasMoreTokens() && (currentObject != null)
                && (currentObject instanceof KeyValueCoding)) {
            accessor = strTok.nextToken();
        ...
        if (currentObject instanceof KeyValueCoding) {
            ...
            if (logger.isLoggable(Level.WARNING))
                logger.warning("Could not find target object for object=" + object + " keyPath="
                        + keyPath + ": must be a KeyValueCoding object (getting " + currentObject
                        + ")");
            return null;
        } else {
            return null;
    ...
    public synchronized boolean hasObjectValue(KeyValueCoding inspectable) throws AccessorInvocationException
    {
    	...
    	try {
    		...
    		e.printStackTrace();
    		if (logger.isLoggable(Level.WARNING))
    			logger.warning("getObjectValue() failed for property " + name
        ...
        try {
        	...
        	if (logger.isLoggable(Level.WARNING))
        		logger.warning("setObjectValue() with " + newValue + " failed for property "
        				+ name + " for object " + inspectable.getClass().getName()
