    		    ...
    		    if (getPropertyModel().hasValueForParameter("icon")) {
    	            try {
    	                ...
    	                if (logger.isLoggable(Level.FINE))
    	                    logger.fine("Get icon file for object" + object + " with icon accessor " + getPropertyModel().getValueForParameter("icon"));
    	                ...
    	                if (currentObject instanceof File) {
    	                    if (logger.isLoggable(Level.FINE))
    	                        logger.fine("Get icon file for object" + object + " with icon accessor " + getPropertyModel().getValueForParameter("icon") + " returns "+currentObject);
    	                    return ((File)currentObject);
    	                } else {
    	                    if (logger.isLoggable(Level.WARNING))
    	                        logger.warning("Property " + getPropertyModel().name + ": succeeded acces to " + listAccessor + " but answer is not a FILE");
    	                    return null;
    	                }
    	            } catch (Exception e) {
    	                if (logger.isLoggable(Level.WARNING))
    	                    logger.warning("getDynamicList() failed for property " + getPropertyModel().name + " for object " + object + " : exception " + e.getMessage());
    	                return null;
    	            }
    	        } else {
    	            return null;
        ...
        if (logger.isLoggable(Level.WARNING))
            logger
                    .warning("There is an error in some configuration file :\n the property named '"
                            + _propertyModel.name
                            + "' has no FILE representation formatter ! Object is a "
