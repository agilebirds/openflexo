...
public class WKFDelete extends FlexoUndoableAction<WKFDelete,WKFObject,WKFObject>
{
    ...
    public static FlexoActionType<WKFDelete,WKFObject,WKFObject> actionType 
    = new FlexoActionType<WKFDelete,WKFObject,WKFObject>(
     ...
     * initialiazer or finalizer of the action. Do not forget to consult the
     * _deletionContext, because objectsToDelete returns the objects that will
     * be deleted, not the ones that have been deleted.
     * 
    ...
    public Vector getObjectsToDelete()
    {
        if (_objectsToDelete == null) {
            ...
            /**
             * The purpose of the loop below is too find all objects on which
             * the delete must be called, so that all the selected objects are
             * removed from the model. The trick is to verify that an object
             * will not already be deleted because another one has been
             * previously removed (causing a double call to delete on that
             * object, which will most likely fail).
             */
