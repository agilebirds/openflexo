...
public abstract class FlexoStorageResource<SRD extends StorageResourceData> extends FlexoFileResource<SRD> {
	...
	public Date getLastKnownMemoryUpdate() {
		...
		if (isLoaded() && lastKnownMemoryUpdate==null) // We have no idea of what is the last memory update, so we default on the disk last modified which is never null
   ...
   /**
    ...
    /**
     * Returns a flag indicating if data related to this resource has been
     * modified. Returns false if data is not loaded
     * 
     * @return
     */
    ...
    protected abstract SRD performLoadResourceData(FlexoProgress progress, ProjectLoadingHandler loadingHandler) throws LoadResourceException, FileNotFoundException, ProjectLoadingCancelledException;
	...
	protected boolean requireUpdateBecauseOf(FlexoResource resource)
    {
        if (super.requireUpdateBecauseOf(resource)) {
             // OK, at first sight 'resource' of which 'this' depends is newer than 'this' so it seems that we need to update 'this'. BUT, maybe 
    ...
    public final void backwardSynchronizeWith(FlexoResourceTree updatedResources) throws ResourceDependancyLoopException, FlexoException, FileNotFoundException
    {
        if (!updatedResources.isEmpty()) {
            if (logger.isLoggable(Level.FINE))
                logger.fine("Resource " + this + " requires backward synchronization for " + updatedResources.getChildNodes().size() + " resources :");
    ...
    @Override
	protected void performUpdating(FlexoResourceTree updatedResources) throws ResourceDependancyLoopException ,FlexoException, LoadResourceException, FileNotFoundException, ProjectLoadingCancelledException
    {
        if (!isLoaded()) {
            try {
            	_resourceData = performLoadResourceData();
