...
public class OEShemaDefinition extends OESLObject implements Sortable {

	protected static final Logger logger = FlexoLogger.getLogger(OEShemaDefinition.class.getPackage().getName());

	private OEShemaFolder _folder;
	private String _shemaName;
	private int index  = -1;
    private OntologyCalc _calc;
    private CalcLibrary _calcLibrary;

    public OEShemaDefinition(OEShemaLibraryBuilder builder) throws DuplicateResourceException
    {
        this(null, builder.shemaLibrary, null, builder.getProject(),false);
        initializeDeserialization(builder);
        _calcLibrary = builder.calcLibrary;
    }

    public OEShemaDefinition(String aShemaName, OEShemaLibrary shemaLibrary, OEShemaFolder aFolder,
    ...
    private void setShemaName(String name) throws DuplicateResourceException, InvalidNameException
    {
        if ((_shemaName != null) && (!_shemaName.equals(name)) && (name != null) && !isDeserializing()) {
        	...
        	if(ReservedKeyword.contains(name))
        		throw new InvalidNameException();
            if (getProject() != null) {
            	OEShemaDefinition cd = getShemaLibrary().getShemaNamed(name);
                if (cd != null && cd!=this)
    ...
    public OEShema createNewShema() 
    {
    	return new OEShema(this,getProject());
    }
    ...
    public void setFolder(OEShemaFolder aFolder)
    {
        _folder = aFolder;
        if (!isDeserializing())
			if (_folder == null)
    ...
    public static class ShemaComparator implements Comparator<OEShemaDefinition>
    {
         ...
         */
        @Override
		public int compare(OEShemaDefinition o1, OEShemaDefinition o2)
        {
     ...
     */
    @Override
	public OEShemaDefinition[] getCollection()
    {
        if (getFolder()==null)
            return null;
        return getFolder().getShemas().toArray(new OEShemaDefinition[0]);
    }
    ...
    {
    	return Inspectors.OE.OE_SHEMA_DEFINITION_INSPECTOR;
    }
