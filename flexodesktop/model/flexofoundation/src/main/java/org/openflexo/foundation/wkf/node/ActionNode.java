...
//        }
    ...
    public long getDisplayOperationFlexoID()
    {
        if (getDisplayOperation() != null)
        	...
        	if(isSerializing())return displayOperationFlexoID;
            return -1;
    }
    ...
    public long getDisplayProcessFlexoID()
    {
        if (getDisplayProcess() != null)
        	...
        	if(isSerializing())return displayProcessFlexoID;
            return -1;
    }
        ...
        public ValidationIssue<ActionTypeMustMatchButtonType,ActionNode> applyValidation(ActionNode node)
        {
            ...
            if (node.getAssociatedButtonWidget() != null) {
                if (node.getAssociatedButtonWidget().getIsDisplayAction()
                        && node.getActionType() != ActionType.DISPLAY_ACTION) {
                    err = new ValidationError<ActionTypeMustMatchButtonType,ActionNode>(this, node, "action_type_must_match_button_action_type");
    ...
    public static class OnlyOneActionNodeMustBeBoundToOperationComponentAction extends ValidationRule<OnlyOneActionNodeMustBeBoundToOperationComponentAction,ActionNode>
    {
        ...
        public ValidationIssue<DisplayActionMustHaveADisplayProcess,ActionNode> applyValidation(ActionNode node)
        {
            ...
            if (node._actionType == ActionType.DISPLAY_ACTION) {
                if (node.getDisplayProcess() == null && node.hasIncomingEdges())
                    err = new ValidationError<DisplayActionMustHaveADisplayProcess,ActionNode>(this, node, "display_action_($object.name)_must_have_a_display_process");
            }
            return err;
    ...
    public static class DisplayActionShouldHaveADisplayOperation extends ValidationRule<DisplayActionShouldHaveADisplayOperation,ActionNode>
    {
        ...
        public ValidationIssue<DisplayActionShouldHaveADisplayOperation,ActionNode> applyValidation(ActionNode node)
        {
            ...
            if (node._actionType == ActionType.DISPLAY_ACTION) {
                if (node.getDisplayProcess() != null && node.getDisplayOperation() == null && node.hasIncomingEdges())
                    w = new ValidationError<DisplayActionShouldHaveADisplayOperation,ActionNode>(this, node, "display_action_($object.name)_should_have_a_display_operation");
            }
            return w;
    ...
    public static class DisplayActionShouldHaveASelectedTab extends ValidationRule<DisplayActionShouldHaveASelectedTab,ActionNode>
    {
        ...
        public ValidationIssue<DisplayActionShouldHaveASelectedTab,ActionNode> applyValidation(ActionNode node)
        {
            ...
            if (node._actionType == ActionType.DISPLAY_ACTION) {
                if (node.getNextOperationTabKey()==null && node.getDisplayProcess() != null && node.getDisplayOperation() != null && node.hasIncomingEdges())
                {
                		...
                		if(nextOpNode != null)
                		{
                        	if(nextOpNode.getOperationComponent()!=null){
                        		...
                        		if(component.hasTabContainer() && component.hasAtLeastOneTabDefined()){
                                	w = new ValidationError<DisplayActionShouldHaveASelectedTab,ActionNode>(this, node, "display_action_($object.name)_should_have_a_selected_tab");
                        		}
                        	}
    ...
    public static class ActionNodeCanBeBoundToFlexoActionOrDisplayActionButton extends ValidationRule<ActionNodeCanBeBoundToFlexoActionOrDisplayActionButton, ActionNode> {

		public ActionNodeCanBeBoundToFlexoActionOrDisplayActionButton() {
			super(ActionNode.class, "action_node_($object.name)_can_only_be_bound_to_buttons_with_hyperlink_type_flexo_action_or_display_action");
		}
		...
		public ValidationIssue<ActionNodeCanBeBoundToFlexoActionOrDisplayActionButton, ActionNode> applyValidation(ActionNode object) {
			if (object.getAssociatedButtonWidget()!=null) {
				...
				if (type==null||(!type.isFlexoAction()&&!type.isDisplayAction())) {
					...
					return new ValidationError<ActionNodeCanBeBoundToFlexoActionOrDisplayActionButton, ActionNode>(this,object,"action_node_can_be_bound_to_buttons_with_hyperlink_type_flexo_action_or_display_action",fixes);
				}
			}
			return null;
		}
		...
		public static class ChangeButtonType extends FixProposal<ActionNodeCanBeBoundToFlexoActionOrDisplayActionButton, ActionNode> {
			...
			private static final Logger logger = Logger.getLogger(ActionNode.ActionNodeCanBeBoundToFlexoActionOrDisplayActionButton.ChangeButtonType.class.getPackage().getName());
     ...
     * @return
     */
    protected List<WorkflowPathToOperationNode> getNextOperationForDisplayAction(WorkflowPathToOperationNode currentWorkflowPathToOperationNode)
    {
        if (getDisplayOperation() != null) {
            if (getDisplayOperation().hasWOComponent()) {
                ...
                return getNextOperationForFlexoNode(new ArrayList<AbstractNode>(), new ArrayList<WorkflowPathToOperationNode>(), getDisplayOperation(), currentWorkflowPathToOperationNode);
            }
        } else {
            ...
            if (process == null) {
                ...
                return getNextOperationForProcess(new ArrayList<AbstractNode>(), new ArrayList<WorkflowPathToOperationNode>(), process, currentWorkflowPathToOperationNode);
            }
        }
    }
    ...
    private List<WorkflowPathToOperationNode> getNextOperationsForFlexoAction()
    {
        ...
        return getNextOperationForFlexoNode(new ArrayList<AbstractNode>(), new ArrayList<WorkflowPathToOperationNode>(), this, currentWorkflowPath);
    }
    
    private static List<WorkflowPathToOperationNode> getNextOperationForFlexoNode(List<AbstractNode> visitedNodes,
            List<WorkflowPathToOperationNode> returnedNodes, AbstractNode node, WorkflowPathToOperationNode currentWorkflowPathToOperationNode)
    {
    	return getNextOperationForFlexoNode(visitedNodes, returnedNodes, node, currentWorkflowPathToOperationNode, true);
    }

    private static List<WorkflowPathToOperationNode> getNextOperationForFlexoNode(List<AbstractNode> visitedNodes,
            List<WorkflowPathToOperationNode> returnedNodes, AbstractNode node, WorkflowPathToOperationNode currentWorkflowPathToOperationNode, boolean includeDeadEndPath)
    {
        ...
        if (node instanceof FlexoNode && ((FlexoNode) node).isEndNode()){ // here we have reach an end node
    		...
    		if (container instanceof AbstractNode)
    		{
    			...
    			getNextOperationForFlexoNode(new ArrayList<AbstractNode>(visitedNodes), returnedNodes, (AbstractNode)container, currentWorkflowPathToOperationNode.clone(), false);
...
//        }
        ...
        if(v.size() > 0)
        {
	        ...
	        while (en.hasMoreElements()) {
	            ...
	            if(post.getIsConditional() && (post.getConditionPrimitive() != null || (post.getConditionDescription() != null && post.getConditionDescription().trim().length() > 0)))
	            	duplicatedWorkflowPath.addCondition(post.getProcess(), post.getConditionDescription(), post.getConditionPrimitive(), true);
	            ...
	            if (end instanceof LoopSubProcessNode) {
	            	if (((PetriGraphNode)end).getNewStatus()!=null)
	            		duplicatedWorkflowPath.addNewStatus(((PetriGraphNode)end).getNewStatus());
	            	if(((LoopSubProcessNode)end).getPortMapRegistery()!=null && ((LoopSubProcessNode)end).getPortMapRegistery().getAllNewPortmaps().size()>0)
	            ...
	            if (end instanceof OperationNode && ((OperationNode) end).hasWOComponent()) {
	                ...
	                getNextOperationForFlexoNode(visitedNodes, returnedNodes, ((FlexoPortMap) end).getOperation().getPort(), duplicatedWorkflowPath);
	            	...
	            	for (FlexoPortMap portMap : portMaps) {
	            		getNextOperationForFlexoNode(new ArrayList<AbstractNode>(visitedNodes), returnedNodes, portMap, duplicatedWorkflowPath.clone());
					}
    ...
    private static List<WorkflowPathToOperationNode> getNextOperationForProcess(List<AbstractNode> visitedNodes, List<WorkflowPathToOperationNode> returnedNodes,
            FlexoProcess process, WorkflowPathToOperationNode currentWorkflowPathToOperationNode)
    {
    ...
    public void setTabComponentName(String aComponentName) throws DuplicateResourceException, OperationAssociatedWithComponentSuccessfully
    {
        ...
        if (newComponent == null) {
            ...
            FlexoComponentFolder selectedFolder = getProject().getFlexoComponentLibrary().getRootFolder().getFolderTyped(
                    FolderType.TAB_FOLDER);
    ...
    public void setTabActionComponentInstance(TabComponentInstance tabComponentInstance)
    {
        ...
        if (tabComponentInstance.getComponentDefinition() != null ||(isCreatedByCloning() && tabComponentInstance.getComponentName()!=null)) {
    		...
    		for(WorkflowPathToOperationNode workflowPath : getNextOperationForDisplayAction())
    		{
    			...
    			if(nextOp != null)
    			{
    				if(nextOp.getOperationComponent().getWOComponent().getTabWidgetForTabComponent(getTabComponent())!=null)
    					return nextOp.getOperationComponent().getWOComponent().getTabWidgetForTabComponent(getTabComponent()).getTabKeyForGenerator();
    				break;
    			}
    		}
		}
    ...
    public static class FlexoActionMustSendToken extends ValidationRule
    {
        ...
        public ValidationIssue applyValidation(Validable object)
        {
            ...
            if (node.getActionType()==ActionType.FLEXO_ACTION && !node.isEndNode() && !node.isBeginNode() && (node.getOutgoingPostConditions()==null || node.getOutgoingPostConditions().size()==0)){
