...
public class DMDelete extends FlexoUndoableAction<DMDelete, DMObject, DMObject> 
{
    ...
    public static FlexoActionType<DMDelete, DMObject, DMObject> actionType = new FlexoActionType<DMDelete, DMObject, DMObject>("delete",FlexoActionType.editGroup,FlexoActionType.DELETE_ACTION_TYPE) {
        ...
        while (en.hasMoreElements()) {
            ...
            if (object.isDeletable()){
                ...
                for (Enumeration en2=getGlobalSelectionAndFocusedObject().elements(); en2.hasMoreElements();) {
                    ...
                    if ((next instanceof DMObject) 
                            && (((DMObject)next).isDeletable()) 
                            && (next != object)) {
                        if (((DMObject)next).getAllEmbeddedDeleted().contains(object)) includesIt = false;
                    }
                }
                if (includesIt) {
