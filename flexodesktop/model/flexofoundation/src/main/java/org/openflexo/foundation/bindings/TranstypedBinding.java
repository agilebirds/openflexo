...
public class TranstypedBinding extends AbstractBinding  {
    ...
    public class TranstypedBindingValue extends FlexoModelObject implements InspectableObject
    {
    	...
    	protected TranstypedBindingValue(DMTranstyperEntry anEntry)
    	{
    		...
    		entry = anEntry;
    		bindingValue = null;
    		bindingDefinition = new BindingDefinition(entry.getName(),entry.getType(),TranstypedBinding.this,BindingDefinitionType.GET,true)
    		{
    			@Override
		...
		public boolean equals(Object object)
	    {
	        ...
	        if (object instanceof TranstypedBindingValue) {
	        	...
	        	return ((o.getEntry() == getEntry())
	        			&& (o.getBindingValue() == getBindingValue()));
	        } else {
	            return false;
	        }
	    }
		...
		public TranstypedBinding convertFromString(String aValue)
    	{
            ...
            try {
            	...
            	if (parsedExpression instanceof Function) {
            		...
            		if (logger.isLoggable(Level.WARNING) && warnOnFailure)
            			logger.warning("Could not decode TranstypedBinding : trying to find transtyper syntax with '" + aValue + " : wrong syntax");
            		return null;
            	}
            }
            catch (ParseException e) {
            	if (logger.isLoggable(Level.WARNING) && warnOnFailure)
            		logger.warning("Could not decode TranstypedBinding : parse error while trying to decode as transtyped binding '" + aValue);
            	return null;
            }
            ...
            if (_bindable.getBindingModel() == null) {
            	if (logger.isLoggable(Level.WARNING) && warnOnFailure)
            		logger
            		.warning(("Could not decode TranstypedBinding '" + aValue + "' : declared bindable has a null binding model !"));
            	return null;
            }
            ...
            if (transtyper == null) {
            	if (logger.isLoggable(Level.WARNING) && warnOnFailure)
            		logger.warning(("Could not decode TranstypedBinding '" + aValue + "' : could not find transtyper "+transtyperName+" for "+declaringEntity.getFullyQualifiedName()));
            	return null;
            }
	...
	public boolean equals(Object object)
    {
        ...
        if (object instanceof TranstypedBinding) {
            ...
            return ((_owner == tb._owner)
            		&& (getTranstyper() == tb.getTranstyper())
            		&& (getValues().equals(tb.getValues())));
        } else {
    ...
    {
       if (transtyper != null) return transtyper.getReturnedType();
       return null;
    }
    ...
    {
    	return getBindingDefinition().getType() == null
    			|| getBindingDefinition().getType().isAssignableFrom(getAccessedType(),true);
    }
	...
	{
		if (getTranstyper() == null) return "null";
