		...
		if (getFocusedObject().getGeneratedCode() != null) 
		{
			...
			if (gc instanceof GeneratedCode) 
			{
				...
				if (getNewTargetType() != CodeType.BPEL) {
					...
					if (includeReader) {
						FlexoProjectFile readerSymbDir = new FlexoProjectFile(project, _newGeneratedCodeRepository.getSourceCodeRepository(),
						"/src/main/resources/reader");
						_newGeneratedCodeRepository.setSymbolicDirectoryForKey(new CGSymbolicDirectory(_newGeneratedCodeRepository,
...
//						//TODO_MOS
						createFiguresSymbolicDir(project, "/./ppt/media/figures/");
						createResourcesSymbolicDir(project, srcSymbDirType, "/./ppt/media");
						//
						break;
						//
				}
				...
				if(srcSymbDirType != null)
				{
					...
					_newGeneratedCodeRepository.setSymbolicDirectoryForKey(new CGSymbolicDirectory(_newGeneratedCodeRepository, srcSymbDirType, srcSymbDir), srcSymbDirType);
				}
			}
		}
	}
	...
	private void createFiguresSymbolicDir(FlexoProject project, String figuresPath) 
	{
		...
		_newGeneratedCodeRepository.setSymbolicDirectoryForKey(new CGSymbolicDirectory(_newGeneratedCodeRepository, CGSymbolicDirectory.FIGURES, figSymbDir), CGSymbolicDirectory.FIGURES);
	}
	...
	private void createJavascriptSymbolicDir(FlexoProject project, String srcSymbDirType) {
		FlexoProjectFile jsProcessSymbDir = new FlexoProjectFile(project, _newGeneratedCodeRepository.getSourceCodeRepository(), "/./processes");
		_newGeneratedCodeRepository.setSymbolicDirectoryForKey(new CGSymbolicDirectory(_newGeneratedCodeRepository,
				srcSymbDirType, jsProcessSymbDir), CGSymbolicDirectory.JS_PROCESSES);
	}
	...
	private void createResourcesSymbolicDir(FlexoProject project, String srcSymbDirType, String resourcesPath) 
	{
		...
		FlexoProjectFile resourcesSymbDir = new FlexoProjectFile(project, _newGeneratedCodeRepository.getSourceCodeRepository(), resourcesPath);
		_newGeneratedCodeRepository.setSymbolicDirectoryForKey(new CGSymbolicDirectory(_newGeneratedCodeRepository, srcSymbDirType, resourcesSymbDir), CGSymbolicDirectory.RESOURCES);
	}
