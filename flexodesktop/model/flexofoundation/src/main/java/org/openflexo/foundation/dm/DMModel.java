...
public class DMModel extends DMObject implements XMLStorageResourceData
{
    ...
    public DMEntity getDMEntity(String fullQualifiedName)
    {
    	...
    	if (returned == null) returned = getEntities().get(DMPackage.DEFAULT_PACKAGE_NAME+"."+fullQualifiedName);
    	return returned;
    }
    ...
    /**
     * Used to sort repositories according to following order: - JDKRepository -
     * WORepository - ComponentRepository - ProjectRepositories, sorted by
     * alphabetic name - ProjectDatabaseRepositories, sorted by alphabetic name -
     * DenaliFoundationRepositories, sorted by alphabetic name -
     * ExternalDatabaseRepositories, sorted by alphabetic name -
     * ExternalRepositories, sorted by alphabetic name - ThesaurusRepositories,
     * sorted by alphabetic name - ThesaurusDatabaseRepositories, sorted by
     * alphabetic name
     *
     * @author sguerin
     *
    ...
    private void importClasses (String jarFileName, Vector<String> classNameToImport)
    {
       	...
       	if (importedJar.exists()) {
            	logger.info("Importing "+importedJar.getAbsolutePath());
             	...
             	try {
        			ExternalRepository javaFoundationRepository
        			= ExternalRepository.createNewExternalRepository(getDMModel(), importedJar, classesToImport);
    ...
    public DMEntity getDefaultParentDMEOEntity()
    {
        ...
        if (wdlGenericRecord != null) return wdlGenericRecord;
        DMEntity eoGenericRecord = getDMEntity("com.webobjects.eocontrol","EOGenericRecord");
        if (eoGenericRecord != null) return eoGenericRecord;
        return null;
    }
		...
		if(driverDirectory.exists()){
			...
			if(content.length>0){
				for(int i=0;i<content.length;i++){
					if(content[i].getName().endsWith(".jar"))return content[i].getName();
				}
			}
		}
		return null;
	...
	private Vector<String> getAllDBMSNames(){
		...
		if(findDBMSName(getGlobalDefaultConnectionString())!=null)v.add(findDBMSName(getGlobalDefaultConnectionString()));
		...
		while(en.hasMoreElements()){
			...
			if(dbms!=null && !v.contains(dbms))v.add(dbms);
		}
		return v;
	}
	...
	private static String findDBMSName(String connectionStr){
		...
		if(connectionStr.toLowerCase().indexOf("oracle")>-1)return "oracle";
		if(connectionStr.toLowerCase().indexOf("frontbase")>-1)return "frontbase";
		if(connectionStr.toLowerCase().indexOf("postgres")>-1)return "postgres";
		if(connectionStr.toLowerCase().indexOf("mysql")>-1)return "mysql";
		if(connectionStr.toLowerCase().indexOf("derby")>-1)return "derby";
		if(connectionStr.toLowerCase().indexOf("hsql")>-1)return "hsql";
		if(connectionStr.toLowerCase().indexOf("h2")>-1)return "h2";
		return null;
	}
	...
	{
		if (aTranstyper.getReturnedType() == null) return;
    ...
    {
    	if (type == null) return EMPTY_TRANSTYPER_VECTOR;
