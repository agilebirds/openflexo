...
public abstract class FlexoNode extends PetriGraphNode implements ExecutableWorkflowElement {
    ...
    /**
     * Attached pre-condition: relevant only for begin nodes of Operation and
     * Action level
     */
    ...
    /**
     * Sets attached pre-condition: relevant only for begin nodes of Operation
     * and Action level
     *
     * @param condition
     */
    ...
    /**
     * Returns attached pre-condition: relevant only for begin nodes of
    ...
    /**
     * The insertion or removal of preConditions has to be done with
     * addToPreConditions or removeFromPreConditions methods.
     *
	...
	public static class InteractiveNodeCannotBePutInsideExecutionPetriGraph extends ValidationRule<InteractiveNodeCannotBePutInsideExecutionPetriGraph,FlexoNode> {
		public InteractiveNodeCannotBePutInsideExecutionPetriGraph() {
			super(FlexoNode.class,
		...
		public ValidationIssue<InteractiveNodeCannotBePutInsideExecutionPetriGraph,FlexoNode> applyValidation(FlexoNode node)
		{
			if (node.isInteractive() && node.isEmbeddedInSelfExecutableNode()) {
				return new ValidationError<InteractiveNodeCannotBePutInsideExecutionPetriGraph,FlexoNode>(
						this, node,
						"interactive_nodes_cannot_be_found_inside_an_execution_petri_graph",
	...
	public static class EndNodeCannotHaveMultipleEdges extends ValidationRule<EndNodeCannotHaveMultipleEdges,FlexoNode> {
		...
		public ValidationIssue<EndNodeCannotHaveMultipleEdges, FlexoNode> applyValidation(FlexoNode node) {
			if (node instanceof ActionNode && node.isEndNode() && node.getOutgoingPostConditions().size()>1) {
				return new ValidationWarning<EndNodeCannotHaveMultipleEdges, FlexoNode>(this, node, "end_node_at_this_level_cannot_have_multiple_outgoing_edges");
			}
			return null;
		}
