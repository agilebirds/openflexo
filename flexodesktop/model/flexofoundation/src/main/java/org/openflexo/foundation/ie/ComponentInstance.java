...
/**
 * A ComponentInstance represents an instance (a use case) of a
 * ComponentDefinition in a given context (this context can be determined with
 * the type of resource data: {@link #getXMLResourceData()}
 ...
 * 2) When the owner is deleted-->delete (perform this call asap so that resource data is still set on the calling owner)
 * 3) When the component instance is removed from the owner-->delete
 *
...
public abstract class ComponentInstance extends IEObject implements Bindable, FlexoObserver, Validable
{
	...
	private void rebuildDependancy(XMLStorageResourceData newResourceData) {
		if (newResourceData!=null && getComponentDefinition()!=null) {
    		...
    		if (logger.isLoggable(Level.WARNING))
				logger.warning("Could not rebuid dependancy for resource data: "+newResourceData+" and component definition is "+getComponentDefinition()+" named: "+getComponentName());
    	}
	}
     ...
     * object is contained relating to storing scheme: here it's the object
     * declared as the container
     *
    	...
    	if (_componentDefinition==null && xmlComponentName!=null) {
			...
			if (aComponentDefinition == null) {
				if (logger.isLoggable(Level.INFO))
					logger.info("A ComponentInstance with component name : " + xmlComponentName
							+ " has no component def. Owner is "+getOwner());
			} else {
    ...
    public Vector<ComponentInstanceBinding> getBindings()
    {
        ...
        if ((_bindings != null) && (getComponentDefinition() != null) && (_bindings.size() != getComponentDefinition().getBindingDefinitions().size())) {
            updateBindings();
        }
        return _bindings;
    }
   ...
   private void updateBindings()
    ...
    /**
     * Stores in an hashtable ComponentInstanceBinding related to a ComponentBindingDefinition
     * Ensure that no new ComponentInstanceBinding is created if a component binding definition is renamed for example
     */
    ...
    /**
     * Return a Vector of embedded IEObjects at this level.
     * NOTE1: that this is NOT a recursive method
     * NOTE2: return EMPTY_VECTOR, since there is no embedded IEObject
     *
     * @return EMPTY_VECTOR
     */
    @Override
    ...
    /**
     * Returns owner for this component instance, if it has been set.
     * The owner of a ComponentInstance is the FlexoModelObject which emmbed this
     * instance.
     */
    ...
    /**
     * Sets owner for this component instance.
     * The owner of a ComponentInstance is the FlexoModelObject which emmbed this
     * instance.
    * @param owner
     */
    public final void setOwner(ComponentInstanceOwner owner)
    {
    	...
    	container = null;
        _owner = owner;
        _componentDefinition=null;// We reset the _componentDefinition so that in case of conversion we will try to retrieve the component definition again.
    ...
    public void rebuildDependancies() {
    	if (getXMLResourceData()!=null) {
    		...
    		if (logger.isLoggable(Level.WARNING))
				logger.warning("Cannot rebuild dependancies because XMLResourceData="+getXMLResourceData()+" and ComponentDefinition="+getComponentDefinition());
    	}
    }
    ...
    public abstract static class CheckAllBindingsRule extends ValidationRule<CheckAllBindingsRule,ComponentInstance>
    {
        ...
        public class MissingRequiredBinding extends ValidationError<CheckAllBindingsRule,ComponentInstance>
        {
            ...
            public MissingRequiredBinding(ComponentInstance ci, ComponentInstanceBinding aComponentInstanceBinding)
            {
                ...
                ownerIdentifier = componentInstanceBinding.getComponentInstance().getOwner()!=null ?
                		((FlexoModelObject)componentInstanceBinding.getComponentInstance().getOwner()).getFullyQualifiedName():
                			"undentified button or link";
            ...
            {
                return FlexoLocalization.localizedForKeyWithParams("binding_named_($bindingName)_required_by_component_($componentName)_is_not_defined_for_widget($ownerIdentifier)",this);
            }

            public String getLocalizedErrorMessageForInvalidAndRequiredValue()
            {
                return FlexoLocalization.localizedForKeyWithParams("binding_named_($bindingName)_required_by_component_($componentName)_has_invalid_value_for_widget($ownerIdentifier)",this);
            }
        ...
        public static class AddEntryToComponentAndSetBinding extends ParameteredFixProposal<CheckAllBindingsRule,ComponentInstance>
        {
            ...
            private static ParameterDefinition[] buildParameters(BindingDefinition bd)
            {
                ...
                returned[1] = new DMEntityParameter("variableType","variable_type",(bd.getType()!=null?bd.getType().getBaseEntity():null));
