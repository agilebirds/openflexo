...
public class AddPSlide extends FlexoAction<AddPSlide, PTOCObject,PTOCObject>
{

    @SuppressWarnings("unused")
	private static final Logger logger = Logger.getLogger(AddPSlide.class.getPackage().getName());

    public static FlexoActionType<AddPSlide, PTOCObject, PTOCObject> actionType = new FlexoActionType<AddPSlide, PTOCObject, PTOCObject>(
         ...
         */
        @Override
        public AddPSlide makeNewAction(PTOCObject focusedObject, Vector<PTOCObject> globalSelection, FlexoEditor editor)
        {
            return new AddPSlide(focusedObject, globalSelection, editor);
        }
    ...
    private String pSlideTitle;
    private PSlideLayout layout;
    private PSlideType type;
    private FlexoModelObject modelObject;

    AddPSlide(PTOCObject focusedObject, Vector<PTOCObject> globalSelection, FlexoEditor editor)
    {
...
//    	}
    	PSlide slide;
    	if (type!=null || modelObject!=null) {
    		if(type!=null && modelObject!=null){
    			slide = getRepository().createPSlide(modelObject,type);
    			slide.setTitle(getPSlideTitle());
    			addPSlideToFocusedObject(slide);
    		}
//    		else if(type!=null)
...
//    		}
    		...
    		slide = new PSlide(getFocusedObject().getData());
//    		if(subType!=null)entry.setSubType(subType);
			slide.setTitle(getPSlideTitle());
			addPSlideToFocusedObject(slide);
    	}
    ...
    private void addPSlideToFocusedObject(PSlide entry) {
   		((PTOCEntry)getFocusedObject()).addToPTocUnits(entry);
    }
    
	public String getPSlideTitle() {
		return pSlideTitle;
	}

	public void setPSlideTitle(String Title) {
		this.pSlideTitle = Title;
	}

	public void setLayout(PSlideLayout layout) {
		this.layout = layout;
	}
	
	public void setSlideType(PSlideType type) {
		this.type = type;
	}
