...
//    			_sourceCodeRepository.setIdentifier(name);
                    ...
                    if (file.getName()!=null && file.getName().indexOf('.')>-1) {
                        if (file.getName().substring(0,file.getName().indexOf('.')).equals(getName())) {
                            if (logger.isLoggable(Level.WARNING))
                                logger.warning("Found a resource without CGFile that is supposed to be in this repository: "+file.getFullyQualifiedName()+ " I will delete it now");
                            file.delete(deleteFiles);
                        }
                    }
                }
	...
	public CGSymbolicDirectory getSymbolicDirectory(CGFile file){
		if(this==file.getRepository()){
			...
			while(en.hasMoreElements()){
				...
				if(filePath.indexOf(reply.getDirectory().getRelativePath())>-1)return reply;
			}
		}
		return null;
	}
	...
	public ProjectExternalRepository getSourceCodeRepository()
	{
		if (_sourceCodeRepository == null) {
			...
			if (_sourceCodeRepository==null)
				_sourceCodeRepository = getProject().setDirectoryForRepositoryName(_displayName!=null?_displayName:"Default", new File(System.getProperty("user.home")+"/"+getProject().getProjectName()+"/"+getTarget().getName()));
		}
	...
	{
		CustomCGTemplateRepository newRepository = getGeneratedCode().getTemplates().getCustomCGTemplateRepositoryForName(_preferredTemplateRepositoryName);
		if (newRepository != null) {
			setPreferredTemplateRepository(newRepository);
		}
	}
   ...
   private File _codeGenerationWorkingDirectory;
    ...
    public File getCodeGenerationWorkingDirectory()
    {
    	if (_codeGenerationWorkingDirectory == null) {
    		_codeGenerationWorkingDirectory = new File(ProjectRestructuration.getExpectedGeneratedCodeDirectory(getProject().getProjectDirectory()),getName());
    	}
	...
	public CGRelease getLastRelease()
	{
		...
		if (_releases.size() > 0) return _releases.lastElement();
		return null;
	}
    ...
    {
    	if (getLastRelease() != null) return getLastRelease().getVersionIdentifier();
