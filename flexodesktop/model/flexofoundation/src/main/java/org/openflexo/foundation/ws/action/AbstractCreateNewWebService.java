...
public class AbstractCreateNewWebService<C extends AbstractCreateNewWebService<C>> extends FlexoAction<C, WSObject, WSObject> {

	private static final Logger logger = Logger.getLogger(AbstractCreateNewWebService.class.getPackage().getName());

	public static final String INTERNAL_WS = "INTERNAL_WS";
	public static final String EXTERNAL_WS = "EXTERNAL_WS";

	protected AbstractCreateNewWebService(FlexoActionType<C, WSObject, WSObject> anActionType, WSObject focusedObject,
	...
	protected void doAction(Object context) throws FlexoException {
		...
		if (_webServiceType.equals(AbstractCreateNewWebService.EXTERNAL_WS)) {
		...
		} else if (_webServiceType.equals(AbstractCreateNewWebService.INTERNAL_WS)) {
	...
	private DMEntity copyEntity(DMEntity entity, WSDLRepository toRep) {
		...
		// 3. copy properties
		Enumeration<String> en = entity.getProperties().keys();
		while (en.hasMoreElements()) {
			DMProperty property = entity.getProperty(en.nextElement());
