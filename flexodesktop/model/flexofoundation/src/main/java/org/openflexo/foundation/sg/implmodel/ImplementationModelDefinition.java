...
public class ImplementationModelDefinition extends FlexoModelObject {
    ...
    public ImplementationModelDefinition(String modelName, GeneratedSources generatedSources,FlexoProject project, boolean checkUnicity) throws DuplicateResourceException
    {
    ...
    public ImplementationModelResource getImplementationModelResource(boolean createIfNotExists)
    {
        if (getProject() != null) {
            ...
            if (returned == null && createIfNotExists) {
                ...
                File implModelFile = new File(ProjectRestructuration.getExpectedImplementationModelDirectory(getProject().getProjectDirectory(),getName()), getName() + ".im");
                ...
                try {
                    ...
                    for (int i = 0; i < 100 && !ok; i++) {
                        try {
                        	implModelFile = new File(ProjectRestructuration.getExpectedImplementationModelDirectory(getProject().getProjectDirectory(),getName()), getName() + i + ".im");
                    ...
                    if (!ok) {
                      	implModelFile = new File(ProjectRestructuration.getExpectedImplementationModelDirectory(getProject().getProjectDirectory(),getName()), FileUtils.getValidFileName(getName())+getFlexoID() + ".im");
                     	resourceImplModelFile = new FlexoProjectFile(implModelFile, getProject());
                        try {
                        	implModelRes = new ImplementationModelResource(getProject(), getName(), gsRes, resourceImplModelFile);
                        } catch (InvalidFileNameException e) {
