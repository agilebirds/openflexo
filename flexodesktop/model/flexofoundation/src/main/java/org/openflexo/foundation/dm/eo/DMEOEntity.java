...
public class DMEOEntity extends DMEntity implements DMEOObject, SourceCodeOwner
{
     ...
     */
    public DMEOEntity(DMModel dmModel, DMEOModel dmEOModel, EOEntity eoEntity)
     ...
     */
    public DMEOEntity(DMModel dmModel, DMEOModel dmEOModel, EOEntity eoEntity,
            boolean isPrototypeEntity) throws EOAccessException
    {
        this(dmModel);
        ...
        if (eoEntity != null) {
             ...
             * try {
             *
             * for (Enumeration en=eoEntity.attributes().objectEnumerator();
             * en.hasMoreElements();) { if (!isPrototypeEntity) { EOAttribute
             * eoAttribute = (EOAttribute)en.nextElement(); DMEOAttribute
             * dmEOAttribute = new DMEOAttribute(dmModel, eoAttribute);
             * registerProperty(dmEOAttribute); } else { EOAttribute eoAttribute =
             * (EOAttribute)en.nextElement(); DMEOPrototype dmEOPrototype = new
             * DMEOPrototype(dmModel, eoAttribute);
             * registerProperty(dmEOPrototype); } }
             *
             * if (!isPrototypeEntity) { for (Enumeration
             * en=eoEntity.relationships().objectEnumerator();
             * en.hasMoreElements();) { EORelationship eoRelationship =
             * (EORelationship)en.nextElement(); DMEORelationship
             * dmEORelationship = new DMEORelationship(dmModel, eoRelationship);
             * registerProperty(dmEORelationship); } } } catch
             * (IllegalArgumentException e) { if
             * (logger.isLoggable(Level.WARNING)) logger.warning("EOAccess
             * management failed :"+e.getMessage()); throw new
             * EOAccessException(e); }
             */
     ...
     */
    public static DMEOEntity createsNewDMEOEntity(DMModel dmModel, String name,
            DMEOModel dmEOModel, String className) throws EOAccessException
    {
        ...
        try {
            ...
            if (logger.isLoggable(Level.WARNING))
                logger.warning("EOAccess management failed while trying to creates entity " + name
                        + " : " + e.getMessage());
    ...
    private void updateFromEOEntity(EOEntity eoEntity, boolean isPrototypeEntity)
            throws EOAccessException
    {
        if (eoEntity != null) {
    ...
    private void updateAttributesFromEOEntity(EOEntity eoEntity, boolean isPrototypeEntity)
            throws EOAccessException
    {
        if (eoEntity != null) {
            if (logger.isLoggable(Level.FINE))
            ...
            try {
                ...
                for (Iterator<EOAttribute> i = new Vector<EOAttribute>(eoEntity.getAttributes()).iterator(); i.hasNext();) {
                    ...
                    if ((foundAttribute != null) && (foundAttribute.getDMEOEntity() != this)) {
                        if (logger.isLoggable(Level.WARNING))
                            logger.warning("Lookup dereferenced EOAttribute "
                                    + foundAttribute.getName() + "! Trying to repair...");
                    ...
                    if (foundAttribute == null) {
                        if (!isPrototypeEntity) {
                            if (logger.isLoggable(Level.FINE))
                                logger.fine("Found NEW EOAttribute " + eoAttribute.getName()
                                        + ". Creates the related DMEOAttribute.");
                            DMEOAttribute newDMEOAttribute = new DMEOAttribute(getDMModel(),
                                    eoAttribute);
                             registerProperty(newDMEOAttribute);
                       } else {
                            if (logger.isLoggable(Level.FINE))
                                logger.fine("Found NEW EOAttribute " + eoAttribute.getName()
                                        + ". Creates the related DMEOPrototype.");
                            DMEOPrototype newDMEOPrototype = new DMEOPrototype(getDMModel(),
                                    eoAttribute);
        ...
        if (eoEntity != null) {
            ...
            try {
                ...
                for (Iterator<EORelationship> i = new Vector<EORelationship>(eoEntity.getRelationships()).iterator();i.hasNext();) {
                    ...
                    DMEORelationship foundRelationship = lookupDMEORelationshipWithName(eoRelationship
                            .getName());
                    if ((foundRelationship != null) && (foundRelationship.getDMEOEntity() != this)) {
                        if (logger.isLoggable(Level.WARNING))
                            logger.warning("Lookup dereferenced EORelationship "
                                    + foundRelationship.getName() + "! Trying to repair...");
                        foundRelationship.delete();
                        foundRelationship = null;
                    }
                    if (foundRelationship == null) {
                        if (logger.isLoggable(Level.FINE))
                            logger.fine("Found NEW EORelationship " + eoRelationship.getName()
                                    + ". Creates the related DMEORelationship.");
                        DMEORelationship newDMEORelationship = new DMEORelationship(getDMModel(),
                                eoRelationship);
    ...
    {
         if (_parentType == null) {
            if ((_parentEOEntity != null)
            		&& (getDMEORepository() != null)) {
  ...
  /* public DMEntity getParentBaseEntity()
    {
    	DMEntity returned = super.getParentBaseEntity();
    	if (returned == null && (_parentEOEntity != null) && (getDMEORepository() != null)) {
    		DMEntity
    	}
        ...
        if (parentEntity == null) {
            if ((_parentEOEntity != null) && (getDMEORepository() != null)) {
                parentEntity = getDMModel().getDMEOEntity(_parentEOEntity);
            }
            else {
                DMEntity defaultParentEntity = getDMModel().getDefaultParentDMEOEntity();
                if (defaultParentEntity != null) {
                   ...
                   if (parentEntity != null) {
                        parentEntity.addToChildEntities(this);
                    }
                 }
                else {
                    //logger.warning("com.webobjects.eocontrol package not imported !");
                }
            }
        }
        return parentEntity;
    ...
    private void internallySetParentType(DMType aType)
    {
    	...
    	if (getEOEntity() != null) {
    		if (newParentEntity instanceof DMEOEntity) {
    			...
    			//  if (logger.isLoggable(Level.WARNING))
    			//    logger.warning("Could not set parent entity which is not a DMEOEntity");
    		}
    	}
    	setChanged();
    	notifyObservers(new DMAttributeDataModification("parentEntity", oldParentEntity,
    			newParentEntity));

    }


    /*public void setParentBaseEntity(DMEntity aParentEntity)
    {
        DMEntity oldParentEntity = getParentBaseEntity();
        parentEntity = aParentEntity;
        if (getEOEntity() != null) {
            if (aParentEntity instanceof DMEOEntity) {
                if (((DMEOEntity) aParentEntity).getEOEntity() != null) {
                    try {
                        getEOEntity().setParentEntity(((DMEOEntity) aParentEntity).getEOEntity());
                        _parentEOEntity = ((DMEOEntity) aParentEntity).getEOEntity();
                    } catch (IllegalArgumentException e) {
                        parentEntity = null;
                        getEOEntity().setParentEntity(null);
                    }
            ...
            } else if (aParentEntity != null) {
                aParentEntity.addToChildEntities(this);
                //  if (logger.isLoggable(Level.WARNING))
                //    logger.warning("Could not set parent entity which is not a DMEOEntity");
            }
        }
        ...
        if (_eoEntity == null) {
            if ((getDMEOModel() != null) && (getDMEOModel().getEOModel() != null)) {
                try {
                    ...
                    if (logger.isLoggable(Level.WARNING))
                        logger.warning("Could not find EOEntity named " + getName()
                                + " : EOControl management failed");
                }
                if (_eoEntity == null) {
                    // if (logger.isLoggable(Level.WARNING))
                    // logger.warning("Could not find EOEntity named
                    // "+getName());
                }
            }
        }
    ...
    @Override
    public void setEntityClassName(String anEntityClassName) throws InvalidNameException,
            DuplicateClassNameException
    {
        if (!isDeserializing()
                && (anEntityClassName == null || !DMRegExp.ENTITY_NAME_PATTERN.matcher(
                        anEntityClassName).matches()))
            throw new InvalidNameException();
        if ((getEntityClassName() == null) || (!getEntityClassName().equals(anEntityClassName))) {
            if (getEOEntity() != null) {
                if (getDMModel().getDMEntity(getEntityPackageName(), anEntityClassName) != null) {
                    throw new DuplicateClassNameException(getEntityPackageName() + "."
                            + anEntityClassName);
                }
                DMRepository containerRepository = getRepository();
                if (containerRepository != null) {
                ...
                getEOEntity().setClassName((getPackage()==null||getPackage().isDefaultPackage()?"":getPackage().getName()+".")+anEntityClassName);
    ...
    public String getEntityPackageName() {
    	...
    	return super.getEntityPackageName();
    }
    ...
    {
        if (!isDeserializing()
                && (externalName == null || !DMRegExp.ENTITY_NAME_PATTERN.matcher(externalName)
                        .matches()))
            throw new InvalidNameException();
        if (getEOEntity() != null) {
            ...
            setChanged();
            notifyObservers(new DMAttributeDataModification("externalName", oldExternalName,
                    externalName));
        }
    }
            ...
            for (Iterator<EOProperty> i = getEOEntity().getClassProperties().iterator(); i.hasNext();) {
                ...
                if (eoProperty instanceof EOAttribute) {
                    ...
                    if (dmEOAttribute != null) {
                        ...
                        // if (logger.isLoggable(Level.WARNING))
                        // logger.warning("Could not find attribute named
                        // "+eoAttribute.name());
                    }
                }
                if (eoProperty instanceof EORelationship) {
                    ...
                    if (dmEORelationship != null) {
                        ...
                        // if (logger.isLoggable(Level.WARNING))
                        // logger.warning("Could not find relationship named
                        // "+eoRelationship.name());
                    }
                }
            }
        ...
        if (dmEORelationship.getEORelationship() != null) {
            if (_relationshipsForEORelationships.get(dmEORelationship.getEORelationship()) != null) {
                if (_relationshipsForEORelationships.get(dmEORelationship.getEORelationship()) != dmEORelationship) {
                    if (logger.isLoggable(Level.WARNING))
                        logger
                                .warning("Trying to redefine DMEORelationship: operation not allowed !");
                } else { // Ignore it !
                }
            } else {
                _relationshipsForEORelationships.put(dmEORelationship.getEORelationship(),
                        dmEORelationship);
            }
        }
    }
    ...
    {
    	if(eoAttribute==null)return null;
        return _attributesForEOAttributes.get(eoAttribute);
    }
    ...
    private DMEOAttribute lookupDMEOAttributeWithName(String attributeName)
    {
        ...
        if (returned != null)
            if (logger.isLoggable(Level.WARNING))
                logger.warning("Found property named " + attributeName
                        + " but doesn't match a DMEOAttribute: " + returned.getClass().getName());
        return null;
    }
    ...
    private DMEORelationship lookupDMEORelationshipWithName(String relationshipName)
    {
        ...
        if (returned != null)
            if (logger.isLoggable(Level.WARNING))
                logger
                        .warning("Found property named " + relationshipName
                                + " but doesn't match a DMEORelationship: "
    ...
    /**
     * Return a vector of up-to-date and reordered DMProperty which are not
     * instances of DMEOAttribute or DMEORelationship
     *
     * @return
     */
    @Override
    ...
    protected void reorderProperties()
    {
        if (propertiesNeedsReordering) {
            ...
            if(_attributesForEOAttributes!=null)_orderedAttributes.addAll(_attributesForEOAttributes.values());
    ...
    {
        boolean b = super.unregisterProperty(property, notify);
        /*
         * if (property instanceof DMEOAttribute)
         * getEOEntity().removeAttribute(((DMEOAttribute)
         * property).getEOAttribute()); else if (property instanceof
         * DMEORelationship)
         * getEOEntity().removeRelationship(((DMEORelationship)
         * property).getEORelationship());
         */
        return b;
    }
   ...
   public static class DMEOEntityMustAtLeastOnePrimaryKey extends ValidationRule
    {
        ...
        public ValidationIssue applyValidation(final Validable object)
        {
            ...
            if (entity.getEOEntity() != null && entity.getEOEntity().getPrimaryKeyAttributes().size()==0) {
                ValidationError error = new ValidationError(this, object, "eoentity_($object.name)_must_have_at_least_one_primary_key_attribute");
	...
	public boolean isIntegerPrimaryKey(){
		...
		if(att.getPrototype().getName().startsWith("id"))return true;
		return false;
	}
	...
	public Vector<DMProperty> getAllNonEOProperties(){
		...
		while(it.hasNext()){
			p = it.next();
			if(!(p instanceof DMEOAttribute) && !(p instanceof DMEORelationship))reply.add(p);
		}
		return reply;
	}
        ...
        while (en.hasMoreElements()) {
            ...
            if (!r.getIsToMany()) {
                if (r.getDestinationEntity()==to)
                    return true;
                else if (r.getDestinationEntity()!=null && !alreadyVisited.contains(r.getDestinationEntity()) && r.getDestinationEntity().hasRelationshipTo(to, alreadyVisited))
                    return true;
            }
        }
        return false;
    ...
    {
    	if (dataModification instanceof TypeResolved
    			&& ((TypeResolved)dataModification).getType() == _parentType) {
    		if (logger.isLoggable(Level.FINE))logger.fine("EOEntity: Parent type "+_parentType+" decoded for "+this);
    		internallySetParentType(((TypeResolved)dataModification).getType());
     	}
    	else super.update(observable, dataModification);
    }
     ...
     * Applicable only if EOCodeGeneration was activated
     *
     * @return
     */
    @Override
    ...
    public ClassSourceCode getGeneratedCode()
    {
    	if (codeNeedsToBeRegenerated && getDMModel().getEOCodeGenerationActivated() && codeIsComputable()) {
    		if (generatedCode == null) {
    			generatedCode = new ClassSourceCode(this/*,"generatedCode","hasClassParseError","classParseErrorWarning"*/) {
					@Override
	...
	public boolean codeIsComputable() {
		...
		while(en.hasMoreElements()){
			if(!en.nextElement().codeIsComputable())return false;
		}
		return true;
	}
