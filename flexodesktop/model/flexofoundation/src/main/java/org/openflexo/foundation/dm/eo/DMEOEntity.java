...
public class DMEOEntity extends DMEntity implements DMEOObject, SourceCodeOwner {
     ...
     */
    public static DMEOEntity createsNewDMEOEntity(DMModel dmModel, String name, DMEOModel dmEOModel,
                                                  String className) throws EOAccessException {
    ...
    /**
     * Return String uniquely identifying inspector template which must be applied when trying to inspect
     * this object
     *
    ...
    public void setName(String newName) throws InvalidNameException {
        ...
        if ((name == null) || (!name.equals(newName))) {
            ...
            if (!isDeserializing() && newName != null && newName.trim().length() > 0 && (getExternalName() == null
                   || getExternalName().equals(ToolBox.convertJavaStringToDBName(oldName)))) {
                setExternalName(ToolBox.convertJavaStringToDBName(newName));
            }
            if (!isDeserializing() && newName != null && newName.trim().length() > 0 && (getEntityClassName() == null
                                                                                         || getEntityClassName().equals(oldName))) {
                try {
        ...
        if ((getEntityClassName() == null) || (!getEntityClassName().equals(anEntityClassName))) {
            if (getEOEntity() != null) {
                ...
                getEOEntity().setClassName(
                        (getPackage() == null || getPackage().isDefaultPackage() ? "" : getPackage().getName() + ".") +
                        anEntityClassName);
    ...
    private DMEOAttribute lookupDMEOAttributeWithName(String attributeName) {
        ...
        if (returned != null) {
            if (logger.isLoggable(Level.WARNING)) {
                logger.warning(
                        "Found property named " + attributeName + " but doesn't match a DMEOAttribute: " + returned.getClass().getName());
            }
        }
        return null;
    }
