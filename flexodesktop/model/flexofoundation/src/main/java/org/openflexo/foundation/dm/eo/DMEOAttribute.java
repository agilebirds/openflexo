 ...
 * database, as a field in a table.
 *
 * @author sguerin
 *
 */
public class DMEOAttribute extends DMEOProperty
{
    ...
    public String getJavaClassName()
    {
        try {
            if (getEOAttribute().getClassName() != null)
                ...
                return getEOAttribute().getPrototype().getClassName().substring(
                        (getEOAttribute().getPrototype().getClassName().lastIndexOf(".")) + 1);
            else {
                if (logger.isLoggable(Level.WARNING))
    ...
    public void setPrototype(DMEOPrototype prototype)
    {
        if (getEOAttribute() != null) {
            ...
            if (!isDeserializing() && oldPrototype != null && oldPrototype.getName().equals(BOOLEAN_PROTOTYPE_NAME) && getDMEOEntity()!=null) {
        ...
        try {
            if (!isDeserializing() && getPrototype() != null && getPrototype().getName().equals(BOOLEAN_PROTOTYPE_NAME)
                ...
                DMProperty p = new DMProperty(
                		getDMModel(),
                		getName() + BOOLEAN_METHOD_POSTFIX,
                		DMType.makeResolvedDMType(getDMModel().getEntityNamed("boolean")),
                        DMCardinality.SINGLE,
                        true,
                        true,
                        DMPropertyImplementationType.PUBLIC_ACCESSORS_ONLY);
                StringBuilder sb = new StringBuilder();
                sb.append("    return ").append(getName()).append("()!=null && ").append(getName()).append("().equals(").append(
                        getProject().getPrefix()).append("Constants.TRUE_VALUE);");
                //p.setGetterCoreCode(sb.toString());

                try {
					p.setGetterCode(getGetterJavadoc()+StringUtils.LINE_SEPARATOR+getGetterHeader()+" { "+StringUtils.LINE_SEPARATOR+sb.toString()+StringUtils.LINE_SEPARATOR+"}");
                ...
                sb.append("    set").append(ToolBox.capitalize(getName(),true)).append("(value?")
                        .append(getProject().getPrefix()).append("Constants.TRUE_VALUE:").append(getProject().getPrefix()).append(
                                "Constants.FALSE_VALUE);");
                //p.setSetterCoreCode(sb.toString());

                try {
                	p.setSetterCode(getSetterJavadoc()+StringUtils.LINE_SEPARATOR+getSetterHeader()+" { "+StringUtils.LINE_SEPARATOR+sb.toString()+StringUtils.LINE_SEPARATOR+"}");
                } catch (ParserNotInstalledException e) {
                	e.printStackTrace();
                } catch (DuplicateMethodSignatureException e) {
                	e.printStackTrace();
   ...
   public static class DMEOAttributeMustHaveAJavaClass extends ValidationRule
    {
        ...
        {
            final DMEOAttribute property = (DMEOAttribute) object;
            if (property.getIsClassProperty()
            		&& (property.getEOAttribute() != null)
            		&& (property.getEOAttribute().getClassName() == null)
