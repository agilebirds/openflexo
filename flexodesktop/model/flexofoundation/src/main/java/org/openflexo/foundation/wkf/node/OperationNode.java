 ...
 *
 */
public class OperationNode extends FatherNode implements ApplicationHelpEntryPoint, ChildNode, ComponentInstanceOwner, Comparable<OperationNode>, MetricsValueOwner, FlexoObserver
{
	...
	{
		if(observable instanceof FlexoProcess && (dataModification instanceof NameChanged || dataModification instanceof WKFAttributeDataModification && "businessDataType".equals(((WKFAttributeDataModification)dataModification).getAttributeName())))
		{ //Update business data accessing methods on screens
	...
	/**
	 * Returns all the action nodes embedded in this operation. This returns also action nodes embedded in LOOPOperator and SelfExecutableNode.
	...
	/**
	 * Returns a sorted vector of all the Action nodes that are in the underlying action petrigraph and the ones embedded by LOOPOperator and SelfExecutableNode.
	...
	public void setComponentInstance(OperationComponentInstance componentInstance) throws OperationAssociatedWithComponentSuccessfully
	{
		...
		if (componentInstance.getComponentDefinition() != null || isCreatedByCloning() && componentInstance.getComponentName()!=null) {
			...
			OperationComponentHasBeenSet dm = new OperationComponentHasBeenSet("operationComponent", componentInstance
					.getComponentDefinition(), this);
			notifyObservers(dm);
	...
	public void setTabComponentName(String aComponentName) throws DuplicateResourceException, OperationAssociatedWithComponentSuccessfully
	{
		...
		if (newComponent == null) {
			...
			newComponent = new TabComponentDefinition(aComponentName, getProject().getFlexoComponentLibrary(), selectedFolder,
					getProject());
		}
	...
	public void setTabOperationComponentInstance(TabComponentInstance tabComponentInstance)
	{
		...
		if (tabComponentInstance.getComponentDefinition() != null || isCreatedByCloning()) {
			...
			setChanged();
			TabOperationComponentHasBeenSet dm = new TabOperationComponentHasBeenSet("WOComponentName", tabComponentInstance
					.getComponentDefinition(), this);
			notifyObservers(dm);
		} else if (logger.isLoggable(Level.SEVERE)) {
	...
	public static class OperationMustHaveAWOComponent extends ValidationRule<OperationMustHaveAWOComponent,OperationNode>
	{
		...
		{
			if (!(node instanceof SelfExecutableOperationNode) && node.getNodeType() == NodeType.NORMAL && node.getComponentInstance() == null) {
				ValidationError<OperationMustHaveAWOComponent,OperationNode> error = new ValidationError<OperationMustHaveAWOComponent,OperationNode>(this, node, "operation_($object.name)_defines_no_wo_component");
	...
	public static class OperationComponentActionsMustBeBoundToAnActionNode extends ValidationRule<OperationComponentActionsMustBeBoundToAnActionNode, OperationNode>
	{
		...
		public ValidationIssue<OperationComponentActionsMustBeBoundToAnActionNode, OperationNode> applyValidation(OperationNode node)
		{
			...
			while (en.hasMoreElements()) {
				...
				if (w.getIsMandatoryFlexoAction() &&
						(w.getHyperlinkType()==null ||
								w.getHyperlinkType()!=HyperlinkType.SEARCH
								&& w.getHyperlinkType()!=HyperlinkType.CANCEL
								&& w.getHyperlinkType()!=HyperlinkType.MAILTO
		...
		public class RemoveMandatoryAttributeFromWidget extends FixProposal<OperationComponentActionsMustBeBoundToAnActionNode, OperationNode>
		{
	...
	protected boolean shouldBeSynchronized()
	{
		for (Enumeration en = getAllActionNodes().elements(); en.hasMoreElements();) {
			...
			for (Enumeration<FlexoPostCondition<AbstractNode, AbstractNode>> en2 = nextAction.getOutgoingPostConditions().elements(); en2.hasMoreElements();) {
	...
	/**
	 * An OperationNode containing at least one ActionNode linked to an END node
	 * should be synchronized.
	 *
	 * @author sguerin
	 * @version $Id: OperationNode.java,v 1.9.2.19 2006/01/06 21:35:06 sguerin
	 *          Exp $
	 */
	public static class OperationShouldBeSynchronized extends ValidationRule<OperationShouldBeSynchronized,OperationNode>
	{
		...
		public ValidationIssue<OperationShouldBeSynchronized,OperationNode> applyValidation(OperationNode node)
		{
			if (node.shouldBeSynchronized() && !node.getIsSynchronized()) {
				ValidationWarning<OperationShouldBeSynchronized,OperationNode> warning = new ValidationWarning<OperationShouldBeSynchronized,OperationNode>(this, node, "operation_($object.name)_should_be_synchronized");
	 ...
	 */
	 ...
	 public static class MandatoryBindingsMustHaveAValue extends ValidationRule<MandatoryBindingsMustHaveAValue,OperationNode> {
		 ...
		 public ValidationIssue<MandatoryBindingsMustHaveAValue,OperationNode> applyValidation(OperationNode node) {
			 ...
			 Enumeration<ComponentBindingDefinition> en = node.getComponentInstance().getComponentDefinition().getBindingDefinitions().elements();
			 while (en.hasMoreElements()) {
				 ...
				 if (cib.getBindingValue()==null) {
					 return new ValidationError<MandatoryBindingsMustHaveAValue,OperationNode>(this,node,"some_bindings_have_no_value_in_operation_($object.name)");
				 }
			 }
			 return null;
		 }
	 ...
	 public static class MandatoryBooleanBindingsMustHaveAValue extends ValidationRule<MandatoryBooleanBindingsMustHaveAValue,OperationNode> {
		 ...
		 public ValidationIssue<MandatoryBooleanBindingsMustHaveAValue,OperationNode> applyValidation(OperationNode node) {
			 ...
			 while (en.hasMoreElements()) {
				 ComponentBindingDefinition bd = en.nextElement();
				 if(bd.getType()==null){
					 return new ValidationError<MandatoryBooleanBindingsMustHaveAValue,OperationNode>(this,node,"component_bindings_for_operation_($object.name)_have_no_type");
				 }
				 ...
				 if (cib ==null || cib.getBindingValue()==null) {
					 return new ValidationError<MandatoryBooleanBindingsMustHaveAValue,OperationNode>(this,node,"some_boolean_bindings_have_no_value_in_operation_($object.name)");
				 }
			 }
			 return null;
		 }
	 ...
	 public static class OperationMustDefineABindingOfBusinessDataType extends ValidationRule<OperationMustDefineABindingOfBusinessDataType, OperationNode> {
		 ...
		 public ValidationIssue<OperationMustDefineABindingOfBusinessDataType, OperationNode> applyValidation(OperationNode node) {
			 ...
			 return new ValidationError<OperationMustDefineABindingOfBusinessDataType, OperationNode>(this,node,"operation_must_define_a_binding_of_business_data_type");
		 }
	 ...
	 public ActionNode getActionNodeForButton(IEHyperlinkWidget button)
	 {
		 ...
		 while (en.hasMoreElements()) {
			 ...
			 if (action.getAssociatedButtonWidget() == button
					 && (action.getActionType() == ActionType.DISPLAY_ACTION || action
							 .getActionType() == ActionType.FLEXO_ACTION)) {
				 return action;
			 }
		 }
		 return null;
	 ...
	 public String getStaticDirectActionUrl(){
		 ...
		 if (getComponentInstance().getComponentDefinition().getHasTabContainer()) {
			 sb.append("&tab=");
			 if (getTabOperationComponentInstance()!=null) {
				 sb.append(getComponentInstance().getComponentDefinition().getWOComponent().getTabWidgetForTabComponent(getTabComponent()).getTabKeyForGenerator());
			 }
		 }
		 return sb.toString();
	 }
	...
	public ApplicationHelpEntryPoint getParentHelpEntry() {
		 return getAbstractActivityNode() instanceof ApplicationHelpEntryPoint ? (ApplicationHelpEntryPoint)getAbstractActivityNode():null;
	 }
