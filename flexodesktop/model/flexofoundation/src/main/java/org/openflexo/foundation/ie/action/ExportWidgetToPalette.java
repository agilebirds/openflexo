...
public class ExportWidgetToPalette extends FlexoAction 
{

    private String _widgetName;
    
    public static FlexoActionType actionType = new FlexoActionType ("export_widget_to_palette",FlexoActionType.helpGroup) {
        ...
        {
            return object instanceof IEWidget;
        }

        @Override
		protected boolean isEnabledForSelection(FlexoModelObject object, Vector globalSelection) 
        {
            return (object instanceof IEWidget && !(object instanceof IEReusableWidget) &&  !(object instanceof IESequenceTR) &&  !(object instanceof IETRWidget)) && (globalSelection==null || globalSelection.size()==0||(globalSelection.size()==1 && globalSelection.firstElement()==object));
        }
                
    };
    
    ExportWidgetToPalette (FlexoModelObject focusedObject, Vector globalSelection, FlexoEditor editor)
    {
    ...
    public IEWidget getWidget(){
    	if (widget==null)
	        if (getFocusedObject() instanceof IEOperator) {
	        	...
	        	widget = (IEWidget) getFocusedObject();
        return widget;
    }
