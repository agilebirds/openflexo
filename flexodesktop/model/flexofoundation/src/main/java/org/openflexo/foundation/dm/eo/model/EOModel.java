...
public class EOModel extends EOObject
{
    ...
    {
    	if(getFile()==null || !getFile().exists())return;
    ...
    /**
     * Returns the entity named <code>name</code>. This methods looks in the
     * model and all the other registered models of this model group.
     * 
     * @param name -
    ...
    /**
     * Returns the entity name <code>name</code> if it is found in this model.
     * This method can be used by third-party but is first intended toward the
     * model.
     * 
     * @param name -
    ...
    public void addEntity(EOEntity entity)
    {
        ...
        if (entityNamedIgnoreCase(entity.getName()) != null)
            throw new IllegalArgumentException("An other entity named " + entityNamedIgnoreCase(entity.getName()).getName() + " already exists in model " + getName());
        entities.add(entity);
