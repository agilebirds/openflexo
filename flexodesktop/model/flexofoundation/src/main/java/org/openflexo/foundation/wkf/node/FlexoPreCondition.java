...
/**
 * A FlexoPreCondition is a recepient container for tokens that came along
 * TokenEdge linked to it.<BR>
 * <U>Main attributes</u><BR>
 * <B>initTokenNbr</B> The number of tokens that are initially found in the
 * container.(Usually 0)<BR>
 *
 * @author benoit
 */
public final class FlexoPreCondition extends Node implements InspectableObject, DeletableObject, LevelledObject, ExecutableWorkflowElement
{
	...
	public String getClassNameKey()
	 ...
	 {
		 if (_executionComputingFactory != null) return _executionComputingFactory.getControlGraph(this);
		 return null;
	 }
		...
		public ValidationIssue<PreConditionMustBeAttachedToANode,FlexoPreCondition> applyValidation(FlexoPreCondition pre)
		 {
			 if (pre.getAttachedNode() == null) {
				 return new ValidationError<PreConditionMustBeAttachedToANode,FlexoPreCondition>(this, pre, "pre_condition_is_not_attached_to_a_node",
		...
		public ValidationIssue<PreConditionMustHaveIncomingEdges, FlexoPreCondition>applyValidation(FlexoPreCondition pre)
		 {
			 if (!pre.hasIncomingPostConditions()) {
				 return new ValidationError<PreConditionMustHaveIncomingEdges, FlexoPreCondition>(this, pre, "pre_condition_has_no_incoming_edges", new DeletionFixProposal<PreConditionMustHaveIncomingEdges, FlexoPreCondition>(
				 "delete_this_pre_condition"));
			 }
			 return null;
		 }
	 }

	 public static class PreConditionMustBeLinkedToABeginNode extends ValidationRule<PreConditionMustBeLinkedToABeginNode, FlexoPreCondition>
	 {
		...
		public ValidationIssue<PreConditionMustBeLinkedToABeginNode, FlexoPreCondition> applyValidation(FlexoPreCondition pre)
		 {
			 if ((pre.getAttachedNode() != null
					 ...
					 && (pre.getAttachedNode() instanceof FatherNode) && (!(pre.getAttachedNode() instanceof AbstractActivityNode)|| ((AbstractActivityNode)pre.getAttachedNode()).mightHaveOperationPetriGraph())) {
				 ValidationError<PreConditionMustBeLinkedToABeginNode, FlexoPreCondition> error = null;
				 if (pre.getAttachedBeginNode() == null) {
					 error = new ValidationError<PreConditionMustBeLinkedToABeginNode, FlexoPreCondition>(this, pre, "pre_condition_is_not_linked_to_a_begin_node");
				 } else if (pre.getAttachedBeginNode().getParentPetriGraph() == null) {
					 error = new ValidationError<PreConditionMustBeLinkedToABeginNode, FlexoPreCondition>(this, pre, "pre_condition_is_linked_to_a_begin_node_outside_a_petri_graph");
				 } else if (pre.getAttachedNode() instanceof SelfExecutableNode) {
					 if (pre.getAttachedBeginNode().getParentPetriGraph() != ((SelfExecutableNode)pre.getAttachedNode()).getExecutionPetriGraph()) {
						 error = new ValidationError<PreConditionMustBeLinkedToABeginNode, FlexoPreCondition>(this, pre, "pre_condition_is_linked_to_a_begin_node_related_to_wrong_petri_graph");
					 }
				 } else if (pre.getAttachedBeginNode().getParentPetriGraph() != ((FatherNode)pre.getAttachedNode()).getContainedPetriGraph()) {
					 error = new ValidationError<PreConditionMustBeLinkedToABeginNode, FlexoPreCondition>(this, pre, "pre_condition_is_linked_to_a_begin_node_related_to_wrong_petri_graph");
				 }
				 if (error != null) {
			...
			protected void fixAction()
			 {
				 ...
				 FatherNode attachedNode = (FatherNode) pre.getAttachedNode();
				 FlexoPetriGraph pg = attachedNode instanceof SelfExecutableNode?((SelfExecutableNode)pre.getAttachedNode()).getExecutionPetriGraph():attachedNode.getContainedPetriGraph();
