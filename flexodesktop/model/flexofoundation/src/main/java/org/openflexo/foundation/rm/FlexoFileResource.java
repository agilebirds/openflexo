...
/**
 * This class represents a File Flexo resource. A File FlexoResource represent
 * an object handled by Flexo Application Suite (all concerned modules), which
 * could be stored in a File, generally located in related {@link FlexoProject}
 * project directory.
 *
 * @author sguerin
 */
public abstract class FlexoFileResource<RD extends FlexoResourceData> extends FlexoResource<RD> implements Serializable
{
     ...
     * effectively write a file on disk after the date it was requested. If file
     * is written after this delay, the ResourceManager will interprete it as a
     * concurrent file modification requiring to be handled properly. In fact,
     * this is not a big problem but resource management may be affected.
     */
            ...
            if (res!=null && res!=this) {
            	if (logger.isLoggable(Level.SEVERE)){
                    logger.severe("The name " + ((FlexoFileResource) res).getResourceFile().getStringRepresentation()
                            + " is equivalent to " + aFile.getStringRepresentation()
                            + " and can therefore not be used twice!!");
    ...
    public final synchronized Date getDiskLastModifiedDate()
    {
        if ((_diskLastModifiedDate == null || _diskLastModifiedDate.getTime() == 0 || (isConnected() && !getFile().exists())) && !_isSaving) {
            ...
            if (_diskLastModifiedDate.getTime() > _lastWrittenOnDisk.getTime() + ACCEPTABLE_FS_DELAY) {
                ...
                // Since we are in this block (diskLastModified was null see the top 'if'), we consider that it is some kind of bug in the FS
                // and we update accordingly so that the resource checking thread won't think that the resource was updated by another application
                _lastWrittenOnDisk = _diskLastModifiedDate;
...
//                            + (new SimpleDateFormat("dd/MM HH:mm:ss SSS")).format(fsDate));
    ...
    /**
     * This method should be used parsimoniously since RM will not detect disk
     * updates. It should only be used in few cases, eg when converting
    ...
    public class FileWritingLock extends Thread
    {
		...
		public void run()
		{
			...
			while ((new Date()).getTime() <= startChecking.getTime()+ACCEPTABLE_FS_DELAY
					&& !fileHasBeenWritten) {
    ...
    public synchronized boolean hasWritePermission()
    {
        return (getFile()==null
        		|| ((!getFile().exists())
        				|| (getFile().canWrite()))
        				&& (getFile().getParentFile() != null)
        				&& (!getFile().getParentFile().exists()
        						|| getFile().getParentFile().canWrite()));
    }

    /**
     * Returns boolean indicating if merge scheme is implemented for this kind
     * of resource If set to true, method #performMerge() must be overriden in
     * related class
     *
     * @return
     */
    ...
    /**
     * Perform merge between data read from a updated resource on disk and data
     * stored in memory NOTE: must be overriden in subclasses if relevant
     */
    ...
    /**
     * Override this if required. Default behaviour is to return false, and
     * thus, never propagate a ResourceUpdateNotification through synchronized
     * resources
     *
    ...
    /**
     * Override this if required. Default behaviour is to propagate
     * ResourceUpdateNotification to all loaded storage resources
     *
     * @param originResource
     * @param targetResource
     * @return
