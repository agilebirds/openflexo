...
public class BindingDefinition extends FlexoModelObject implements InspectableObject, Typed
{
    ...
    public BindingDefinition(FlexoModelObject owner)
    {
        ...
        _owner = owner;
        if (owner!=null) _project = owner.getProject();
    }
    ...
    {
    	if (_variableName == null) return "null";
    ...
    private Vector<BindingValue> searchMatchingBindingValue (Bindable bindable, BindingValue current, DMType currentType, int depth)
    {
        ...
        if (getType() == null) return returned;
        if (currentType == null) return returned;
        if (getType().getBaseEntity() == null) return returned;
    ...
    public boolean supportDirectActionEncoding()
    {
    	...
    	if(getType().isString())return true;
    	if(getType().isInteger())return true;
    	if(getType().isChar())return true;
    	if(getType().isBoolean())return true;
    ...
    private boolean isASerializablePrimaryKey(DMEOAttribute pk){
    	if(pk.getType()==null)return false;
    	if(pk.getType().isString())return true;
    	if(pk.getType().isInteger())return true;
    	return false;
    }
    ...
    public String getTypeStringRepresentation()
    {
    	...
    	else return getType().getSimplifiedStringRepresentation();
     }
