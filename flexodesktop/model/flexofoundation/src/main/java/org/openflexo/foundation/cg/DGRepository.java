...
public class DGRepository extends GenerationRepository {
	...
	private static final Logger logger = FlexoLogger
			.getLogger(DGRepository.class.getPackage().getName());

	protected DocType docType;

	protected String postProductName;
	...
	protected ProjectExternalRepository postBuildRepository;
	...
	protected Vector<CGRepository> repositoriedUsingAsReader;
	 ...
	 * 
	 */
	public DGRepository(GeneratedDoc generatedDoc, String name,
			DocType docType, Format format, File directory)
			throws DuplicateCodeRepositoryNameException {
	...
	protected void deleteExternalRepositories() {
		if (getPostBuildRepository() != null) {
			getProject().removeFromExternalRepositories(
					getPostBuildRepository());
		}
	...
	public boolean isEnabled() {
		return super.isEnabled()
				&& (getFormat() == Format.HTML || getTocRepository() != null);
	}
	...
	public ProjectExternalRepository getPostBuildRepository() {
		if (postBuildRepository == null) {
			postBuildRepository = getProject().getExternalRepositoryWithKey(
					getName() + getFormat().getPostBuildKey());
			if (postBuildRepository == null)
				postBuildRepository = getProject()
						.setDirectoryForRepositoryName(
								getName() + getFormat().getPostBuildKey(),
								getDirectory() != null ? getDirectory()
										.getParentFile() : new File(System
										.getProperty("user.home")
										+ "/"
										+ getFormat().getPostBuildKey()
										+ "/"
										+ getName()));
			if (postBuildRepository.getDirectory() == null)
				postBuildRepository
						.setDirectory(getDirectory() != null ? getDirectory()
								.getParentFile() : new File(System
								.getProperty("user.home")
								+ "/"
								+ getFormat().getPostBuildKey()
	...
	protected String docTypeAsString;
	...
	public void setPostBuildDirectory(File selectedFile) {
		if (getPostBuildRepository() != null) {
			...
			setChanged();
			notifyObservers(new CGDataModification("pdfDirectory", old,
					selectedFile));
	...
	public String getPostProductName() {
		if (postProductName == null) {
			postProductName = getName()
					+ getFormat().getPostBuildFileExtension();
		}
	...
	public void setSystemVersion(String systemVersion) {
		...
		lastUpdateDate = new Date();
		setChanged();
		notifyObservers(new CGDataModification("systemVersion", null,
				systemVersion));
	}
	...
	public File getPostBuildFile() {
		if (!getPostProductName().endsWith(
				getFormat().getPostBuildFileExtension())) {
			setPostProductName(getPostProductName()
					+ getFormat().getPostBuildFileExtension());
		}
    ...
    */
	...
	public void setTocRepository(TOCRepository tocRepository) {
		...
		if (tocRepository != null) {
			tocRepositoryRef = new FlexoModelObjectReference<TOCRepository>(
					getProject(), tocRepository);
		} else {
			tocRepositoryRef = null;
		}
		setChanged();
	...
	public void setTocRepositoryRef(
			FlexoModelObjectReference<TOCRepository> tocRepositoryRef) {
