...
public abstract class ValidationModel extends FlexoListModel
{
    ...
    /**
     * Validate supplied Validable object by returning boolean indicating if
     * validation throw errors (warnings are not considered as invalid model).
     * 
     * @param object
     * @return
     */
    ...
    /**
     * Validate supplied Validable object by building and returning a new
    ...
    /**
     * Validate supplied Validable object by appending ValidationIssues object
     * to supplied ValidationReport. Return true if no validation issues were
        ...
        if (issue != null) {
			...
			if (fixAutomaticallyIfOneFixProposal()) {
				if (issue instanceof ProblemIssue && ((ProblemIssue<R,V>) issue).getSize() == 1) {
					...
					report.addToValidationIssues(new InformationIssue<R,V>(next, FlexoLocalization.localizedForKey("fixed_automatically:") + " " + issue.getLocalizedMessage() + " : " + ((ProblemIssue<R,V>) issue).getElementAt(0).getLocalizedMessage(), false));
					...
					for (ValidationIssue<R,V> containedIssue : ((CompoundIssue<R,V>) issue).getContainedIssues()) {
						if ((containedIssue instanceof ProblemIssue) && (containedIssue.getSize() == 1)) {
							...
							report.addToValidationIssues(new InformationIssue<R,V>(containedIssue.getObject(), FlexoLocalization.localizedForKey("fixed_automatically:") + " " + containedIssue.getLocalizedMessage() + " : " + ((ProblemIssue<R,V>) containedIssue).getElementAt(0).getLocalizedMessage(), false));
						}
					}
				}
			}
    ...
    protected void unregisterRule(ValidationRule validationRule)
    {
     ...
     /**
     * Return all known ValidationRule object known for supplied object type
     * (supports basic class inheritance scheme)
     * 
     * @param objectType
     * @return
     */
        ...
        if (_inheritedRules.get(objectType) != null) {
            ...
            while (current.getSuperclass() != null) {
                if (_inheritedRules.get(current.getSuperclass()) != null) {
                    ValidationRuleSet newRuleSet = new ValidationRuleSet(objectType, _inheritedRules.get(current.getSuperclass()).getRules());
    ...
    /**
     * Internally used to maintain association between classes and a related set
        ...
        for (Enumeration e1 = _rules.keys(); e1.hasMoreElements();) {
            ...
            if (_project != null) {
                for (Enumeration e2 = rulesForType.elements(); e2.hasMoreElements();) {
                    ...
                    if ((!rule.isValidForTarget(getTargetType()))
                            || ((filter != null) && !(filter.accept(rule)))) {
