...
public class DropIEElement extends FlexoAction<DropIEElement, IEObject, IEObject>
{
    ...
    {
        logger.info(new StringBuffer("Insert IE element index=").append(getIndex()).append(" row=").append(getRow()).append(" col=").append(getCol()).toString());
        ...
        if (droppedWidget instanceof IEButtonWidget) {
        	if (((IEButtonWidget)droppedWidget).getImageName()!=null && ((IEButtonWidget)droppedWidget).getImageName().toLowerCase().indexOf("help")>-1) {
    ...
    public static DropIEElement insertWidgetInTable(IEHTMLTableWidget table, WidgetType widgetType, int row, int col, int index, FlexoEditor editor)
    {
