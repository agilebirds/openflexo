...
public class DMEntity extends DMObject implements DMGenericDeclaration, DMTypeOwner, Typed
{
     ...
     */
    public DMEntity(DMModel dmModel, String name, String packageName, String className,
            DMType parentType)
    {
        this(dmModel);
        this.name = name;
    ...
    public void setEntityClassName(String newEntityClassName) throws DuplicateClassNameException, InvalidNameException
    {
       /* if (entityClassName == null) {
        ...
        if (!isDeserializing()
        		&& ((newEntityClassName==null) || !DMRegExp.ENTITY_NAME_PATTERN.matcher(newEntityClassName).matches())) {
    ...
    public void setEntityPackageName(String anEntityPackageName)
    {
        ...
        if ((oldEntityPackageName == null)
        		|| (anEntityPackageName==null)
        		|| !oldEntityPackageName.equals(anEntityPackageName)) {
        	entityPackageName = anEntityPackageName;
            setChanged();
            notifyObservers(new DMAttributeDataModification("entityPackageName", oldEntityPackageName,
                    entityPackageName));
        }
    }
   ...
   /* private String parentTypeAsString;
    ...
    /**
     * Return a vector of up-to-date and ordered DMMethod vector representing
     * all methods declared for this entity
     *
    ...
    /**
     * Return a vector of accessible DMMethods. An accessible mehod is defined
     * as a method defined in this entity of somewhere in an ancestor entity.
     *
    ...
    {
    	if ((method._getRegisteredWithSignature() != null)
    			&& !method._getRegisteredWithSignature().equals(methodSignature)
    ...
    public boolean registerMethod(DMMethod method, boolean notify)
    {
        if (methods.get(method.getSignature()) == null) {
            ...
            if (logger.isLoggable(Level.WARNING)) {
				logger.warning("Trying to redefine method " + method.getSignature()
     ...
     * @return true if method has been effectively un-registered, false
     *         otherwise
     */
    ...
    /**
     * Return method whose signature matches 'methodSignature' explicitely
    ...
    /**
     * Return method whose signature matches 'methodSignature' accessible from
     * this entity. This method allows to access inherited methods from parent
     * classes.
     *
    ...
    /**
     * Return a vector of up-to-date and ordered DMProperty vector representing
     * all properties declared for this entity
     *
   ...
   /**
     * Return a vector of accessible DMProperties An accessible property is
     * defined as a property defined in this entity of somewhere in an ancestor
     * entity. If this entity is a DMEOProperty, it must be a class property.
     *
     * @return
     */
         ...
         if (properties.get(property.getName()) == null) {
            ...
            if (logger.isLoggable(Level.WARNING)) {
                 logger.warning("You are trying to redefine property "+property.getName()+"("+property.getType()+") "
                		+" but property "+property.getName()+"("+properties.get(property.getName()).getType()+") is already existing");
            }
        }
        return false;
    }
     ...
     * @return true if property has been effectively un-registered, false
     *         otherwise
     */
    ...
    /**
     * Return property named 'propertyName' accessible from this entity. This
     * method allows to access inherited properties from parent classes.
     *
     * @param propertyName
     * @return
     */
    ...
    /**
     * Returns boolean indicating if this entity is an ancestor of supplied
     * entity. Returns true if entity equals this entity, or if this entity is a
     * parent of supplied entity (parent class or parent interface)
     *
     * Note that both 'extends' and 'implements' schemes are implemented by
     * this method
     *
     * @param entity
     * @return a boolean
    ...
    /**
     * Returns most direct ancestor of supplied entity, on it's way to this type as ancestor,
     * assuming this type is an ancestor of supplied entity
     *
     * @param entity
     * @return
     */
...
//        }
    ...
    {
        String returned = "Name = " + getName() + " inherits from "
                + (getParentType() != null ? getParentType().getName() : "none") + "\n";
    ...
    protected synchronized void reorderMethods()
    {
        if (methodsNeedsReordering) {
            ...
            while (currentParent != null) {
                for (Enumeration en = currentParent.getAccessibleMethods().elements(); en
                        .hasMoreElements();) {
                    DMMethod method = (DMMethod) en.nextElement();
    ...
    protected synchronized void reorderProperties()
    {
        if (propertiesNeedsReordering) {
            ...
            DMEntity currentParent = getParentBaseEntity();
            while (currentParent != null) {
                for (Enumeration en = currentParent.getAccessibleProperties().elements(); en
                        .hasMoreElements();) {
                    DMProperty property = (DMProperty) en.nextElement();
                    boolean isOverriding = false;
   ...
   public String getTypesVariablesAsString()
   {
    ...
    public static class EntityInPackageCannotUseEntityOfTheDefaultPackage extends ValidationRule<EntityInPackageCannotUseEntityOfTheDefaultPackage, DMEntity> {
		...
		public ValidationIssue<EntityInPackageCannotUseEntityOfTheDefaultPackage, DMEntity> applyValidation(DMEntity entity) {
	    	...
	    	if (v.size()>0) {
	    		StringBuilder sb = new StringBuilder(FlexoLocalization.localizedForKey("the_following_entities_are_in_the_default_package_and_cannot_be_used_by")+" "+entity.getFullQualifiedName()).append(": ");
	    		for (DMEntity e : v) {
					sb.append(e.getName()).append(" ");
				}
	    		ValidationError<EntityInPackageCannotUseEntityOfTheDefaultPackage, DMEntity> vr = new ValidationError<EntityInPackageCannotUseEntityOfTheDefaultPackage, DMEntity>(this,entity,sb.toString());
