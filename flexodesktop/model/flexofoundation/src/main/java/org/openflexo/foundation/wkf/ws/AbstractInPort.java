...
/**
 * Abstract representation of a IN port associated to a PortRegistery associated
 * to a FlexoProcess: such a port receive message and could be of type IN
 * (normal), NEW (new instance) or DELETE (delete instance)
 * 
 * @author sguerin
 * 
 */
public abstract class AbstractInPort extends FlexoPort 
{
    ...
    public void setInputMessageDefinition(MessageDefinition aMessageDefinition)
    {
        if (aMessageDefinition != _inputMessageDefinition) {
            if (aMessageDefinition != null) aMessageDefinition.setPort(this);
