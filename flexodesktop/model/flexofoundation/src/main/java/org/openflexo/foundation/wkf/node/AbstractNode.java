...
/**
 * A AbstractNode is the base element representing a node in a PetriGraph. A
 * AbstractNode is abstract and must be subsequently subclassed with FlexoNode,
 * OperatorNode and FlexoPort
 *
 * @author sguerin
 */
public abstract class AbstractNode extends WKFNode implements InspectableObject, DeletableObject, LevelledObject
{
	 ...
	 */
	 ...
	 {
		if (!_incomingPostConditions.contains(post) && post.getEndNodeClass().isAssignableFrom(getClass()) && mayHaveIncomingPostConditions()) {
	...
	public void addToOutgoingPostConditions(FlexoPostCondition post) {
		if (!_outgoingPostConditions.contains(post) && post.getStartNodeClass().isAssignableFrom(getClass()) && mayHaveOutgoingPostConditions()) {
	...
	public static class NodeShouldHaveNonAmbigousName extends ValidationRule<NodeShouldHaveNonAmbigousName,AbstractNode>
	{
		...
		public ValidationIssue<NodeShouldHaveNonAmbigousName,AbstractNode> applyValidation(final AbstractNode node)
		{
			...
			if (nodesWithSameName.size() > 0) {
				ValidationWarning<NodeShouldHaveNonAmbigousName,AbstractNode> warning = new ValidationWarning<NodeShouldHaveNonAmbigousName,AbstractNode>(this, node, "node_($object.name)_has_ambigous_name");
	...
	public static class NodeCannotHaveMoreThanOneDefaultOutgoingTokenEdge extends ValidationRule<NodeCannotHaveMoreThanOneDefaultOutgoingTokenEdge,AbstractNode>
	{
		...
		public ValidationIssue<NodeCannotHaveMoreThanOneDefaultOutgoingTokenEdge,AbstractNode> applyValidation(final AbstractNode node)
		{
			...
			for(FlexoPostCondition p:node.getOutgoingPostConditions()){
				if(p instanceof TokenEdge && p.getIsDefaultFlow()){
					if(hasAlreadyOne){
						ValidationWarning<NodeCannotHaveMoreThanOneDefaultOutgoingTokenEdge,AbstractNode> warning = new ValidationWarning<NodeCannotHaveMoreThanOneDefaultOutgoingTokenEdge,AbstractNode>(this, node, "node_($object.name)_has_more_than_one_default_outgoing_edge");
						return warning;
					}
					else hasAlreadyOne = true;
				}
			}
			return null;
		}
	...
	public static class NodeWithConditionalEdgeOrDefaultEdgeMustHaveMoreThanOneEdge extends ValidationRule<NodeWithConditionalEdgeOrDefaultEdgeMustHaveMoreThanOneEdge,AbstractNode>
	{
		...
		@Override
		public ValidationIssue<NodeWithConditionalEdgeOrDefaultEdgeMustHaveMoreThanOneEdge,AbstractNode> applyValidation(final AbstractNode node)
		{
			int defaultCount = 0;
			...
			if(defaultCount>0 && (regular+conditionalCount) == 0) {
				if (node instanceof OperatorNode && ((OperatorNode)node).isExclusiveGateway()) {
					...
					ValidationWarning<NodeWithConditionalEdgeOrDefaultEdgeMustHaveMoreThanOneEdge,AbstractNode> warning = new ValidationWarning<NodeWithConditionalEdgeOrDefaultEdgeMustHaveMoreThanOneEdge,AbstractNode>(this, node, "node_($object.name)_has_one_default_outgoing_edge_should_have_at_least_another_outgoing_edge");
					return warning;
				}
			}
			if(conditionalCount==1 && regular == 0 && defaultCount==0 && !listOfAllPostToConsider.firstElement().mustBeConditional()) {
				if (node instanceof OperatorNode && ((OperatorNode)node).isExclusiveGateway()) {
					...
					ValidationWarning<NodeWithConditionalEdgeOrDefaultEdgeMustHaveMoreThanOneEdge,AbstractNode> warning = new ValidationWarning<NodeWithConditionalEdgeOrDefaultEdgeMustHaveMoreThanOneEdge,AbstractNode>(this, node, "node_($object.name)_has_one_conditional_outgoing_edge_should_have_at_least_another_outgoing_edge");
					return warning;
				}
			}
			return null;
	...
	public static class NodeWithDefaultFlowMustHaveConditionOneOtherEdge extends ValidationRule<NodeWithDefaultFlowMustHaveConditionOneOtherEdge,AbstractNode>
	{
		...
		public ValidationIssue<NodeWithDefaultFlowMustHaveConditionOneOtherEdge,AbstractNode> applyValidation(final AbstractNode node)
		{
			...
			if(defaultCount>0 && totalCount!=defaultCount+conditionalCount) {
				if (node instanceof OperatorNode && ((OperatorNode)node).isExclusiveGateway()) {
					...
					ValidationWarning<NodeWithDefaultFlowMustHaveConditionOneOtherEdge,AbstractNode> warning = new ValidationWarning<NodeWithDefaultFlowMustHaveConditionOneOtherEdge,AbstractNode>(this, node, "node_($object.name)_has_one_default_outgoing_flow_and_so_other_edges_must_be_conditionnal");
					return warning;
				}
			}
			return null;
