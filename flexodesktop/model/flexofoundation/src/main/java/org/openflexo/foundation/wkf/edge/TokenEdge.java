...
/**
 * Edge linking 2 FlexoNode in the context of petri graphs (carry tokens)
 * <U>Main attributes</U><BR>
 * <B>Token increment</B>: the number of tokens that are sent to the next
 * precondition when the node is executed.<BR>
 * <B>Delay</B>: the time taken by the tokens to go to the next precondition
 * (usually: no delay).<BR>
 * 
 * @author sguerin
 * 
...
public final class TokenEdge extends FlexoPostCondition<PetriGraphNode,Node> implements Bindable
{
    ...
    public TokenEdge(PetriGraphNode startNode, Node endNode, FlexoProcess process) throws InvalidEdgeException
    {
       ...
       if (!isEdgeValid()) {
        ...
        if ((startNode instanceof ActionNode) 
                && (endNode.getNode() instanceof ActionNode)
                && (((ActionNode)endNode.getNode()).isEndNode())) {
            ActionNode s = (ActionNode)startNode;
            ActionNode e = (ActionNode)endNode.getNode();
            if ((s.getOperationNode() == e.getOperationNode())
                    && (s.getOperationNode() != null)) {
	...
	public FlexoLevel getLevel()
    {
        if (getStartNode() != null) {
        	if (getStartNode() instanceof FlexoNode && ((FlexoNode)getStartNode()).isEndNode()) {
        		if (getStartNode() instanceof OperationNode) return FlexoLevel.ACTIVITY;
           		if (getStartNode() instanceof ActionNode) return FlexoLevel.OPERATION;
        	}
    ...
    public static class TokenEdgeMustBeValid extends ValidationRule<TokenEdgeMustBeValid,TokenEdge>
    {
        ...
        {
            if (!edge.isEdgeValid()) {
                ValidationError<TokenEdgeMustBeValid,TokenEdge> error = new ValidationError<TokenEdgeMustBeValid,TokenEdge>(this, edge, "token_edge_is_not_valid");
    ...
    public static class TokenEdgeShouldHaveNonNullTokenIncrement extends ValidationRule<TokenEdgeShouldHaveNonNullTokenIncrement,TokenEdge>
    {
		...
		public ValidationIssue<TokenEdgeShouldHaveNonNullTokenIncrement,TokenEdge> applyValidation(TokenEdge edge)
        {
            if (edge.getTokenIncrem() == 0) {
                ValidationWarning<TokenEdgeShouldHaveNonNullTokenIncrement,TokenEdge> warning = new ValidationWarning<TokenEdgeShouldHaveNonNullTokenIncrement,TokenEdge>(this, edge, "token_edge_has_a_null_token_increment");
