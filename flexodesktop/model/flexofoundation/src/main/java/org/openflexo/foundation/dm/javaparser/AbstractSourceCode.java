...
public abstract class AbstractSourceCode extends TemporaryFlexoModelObject 
{
	...
	{
		if ((warning == null && _parseErrorWarning != null)
				|| (warning != null && !warning.equals(_parseErrorWarning))) {
	...
	public String getCode() 
	{
		...
		if (getOwner().isSerializing() && !isEdited()) return null;
    	
    	if (isEdited()) return _editedCode;
	...
	/**
	 * Sets code this method represents, and when not deserializing,
	 * update owner object with informations contained in
	 * supplied qualified code.
	 * 
	 * @param qualifiedCode
	...
	/**
	 * Sets code this method represents, and if parseCode flag is set to true,
	 * update owner object with informations contained in
	 * supplied qualified code.
	 * 
	 * @param someCode
	...
	public void setCode(String someCode, boolean parseCode) throws ParserNotInstalledException, DuplicateMethodSignatureException 
	{
		...
		someCode = getCleanCode(someCode);
		if (_computedCode == null && getOwner().codeIsComputable()) _computedCode = makeComputedCode();
	...
	public void replaceJavadocInEditedCode (String newJavadoc)
	{
		...
		if (javadocBeginIndex > -1) {
			...
			_editedCode = _editedCode.substring(0,javadocBeginIndex)
			+ newJavadoc
			+ _editedCode.substring(endJavadocIndex);
		}
		else {
