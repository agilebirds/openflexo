...
/**
 * Represents a connex sub-graph composed of Role elements linked with
 * RoleSpecialization. Note that a RoleCompound is created
...
public final class RoleCompound extends WorkflowModelObject
{
    ...
    private void makeConnex(RoleCompound clone, Vector<Role> originalSelectedRoles)
    {
		...
		while (i.hasNext()) {
			...
			if (rs.getRole()==null || rs.getParentRole()==null || !originalSet.contains(rs.getRole()) || !originalSet.contains(rs.getParentRole())) {
				rs.delete();
			}
		}
	}
    	...
    	for (Role role : getRoles()) {
         	if (upperLeftCorner==null) {
        		...
        		upperLeftCorner.setLocation(
        				Math.min(upperLeftCorner.getX(), role.getX(context)),
