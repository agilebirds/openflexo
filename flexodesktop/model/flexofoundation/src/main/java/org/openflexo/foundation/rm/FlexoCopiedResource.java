 ...
 *
 */
public class FlexoCopiedResource extends CGRepositoryFileResource<CopiedFileData,IFlexoResourceGenerator, CGFile> implements GenerationAvailableFileResourceInterface, FlexoObserver
{
    ...
    public String getName()
    {
        if (getResourceToCopy() != null) {
            ...
            if ((getFileName() != null) && (getResourceToCopy().getFileName() != null) && !getFileName().equals(getResourceToCopy().getFileName())
                    && !isDeleted()) {
				try {
    ...
    public boolean checkIntegrity() {
    	if (!isDeleted() && !project.isDeserializing() && (getProject()!=null) && (getProject().getResourceManagerInstance()!=null) && !getProject().getResourceManagerInstance().isLoadingAProject()) {
            if (resourceToCopy == null) {
                ...
                if (logger.isLoggable(Level.INFO)) {
					logger.info("This copied resource is no more acceptable because resourceToCopy "+resourceToCopy.getFullyQualifiedName()+" does not pass integrity check");
				}
                return false;
	...
	public void update(FlexoObservable observable, DataModification dataModification) {
		if ((dataModification instanceof ResourceRemoved)
				&& (((ResourceRemoved)dataModification).getRemovedResource()==resourceToCopy)) {
			if (getCGFile()==null) {
				this.delete();
			} else {
