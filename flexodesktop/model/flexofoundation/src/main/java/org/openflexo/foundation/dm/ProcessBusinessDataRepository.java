...
public class ProcessBusinessDataRepository extends DMRepository {
	...
	public void generateProcessBusinessData(List<AutoGeneratedProcessBusinessDataDMEntity> entitiesToGenerate) {
		...
		while (processes.hasMoreElements()) {
			...
			if (!process.isImported()) {
				...
				if (process.getBusinessDataType() == null && entitiesToGenerate == null) {
					processBusinessDataEntity = getAutoGeneratedProcessBusinessDataDMEntity(process);
				} else if (process.getBusinessDataType() instanceof AutoGeneratedProcessBusinessDataDMEntity && (entitiesToGenerate == null || entitiesToGenerate
						.contains(process.getBusinessDataType()))) {
		...
		for (DMEntity entity : new ArrayList<DMEntity>(getBusinessDataPackage().getEntities())) {
			if (entity instanceof AutoGeneratedProcessBusinessDataDMEntity && !keptEntitiesWithProperties.containsKey(entity) && (entitiesToGenerate == null || entitiesToGenerate
					.contains(entity))) {
				entity.delete();
			}
		}
	...
	public AutoGeneratedProcessBusinessDataDMEntity getAutoGeneratedProcessBusinessDataDMEntity(FlexoProcess process) {
		...
		logger.severe("Cannot generate AutoGeneratedProcessBusinessDataDMEntity for process " + process.getName() + " because another DMEntity already exists with the same name");
		return null;
	}
