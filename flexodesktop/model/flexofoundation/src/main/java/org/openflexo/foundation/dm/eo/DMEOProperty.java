...
public abstract class DMEOProperty extends DMProperty implements DMEOObject
{
    ...
    public boolean getIsStaticallyDefinedInTemplate()
    {
    	...
    	// GPO: Well, now that you have changed everything, of course, it is changed, but before it was mandatory that this method returned true, trust me.
        return false;
    }
    /*public boolean getIsStaticallyDefinedInTemplate()
    {
    		...
    		if (getDMEOEntity().getGeneratedCode() != null) {
    			if (getDMEOEntity().getGeneratedCode().getParsedClass() != null) {
    				if (getImplementationType().requiresField()) {
    					...
    					if (logger.isLoggable(Level.FINE)) logger.fine("Tried to retrieve field "+getFieldName()+" FOUND: "+generatedCodeField);
    				}
    				if (getImplementationType().requiresAccessors()) {
    					generatedCodeGetter = searchMethod(getDMEOEntity().getGeneratedCode().getParsedClass(),getGetterSignatureCandidates());
    					if (logger.isLoggable(Level.FINE)) logger.fine("Tried to retrieve getter, FOUND: "+generatedCodeGetter);
    					if (getIsSettable()) {
    						generatedCodeSetter = searchMethod(getDMEOEntity().getGeneratedCode().getParsedClass(),getSetterSignatureCandidates());
    						if (logger.isLoggable(Level.FINE)) logger.fine("Tried to retrieve setter, FOUND: "+generatedCodeSetter);
    						if (getCardinality().isMultiple()) {
    							generatedCodeAdditionMethod = searchMethod(getDMEOEntity().getGeneratedCode().getParsedClass(),getAdditionAccessorSignatureCandidates());
    							if (logger.isLoggable(Level.FINE)) logger.fine("Tried to retrieve addition method, FOUND: "+generatedCodeAdditionMethod);
    							generatedCodeRemovalMethod = searchMethod(getDMEOEntity().getGeneratedCode().getParsedClass(),getRemovalAccessorSignatureCandidates());
    							if (logger.isLoggable(Level.FINE)) logger.fine("Tried to retrieve removal method, FOUND: "+generatedCodeRemovalMethod);
    						}
    					}
    				}
    ...
    protected String getFieldDefaultInitializationExpression()
    {
		if (getDMModel().getEOEntityCodeGenerator() == null || getDMEOEntity() == null) {
			return "{"+StringUtils.LINE_SEPARATOR+"// EOEntityCodeGenerator not installed: code not available"+StringUtils.LINE_SEPARATOR+"}";
		}
		else {
			...
			if (codeNeedsToBeRegenerated) regenerateCode();
			if (generatedCodeField != null) return generatedCodeField.getInitializationExpression();
    ...
    protected String getGetterDefaultCoreCode()
	{
		if (getDMModel().getEOEntityCodeGenerator() == null || getDMEOEntity() == null) {
			...
			if (!isCodeGenerationActivated())
				return "{"+StringUtils.LINE_SEPARATOR+"// EOF code generation not activated"+StringUtils.LINE_SEPARATOR+"}";
			if (codeNeedsToBeRegenerated) regenerateCode();
			if (generatedCodeGetter != null) return generatedCodeGetter.getCoreCode();
			return "{"+StringUtils.LINE_SEPARATOR+"// WARNING: could not retrieve generated code for getter"+StringUtils.LINE_SEPARATOR+"}";
		}
	}
    ...
    protected String getSetterDefaultCoreCode()
	{
		if (getDMModel().getEOEntityCodeGenerator() == null || getDMEOEntity() == null) {
			...
			if (!isCodeGenerationActivated())
				return "{"+StringUtils.LINE_SEPARATOR+"// EOF code generation not activated"+StringUtils.LINE_SEPARATOR+"}";
			if (codeNeedsToBeRegenerated) regenerateCode();
			if (generatedCodeSetter != null) return generatedCodeSetter.getCoreCode();
			return "{"+StringUtils.LINE_SEPARATOR+"// WARNING: could not retrieve generated code for setter"+StringUtils.LINE_SEPARATOR+"}";
		}
	}
    ...
    protected String getAdditionAccessorDefaultCoreCode()
	{
		if (getDMModel().getEOEntityCodeGenerator() == null || getDMEOEntity() == null) {
			...
			if (!isCodeGenerationActivated())
				return "{"+StringUtils.LINE_SEPARATOR+"// EOF code generation not activated"+StringUtils.LINE_SEPARATOR+"}";
			if (codeNeedsToBeRegenerated) regenerateCode();
			if (generatedCodeAdditionMethod != null) return generatedCodeAdditionMethod.getCoreCode();
			return "{"+StringUtils.LINE_SEPARATOR+"// WARNING: could not retrieve generated code for addition method"+StringUtils.LINE_SEPARATOR+"}";
		}
	}
    ...
    protected String getRemovalAccessorDefaultCoreCode()
	{
		if (getDMModel().getEOEntityCodeGenerator() == null || getDMEOEntity() == null) {
			...
			if (!isCodeGenerationActivated())
				return "{"+StringUtils.LINE_SEPARATOR+"// EOF code generation not activated"+StringUtils.LINE_SEPARATOR+"}";
			if (codeNeedsToBeRegenerated) regenerateCode();
			if (generatedCodeRemovalMethod != null) return generatedCodeRemovalMethod.getCoreCode();
			return "{"+StringUtils.LINE_SEPARATOR+"// WARNING: could not retrieve generated code for removal method"+StringUtils.LINE_SEPARATOR+"}";
		}
	}
