...
/**
 * Represents a binding value. A binding value MUST refer to a BindingDefinition object, and an owner which is the Bindable objet used to get the BindingModel. The root object defining the binding
 * value is the BindingVariable object, while properties path defined as a Vector defines all the properties leading to accessed value.
 * 
 * @author sguerin
 * 
 */
public class BindingValue extends AbstractBinding {
	...
	public void setsWith(AbstractBinding aValue) {
		...
		if (aValue != null) {
			if (aValue instanceof BindingValue) {
				...
				if ((getBindingVariable() != null) && (getBindingVariable().getType() != null) && (getBindingVariable().getType().getBaseEntity() != null))
	...
	public boolean isBindingValid() {
		...
		if ((getBindingDefinition() != null) && (getBindingDefinition().getIsSettable())) {
			if ((getBindingPathLastElement() == null) || (!(getBindingPathLastElement() instanceof DMProperty)) || (!((DMProperty) getBindingPathLastElement()).getIsSettable())) {
				if (logger.isLoggable(Level.FINE))
	...
	private boolean _checkBindingPathValid() {
		...
		for (int i = 0; i < _bindingPath.size(); i++) {
			...
			if (element.getEntity() == null || currentType.getBaseEntity() == null || !element.getEntity().isAncestorOf(currentType.getBaseEntity()))
				return false;
	...
	public BindingVariable getBindingVariable() {
		if (logger.isLoggable(Level.FINE))
			logger.fine("Binding Variable of bindingValue for Entry:" + (getBindingDefinition() != null ? getBindingDefinition().getVariableName() : null) + " var:"
			...
			for (DMMethod method : possiblyMatchingMethods) {
				...
				for (int i = 0; i < method.getParameters().size(); i++) {
					...
					if ((paramBindingValue != null)) {
						...
						if (type != null && paramBindingValue.getAccessedType() != null && type.isAssignableFrom(paramBindingValue.getAccessedType(), true)) {
							if (logger.isLoggable(Level.FINE))
							...
							if (logger.isLoggable(Level.FINE))
								logger.fine("Lookup on type " + type + " failed (wrong type): " + paramBindingValue.getStringRepresentation() + "types: " + "looking " + type + " found "
				...
				if (st.hasMoreTokens()) {
					...
					if (bv == null) {
						...
						while (st.hasMoreTokens()) {
							...
							if (nextElement == null) {
								if (logger.isLoggable(Level.WARNING) && warnOnFailure)
									logger.warning(("Could not decode BindingValue '" + value + "' : cannot find property nor method matching '" + nextTokenName + "' for type " + currentType + " !"));
								return null;
							} else {
	...
	public void update(FlexoObservable observable, DataModification dataModification) {
		if (logger.isLoggable(Level.FINE))
			logger.fine("update in BindingValue " + getStringRepresentation() + " for " + dataModification.getClass().getName() + " owner is " + getOwner());
		if (dataModification instanceof DMObjectDeleted && getBindingVariable() != null && dataModification.oldValue() == getBindingVariable().getType()) {
	...
	/**
	 * Internal class extending Vector to provide observing management for BindingPathElement Additionnaly, provides generics type resolving by maintaining a resulting type vector
	 * 
	 * @author sguerin
	 * 
	 */
