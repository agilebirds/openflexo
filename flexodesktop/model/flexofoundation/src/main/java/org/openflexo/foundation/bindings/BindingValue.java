...
/**
 * Represents a binding value. A binding value MUST refer to a BindingDefinition
 * object, and an owner which is the Bindable objet used to get the
 * BindingModel. The root object defining the binding value is the
 * BindingVariable object, while properties path defined as a Vector defines all
 * the properties leading to accessed value.
 * 
 * @author sguerin
 * 
 */
public class BindingValue extends AbstractBinding
{
    	...
    	if (aValue != null) {
    		if (aValue instanceof BindingValue) {
    			...
    			if ((getBindingVariable() != null)
    					&& (getBindingVariable().getType() != null)
    ...
    {   	
    	...
    	if ((getBindingDefinition() != null) && (getBindingDefinition().getIsSettable())) {
    		if ((getBindingPathLastElement() == null)
    				|| (!(getBindingPathLastElement() instanceof DMProperty))
    	...
    	if (logger.isLoggable(Level.FINEST)) {
    		...
    		logger.finest("_bindingDefinition.getType().isAssignableFrom(getAccessedType())="+getBindingDefinition().getType().isAssignableFrom(getAccessedType(),true));
    	}
    	...
    	if (logger.isLoggable(Level.FINE))
    		logger.fine("Invalid binding because types are not matching searched "+getBindingDefinition().getType()+" having "+getAccessedType());
    	return false;
    ...
    public DMType getAccessedType()
    {
        ...
        if (_bindingPath.size() == 0) {
        	if (_bindingVariable != null) return _bindingVariable.getType();
        	return null;
        }
         ...
         for (int i=0; i<_bindingPath.size(); i++) {
        	BindingPathElement element = _bindingPath.elementAt(i);
        	 if (element.getEntity() == null 
       			 || currentType.getBaseEntity() == null
    ...
    public void setBindingVariable(BindingVariable bindingVariable)
    {
        ...
        if (bindingVariable != _bindingVariable) {
            ...
            if ((_bindingVariable != null) && (_bindingVariable.getType() != null)) {
               if (_bindingVariable.getType().getBaseEntity()!=null)  _bindingVariable.getType().getBaseEntity().addObserver(this);
            }
    ...
    {
    	unparsableValue = null;
    	if(_bindingPath!=null && _bindingPath.lastElement()!=null && _bindingPath.lastElement().equals(requestedLast))return;
   ...
   public boolean isLastBindingPathElement(BindingPathElement element, int index)
    ...
    public DMType getBindingPathLastElementType(){
    	if(getBindingPath()!=null 
    			...
    			&& getBindingPath().lastElement()!=null)
   ...
   public String getStringRepresentation()
    {
    	if (getUnparsableValue() != null) return getUnparsableValue();
         if (_bindingVariable != null) {
            StringBuilder sb = new StringBuilder(_bindingVariable.getVariableName());
            for (BindingPathElement element : _bindingPath) {
                if (element != null)
    ...
    public boolean isPointingToSessionVariable(){
    	if(_bindingPath.size()!=2)return false;
    ...
    {
    	if (_bindingVariable == null) return "???";
        ...
        // TODO: we must find a way to handle properly those issues
        // by supplying a parameter to that method for example
        if(reply.startsWith("session()."))return "getSession()."+reply.substring(10);
        return reply;
    }
    ...
    {
        if (getBindingDefinition() != null) {
            return new StringBuffer("BINDING_VALUE.").append(getBindingDefinition().getVariableName()).append("=").append(getCodeStringRepresentation()).toString();
        } else {
    ...
    public static class BindingValueStringConverter extends AbstractBindingStringConverter<BindingValue>
    {
        ...
        private MethodCall tryToDecodeAsMethodCall(BindingValue owner, DMType currentType, String aValue)
        {
            ...
            try {
            	...
            	if (parsedExpression instanceof Function) {
            		...
            		if (logger.isLoggable(Level.WARNING) && warnOnFailure)
            			logger.warning("Could not decode BindingValue : trying to find method call matching '" + aValue + " this is not a function call");
            		return null;
            	}
            }
            ...
            for (DMMethod method : allMethods) {
                if ((method.getName() != null) && (method.getName().equals(methodName)) && (method.getParameters().size() == paramsAsString.size()))
    ...
    public void update(FlexoObservable observable, DataModification dataModification)
    {
        ...
        if ((dataModification instanceof EntityDeleted) && (getBindingVariable() != null)
            ...
            if (getBindingPath().contains(propertyDeleted.oldValue())) {
            	setBindingVariable(null);
// For me it is not consistant to keep a part of the binding if one previously selected part has been deleted, thus I prefer to delete the entire binding
...
//                }
    ...
    /**
     * Internal class extending Vector to provide observing management for
     * BindingPathElement
     * Additionnaly, provides generics type resolving by maintaining a resulting type vector
     * 
     * @author sguerin
     * 
     */
    public class BindingPath extends Vector<BindingPathElement>
    {
        ...
        private void setResultingTypeAtIndex (int index)
        {
        	if (_bindingVariable != null) {
        		...
        		if (type == null) {
        			logger.warning("Cannot find resuling type for bindingPathElement "+bpe.getSerializationRepresentation()+ " in path :"+getStringRepresentation());
        			return;
        		}
        		...
        		if (index > 0 && _resultingTypes.size()>=index) parentType = _resultingTypes.elementAt(index-1);
        		else parentType = _bindingVariable.getType();
        		...
        		else if (_resultingTypes.size() == index) _resultingTypes.add(type);
        		else {
        ...
        {
        	if (index < _resultingTypes.size()) return _resultingTypes.elementAt(index);
        	return null;
        }
    }
    ...
    public BindingPathElement findFirstBindingPathElementOfType(DMEntity entity)
    {
    	...
    	while(en.hasMoreElements()){
    		...
    		if(reply.getType().getBaseEntity().equals(entity))return reply;
    	}
    	return null;
    }
	public void addBindingPathElementAfterFirstPathElementOfType(DMEntity entity, DMProperty newProperty) 
	{
		...
		if(beforeLast!=null)removeBindingPathElementAfter(beforeLast);
		else {
			while(getBindingPathLastElement()!=null)getBindingPath().removeAllElements();
		}
	...
	public DMEntity getLastEditableEntity() 
	{
		...
		while(en.hasMoreElements()){
			...
			if(!temp.getIsReadOnly())reply = temp;
		}
		return reply;
	}
	...
	public BindingPathElement getLastBindingPathElementOfType(DMEntity newEntryEntity) 
	{
		...
		while(en.hasMoreElements()){
			temp = en.nextElement();
			if(!temp.getType().getBaseEntity().equals(newEntryEntity))reply = temp;
		}
		return reply;
	}
