...
/**
 * Represents a class and its definition for objects accessible from a Class object.
 * That could be classes accessible in current ClassLoader or classes imported from an
 * external jar-file (see ExternalRepository).
 * 
 * @author sguerin
 * 
 */
public class LoadableDMEntity extends DMEntity
{
     ...
     * Static builder used for dynamic creation
     */
    public static LoadableDMEntity createLoadableDMEntity(/*DMRepository repository,*/ Class aClass, DMModel dataModel, boolean includesGetOnlyProperties, boolean includesMethods)
    {
    ...
    {
        if (javaType == null) javaType = retrieveJavaType();
        return javaType;
    }
	...
	public boolean isAncestorOf(DMEntity entity) {
		...
		if(entityPackageName.equals("java.lang")){
			if (entityClassName.equals("Boolean") && entity.getName().equals("boolean"))return true;
			if (entityClassName.equals("Integer") && entity.getName().equals("int"))return true;
			if (entityClassName.equals("Long") && entity.getName().equals("long"))return true;
			if (entityClassName.equals("Short") && entity.getName().equals("short"))return true;
			if (entityClassName.equals("Float") && entity.getName().equals("float"))return true;
			if (entityClassName.equals("Double") && entity.getName().equals("double"))return true;
			if (entityClassName.equals("Character") && entity.getName().equals("char"))return true;
			if (entityClassName.equals("Byte") && entity.getName().equals("byte"))return true;
		}
    ...
    private void initializeFromClass(boolean includesGetOnlyProperties, boolean includesMethods)
    {
        ...
        for (Type t : javaType.getGenericInterfaces()) {
        	...
        	if (logger.isLoggable(Level.FINE))logger.fine("Implements "+implementsType);
    ...
    {
        for (Enumeration en=searchForMethods(javaType,getDMModel(),getRepository(),true,excludedSignatures).elements(); en.hasMoreElements();) {
    ...
    /**
     * Build a new Vector containing unregistered DMMethod instances, 
     * given flags includesGetOnlyProperties and includesMethods
     */
    public static Vector<DMMethod> searchForMethods(Class type, DMModel dmModel, DMRepository repository,boolean importReferencedEntities, Vector<String> excludedSignatures)
    {
        Vector<DMMethod> returned = new Vector<DMMethod>();

        if (logger.isLoggable(Level.FINE))logger.fine("searchForMethods()");
     ...
     */
    private static DMMethod makeMethod(Class type, String signature, DMModel dmModel, DMRepository repository, boolean importReferencedEntities)
    {
    		...
    		if (gd instanceof Class) {
    			...
    			if (owner != null) {
    				...
    				for (DMTypeVariable tv : owner.getTypeVariables()) {
    					if (tv.getName().equals(typeAsTypeVariable.getName())) typeVariable = tv;
    				}
    				if (typeVariable == null) {
    		       		typeVariable = new DMTypeVariable(dmModel,owner);
    	        		typeVariable.setName(typeAsTypeVariable.getName());
    	                if (typeAsTypeVariable.getBounds().length > 0) typeVariable.setBounds(typeAsTypeVariable.getBounds()[0].toString());
    ...
    private static DMMethod makeMethod(Method method, DMModel dmModel, DMRepository repository, boolean importReferencedEntities)
    {
        ...
        if (Modifier.isPublic(method.getModifiers())) visibility = DMVisibilityType.PUBLIC;
        else if (Modifier.isProtected(method.getModifiers())) visibility = DMVisibilityType.PROTECTED;
        else if (Modifier.isPrivate(method.getModifiers())) visibility = DMVisibilityType.PRIVATE;
        else visibility = DMVisibilityType.NONE;
        ...
        if (parameters != null) {
            for (int j=0; j<parameters.length; j++) {
                ...
                String name = "arg"+j;
                if (paramDMType.getBaseEntity() != null) name = paramDMType.getBaseEntity().getNameAsMethodArgument();
                else if (paramDMType.getTypeVariable() != null) name = paramDMType.getTypeVariable().getName().toLowerCase();
   ...
   private Vector<String> registerProperties (boolean includesGetOnlyProperties)
    {
        ...
        Vector<DMProperty> properties = searchForProperties(javaType,getDMModel(),getRepository(),includesGetOnlyProperties,true,excludedSignatures);
        if (logger.isLoggable(Level.FINE))
    ...
    /**
     * Build a new Vector containing unregistered DMProperty instances, 
     * given flags includesGetOnlyProperties and includesMethods
     */
    public static Vector<DMProperty> searchForProperties(Class type, DMModel dmModel, DMRepository repository, boolean includesGetOnlyProperties, boolean importReferencedEntities, Vector<String> excludedSignatures)
    {
        ...
        try {
            ...
            for (int i = 0; i < declaredMethods.length; i++) {
                Method method = declaredMethods[i];
                 DMProperty newProperty = makeProperty(method,dmModel,repository,includesGetOnlyProperties,importReferencedEntities,excludedSignatures);
     ...
     */
    private static DMProperty makeProperty(Class type, String propertyName, DMModel dmModel, DMRepository repository, boolean includesGetOnlyProperties, boolean importReferencedEntities, Vector<String> excludedSignatures)
    {
    	if (logger.isLoggable(Level.FINE)) logger.fine("makeProperty() "+propertyName+" for "+type);
        Method method = searchMatchingGetMethod(type,propertyName);
        if (method != null) {
        	if (logger.isLoggable(Level.FINE)) logger.fine("Found method "+method);
            return makeProperty(method,dmModel,repository,includesGetOnlyProperties,importReferencedEntities,excludedSignatures);
        }
        else {
        	if (logger.isLoggable(Level.FINE)) logger.fine("Not found method, try field");
     ...
     * Build a new DMProperty
     */
    private static DMProperty makeProperty(Method method, DMModel dmModel, DMRepository repository, boolean includesGetOnlyProperties, boolean importReferencedEntities, Vector<String> excludedSignatures)
    {
        ...
        if ((returnType != Void.TYPE) && (Modifier.isPublic(method.getModifiers())) && (!Modifier.isStatic(method.getModifiers()))
            ...
            if (cardinality == DMCardinality.VECTOR) {
                ...
                if (excludedSignatures != null) {
                    ...
                    if ((addToMethods.size() == 0) || (removeFromMethods.size() == 0)) isSettable = false;
                }
            }
            ...
            if (cardinality == DMCardinality.HASHTABLE) {
                ...
                if (excludedSignatures != null) {
                    ...
                    if ((setMethods.size() == 0) || (removeMethods.size() == 0)) isSettable = false;
                }
            }
     ...
     * Build a new DMProperty
     */
    private static DMProperty makeProperty(Field field, DMModel dmModel, DMRepository repository, /*boolean includesGetOnlyProperties, boolean includesMethods,*/ boolean importReferencedEntities/*, Vector excludedSignatures*/)
    {
    ...
    private static Method getMethod (Class type, String methodName, Type... params) throws NoSuchMethodException
    {
    	...
    	if (logger.isLoggable(Level.FINE)) {
    		sb	= new StringBuffer();
    		for (Type t : params) sb.append(" "+t.toString());
		   	if (logger.isLoggable(Level.FINE))
    	...
    	for (Method m : type.getMethods()) {
    		...
    		if (m.getName().equals(methodName) && m.getGenericParameterTypes().length == params.length) {
    			...
    			for (int i=0; i<params.length; i++) {
    				if (!params[i].equals(m.getGenericParameterTypes()[i])) paramMatches = false;
    			}
    			if (paramMatches) {
    			   	if (logger.isLoggable(Level.FINE))
     ...
     * NB: 'addTo...' methods are methods with general form:
     * <code>addToXXX(Class anObject)</code> or
     * <code>_addToXXX(Class anObject)</code>, where XXX is the property name
     * (try with or without a terminal 's' character), and Class could be
     * anything... Returns an ordered TreeSet of {@link AccessorMethod} objects
     */
     ...
     * NB: 'removeFrom...' methods are methods with general form:
     * <code>removeFromXXX(Class anObject)</code> or
     * <code>_removeFromXXX(Class anObject)</code>, where XXX is the property
     * name (try with or without a terminal 's' character), and Class could be
     * anything... Returns an ordered TreeSet of {@link AccessorMethod} objects
     */
     ...
     * regarding parameters classes), we store these
     * <code>AccessorMethods</code> in a particular order depending on the
     * parameters specialization. This order is implemented in this class
     * through {@link Comparable} interface implementation. Note: this class has
     * a natural ordering that is inconsistent with equals, which means that
     * <code>(x.compareTo(y)==0) == (x.equals(y))</code> condition is
     * violated.
     */
    ...
    /**
     * Update property
      * Delete supplied property when not found in Class
    * 
     * @param property
    ...
    private DMProperty createProperty(PropertyReference propertyReference)
    {
        ...
        DMProperty returnedProperty = makeProperty(getJavaType(),propertyReference.getName(),getDMModel(),getRepository(),true,true,null);
        registerProperty(returnedProperty,false);
        return returnedProperty;
    }
    
    /**
     * Update method using defined type as Class
     * Delete supplied method when not found in Class
     * 
     * @param method
    ...
    private void updateMethod(DMMethod method) throws DuplicateMethodSignatureException
    {
        ...
        if (updatedMethod == null) {
        	...
        	if (logger.isLoggable(Level.FINE)) logger.fine("Update method "+method.getSignature()+" with "+updatedMethod.getSignature());
    ...
    public void update (ClassReference aClassReference)
    {
        if (getJavaType() == null) {
        	...
        	if (logger.isLoggable(Level.FINE)) logger.fine("Update "+getName()+" with "+aClassReference.getName());
    ...
    public void update(boolean includeGetOnlyProperties, boolean includeMethods) 
    {
        if (getJavaType() == null) {
            ...
            if (logger.isLoggable(Level.FINE)) logger.fine("Update "+this+" with "+includeGetOnlyProperties+" and "+includeMethods);
            ...
            Vector<String> excludedSignatures = new Vector<String>();
            Vector<DMProperty> allRequiredProperties = LoadableDMEntity.searchForProperties(getJavaType(),getDMModel(),getRepository(),includeGetOnlyProperties,false,excludedSignatures);
            Vector<DMMethod> allRequiredMethods;
            if (includeMethods) {
                allRequiredMethods = LoadableDMEntity.searchForMethods(getJavaType(),getDMModel(),getRepository(),false,excludedSignatures);
            }
            else {
            ...
            for (Enumeration en = properties.elements(); en.hasMoreElements();) {
                ...
                if (logger.isLoggable(Level.FINE)) logger.fine("Update property "+nextProperty);
                try {
            ...
            for (Enumeration en=allRequiredProperties.elements(); en.hasMoreElements();) {
                DMProperty nextProperty = (DMProperty)en.nextElement();
                if (logger.isLoggable(Level.FINE)) logger.fine("Create property "+nextProperty);
            ...
            for (Enumeration en = methods.elements(); en.hasMoreElements();) {
                ...
                if (logger.isLoggable(Level.FINE)) logger.fine("Update method "+nextMethod);
                try {
            ...
            for (Enumeration en=allRequiredMethods.elements(); en.hasMoreElements();) {
                DMMethod nextMethod = (DMMethod)en.nextElement();
                if (logger.isLoggable(Level.FINE)) logger.fine("Create method "+nextMethod);
     ...
     * Always false for compiled classes
     * 
     * @return
     */
