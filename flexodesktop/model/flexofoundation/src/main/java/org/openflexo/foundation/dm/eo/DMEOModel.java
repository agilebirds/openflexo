...
public class DMEOModel extends DMObject implements DMEOObject
{
    ...
    /**
     * Constructor for dynamic creation: the resource is explicitely created and
     * registered
     * 
...
//    	reply.writeToFile(_eoModelResource.getFile());
    ...
    public static DMEOModel createsNewDMEOModelFromExistingEOModel(DMModel dmModel, DMEORepository repository,
            FlexoProjectFile eoModelFile, FlexoDMResource dmRes) throws InvalidEOModelFileException, DuplicateResourceException, InvalidFileNameException
    {
    ...
    /**
     * Constructor for dynamic creation: the resource is explicitely created and
     * registered and the EOModel is created from scratch
     * 
     * @throws DuplicateResourceException
     * @throws InvalidFileNameException 
     * @throws IOException 
    ...
    public String derivePackageName(){
    	return "org.openflexo."+getProject().getPrefix().toLowerCase()+"."+ToolBox.convertStringToJavaString(getNameWithoutSuffix()).toLowerCase()+".db";
    }
            ...
            if (!isDeserializing()) {
            	getRepository().renameEOModel(this, oldName, aName);
              	try {
					...
					e.printStackTrace();
	              	logger.warning("Unexpected InvalidFileNameException raised when trying to rename EOModel from "+oldName+" to "+aName);
				} catch (DuplicateResourceException e) {
              		// TODO: gerer ca mieux un jour
					e.printStackTrace();
	              	logger.warning("Unexpected DuplicateResourceException raised when trying to rename EOModel from "+oldName+" to "+aName);
				}
              	setChanged();
    ...
    /**
     * Return all the entities contained in this eomodel, as a Vector of
     * DMEntity
     * 
	...
	public String getNextDefautEntityName(DMPackage aPackage) {
        ...
        int test = 0;
        while ((entities.get(aPackage + "." + testMe) != null) || ((getEOModel()!=null) && (getEOModel()._entityNamedIgnoreCase(testMe)!=null))|| ((getEOModel()!=null) && (getEOModel().getModelGroup().entityNamedIgnoreCase(testMe)!=null))) {
            test++;
