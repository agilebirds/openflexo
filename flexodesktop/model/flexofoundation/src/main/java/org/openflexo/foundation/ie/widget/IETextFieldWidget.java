...
public class IETextFieldWidget extends IEEditableTextWidget
{
    ...
    private boolean isDateOrIntegerOrFloat(){
    	return getFieldType() != null && 
			(getFieldType().equals(TextFieldType.DATE) || getFieldType().equals(TextFieldType.INTEGER) 
    			|| getFieldType().equals(TextFieldType.FLOAT));
    }
    public String getOperatorCodeStringRepresentation(){
       	if (isDateOrIntegerOrFloat()) {
        	if(getOperator()==null)return IEControlOperator.EQUAL.getSign();
        	if(getOperator()==IEControlOperator.LIKE || getOperator()==IEControlOperator.CASEINSENSITIVELIKE)return IEControlOperator.EQUAL.getSign();
        	return getOperator().getSign();
       	}else{
       		if(getOperator()==null)return IEControlOperator.CASEINSENSITIVELIKE.getSign();
       		if(getOperator()==IEControlOperator.LIKE || getOperator()==IEControlOperator.CASEINSENSITIVELIKE)return getOperator().getSign();
