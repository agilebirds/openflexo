...
public class FlexoDMResource extends FlexoXMLStorageResource<DMModel>
{
    ...
    @Override
	public DMModel performLoadResourceData(FlexoProgress progress, ProjectLoadingHandler loadingHandler) throws LoadXMLResourceException, ProjectLoadingCancelledException, MalformedXMLException
    {
        DMModel dmModel;
        if (progress != null) {
     ...
     * method implements conversion from v1.0 to v1.1
     * 
     * @param v1
     * @param v2
    	...
    	if (v1.equals(new FlexoVersion("1.0")) && v2.equals(new FlexoVersion("1.1"))) {
        	...
        	for (DMEntity componentEntity : getResourceData().getComponentRepository().getEntities().values()) {
        		if (componentEntity instanceof ComponentDMEntity) {
        			logger.info("Sets "+componentEntity+" to have "+getResourceData().getWORepository().getCustomComponentEntity()+" as parent entity");
        			componentEntity.setParentType(DMType.makeResolvedDMType(getResourceData().getWORepository().getCustomComponentEntity()), true);
        		}
        	}
