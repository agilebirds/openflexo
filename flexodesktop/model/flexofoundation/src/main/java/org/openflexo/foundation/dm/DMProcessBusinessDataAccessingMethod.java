...
public class DMProcessBusinessDataAccessingMethod extends DMMethod {
		...
		if (getReturnType() != null && _isRegistered) {
			...
			if (codeType != null) {
				...
				case CUSTOMCOMPONENT_CURRENTBUSINESSDATA:
					return "return (" + getReturnType().getSimplifiedStringRepresentation() + ")"
							+ ProcessBusinessDataRepository.getCustomComponentCurrentProcessBusinessDataGetterName() + "(\"" + processKey
							+ "\");";
				case AUTOGENERATEDBUSINESSDATA_CHILD:
					StringBuilder sb = new StringBuilder();
					sb.append("Vector<?> list = " + ProcessBusinessDataRepository.getChildrenProcessBusinessDataGetterName() + "(\""
							+ processKey + "\");");
					sb.append(StringUtils.LINE_SEPARATOR + "if(list.size() > 0)  return ("
							+ getReturnType().getSimplifiedStringRepresentation() + ") list.get(0);");
					sb.append(StringUtils.LINE_SEPARATOR + "return null;");
					return sb.toString();
				case AUTOGENERATEDBUSINESSDATA_PARENT:
					return "return (" + getReturnType().getSimplifiedStringRepresentation() + ")"
							+ ProcessBusinessDataRepository.getParentProcessBusinessDataGetterName() + "(\"" + processKey + "\");";
				}
			}
		}
	...
	public void updateProcess(FlexoProcess process) {
		if (process.getBusinessDataType() == null) {
			...
			try {
				...
				logger.log(Level.WARNING, "Cannot change process business data accessing method name to "
						+ getAccessingBusinessDataMethodName(process), e);
			}
		}
	}
	...
	public static DMProcessBusinessDataAccessingMethod getProcessBusinessDataAccessingMethod(DMEntity entity, FlexoProcess process,
			CodeType codeType) {
